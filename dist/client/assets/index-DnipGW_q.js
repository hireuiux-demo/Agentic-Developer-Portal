const __vite__mapDeps = (
  i,
  m = __vite__mapDeps,
  d = m.f ||
    (m.f = [
      "assets/index-CYQxVFKm.js",
      "assets/Home-gInC6HRR.js",
      "assets/repo._repoId-HD62IApT.js",
    ]),
) => i.map((i) => d[i]);
function c0(a, i) {
  for (var u = 0; u < i.length; u++) {
    const r = i[u];
    if (typeof r != "string" && !Array.isArray(r)) {
      for (const c in r)
        if (c !== "default" && !(c in a)) {
          const f = Object.getOwnPropertyDescriptor(r, c);
          f &&
            Object.defineProperty(
              a,
              c,
              f.get ? f : { enumerable: !0, get: () => r[c] },
            );
        }
    }
  }
  return Object.freeze(
    Object.defineProperty(a, Symbol.toStringTag, { value: "Module" }),
  );
}
function Ay(a) {
  return a && a.__esModule && Object.prototype.hasOwnProperty.call(a, "default")
    ? a.default
    : a;
}
var Zo = { exports: {} },
  Ai = {};
var zm;
function f0() {
  if (zm) return Ai;
  zm = 1;
  var a = Symbol.for("react.transitional.element"),
    i = Symbol.for("react.fragment");
  function u(r, c, f) {
    var h = null;
    if (
      (f !== void 0 && (h = "" + f),
      c.key !== void 0 && (h = "" + c.key),
      "key" in c)
    ) {
      f = {};
      for (var g in c) g !== "key" && (f[g] = c[g]);
    } else f = c;
    return (
      (c = f.ref),
      { $$typeof: a, type: r, key: h, ref: c !== void 0 ? c : null, props: f }
    );
  }
  return ((Ai.Fragment = i), (Ai.jsx = u), (Ai.jsxs = u), Ai);
}
var Om;
function d0() {
  return (Om || ((Om = 1), (Zo.exports = f0())), Zo.exports);
}
var K = d0(),
  Ko = { exports: {} },
  ot = {};
var Cm;
function h0() {
  if (Cm) return ot;
  Cm = 1;
  var a = Symbol.for("react.transitional.element"),
    i = Symbol.for("react.portal"),
    u = Symbol.for("react.fragment"),
    r = Symbol.for("react.strict_mode"),
    c = Symbol.for("react.profiler"),
    f = Symbol.for("react.consumer"),
    h = Symbol.for("react.context"),
    g = Symbol.for("react.forward_ref"),
    y = Symbol.for("react.suspense"),
    m = Symbol.for("react.memo"),
    S = Symbol.for("react.lazy"),
    p = Symbol.for("react.activity"),
    E = Symbol.iterator;
  function _(M) {
    return M === null || typeof M != "object"
      ? null
      : ((M = (E && M[E]) || M["@@iterator"]),
        typeof M == "function" ? M : null);
  }
  var z = {
      isMounted: function () {
        return !1;
      },
      enqueueForceUpdate: function () {},
      enqueueReplaceState: function () {},
      enqueueSetState: function () {},
    },
    L = Object.assign,
    x = {};
  function C(M, X, k) {
    ((this.props = M),
      (this.context = X),
      (this.refs = x),
      (this.updater = k || z));
  }
  ((C.prototype.isReactComponent = {}),
    (C.prototype.setState = function (M, X) {
      if (typeof M != "object" && typeof M != "function" && M != null)
        throw Error(
          "takes an object of state variables to update or a function which returns an object of state variables.",
        );
      this.updater.enqueueSetState(this, M, X, "setState");
    }),
    (C.prototype.forceUpdate = function (M) {
      this.updater.enqueueForceUpdate(this, M, "forceUpdate");
    }));
  function Y() {}
  Y.prototype = C.prototype;
  function Q(M, X, k) {
    ((this.props = M),
      (this.context = X),
      (this.refs = x),
      (this.updater = k || z));
  }
  var H = (Q.prototype = new Y());
  ((H.constructor = Q), L(H, C.prototype), (H.isPureReactComponent = !0));
  var W = Array.isArray;
  function F() {}
  var G = { H: null, A: null, T: null, S: null },
    Z = Object.prototype.hasOwnProperty;
  function P(M, X, k) {
    var I = k.ref;
    return {
      $$typeof: a,
      type: M,
      key: X,
      ref: I !== void 0 ? I : null,
      props: k,
    };
  }
  function rt(M, X) {
    return P(M.type, X, M.props);
  }
  function ut(M) {
    return typeof M == "object" && M !== null && M.$$typeof === a;
  }
  function it(M) {
    var X = { "=": "=0", ":": "=2" };
    return (
      "$" +
      M.replace(/[=:]/g, function (k) {
        return X[k];
      })
    );
  }
  var $t = /\/+/g;
  function Bt(M, X) {
    return typeof M == "object" && M !== null && M.key != null
      ? it("" + M.key)
      : X.toString(36);
  }
  function Gt(M) {
    switch (M.status) {
      case "fulfilled":
        return M.value;
      case "rejected":
        throw M.reason;
      default:
        switch (
          (typeof M.status == "string"
            ? M.then(F, F)
            : ((M.status = "pending"),
              M.then(
                function (X) {
                  M.status === "pending" &&
                    ((M.status = "fulfilled"), (M.value = X));
                },
                function (X) {
                  M.status === "pending" &&
                    ((M.status = "rejected"), (M.reason = X));
                },
              )),
          M.status)
        ) {
          case "fulfilled":
            return M.value;
          case "rejected":
            throw M.reason;
        }
    }
    throw M;
  }
  function B(M, X, k, I, et) {
    var dt = typeof M;
    (dt === "undefined" || dt === "boolean") && (M = null);
    var vt = !1;
    if (M === null) vt = !0;
    else
      switch (dt) {
        case "bigint":
        case "string":
        case "number":
          vt = !0;
          break;
        case "object":
          switch (M.$$typeof) {
            case a:
            case i:
              vt = !0;
              break;
            case S:
              return ((vt = M._init), B(vt(M._payload), X, k, I, et));
          }
      }
    if (vt)
      return (
        (et = et(M)),
        (vt = I === "" ? "." + Bt(M, 0) : I),
        W(et)
          ? ((k = ""),
            vt != null && (k = vt.replace($t, "$&/") + "/"),
            B(et, X, k, "", function (tn) {
              return tn;
            }))
          : et != null &&
            (ut(et) &&
              (et = rt(
                et,
                k +
                  (et.key == null || (M && M.key === et.key)
                    ? ""
                    : ("" + et.key).replace($t, "$&/") + "/") +
                  vt,
              )),
            X.push(et)),
        1
      );
    vt = 0;
    var Xt = I === "" ? "." : I + ":";
    if (W(M))
      for (var Ct = 0; Ct < M.length; Ct++)
        ((I = M[Ct]), (dt = Xt + Bt(I, Ct)), (vt += B(I, X, k, dt, et)));
    else if (((Ct = _(M)), typeof Ct == "function"))
      for (M = Ct.call(M), Ct = 0; !(I = M.next()).done; )
        ((I = I.value), (dt = Xt + Bt(I, Ct++)), (vt += B(I, X, k, dt, et)));
    else if (dt === "object") {
      if (typeof M.then == "function") return B(Gt(M), X, k, I, et);
      throw (
        (X = String(M)),
        Error(
          "Objects are not valid as a React child (found: " +
            (X === "[object Object]"
              ? "object with keys {" + Object.keys(M).join(", ") + "}"
              : X) +
            "). If you meant to render a collection of children, use an array instead.",
        )
      );
    }
    return vt;
  }
  function J(M, X, k) {
    if (M == null) return M;
    var I = [],
      et = 0;
    return (
      B(M, I, "", "", function (dt) {
        return X.call(k, dt, et++);
      }),
      I
    );
  }
  function lt(M) {
    if (M._status === -1) {
      var X = M._result;
      ((X = X()),
        X.then(
          function (k) {
            (M._status === 0 || M._status === -1) &&
              ((M._status = 1), (M._result = k));
          },
          function (k) {
            (M._status === 0 || M._status === -1) &&
              ((M._status = 2), (M._result = k));
          },
        ),
        M._status === -1 && ((M._status = 0), (M._result = X)));
    }
    if (M._status === 1) return M._result.default;
    throw M._result;
  }
  var Et =
      typeof reportError == "function"
        ? reportError
        : function (M) {
            if (
              typeof window == "object" &&
              typeof window.ErrorEvent == "function"
            ) {
              var X = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof M == "object" &&
                  M !== null &&
                  typeof M.message == "string"
                    ? String(M.message)
                    : String(M),
                error: M,
              });
              if (!window.dispatchEvent(X)) return;
            } else if (
              typeof process == "object" &&
              typeof process.emit == "function"
            ) {
              process.emit("uncaughtException", M);
              return;
            }
            console.error(M);
          },
    Tt = {
      map: J,
      forEach: function (M, X, k) {
        J(
          M,
          function () {
            X.apply(this, arguments);
          },
          k,
        );
      },
      count: function (M) {
        var X = 0;
        return (
          J(M, function () {
            X++;
          }),
          X
        );
      },
      toArray: function (M) {
        return (
          J(M, function (X) {
            return X;
          }) || []
        );
      },
      only: function (M) {
        if (!ut(M))
          throw Error(
            "React.Children.only expected to receive a single React element child.",
          );
        return M;
      },
    };
  return (
    (ot.Activity = p),
    (ot.Children = Tt),
    (ot.Component = C),
    (ot.Fragment = u),
    (ot.Profiler = c),
    (ot.PureComponent = Q),
    (ot.StrictMode = r),
    (ot.Suspense = y),
    (ot.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = G),
    (ot.__COMPILER_RUNTIME = {
      __proto__: null,
      c: function (M) {
        return G.H.useMemoCache(M);
      },
    }),
    (ot.cache = function (M) {
      return function () {
        return M.apply(null, arguments);
      };
    }),
    (ot.cacheSignal = function () {
      return null;
    }),
    (ot.cloneElement = function (M, X, k) {
      if (M == null)
        throw Error(
          "The argument must be a React element, but you passed " + M + ".",
        );
      var I = L({}, M.props),
        et = M.key;
      if (X != null)
        for (dt in (X.key !== void 0 && (et = "" + X.key), X))
          !Z.call(X, dt) ||
            dt === "key" ||
            dt === "__self" ||
            dt === "__source" ||
            (dt === "ref" && X.ref === void 0) ||
            (I[dt] = X[dt]);
      var dt = arguments.length - 2;
      if (dt === 1) I.children = k;
      else if (1 < dt) {
        for (var vt = Array(dt), Xt = 0; Xt < dt; Xt++)
          vt[Xt] = arguments[Xt + 2];
        I.children = vt;
      }
      return P(M.type, et, I);
    }),
    (ot.createContext = function (M) {
      return (
        (M = {
          $$typeof: h,
          _currentValue: M,
          _currentValue2: M,
          _threadCount: 0,
          Provider: null,
          Consumer: null,
        }),
        (M.Provider = M),
        (M.Consumer = { $$typeof: f, _context: M }),
        M
      );
    }),
    (ot.createElement = function (M, X, k) {
      var I,
        et = {},
        dt = null;
      if (X != null)
        for (I in (X.key !== void 0 && (dt = "" + X.key), X))
          Z.call(X, I) &&
            I !== "key" &&
            I !== "__self" &&
            I !== "__source" &&
            (et[I] = X[I]);
      var vt = arguments.length - 2;
      if (vt === 1) et.children = k;
      else if (1 < vt) {
        for (var Xt = Array(vt), Ct = 0; Ct < vt; Ct++)
          Xt[Ct] = arguments[Ct + 2];
        et.children = Xt;
      }
      if (M && M.defaultProps)
        for (I in ((vt = M.defaultProps), vt))
          et[I] === void 0 && (et[I] = vt[I]);
      return P(M, dt, et);
    }),
    (ot.createRef = function () {
      return { current: null };
    }),
    (ot.forwardRef = function (M) {
      return { $$typeof: g, render: M };
    }),
    (ot.isValidElement = ut),
    (ot.lazy = function (M) {
      return { $$typeof: S, _payload: { _status: -1, _result: M }, _init: lt };
    }),
    (ot.memo = function (M, X) {
      return { $$typeof: m, type: M, compare: X === void 0 ? null : X };
    }),
    (ot.startTransition = function (M) {
      var X = G.T,
        k = {};
      G.T = k;
      try {
        var I = M(),
          et = G.S;
        (et !== null && et(k, I),
          typeof I == "object" &&
            I !== null &&
            typeof I.then == "function" &&
            I.then(F, Et));
      } catch (dt) {
        Et(dt);
      } finally {
        (X !== null && k.types !== null && (X.types = k.types), (G.T = X));
      }
    }),
    (ot.unstable_useCacheRefresh = function () {
      return G.H.useCacheRefresh();
    }),
    (ot.use = function (M) {
      return G.H.use(M);
    }),
    (ot.useActionState = function (M, X, k) {
      return G.H.useActionState(M, X, k);
    }),
    (ot.useCallback = function (M, X) {
      return G.H.useCallback(M, X);
    }),
    (ot.useContext = function (M) {
      return G.H.useContext(M);
    }),
    (ot.useDebugValue = function () {}),
    (ot.useDeferredValue = function (M, X) {
      return G.H.useDeferredValue(M, X);
    }),
    (ot.useEffect = function (M, X) {
      return G.H.useEffect(M, X);
    }),
    (ot.useEffectEvent = function (M) {
      return G.H.useEffectEvent(M);
    }),
    (ot.useId = function () {
      return G.H.useId();
    }),
    (ot.useImperativeHandle = function (M, X, k) {
      return G.H.useImperativeHandle(M, X, k);
    }),
    (ot.useInsertionEffect = function (M, X) {
      return G.H.useInsertionEffect(M, X);
    }),
    (ot.useLayoutEffect = function (M, X) {
      return G.H.useLayoutEffect(M, X);
    }),
    (ot.useMemo = function (M, X) {
      return G.H.useMemo(M, X);
    }),
    (ot.useOptimistic = function (M, X) {
      return G.H.useOptimistic(M, X);
    }),
    (ot.useReducer = function (M, X, k) {
      return G.H.useReducer(M, X, k);
    }),
    (ot.useRef = function (M) {
      return G.H.useRef(M);
    }),
    (ot.useState = function (M) {
      return G.H.useState(M);
    }),
    (ot.useSyncExternalStore = function (M, X, k) {
      return G.H.useSyncExternalStore(M, X, k);
    }),
    (ot.useTransition = function () {
      return G.H.useTransition();
    }),
    (ot.version = "19.2.5"),
    ot
  );
}
var wm;
function Yi() {
  return (wm || ((wm = 1), (Ko.exports = h0())), Ko.exports);
}
var nt = Yi();
const Ol = Ay(nt),
  WE = c0({ __proto__: null, default: Ol }, [nt]);
var Jo = { exports: {} },
  Mi = {},
  ko = { exports: {} },
  Fo = {};
var Dm;
function m0() {
  return (
    Dm ||
      ((Dm = 1),
      (function (a) {
        function i(B, J) {
          var lt = B.length;
          B.push(J);
          t: for (; 0 < lt; ) {
            var Et = (lt - 1) >>> 1,
              Tt = B[Et];
            if (0 < c(Tt, J)) ((B[Et] = J), (B[lt] = Tt), (lt = Et));
            else break t;
          }
        }
        function u(B) {
          return B.length === 0 ? null : B[0];
        }
        function r(B) {
          if (B.length === 0) return null;
          var J = B[0],
            lt = B.pop();
          if (lt !== J) {
            B[0] = lt;
            t: for (var Et = 0, Tt = B.length, M = Tt >>> 1; Et < M; ) {
              var X = 2 * (Et + 1) - 1,
                k = B[X],
                I = X + 1,
                et = B[I];
              if (0 > c(k, lt))
                I < Tt && 0 > c(et, k)
                  ? ((B[Et] = et), (B[I] = lt), (Et = I))
                  : ((B[Et] = k), (B[X] = lt), (Et = X));
              else if (I < Tt && 0 > c(et, lt))
                ((B[Et] = et), (B[I] = lt), (Et = I));
              else break t;
            }
          }
          return J;
        }
        function c(B, J) {
          var lt = B.sortIndex - J.sortIndex;
          return lt !== 0 ? lt : B.id - J.id;
        }
        if (
          ((a.unstable_now = void 0),
          typeof performance == "object" &&
            typeof performance.now == "function")
        ) {
          var f = performance;
          a.unstable_now = function () {
            return f.now();
          };
        } else {
          var h = Date,
            g = h.now();
          a.unstable_now = function () {
            return h.now() - g;
          };
        }
        var y = [],
          m = [],
          S = 1,
          p = null,
          E = 3,
          _ = !1,
          z = !1,
          L = !1,
          x = !1,
          C = typeof setTimeout == "function" ? setTimeout : null,
          Y = typeof clearTimeout == "function" ? clearTimeout : null,
          Q = typeof setImmediate < "u" ? setImmediate : null;
        function H(B) {
          for (var J = u(m); J !== null; ) {
            if (J.callback === null) r(m);
            else if (J.startTime <= B)
              (r(m), (J.sortIndex = J.expirationTime), i(y, J));
            else break;
            J = u(m);
          }
        }
        function W(B) {
          if (((L = !1), H(B), !z))
            if (u(y) !== null) ((z = !0), F || ((F = !0), it()));
            else {
              var J = u(m);
              J !== null && Gt(W, J.startTime - B);
            }
        }
        var F = !1,
          G = -1,
          Z = 5,
          P = -1;
        function rt() {
          return x ? !0 : !(a.unstable_now() - P < Z);
        }
        function ut() {
          if (((x = !1), F)) {
            var B = a.unstable_now();
            P = B;
            var J = !0;
            try {
              t: {
                ((z = !1), L && ((L = !1), Y(G), (G = -1)), (_ = !0));
                var lt = E;
                try {
                  e: {
                    for (
                      H(B), p = u(y);
                      p !== null && !(p.expirationTime > B && rt());
                    ) {
                      var Et = p.callback;
                      if (typeof Et == "function") {
                        ((p.callback = null), (E = p.priorityLevel));
                        var Tt = Et(p.expirationTime <= B);
                        if (((B = a.unstable_now()), typeof Tt == "function")) {
                          ((p.callback = Tt), H(B), (J = !0));
                          break e;
                        }
                        (p === u(y) && r(y), H(B));
                      } else r(y);
                      p = u(y);
                    }
                    if (p !== null) J = !0;
                    else {
                      var M = u(m);
                      (M !== null && Gt(W, M.startTime - B), (J = !1));
                    }
                  }
                  break t;
                } finally {
                  ((p = null), (E = lt), (_ = !1));
                }
                J = void 0;
              }
            } finally {
              J ? it() : (F = !1);
            }
          }
        }
        var it;
        if (typeof Q == "function")
          it = function () {
            Q(ut);
          };
        else if (typeof MessageChannel < "u") {
          var $t = new MessageChannel(),
            Bt = $t.port2;
          (($t.port1.onmessage = ut),
            (it = function () {
              Bt.postMessage(null);
            }));
        } else
          it = function () {
            C(ut, 0);
          };
        function Gt(B, J) {
          G = C(function () {
            B(a.unstable_now());
          }, J);
        }
        ((a.unstable_IdlePriority = 5),
          (a.unstable_ImmediatePriority = 1),
          (a.unstable_LowPriority = 4),
          (a.unstable_NormalPriority = 3),
          (a.unstable_Profiling = null),
          (a.unstable_UserBlockingPriority = 2),
          (a.unstable_cancelCallback = function (B) {
            B.callback = null;
          }),
          (a.unstable_forceFrameRate = function (B) {
            0 > B || 125 < B
              ? console.error(
                  "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
                )
              : (Z = 0 < B ? Math.floor(1e3 / B) : 5);
          }),
          (a.unstable_getCurrentPriorityLevel = function () {
            return E;
          }),
          (a.unstable_next = function (B) {
            switch (E) {
              case 1:
              case 2:
              case 3:
                var J = 3;
                break;
              default:
                J = E;
            }
            var lt = E;
            E = J;
            try {
              return B();
            } finally {
              E = lt;
            }
          }),
          (a.unstable_requestPaint = function () {
            x = !0;
          }),
          (a.unstable_runWithPriority = function (B, J) {
            switch (B) {
              case 1:
              case 2:
              case 3:
              case 4:
              case 5:
                break;
              default:
                B = 3;
            }
            var lt = E;
            E = B;
            try {
              return J();
            } finally {
              E = lt;
            }
          }),
          (a.unstable_scheduleCallback = function (B, J, lt) {
            var Et = a.unstable_now();
            switch (
              (typeof lt == "object" && lt !== null
                ? ((lt = lt.delay),
                  (lt = typeof lt == "number" && 0 < lt ? Et + lt : Et))
                : (lt = Et),
              B)
            ) {
              case 1:
                var Tt = -1;
                break;
              case 2:
                Tt = 250;
                break;
              case 5:
                Tt = 1073741823;
                break;
              case 4:
                Tt = 1e4;
                break;
              default:
                Tt = 5e3;
            }
            return (
              (Tt = lt + Tt),
              (B = {
                id: S++,
                callback: J,
                priorityLevel: B,
                startTime: lt,
                expirationTime: Tt,
                sortIndex: -1,
              }),
              lt > Et
                ? ((B.sortIndex = lt),
                  i(m, B),
                  u(y) === null &&
                    B === u(m) &&
                    (L ? (Y(G), (G = -1)) : (L = !0), Gt(W, lt - Et)))
                : ((B.sortIndex = Tt),
                  i(y, B),
                  z || _ || ((z = !0), F || ((F = !0), it()))),
              B
            );
          }),
          (a.unstable_shouldYield = rt),
          (a.unstable_wrapCallback = function (B) {
            var J = E;
            return function () {
              var lt = E;
              E = J;
              try {
                return B.apply(this, arguments);
              } finally {
                E = lt;
              }
            };
          }));
      })(Fo)),
    Fo
  );
}
var Lm;
function y0() {
  return (Lm || ((Lm = 1), (ko.exports = m0())), ko.exports);
}
var Po = { exports: {} },
  fe = {};
var Um;
function p0() {
  if (Um) return fe;
  Um = 1;
  var a = Yi();
  function i(y) {
    var m = "https://react.dev/errors/" + y;
    if (1 < arguments.length) {
      m += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var S = 2; S < arguments.length; S++)
        m += "&args[]=" + encodeURIComponent(arguments[S]);
    }
    return (
      "Minified React error #" +
      y +
      "; visit " +
      m +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function u() {}
  var r = {
      d: {
        f: u,
        r: function () {
          throw Error(i(522));
        },
        D: u,
        C: u,
        L: u,
        m: u,
        X: u,
        S: u,
        M: u,
      },
      p: 0,
      findDOMNode: null,
    },
    c = Symbol.for("react.portal");
  function f(y, m, S) {
    var p =
      3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: c,
      key: p == null ? null : "" + p,
      children: y,
      containerInfo: m,
      implementation: S,
    };
  }
  var h = a.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function g(y, m) {
    if (y === "font") return "";
    if (typeof m == "string") return m === "use-credentials" ? m : "";
  }
  return (
    (fe.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = r),
    (fe.createPortal = function (y, m) {
      var S =
        2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
      if (!m || (m.nodeType !== 1 && m.nodeType !== 9 && m.nodeType !== 11))
        throw Error(i(299));
      return f(y, m, null, S);
    }),
    (fe.flushSync = function (y) {
      var m = h.T,
        S = r.p;
      try {
        if (((h.T = null), (r.p = 2), y)) return y();
      } finally {
        ((h.T = m), (r.p = S), r.d.f());
      }
    }),
    (fe.preconnect = function (y, m) {
      typeof y == "string" &&
        (m
          ? ((m = m.crossOrigin),
            (m =
              typeof m == "string"
                ? m === "use-credentials"
                  ? m
                  : ""
                : void 0))
          : (m = null),
        r.d.C(y, m));
    }),
    (fe.prefetchDNS = function (y) {
      typeof y == "string" && r.d.D(y);
    }),
    (fe.preinit = function (y, m) {
      if (typeof y == "string" && m && typeof m.as == "string") {
        var S = m.as,
          p = g(S, m.crossOrigin),
          E = typeof m.integrity == "string" ? m.integrity : void 0,
          _ = typeof m.fetchPriority == "string" ? m.fetchPriority : void 0;
        S === "style"
          ? r.d.S(y, typeof m.precedence == "string" ? m.precedence : void 0, {
              crossOrigin: p,
              integrity: E,
              fetchPriority: _,
            })
          : S === "script" &&
            r.d.X(y, {
              crossOrigin: p,
              integrity: E,
              fetchPriority: _,
              nonce: typeof m.nonce == "string" ? m.nonce : void 0,
            });
      }
    }),
    (fe.preinitModule = function (y, m) {
      if (typeof y == "string")
        if (typeof m == "object" && m !== null) {
          if (m.as == null || m.as === "script") {
            var S = g(m.as, m.crossOrigin);
            r.d.M(y, {
              crossOrigin: S,
              integrity: typeof m.integrity == "string" ? m.integrity : void 0,
              nonce: typeof m.nonce == "string" ? m.nonce : void 0,
            });
          }
        } else m == null && r.d.M(y);
    }),
    (fe.preload = function (y, m) {
      if (
        typeof y == "string" &&
        typeof m == "object" &&
        m !== null &&
        typeof m.as == "string"
      ) {
        var S = m.as,
          p = g(S, m.crossOrigin);
        r.d.L(y, S, {
          crossOrigin: p,
          integrity: typeof m.integrity == "string" ? m.integrity : void 0,
          nonce: typeof m.nonce == "string" ? m.nonce : void 0,
          type: typeof m.type == "string" ? m.type : void 0,
          fetchPriority:
            typeof m.fetchPriority == "string" ? m.fetchPriority : void 0,
          referrerPolicy:
            typeof m.referrerPolicy == "string" ? m.referrerPolicy : void 0,
          imageSrcSet:
            typeof m.imageSrcSet == "string" ? m.imageSrcSet : void 0,
          imageSizes: typeof m.imageSizes == "string" ? m.imageSizes : void 0,
          media: typeof m.media == "string" ? m.media : void 0,
        });
      }
    }),
    (fe.preloadModule = function (y, m) {
      if (typeof y == "string")
        if (m) {
          var S = g(m.as, m.crossOrigin);
          r.d.m(y, {
            as: typeof m.as == "string" && m.as !== "script" ? m.as : void 0,
            crossOrigin: S,
            integrity: typeof m.integrity == "string" ? m.integrity : void 0,
          });
        } else r.d.m(y);
    }),
    (fe.requestFormReset = function (y) {
      r.d.r(y);
    }),
    (fe.unstable_batchedUpdates = function (y, m) {
      return y(m);
    }),
    (fe.useFormState = function (y, m, S) {
      return h.H.useFormState(y, m, S);
    }),
    (fe.useFormStatus = function () {
      return h.H.useHostTransitionStatus();
    }),
    (fe.version = "19.2.5"),
    fe
  );
}
var Nm;
function My() {
  if (Nm) return Po.exports;
  Nm = 1;
  function a() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(a);
      } catch (i) {
        console.error(i);
      }
  }
  return (a(), (Po.exports = p0()), Po.exports);
}
var Bm;
function g0() {
  if (Bm) return Mi;
  Bm = 1;
  var a = y0(),
    i = Yi(),
    u = My();
  function r(t) {
    var e = "https://react.dev/errors/" + t;
    if (1 < arguments.length) {
      e += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var n = 2; n < arguments.length; n++)
        e += "&args[]=" + encodeURIComponent(arguments[n]);
    }
    return (
      "Minified React error #" +
      t +
      "; visit " +
      e +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function c(t) {
    return !(!t || (t.nodeType !== 1 && t.nodeType !== 9 && t.nodeType !== 11));
  }
  function f(t) {
    var e = t,
      n = t;
    if (t.alternate) for (; e.return; ) e = e.return;
    else {
      t = e;
      do ((e = t), (e.flags & 4098) !== 0 && (n = e.return), (t = e.return));
      while (t);
    }
    return e.tag === 3 ? n : null;
  }
  function h(t) {
    if (t.tag === 13) {
      var e = t.memoizedState;
      if (
        (e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)),
        e !== null)
      )
        return e.dehydrated;
    }
    return null;
  }
  function g(t) {
    if (t.tag === 31) {
      var e = t.memoizedState;
      if (
        (e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)),
        e !== null)
      )
        return e.dehydrated;
    }
    return null;
  }
  function y(t) {
    if (f(t) !== t) throw Error(r(188));
  }
  function m(t) {
    var e = t.alternate;
    if (!e) {
      if (((e = f(t)), e === null)) throw Error(r(188));
      return e !== t ? null : t;
    }
    for (var n = t, l = e; ; ) {
      var s = n.return;
      if (s === null) break;
      var o = s.alternate;
      if (o === null) {
        if (((l = s.return), l !== null)) {
          n = l;
          continue;
        }
        break;
      }
      if (s.child === o.child) {
        for (o = s.child; o; ) {
          if (o === n) return (y(s), t);
          if (o === l) return (y(s), e);
          o = o.sibling;
        }
        throw Error(r(188));
      }
      if (n.return !== l.return) ((n = s), (l = o));
      else {
        for (var d = !1, v = s.child; v; ) {
          if (v === n) {
            ((d = !0), (n = s), (l = o));
            break;
          }
          if (v === l) {
            ((d = !0), (l = s), (n = o));
            break;
          }
          v = v.sibling;
        }
        if (!d) {
          for (v = o.child; v; ) {
            if (v === n) {
              ((d = !0), (n = o), (l = s));
              break;
            }
            if (v === l) {
              ((d = !0), (l = o), (n = s));
              break;
            }
            v = v.sibling;
          }
          if (!d) throw Error(r(189));
        }
      }
      if (n.alternate !== l) throw Error(r(190));
    }
    if (n.tag !== 3) throw Error(r(188));
    return n.stateNode.current === n ? t : e;
  }
  function S(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t;
    for (t = t.child; t !== null; ) {
      if (((e = S(t)), e !== null)) return e;
      t = t.sibling;
    }
    return null;
  }
  var p = Object.assign,
    E = Symbol.for("react.element"),
    _ = Symbol.for("react.transitional.element"),
    z = Symbol.for("react.portal"),
    L = Symbol.for("react.fragment"),
    x = Symbol.for("react.strict_mode"),
    C = Symbol.for("react.profiler"),
    Y = Symbol.for("react.consumer"),
    Q = Symbol.for("react.context"),
    H = Symbol.for("react.forward_ref"),
    W = Symbol.for("react.suspense"),
    F = Symbol.for("react.suspense_list"),
    G = Symbol.for("react.memo"),
    Z = Symbol.for("react.lazy"),
    P = Symbol.for("react.activity"),
    rt = Symbol.for("react.memo_cache_sentinel"),
    ut = Symbol.iterator;
  function it(t) {
    return t === null || typeof t != "object"
      ? null
      : ((t = (ut && t[ut]) || t["@@iterator"]),
        typeof t == "function" ? t : null);
  }
  var $t = Symbol.for("react.client.reference");
  function Bt(t) {
    if (t == null) return null;
    if (typeof t == "function")
      return t.$$typeof === $t ? null : t.displayName || t.name || null;
    if (typeof t == "string") return t;
    switch (t) {
      case L:
        return "Fragment";
      case C:
        return "Profiler";
      case x:
        return "StrictMode";
      case W:
        return "Suspense";
      case F:
        return "SuspenseList";
      case P:
        return "Activity";
    }
    if (typeof t == "object")
      switch (t.$$typeof) {
        case z:
          return "Portal";
        case Q:
          return t.displayName || "Context";
        case Y:
          return (t._context.displayName || "Context") + ".Consumer";
        case H:
          var e = t.render;
          return (
            (t = t.displayName),
            t ||
              ((t = e.displayName || e.name || ""),
              (t = t !== "" ? "ForwardRef(" + t + ")" : "ForwardRef")),
            t
          );
        case G:
          return (
            (e = t.displayName || null),
            e !== null ? e : Bt(t.type) || "Memo"
          );
        case Z:
          ((e = t._payload), (t = t._init));
          try {
            return Bt(t(e));
          } catch {}
      }
    return null;
  }
  var Gt = Array.isArray,
    B = i.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    J = u.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    lt = { pending: !1, data: null, method: null, action: null },
    Et = [],
    Tt = -1;
  function M(t) {
    return { current: t };
  }
  function X(t) {
    0 > Tt || ((t.current = Et[Tt]), (Et[Tt] = null), Tt--);
  }
  function k(t, e) {
    (Tt++, (Et[Tt] = t.current), (t.current = e));
  }
  var I = M(null),
    et = M(null),
    dt = M(null),
    vt = M(null);
  function Xt(t, e) {
    switch ((k(dt, e), k(et, t), k(I, null), e.nodeType)) {
      case 9:
      case 11:
        t = (t = e.documentElement) && (t = t.namespaceURI) ? Ih(t) : 0;
        break;
      default:
        if (((t = e.tagName), (e = e.namespaceURI)))
          ((e = Ih(e)), (t = Wh(e, t)));
        else
          switch (t) {
            case "svg":
              t = 1;
              break;
            case "math":
              t = 2;
              break;
            default:
              t = 0;
          }
    }
    (X(I), k(I, t));
  }
  function Ct() {
    (X(I), X(et), X(dt));
  }
  function tn(t) {
    t.memoizedState !== null && k(vt, t);
    var e = I.current,
      n = Wh(e, t.type);
    e !== n && (k(et, t), k(I, n));
  }
  function en(t) {
    (et.current === t && (X(I), X(et)),
      vt.current === t && (X(vt), (Ei._currentValue = lt)));
  }
  var Mn, wl;
  function Ke(t) {
    if (Mn === void 0)
      try {
        throw Error();
      } catch (n) {
        var e = n.stack.trim().match(/\n( *(at )?)/);
        ((Mn = (e && e[1]) || ""),
          (wl =
            -1 <
            n.stack.indexOf(`
    at`)
              ? " (<anonymous>)"
              : -1 < n.stack.indexOf("@")
                ? "@unknown:0:0"
                : ""));
      }
    return (
      `
` +
      Mn +
      t +
      wl
    );
  }
  var Dl = !1;
  function ja(t, e) {
    if (!t || Dl) return "";
    Dl = !0;
    var n = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var l = {
        DetermineComponentFrameRoot: function () {
          try {
            if (e) {
              var V = function () {
                throw Error();
              };
              if (
                (Object.defineProperty(V.prototype, "props", {
                  set: function () {
                    throw Error();
                  },
                }),
                typeof Reflect == "object" && Reflect.construct)
              ) {
                try {
                  Reflect.construct(V, []);
                } catch (N) {
                  var U = N;
                }
                Reflect.construct(t, [], V);
              } else {
                try {
                  V.call();
                } catch (N) {
                  U = N;
                }
                t.call(V.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (N) {
                U = N;
              }
              (V = t()) &&
                typeof V.catch == "function" &&
                V.catch(function () {});
            }
          } catch (N) {
            if (N && U && typeof N.stack == "string") return [N.stack, U.stack];
          }
          return [null, null];
        },
      };
      l.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var s = Object.getOwnPropertyDescriptor(
        l.DetermineComponentFrameRoot,
        "name",
      );
      s &&
        s.configurable &&
        Object.defineProperty(l.DetermineComponentFrameRoot, "name", {
          value: "DetermineComponentFrameRoot",
        });
      var o = l.DetermineComponentFrameRoot(),
        d = o[0],
        v = o[1];
      if (d && v) {
        var R = d.split(`
`),
          D = v.split(`
`);
        for (
          s = l = 0;
          l < R.length && !R[l].includes("DetermineComponentFrameRoot");
        )
          l++;
        for (; s < D.length && !D[s].includes("DetermineComponentFrameRoot"); )
          s++;
        if (l === R.length || s === D.length)
          for (
            l = R.length - 1, s = D.length - 1;
            1 <= l && 0 <= s && R[l] !== D[s];
          )
            s--;
        for (; 1 <= l && 0 <= s; l--, s--)
          if (R[l] !== D[s]) {
            if (l !== 1 || s !== 1)
              do
                if ((l--, s--, 0 > s || R[l] !== D[s])) {
                  var j =
                    `
` + R[l].replace(" at new ", " at ");
                  return (
                    t.displayName &&
                      j.includes("<anonymous>") &&
                      (j = j.replace("<anonymous>", t.displayName)),
                    j
                  );
                }
              while (1 <= l && 0 <= s);
            break;
          }
      }
    } finally {
      ((Dl = !1), (Error.prepareStackTrace = n));
    }
    return (n = t ? t.displayName || t.name : "") ? Ke(n) : "";
  }
  function Xi(t, e) {
    switch (t.tag) {
      case 26:
      case 27:
      case 5:
        return Ke(t.type);
      case 16:
        return Ke("Lazy");
      case 13:
        return t.child !== e && e !== null
          ? Ke("Suspense Fallback")
          : Ke("Suspense");
      case 19:
        return Ke("SuspenseList");
      case 0:
      case 15:
        return ja(t.type, !1);
      case 11:
        return ja(t.type.render, !1);
      case 1:
        return ja(t.type, !0);
      case 31:
        return Ke("Activity");
      default:
        return "";
    }
  }
  function nn(t) {
    try {
      var e = "",
        n = null;
      do ((e += Xi(t, n)), (n = t), (t = t.return));
      while (t);
      return e;
    } catch (l) {
      return (
        `
Error generating stack: ` +
        l.message +
        `
` +
        l.stack
      );
    }
  }
  var oa = Object.prototype.hasOwnProperty,
    Xe = a.unstable_scheduleCallback,
    Ll = a.unstable_cancelCallback,
    Vi = a.unstable_shouldYield,
    ws = a.unstable_requestPaint,
    ce = a.unstable_now,
    Lt = a.unstable_getCurrentPriorityLevel,
    ae = a.unstable_ImmediatePriority,
    Je = a.unstable_UserBlockingPriority,
    Ha = a.unstable_NormalPriority,
    Zp = a.unstable_LowPriority,
    Cc = a.unstable_IdlePriority,
    Kp = a.log,
    Jp = a.unstable_setDisableYieldValue,
    Ul = null,
    Re = null;
  function zn(t) {
    if (
      (typeof Kp == "function" && Jp(t),
      Re && typeof Re.setStrictMode == "function")
    )
      try {
        Re.setStrictMode(Ul, t);
      } catch {}
  }
  var Te = Math.clz32 ? Math.clz32 : Pp,
    kp = Math.log,
    Fp = Math.LN2;
  function Pp(t) {
    return ((t >>>= 0), t === 0 ? 32 : (31 - ((kp(t) / Fp) | 0)) | 0);
  }
  var Qi = 256,
    Zi = 262144,
    Ki = 4194304;
  function ca(t) {
    var e = t & 42;
    if (e !== 0) return e;
    switch (t & -t) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
        return t & 261888;
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t & 3932160;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return t & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return t;
    }
  }
  function Ji(t, e, n) {
    var l = t.pendingLanes;
    if (l === 0) return 0;
    var s = 0,
      o = t.suspendedLanes,
      d = t.pingedLanes;
    t = t.warmLanes;
    var v = l & 134217727;
    return (
      v !== 0
        ? ((l = v & ~o),
          l !== 0
            ? (s = ca(l))
            : ((d &= v),
              d !== 0
                ? (s = ca(d))
                : n || ((n = v & ~t), n !== 0 && (s = ca(n)))))
        : ((v = l & ~o),
          v !== 0
            ? (s = ca(v))
            : d !== 0
              ? (s = ca(d))
              : n || ((n = l & ~t), n !== 0 && (s = ca(n)))),
      s === 0
        ? 0
        : e !== 0 &&
            e !== s &&
            (e & o) === 0 &&
            ((o = s & -s),
            (n = e & -e),
            o >= n || (o === 32 && (n & 4194048) !== 0))
          ? e
          : s
    );
  }
  function Nl(t, e) {
    return (t.pendingLanes & ~(t.suspendedLanes & ~t.pingedLanes) & e) === 0;
  }
  function Ip(t, e) {
    switch (t) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return e + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return e + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function wc() {
    var t = Ki;
    return ((Ki <<= 1), (Ki & 62914560) === 0 && (Ki = 4194304), t);
  }
  function Ds(t) {
    for (var e = [], n = 0; 31 > n; n++) e.push(t);
    return e;
  }
  function Bl(t, e) {
    ((t.pendingLanes |= e),
      e !== 268435456 &&
        ((t.suspendedLanes = 0), (t.pingedLanes = 0), (t.warmLanes = 0)));
  }
  function Wp(t, e, n, l, s, o) {
    var d = t.pendingLanes;
    ((t.pendingLanes = n),
      (t.suspendedLanes = 0),
      (t.pingedLanes = 0),
      (t.warmLanes = 0),
      (t.expiredLanes &= n),
      (t.entangledLanes &= n),
      (t.errorRecoveryDisabledLanes &= n),
      (t.shellSuspendCounter = 0));
    var v = t.entanglements,
      R = t.expirationTimes,
      D = t.hiddenUpdates;
    for (n = d & ~n; 0 < n; ) {
      var j = 31 - Te(n),
        V = 1 << j;
      ((v[j] = 0), (R[j] = -1));
      var U = D[j];
      if (U !== null)
        for (D[j] = null, j = 0; j < U.length; j++) {
          var N = U[j];
          N !== null && (N.lane &= -536870913);
        }
      n &= ~V;
    }
    (l !== 0 && Dc(t, l, 0),
      o !== 0 && s === 0 && t.tag !== 0 && (t.suspendedLanes |= o & ~(d & ~e)));
  }
  function Dc(t, e, n) {
    ((t.pendingLanes |= e), (t.suspendedLanes &= ~e));
    var l = 31 - Te(e);
    ((t.entangledLanes |= e),
      (t.entanglements[l] = t.entanglements[l] | 1073741824 | (n & 261930)));
  }
  function Lc(t, e) {
    var n = (t.entangledLanes |= e);
    for (t = t.entanglements; n; ) {
      var l = 31 - Te(n),
        s = 1 << l;
      ((s & e) | (t[l] & e) && (t[l] |= e), (n &= ~s));
    }
  }
  function Uc(t, e) {
    var n = e & -e;
    return (
      (n = (n & 42) !== 0 ? 1 : Ls(n)),
      (n & (t.suspendedLanes | e)) !== 0 ? 0 : n
    );
  }
  function Ls(t) {
    switch (t) {
      case 2:
        t = 1;
        break;
      case 8:
        t = 4;
        break;
      case 32:
        t = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        t = 128;
        break;
      case 268435456:
        t = 134217728;
        break;
      default:
        t = 0;
    }
    return t;
  }
  function Us(t) {
    return (
      (t &= -t),
      2 < t ? (8 < t ? ((t & 134217727) !== 0 ? 32 : 268435456) : 8) : 2
    );
  }
  function Nc() {
    var t = J.p;
    return t !== 0 ? t : ((t = window.event), t === void 0 ? 32 : _m(t.type));
  }
  function Bc(t, e) {
    var n = J.p;
    try {
      return ((J.p = t), e());
    } finally {
      J.p = n;
    }
  }
  var On = Math.random().toString(36).slice(2),
    le = "__reactFiber$" + On,
    he = "__reactProps$" + On,
    qa = "__reactContainer$" + On,
    Ns = "__reactEvents$" + On,
    $p = "__reactListeners$" + On,
    tg = "__reactHandles$" + On,
    jc = "__reactResources$" + On,
    jl = "__reactMarker$" + On;
  function Bs(t) {
    (delete t[le], delete t[he], delete t[Ns], delete t[$p], delete t[tg]);
  }
  function Ya(t) {
    var e = t[le];
    if (e) return e;
    for (var n = t.parentNode; n; ) {
      if ((e = n[qa] || n[le])) {
        if (
          ((n = e.alternate),
          e.child !== null || (n !== null && n.child !== null))
        )
          for (t = im(t); t !== null; ) {
            if ((n = t[le])) return n;
            t = im(t);
          }
        return e;
      }
      ((t = n), (n = t.parentNode));
    }
    return null;
  }
  function Ga(t) {
    if ((t = t[le] || t[qa])) {
      var e = t.tag;
      if (
        e === 5 ||
        e === 6 ||
        e === 13 ||
        e === 31 ||
        e === 26 ||
        e === 27 ||
        e === 3
      )
        return t;
    }
    return null;
  }
  function Hl(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t.stateNode;
    throw Error(r(33));
  }
  function Xa(t) {
    var e = t[jc];
    return (
      e ||
        (e = t[jc] =
          { hoistableStyles: new Map(), hoistableScripts: new Map() }),
      e
    );
  }
  function te(t) {
    t[jl] = !0;
  }
  var Hc = new Set(),
    qc = {};
  function fa(t, e) {
    (Va(t, e), Va(t + "Capture", e));
  }
  function Va(t, e) {
    for (qc[t] = e, t = 0; t < e.length; t++) Hc.add(e[t]);
  }
  var eg = RegExp(
      "^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$",
    ),
    Yc = {},
    Gc = {};
  function ng(t) {
    return oa.call(Gc, t)
      ? !0
      : oa.call(Yc, t)
        ? !1
        : eg.test(t)
          ? (Gc[t] = !0)
          : ((Yc[t] = !0), !1);
  }
  function ki(t, e, n) {
    if (ng(e))
      if (n === null) t.removeAttribute(e);
      else {
        switch (typeof n) {
          case "undefined":
          case "function":
          case "symbol":
            t.removeAttribute(e);
            return;
          case "boolean":
            var l = e.toLowerCase().slice(0, 5);
            if (l !== "data-" && l !== "aria-") {
              t.removeAttribute(e);
              return;
            }
        }
        t.setAttribute(e, "" + n);
      }
  }
  function Fi(t, e, n) {
    if (n === null) t.removeAttribute(e);
    else {
      switch (typeof n) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(e);
          return;
      }
      t.setAttribute(e, "" + n);
    }
  }
  function an(t, e, n, l) {
    if (l === null) t.removeAttribute(n);
    else {
      switch (typeof l) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(n);
          return;
      }
      t.setAttributeNS(e, n, "" + l);
    }
  }
  function De(t) {
    switch (typeof t) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return t;
      case "object":
        return t;
      default:
        return "";
    }
  }
  function Xc(t) {
    var e = t.type;
    return (
      (t = t.nodeName) &&
      t.toLowerCase() === "input" &&
      (e === "checkbox" || e === "radio")
    );
  }
  function ag(t, e, n) {
    var l = Object.getOwnPropertyDescriptor(t.constructor.prototype, e);
    if (
      !t.hasOwnProperty(e) &&
      typeof l < "u" &&
      typeof l.get == "function" &&
      typeof l.set == "function"
    ) {
      var s = l.get,
        o = l.set;
      return (
        Object.defineProperty(t, e, {
          configurable: !0,
          get: function () {
            return s.call(this);
          },
          set: function (d) {
            ((n = "" + d), o.call(this, d));
          },
        }),
        Object.defineProperty(t, e, { enumerable: l.enumerable }),
        {
          getValue: function () {
            return n;
          },
          setValue: function (d) {
            n = "" + d;
          },
          stopTracking: function () {
            ((t._valueTracker = null), delete t[e]);
          },
        }
      );
    }
  }
  function js(t) {
    if (!t._valueTracker) {
      var e = Xc(t) ? "checked" : "value";
      t._valueTracker = ag(t, e, "" + t[e]);
    }
  }
  function Vc(t) {
    if (!t) return !1;
    var e = t._valueTracker;
    if (!e) return !0;
    var n = e.getValue(),
      l = "";
    return (
      t && (l = Xc(t) ? (t.checked ? "true" : "false") : t.value),
      (t = l),
      t !== n ? (e.setValue(t), !0) : !1
    );
  }
  function Pi(t) {
    if (
      ((t = t || (typeof document < "u" ? document : void 0)), typeof t > "u")
    )
      return null;
    try {
      return t.activeElement || t.body;
    } catch {
      return t.body;
    }
  }
  var lg = /[\n"\\]/g;
  function Le(t) {
    return t.replace(lg, function (e) {
      return "\\" + e.charCodeAt(0).toString(16) + " ";
    });
  }
  function Hs(t, e, n, l, s, o, d, v) {
    ((t.name = ""),
      d != null &&
      typeof d != "function" &&
      typeof d != "symbol" &&
      typeof d != "boolean"
        ? (t.type = d)
        : t.removeAttribute("type"),
      e != null
        ? d === "number"
          ? ((e === 0 && t.value === "") || t.value != e) &&
            (t.value = "" + De(e))
          : t.value !== "" + De(e) && (t.value = "" + De(e))
        : (d !== "submit" && d !== "reset") || t.removeAttribute("value"),
      e != null
        ? qs(t, d, De(e))
        : n != null
          ? qs(t, d, De(n))
          : l != null && t.removeAttribute("value"),
      s == null && o != null && (t.defaultChecked = !!o),
      s != null &&
        (t.checked = s && typeof s != "function" && typeof s != "symbol"),
      v != null &&
      typeof v != "function" &&
      typeof v != "symbol" &&
      typeof v != "boolean"
        ? (t.name = "" + De(v))
        : t.removeAttribute("name"));
  }
  function Qc(t, e, n, l, s, o, d, v) {
    if (
      (o != null &&
        typeof o != "function" &&
        typeof o != "symbol" &&
        typeof o != "boolean" &&
        (t.type = o),
      e != null || n != null)
    ) {
      if (!((o !== "submit" && o !== "reset") || e != null)) {
        js(t);
        return;
      }
      ((n = n != null ? "" + De(n) : ""),
        (e = e != null ? "" + De(e) : n),
        v || e === t.value || (t.value = e),
        (t.defaultValue = e));
    }
    ((l = l ?? s),
      (l = typeof l != "function" && typeof l != "symbol" && !!l),
      (t.checked = v ? t.checked : !!l),
      (t.defaultChecked = !!l),
      d != null &&
        typeof d != "function" &&
        typeof d != "symbol" &&
        typeof d != "boolean" &&
        (t.name = d),
      js(t));
  }
  function qs(t, e, n) {
    (e === "number" && Pi(t.ownerDocument) === t) ||
      t.defaultValue === "" + n ||
      (t.defaultValue = "" + n);
  }
  function Qa(t, e, n, l) {
    if (((t = t.options), e)) {
      e = {};
      for (var s = 0; s < n.length; s++) e["$" + n[s]] = !0;
      for (n = 0; n < t.length; n++)
        ((s = e.hasOwnProperty("$" + t[n].value)),
          t[n].selected !== s && (t[n].selected = s),
          s && l && (t[n].defaultSelected = !0));
    } else {
      for (n = "" + De(n), e = null, s = 0; s < t.length; s++) {
        if (t[s].value === n) {
          ((t[s].selected = !0), l && (t[s].defaultSelected = !0));
          return;
        }
        e !== null || t[s].disabled || (e = t[s]);
      }
      e !== null && (e.selected = !0);
    }
  }
  function Zc(t, e, n) {
    if (
      e != null &&
      ((e = "" + De(e)), e !== t.value && (t.value = e), n == null)
    ) {
      t.defaultValue !== e && (t.defaultValue = e);
      return;
    }
    t.defaultValue = n != null ? "" + De(n) : "";
  }
  function Kc(t, e, n, l) {
    if (e == null) {
      if (l != null) {
        if (n != null) throw Error(r(92));
        if (Gt(l)) {
          if (1 < l.length) throw Error(r(93));
          l = l[0];
        }
        n = l;
      }
      (n == null && (n = ""), (e = n));
    }
    ((n = De(e)),
      (t.defaultValue = n),
      (l = t.textContent),
      l === n && l !== "" && l !== null && (t.value = l),
      js(t));
  }
  function Za(t, e) {
    if (e) {
      var n = t.firstChild;
      if (n && n === t.lastChild && n.nodeType === 3) {
        n.nodeValue = e;
        return;
      }
    }
    t.textContent = e;
  }
  var ig = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " ",
    ),
  );
  function Jc(t, e, n) {
    var l = e.indexOf("--") === 0;
    n == null || typeof n == "boolean" || n === ""
      ? l
        ? t.setProperty(e, "")
        : e === "float"
          ? (t.cssFloat = "")
          : (t[e] = "")
      : l
        ? t.setProperty(e, n)
        : typeof n != "number" || n === 0 || ig.has(e)
          ? e === "float"
            ? (t.cssFloat = n)
            : (t[e] = ("" + n).trim())
          : (t[e] = n + "px");
  }
  function kc(t, e, n) {
    if (e != null && typeof e != "object") throw Error(r(62));
    if (((t = t.style), n != null)) {
      for (var l in n)
        !n.hasOwnProperty(l) ||
          (e != null && e.hasOwnProperty(l)) ||
          (l.indexOf("--") === 0
            ? t.setProperty(l, "")
            : l === "float"
              ? (t.cssFloat = "")
              : (t[l] = ""));
      for (var s in e)
        ((l = e[s]), e.hasOwnProperty(s) && n[s] !== l && Jc(t, s, l));
    } else for (var o in e) e.hasOwnProperty(o) && Jc(t, o, e[o]);
  }
  function Ys(t) {
    if (t.indexOf("-") === -1) return !1;
    switch (t) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var ug = new Map([
      ["acceptCharset", "accept-charset"],
      ["htmlFor", "for"],
      ["httpEquiv", "http-equiv"],
      ["crossOrigin", "crossorigin"],
      ["accentHeight", "accent-height"],
      ["alignmentBaseline", "alignment-baseline"],
      ["arabicForm", "arabic-form"],
      ["baselineShift", "baseline-shift"],
      ["capHeight", "cap-height"],
      ["clipPath", "clip-path"],
      ["clipRule", "clip-rule"],
      ["colorInterpolation", "color-interpolation"],
      ["colorInterpolationFilters", "color-interpolation-filters"],
      ["colorProfile", "color-profile"],
      ["colorRendering", "color-rendering"],
      ["dominantBaseline", "dominant-baseline"],
      ["enableBackground", "enable-background"],
      ["fillOpacity", "fill-opacity"],
      ["fillRule", "fill-rule"],
      ["floodColor", "flood-color"],
      ["floodOpacity", "flood-opacity"],
      ["fontFamily", "font-family"],
      ["fontSize", "font-size"],
      ["fontSizeAdjust", "font-size-adjust"],
      ["fontStretch", "font-stretch"],
      ["fontStyle", "font-style"],
      ["fontVariant", "font-variant"],
      ["fontWeight", "font-weight"],
      ["glyphName", "glyph-name"],
      ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
      ["glyphOrientationVertical", "glyph-orientation-vertical"],
      ["horizAdvX", "horiz-adv-x"],
      ["horizOriginX", "horiz-origin-x"],
      ["imageRendering", "image-rendering"],
      ["letterSpacing", "letter-spacing"],
      ["lightingColor", "lighting-color"],
      ["markerEnd", "marker-end"],
      ["markerMid", "marker-mid"],
      ["markerStart", "marker-start"],
      ["overlinePosition", "overline-position"],
      ["overlineThickness", "overline-thickness"],
      ["paintOrder", "paint-order"],
      ["panose-1", "panose-1"],
      ["pointerEvents", "pointer-events"],
      ["renderingIntent", "rendering-intent"],
      ["shapeRendering", "shape-rendering"],
      ["stopColor", "stop-color"],
      ["stopOpacity", "stop-opacity"],
      ["strikethroughPosition", "strikethrough-position"],
      ["strikethroughThickness", "strikethrough-thickness"],
      ["strokeDasharray", "stroke-dasharray"],
      ["strokeDashoffset", "stroke-dashoffset"],
      ["strokeLinecap", "stroke-linecap"],
      ["strokeLinejoin", "stroke-linejoin"],
      ["strokeMiterlimit", "stroke-miterlimit"],
      ["strokeOpacity", "stroke-opacity"],
      ["strokeWidth", "stroke-width"],
      ["textAnchor", "text-anchor"],
      ["textDecoration", "text-decoration"],
      ["textRendering", "text-rendering"],
      ["transformOrigin", "transform-origin"],
      ["underlinePosition", "underline-position"],
      ["underlineThickness", "underline-thickness"],
      ["unicodeBidi", "unicode-bidi"],
      ["unicodeRange", "unicode-range"],
      ["unitsPerEm", "units-per-em"],
      ["vAlphabetic", "v-alphabetic"],
      ["vHanging", "v-hanging"],
      ["vIdeographic", "v-ideographic"],
      ["vMathematical", "v-mathematical"],
      ["vectorEffect", "vector-effect"],
      ["vertAdvY", "vert-adv-y"],
      ["vertOriginX", "vert-origin-x"],
      ["vertOriginY", "vert-origin-y"],
      ["wordSpacing", "word-spacing"],
      ["writingMode", "writing-mode"],
      ["xmlnsXlink", "xmlns:xlink"],
      ["xHeight", "x-height"],
    ]),
    sg =
      /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function Ii(t) {
    return sg.test("" + t)
      ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
      : t;
  }
  function ln() {}
  var Gs = null;
  function Xs(t) {
    return (
      (t = t.target || t.srcElement || window),
      t.correspondingUseElement && (t = t.correspondingUseElement),
      t.nodeType === 3 ? t.parentNode : t
    );
  }
  var Ka = null,
    Ja = null;
  function Fc(t) {
    var e = Ga(t);
    if (e && (t = e.stateNode)) {
      var n = t[he] || null;
      t: switch (((t = e.stateNode), e.type)) {
        case "input":
          if (
            (Hs(
              t,
              n.value,
              n.defaultValue,
              n.defaultValue,
              n.checked,
              n.defaultChecked,
              n.type,
              n.name,
            ),
            (e = n.name),
            n.type === "radio" && e != null)
          ) {
            for (n = t; n.parentNode; ) n = n.parentNode;
            for (
              n = n.querySelectorAll(
                'input[name="' + Le("" + e) + '"][type="radio"]',
              ),
                e = 0;
              e < n.length;
              e++
            ) {
              var l = n[e];
              if (l !== t && l.form === t.form) {
                var s = l[he] || null;
                if (!s) throw Error(r(90));
                Hs(
                  l,
                  s.value,
                  s.defaultValue,
                  s.defaultValue,
                  s.checked,
                  s.defaultChecked,
                  s.type,
                  s.name,
                );
              }
            }
            for (e = 0; e < n.length; e++)
              ((l = n[e]), l.form === t.form && Vc(l));
          }
          break t;
        case "textarea":
          Zc(t, n.value, n.defaultValue);
          break t;
        case "select":
          ((e = n.value), e != null && Qa(t, !!n.multiple, e, !1));
      }
    }
  }
  var Vs = !1;
  function Pc(t, e, n) {
    if (Vs) return t(e, n);
    Vs = !0;
    try {
      var l = t(e);
      return l;
    } finally {
      if (
        ((Vs = !1),
        (Ka !== null || Ja !== null) &&
          (Hu(), Ka && ((e = Ka), (t = Ja), (Ja = Ka = null), Fc(e), t)))
      )
        for (e = 0; e < t.length; e++) Fc(t[e]);
    }
  }
  function ql(t, e) {
    var n = t.stateNode;
    if (n === null) return null;
    var l = n[he] || null;
    if (l === null) return null;
    n = l[e];
    t: switch (e) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        ((l = !l.disabled) ||
          ((t = t.type),
          (l = !(
            t === "button" ||
            t === "input" ||
            t === "select" ||
            t === "textarea"
          ))),
          (t = !l));
        break t;
      default:
        t = !1;
    }
    if (t) return null;
    if (n && typeof n != "function") throw Error(r(231, e, typeof n));
    return n;
  }
  var un = !(
      typeof window > "u" ||
      typeof window.document > "u" ||
      typeof window.document.createElement > "u"
    ),
    Qs = !1;
  if (un)
    try {
      var Yl = {};
      (Object.defineProperty(Yl, "passive", {
        get: function () {
          Qs = !0;
        },
      }),
        window.addEventListener("test", Yl, Yl),
        window.removeEventListener("test", Yl, Yl));
    } catch {
      Qs = !1;
    }
  var Cn = null,
    Zs = null,
    Wi = null;
  function Ic() {
    if (Wi) return Wi;
    var t,
      e = Zs,
      n = e.length,
      l,
      s = "value" in Cn ? Cn.value : Cn.textContent,
      o = s.length;
    for (t = 0; t < n && e[t] === s[t]; t++);
    var d = n - t;
    for (l = 1; l <= d && e[n - l] === s[o - l]; l++);
    return (Wi = s.slice(t, 1 < l ? 1 - l : void 0));
  }
  function $i(t) {
    var e = t.keyCode;
    return (
      "charCode" in t
        ? ((t = t.charCode), t === 0 && e === 13 && (t = 13))
        : (t = e),
      t === 10 && (t = 13),
      32 <= t || t === 13 ? t : 0
    );
  }
  function tu() {
    return !0;
  }
  function Wc() {
    return !1;
  }
  function me(t) {
    function e(n, l, s, o, d) {
      ((this._reactName = n),
        (this._targetInst = s),
        (this.type = l),
        (this.nativeEvent = o),
        (this.target = d),
        (this.currentTarget = null));
      for (var v in t)
        t.hasOwnProperty(v) && ((n = t[v]), (this[v] = n ? n(o) : o[v]));
      return (
        (this.isDefaultPrevented = (
          o.defaultPrevented != null ? o.defaultPrevented : o.returnValue === !1
        )
          ? tu
          : Wc),
        (this.isPropagationStopped = Wc),
        this
      );
    }
    return (
      p(e.prototype, {
        preventDefault: function () {
          this.defaultPrevented = !0;
          var n = this.nativeEvent;
          n &&
            (n.preventDefault
              ? n.preventDefault()
              : typeof n.returnValue != "unknown" && (n.returnValue = !1),
            (this.isDefaultPrevented = tu));
        },
        stopPropagation: function () {
          var n = this.nativeEvent;
          n &&
            (n.stopPropagation
              ? n.stopPropagation()
              : typeof n.cancelBubble != "unknown" && (n.cancelBubble = !0),
            (this.isPropagationStopped = tu));
        },
        persist: function () {},
        isPersistent: tu,
      }),
      e
    );
  }
  var da = {
      eventPhase: 0,
      bubbles: 0,
      cancelable: 0,
      timeStamp: function (t) {
        return t.timeStamp || Date.now();
      },
      defaultPrevented: 0,
      isTrusted: 0,
    },
    eu = me(da),
    Gl = p({}, da, { view: 0, detail: 0 }),
    rg = me(Gl),
    Ks,
    Js,
    Xl,
    nu = p({}, Gl, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
      pageX: 0,
      pageY: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      getModifierState: Fs,
      button: 0,
      buttons: 0,
      relatedTarget: function (t) {
        return t.relatedTarget === void 0
          ? t.fromElement === t.srcElement
            ? t.toElement
            : t.fromElement
          : t.relatedTarget;
      },
      movementX: function (t) {
        return "movementX" in t
          ? t.movementX
          : (t !== Xl &&
              (Xl && t.type === "mousemove"
                ? ((Ks = t.screenX - Xl.screenX), (Js = t.screenY - Xl.screenY))
                : (Js = Ks = 0),
              (Xl = t)),
            Ks);
      },
      movementY: function (t) {
        return "movementY" in t ? t.movementY : Js;
      },
    }),
    $c = me(nu),
    og = p({}, nu, { dataTransfer: 0 }),
    cg = me(og),
    fg = p({}, Gl, { relatedTarget: 0 }),
    ks = me(fg),
    dg = p({}, da, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
    hg = me(dg),
    mg = p({}, da, {
      clipboardData: function (t) {
        return "clipboardData" in t ? t.clipboardData : window.clipboardData;
      },
    }),
    yg = me(mg),
    pg = p({}, da, { data: 0 }),
    tf = me(pg),
    gg = {
      Esc: "Escape",
      Spacebar: " ",
      Left: "ArrowLeft",
      Up: "ArrowUp",
      Right: "ArrowRight",
      Down: "ArrowDown",
      Del: "Delete",
      Win: "OS",
      Menu: "ContextMenu",
      Apps: "ContextMenu",
      Scroll: "ScrollLock",
      MozPrintableKey: "Unidentified",
    },
    vg = {
      8: "Backspace",
      9: "Tab",
      12: "Clear",
      13: "Enter",
      16: "Shift",
      17: "Control",
      18: "Alt",
      19: "Pause",
      20: "CapsLock",
      27: "Escape",
      32: " ",
      33: "PageUp",
      34: "PageDown",
      35: "End",
      36: "Home",
      37: "ArrowLeft",
      38: "ArrowUp",
      39: "ArrowRight",
      40: "ArrowDown",
      45: "Insert",
      46: "Delete",
      112: "F1",
      113: "F2",
      114: "F3",
      115: "F4",
      116: "F5",
      117: "F6",
      118: "F7",
      119: "F8",
      120: "F9",
      121: "F10",
      122: "F11",
      123: "F12",
      144: "NumLock",
      145: "ScrollLock",
      224: "Meta",
    },
    Sg = {
      Alt: "altKey",
      Control: "ctrlKey",
      Meta: "metaKey",
      Shift: "shiftKey",
    };
  function bg(t) {
    var e = this.nativeEvent;
    return e.getModifierState
      ? e.getModifierState(t)
      : (t = Sg[t])
        ? !!e[t]
        : !1;
  }
  function Fs() {
    return bg;
  }
  var _g = p({}, Gl, {
      key: function (t) {
        if (t.key) {
          var e = gg[t.key] || t.key;
          if (e !== "Unidentified") return e;
        }
        return t.type === "keypress"
          ? ((t = $i(t)), t === 13 ? "Enter" : String.fromCharCode(t))
          : t.type === "keydown" || t.type === "keyup"
            ? vg[t.keyCode] || "Unidentified"
            : "";
      },
      code: 0,
      location: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      repeat: 0,
      locale: 0,
      getModifierState: Fs,
      charCode: function (t) {
        return t.type === "keypress" ? $i(t) : 0;
      },
      keyCode: function (t) {
        return t.type === "keydown" || t.type === "keyup" ? t.keyCode : 0;
      },
      which: function (t) {
        return t.type === "keypress"
          ? $i(t)
          : t.type === "keydown" || t.type === "keyup"
            ? t.keyCode
            : 0;
      },
    }),
    Eg = me(_g),
    Rg = p({}, nu, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
      tangentialPressure: 0,
      tiltX: 0,
      tiltY: 0,
      twist: 0,
      pointerType: 0,
      isPrimary: 0,
    }),
    ef = me(Rg),
    Tg = p({}, Gl, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
      metaKey: 0,
      ctrlKey: 0,
      shiftKey: 0,
      getModifierState: Fs,
    }),
    xg = me(Tg),
    Ag = p({}, da, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
    Mg = me(Ag),
    zg = p({}, nu, {
      deltaX: function (t) {
        return "deltaX" in t
          ? t.deltaX
          : "wheelDeltaX" in t
            ? -t.wheelDeltaX
            : 0;
      },
      deltaY: function (t) {
        return "deltaY" in t
          ? t.deltaY
          : "wheelDeltaY" in t
            ? -t.wheelDeltaY
            : "wheelDelta" in t
              ? -t.wheelDelta
              : 0;
      },
      deltaZ: 0,
      deltaMode: 0,
    }),
    Og = me(zg),
    Cg = p({}, da, { newState: 0, oldState: 0 }),
    wg = me(Cg),
    Dg = [9, 13, 27, 32],
    Ps = un && "CompositionEvent" in window,
    Vl = null;
  un && "documentMode" in document && (Vl = document.documentMode);
  var Lg = un && "TextEvent" in window && !Vl,
    nf = un && (!Ps || (Vl && 8 < Vl && 11 >= Vl)),
    af = " ",
    lf = !1;
  function uf(t, e) {
    switch (t) {
      case "keyup":
        return Dg.indexOf(e.keyCode) !== -1;
      case "keydown":
        return e.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function sf(t) {
    return (
      (t = t.detail),
      typeof t == "object" && "data" in t ? t.data : null
    );
  }
  var ka = !1;
  function Ug(t, e) {
    switch (t) {
      case "compositionend":
        return sf(e);
      case "keypress":
        return e.which !== 32 ? null : ((lf = !0), af);
      case "textInput":
        return ((t = e.data), t === af && lf ? null : t);
      default:
        return null;
    }
  }
  function Ng(t, e) {
    if (ka)
      return t === "compositionend" || (!Ps && uf(t, e))
        ? ((t = Ic()), (Wi = Zs = Cn = null), (ka = !1), t)
        : null;
    switch (t) {
      case "paste":
        return null;
      case "keypress":
        if (!(e.ctrlKey || e.altKey || e.metaKey) || (e.ctrlKey && e.altKey)) {
          if (e.char && 1 < e.char.length) return e.char;
          if (e.which) return String.fromCharCode(e.which);
        }
        return null;
      case "compositionend":
        return nf && e.locale !== "ko" ? null : e.data;
      default:
        return null;
    }
  }
  var Bg = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0,
  };
  function rf(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return e === "input" ? !!Bg[t.type] : e === "textarea";
  }
  function of(t, e, n, l) {
    (Ka ? (Ja ? Ja.push(l) : (Ja = [l])) : (Ka = l),
      (e = Zu(e, "onChange")),
      0 < e.length &&
        ((n = new eu("onChange", "change", null, n, l)),
        t.push({ event: n, listeners: e })));
  }
  var Ql = null,
    Zl = null;
  function jg(t) {
    Zh(t, 0);
  }
  function au(t) {
    var e = Hl(t);
    if (Vc(e)) return t;
  }
  function cf(t, e) {
    if (t === "change") return e;
  }
  var ff = !1;
  if (un) {
    var Is;
    if (un) {
      var Ws = "oninput" in document;
      if (!Ws) {
        var df = document.createElement("div");
        (df.setAttribute("oninput", "return;"),
          (Ws = typeof df.oninput == "function"));
      }
      Is = Ws;
    } else Is = !1;
    ff = Is && (!document.documentMode || 9 < document.documentMode);
  }
  function hf() {
    Ql && (Ql.detachEvent("onpropertychange", mf), (Zl = Ql = null));
  }
  function mf(t) {
    if (t.propertyName === "value" && au(Zl)) {
      var e = [];
      (of(e, Zl, t, Xs(t)), Pc(jg, e));
    }
  }
  function Hg(t, e, n) {
    t === "focusin"
      ? (hf(), (Ql = e), (Zl = n), Ql.attachEvent("onpropertychange", mf))
      : t === "focusout" && hf();
  }
  function qg(t) {
    if (t === "selectionchange" || t === "keyup" || t === "keydown")
      return au(Zl);
  }
  function Yg(t, e) {
    if (t === "click") return au(e);
  }
  function Gg(t, e) {
    if (t === "input" || t === "change") return au(e);
  }
  function Xg(t, e) {
    return (t === e && (t !== 0 || 1 / t === 1 / e)) || (t !== t && e !== e);
  }
  var xe = typeof Object.is == "function" ? Object.is : Xg;
  function Kl(t, e) {
    if (xe(t, e)) return !0;
    if (
      typeof t != "object" ||
      t === null ||
      typeof e != "object" ||
      e === null
    )
      return !1;
    var n = Object.keys(t),
      l = Object.keys(e);
    if (n.length !== l.length) return !1;
    for (l = 0; l < n.length; l++) {
      var s = n[l];
      if (!oa.call(e, s) || !xe(t[s], e[s])) return !1;
    }
    return !0;
  }
  function yf(t) {
    for (; t && t.firstChild; ) t = t.firstChild;
    return t;
  }
  function pf(t, e) {
    var n = yf(t);
    t = 0;
    for (var l; n; ) {
      if (n.nodeType === 3) {
        if (((l = t + n.textContent.length), t <= e && l >= e))
          return { node: n, offset: e - t };
        t = l;
      }
      t: {
        for (; n; ) {
          if (n.nextSibling) {
            n = n.nextSibling;
            break t;
          }
          n = n.parentNode;
        }
        n = void 0;
      }
      n = yf(n);
    }
  }
  function gf(t, e) {
    return t && e
      ? t === e
        ? !0
        : t && t.nodeType === 3
          ? !1
          : e && e.nodeType === 3
            ? gf(t, e.parentNode)
            : "contains" in t
              ? t.contains(e)
              : t.compareDocumentPosition
                ? !!(t.compareDocumentPosition(e) & 16)
                : !1
      : !1;
  }
  function vf(t) {
    t =
      t != null &&
      t.ownerDocument != null &&
      t.ownerDocument.defaultView != null
        ? t.ownerDocument.defaultView
        : window;
    for (var e = Pi(t.document); e instanceof t.HTMLIFrameElement; ) {
      try {
        var n = typeof e.contentWindow.location.href == "string";
      } catch {
        n = !1;
      }
      if (n) t = e.contentWindow;
      else break;
      e = Pi(t.document);
    }
    return e;
  }
  function $s(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return (
      e &&
      ((e === "input" &&
        (t.type === "text" ||
          t.type === "search" ||
          t.type === "tel" ||
          t.type === "url" ||
          t.type === "password")) ||
        e === "textarea" ||
        t.contentEditable === "true")
    );
  }
  var Vg = un && "documentMode" in document && 11 >= document.documentMode,
    Fa = null,
    tr = null,
    Jl = null,
    er = !1;
  function Sf(t, e, n) {
    var l =
      n.window === n ? n.document : n.nodeType === 9 ? n : n.ownerDocument;
    er ||
      Fa == null ||
      Fa !== Pi(l) ||
      ((l = Fa),
      "selectionStart" in l && $s(l)
        ? (l = { start: l.selectionStart, end: l.selectionEnd })
        : ((l = (
            (l.ownerDocument && l.ownerDocument.defaultView) ||
            window
          ).getSelection()),
          (l = {
            anchorNode: l.anchorNode,
            anchorOffset: l.anchorOffset,
            focusNode: l.focusNode,
            focusOffset: l.focusOffset,
          })),
      (Jl && Kl(Jl, l)) ||
        ((Jl = l),
        (l = Zu(tr, "onSelect")),
        0 < l.length &&
          ((e = new eu("onSelect", "select", null, e, n)),
          t.push({ event: e, listeners: l }),
          (e.target = Fa))));
  }
  function ha(t, e) {
    var n = {};
    return (
      (n[t.toLowerCase()] = e.toLowerCase()),
      (n["Webkit" + t] = "webkit" + e),
      (n["Moz" + t] = "moz" + e),
      n
    );
  }
  var Pa = {
      animationend: ha("Animation", "AnimationEnd"),
      animationiteration: ha("Animation", "AnimationIteration"),
      animationstart: ha("Animation", "AnimationStart"),
      transitionrun: ha("Transition", "TransitionRun"),
      transitionstart: ha("Transition", "TransitionStart"),
      transitioncancel: ha("Transition", "TransitionCancel"),
      transitionend: ha("Transition", "TransitionEnd"),
    },
    nr = {},
    bf = {};
  un &&
    ((bf = document.createElement("div").style),
    "AnimationEvent" in window ||
      (delete Pa.animationend.animation,
      delete Pa.animationiteration.animation,
      delete Pa.animationstart.animation),
    "TransitionEvent" in window || delete Pa.transitionend.transition);
  function ma(t) {
    if (nr[t]) return nr[t];
    if (!Pa[t]) return t;
    var e = Pa[t],
      n;
    for (n in e) if (e.hasOwnProperty(n) && n in bf) return (nr[t] = e[n]);
    return t;
  }
  var _f = ma("animationend"),
    Ef = ma("animationiteration"),
    Rf = ma("animationstart"),
    Qg = ma("transitionrun"),
    Zg = ma("transitionstart"),
    Kg = ma("transitioncancel"),
    Tf = ma("transitionend"),
    xf = new Map(),
    ar =
      "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
        " ",
      );
  ar.push("scrollEnd");
  function Ve(t, e) {
    (xf.set(t, e), fa(e, [t]));
  }
  var lu =
      typeof reportError == "function"
        ? reportError
        : function (t) {
            if (
              typeof window == "object" &&
              typeof window.ErrorEvent == "function"
            ) {
              var e = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof t == "object" &&
                  t !== null &&
                  typeof t.message == "string"
                    ? String(t.message)
                    : String(t),
                error: t,
              });
              if (!window.dispatchEvent(e)) return;
            } else if (
              typeof process == "object" &&
              typeof process.emit == "function"
            ) {
              process.emit("uncaughtException", t);
              return;
            }
            console.error(t);
          },
    Ue = [],
    Ia = 0,
    lr = 0;
  function iu() {
    for (var t = Ia, e = (lr = Ia = 0); e < t; ) {
      var n = Ue[e];
      Ue[e++] = null;
      var l = Ue[e];
      Ue[e++] = null;
      var s = Ue[e];
      Ue[e++] = null;
      var o = Ue[e];
      if (((Ue[e++] = null), l !== null && s !== null)) {
        var d = l.pending;
        (d === null ? (s.next = s) : ((s.next = d.next), (d.next = s)),
          (l.pending = s));
      }
      o !== 0 && Af(n, s, o);
    }
  }
  function uu(t, e, n, l) {
    ((Ue[Ia++] = t),
      (Ue[Ia++] = e),
      (Ue[Ia++] = n),
      (Ue[Ia++] = l),
      (lr |= l),
      (t.lanes |= l),
      (t = t.alternate),
      t !== null && (t.lanes |= l));
  }
  function ir(t, e, n, l) {
    return (uu(t, e, n, l), su(t));
  }
  function ya(t, e) {
    return (uu(t, null, null, e), su(t));
  }
  function Af(t, e, n) {
    t.lanes |= n;
    var l = t.alternate;
    l !== null && (l.lanes |= n);
    for (var s = !1, o = t.return; o !== null; )
      ((o.childLanes |= n),
        (l = o.alternate),
        l !== null && (l.childLanes |= n),
        o.tag === 22 &&
          ((t = o.stateNode), t === null || t._visibility & 1 || (s = !0)),
        (t = o),
        (o = o.return));
    return t.tag === 3
      ? ((o = t.stateNode),
        s &&
          e !== null &&
          ((s = 31 - Te(n)),
          (t = o.hiddenUpdates),
          (l = t[s]),
          l === null ? (t[s] = [e]) : l.push(e),
          (e.lane = n | 536870912)),
        o)
      : null;
  }
  function su(t) {
    if (50 < yi) throw ((yi = 0), (yo = null), Error(r(185)));
    for (var e = t.return; e !== null; ) ((t = e), (e = t.return));
    return t.tag === 3 ? t.stateNode : null;
  }
  var Wa = {};
  function Jg(t, e, n, l) {
    ((this.tag = t),
      (this.key = n),
      (this.sibling =
        this.child =
        this.return =
        this.stateNode =
        this.type =
        this.elementType =
          null),
      (this.index = 0),
      (this.refCleanup = this.ref = null),
      (this.pendingProps = e),
      (this.dependencies =
        this.memoizedState =
        this.updateQueue =
        this.memoizedProps =
          null),
      (this.mode = l),
      (this.subtreeFlags = this.flags = 0),
      (this.deletions = null),
      (this.childLanes = this.lanes = 0),
      (this.alternate = null));
  }
  function Ae(t, e, n, l) {
    return new Jg(t, e, n, l);
  }
  function ur(t) {
    return ((t = t.prototype), !(!t || !t.isReactComponent));
  }
  function sn(t, e) {
    var n = t.alternate;
    return (
      n === null
        ? ((n = Ae(t.tag, e, t.key, t.mode)),
          (n.elementType = t.elementType),
          (n.type = t.type),
          (n.stateNode = t.stateNode),
          (n.alternate = t),
          (t.alternate = n))
        : ((n.pendingProps = e),
          (n.type = t.type),
          (n.flags = 0),
          (n.subtreeFlags = 0),
          (n.deletions = null)),
      (n.flags = t.flags & 65011712),
      (n.childLanes = t.childLanes),
      (n.lanes = t.lanes),
      (n.child = t.child),
      (n.memoizedProps = t.memoizedProps),
      (n.memoizedState = t.memoizedState),
      (n.updateQueue = t.updateQueue),
      (e = t.dependencies),
      (n.dependencies =
        e === null ? null : { lanes: e.lanes, firstContext: e.firstContext }),
      (n.sibling = t.sibling),
      (n.index = t.index),
      (n.ref = t.ref),
      (n.refCleanup = t.refCleanup),
      n
    );
  }
  function Mf(t, e) {
    t.flags &= 65011714;
    var n = t.alternate;
    return (
      n === null
        ? ((t.childLanes = 0),
          (t.lanes = e),
          (t.child = null),
          (t.subtreeFlags = 0),
          (t.memoizedProps = null),
          (t.memoizedState = null),
          (t.updateQueue = null),
          (t.dependencies = null),
          (t.stateNode = null))
        : ((t.childLanes = n.childLanes),
          (t.lanes = n.lanes),
          (t.child = n.child),
          (t.subtreeFlags = 0),
          (t.deletions = null),
          (t.memoizedProps = n.memoizedProps),
          (t.memoizedState = n.memoizedState),
          (t.updateQueue = n.updateQueue),
          (t.type = n.type),
          (e = n.dependencies),
          (t.dependencies =
            e === null
              ? null
              : { lanes: e.lanes, firstContext: e.firstContext })),
      t
    );
  }
  function ru(t, e, n, l, s, o) {
    var d = 0;
    if (((l = t), typeof t == "function")) ur(t) && (d = 1);
    else if (typeof t == "string")
      d = Wv(t, n, I.current)
        ? 26
        : t === "html" || t === "head" || t === "body"
          ? 27
          : 5;
    else
      t: switch (t) {
        case P:
          return ((t = Ae(31, n, e, s)), (t.elementType = P), (t.lanes = o), t);
        case L:
          return pa(n.children, s, o, e);
        case x:
          ((d = 8), (s |= 24));
          break;
        case C:
          return (
            (t = Ae(12, n, e, s | 2)),
            (t.elementType = C),
            (t.lanes = o),
            t
          );
        case W:
          return ((t = Ae(13, n, e, s)), (t.elementType = W), (t.lanes = o), t);
        case F:
          return ((t = Ae(19, n, e, s)), (t.elementType = F), (t.lanes = o), t);
        default:
          if (typeof t == "object" && t !== null)
            switch (t.$$typeof) {
              case Q:
                d = 10;
                break t;
              case Y:
                d = 9;
                break t;
              case H:
                d = 11;
                break t;
              case G:
                d = 14;
                break t;
              case Z:
                ((d = 16), (l = null));
                break t;
            }
          ((d = 29),
            (n = Error(r(130, t === null ? "null" : typeof t, ""))),
            (l = null));
      }
    return (
      (e = Ae(d, n, e, s)),
      (e.elementType = t),
      (e.type = l),
      (e.lanes = o),
      e
    );
  }
  function pa(t, e, n, l) {
    return ((t = Ae(7, t, l, e)), (t.lanes = n), t);
  }
  function sr(t, e, n) {
    return ((t = Ae(6, t, null, e)), (t.lanes = n), t);
  }
  function zf(t) {
    var e = Ae(18, null, null, 0);
    return ((e.stateNode = t), e);
  }
  function rr(t, e, n) {
    return (
      (e = Ae(4, t.children !== null ? t.children : [], t.key, e)),
      (e.lanes = n),
      (e.stateNode = {
        containerInfo: t.containerInfo,
        pendingChildren: null,
        implementation: t.implementation,
      }),
      e
    );
  }
  var Of = new WeakMap();
  function Ne(t, e) {
    if (typeof t == "object" && t !== null) {
      var n = Of.get(t);
      return n !== void 0
        ? n
        : ((e = { value: t, source: e, stack: nn(e) }), Of.set(t, e), e);
    }
    return { value: t, source: e, stack: nn(e) };
  }
  var $a = [],
    tl = 0,
    ou = null,
    kl = 0,
    Be = [],
    je = 0,
    wn = null,
    ke = 1,
    Fe = "";
  function rn(t, e) {
    (($a[tl++] = kl), ($a[tl++] = ou), (ou = t), (kl = e));
  }
  function Cf(t, e, n) {
    ((Be[je++] = ke), (Be[je++] = Fe), (Be[je++] = wn), (wn = t));
    var l = ke;
    t = Fe;
    var s = 32 - Te(l) - 1;
    ((l &= ~(1 << s)), (n += 1));
    var o = 32 - Te(e) + s;
    if (30 < o) {
      var d = s - (s % 5);
      ((o = (l & ((1 << d) - 1)).toString(32)),
        (l >>= d),
        (s -= d),
        (ke = (1 << (32 - Te(e) + s)) | (n << s) | l),
        (Fe = o + t));
    } else ((ke = (1 << o) | (n << s) | l), (Fe = t));
  }
  function or(t) {
    t.return !== null && (rn(t, 1), Cf(t, 1, 0));
  }
  function cr(t) {
    for (; t === ou; )
      ((ou = $a[--tl]), ($a[tl] = null), (kl = $a[--tl]), ($a[tl] = null));
    for (; t === wn; )
      ((wn = Be[--je]),
        (Be[je] = null),
        (Fe = Be[--je]),
        (Be[je] = null),
        (ke = Be[--je]),
        (Be[je] = null));
  }
  function wf(t, e) {
    ((Be[je++] = ke),
      (Be[je++] = Fe),
      (Be[je++] = wn),
      (ke = e.id),
      (Fe = e.overflow),
      (wn = t));
  }
  var ie = null,
    Ut = null,
    gt = !1,
    Dn = null,
    He = !1,
    fr = Error(r(519));
  function Ln(t) {
    var e = Error(
      r(
        418,
        1 < arguments.length && arguments[1] !== void 0 && arguments[1]
          ? "text"
          : "HTML",
        "",
      ),
    );
    throw (Fl(Ne(e, t)), fr);
  }
  function Df(t) {
    var e = t.stateNode,
      n = t.type,
      l = t.memoizedProps;
    switch (((e[le] = t), (e[he] = l), n)) {
      case "dialog":
        (mt("cancel", e), mt("close", e));
        break;
      case "iframe":
      case "object":
      case "embed":
        mt("load", e);
        break;
      case "video":
      case "audio":
        for (n = 0; n < gi.length; n++) mt(gi[n], e);
        break;
      case "source":
        mt("error", e);
        break;
      case "img":
      case "image":
      case "link":
        (mt("error", e), mt("load", e));
        break;
      case "details":
        mt("toggle", e);
        break;
      case "input":
        (mt("invalid", e),
          Qc(
            e,
            l.value,
            l.defaultValue,
            l.checked,
            l.defaultChecked,
            l.type,
            l.name,
            !0,
          ));
        break;
      case "select":
        mt("invalid", e);
        break;
      case "textarea":
        (mt("invalid", e), Kc(e, l.value, l.defaultValue, l.children));
    }
    ((n = l.children),
      (typeof n != "string" && typeof n != "number" && typeof n != "bigint") ||
      e.textContent === "" + n ||
      l.suppressHydrationWarning === !0 ||
      Fh(e.textContent, n)
        ? (l.popover != null && (mt("beforetoggle", e), mt("toggle", e)),
          l.onScroll != null && mt("scroll", e),
          l.onScrollEnd != null && mt("scrollend", e),
          l.onClick != null && (e.onclick = ln),
          (e = !0))
        : (e = !1),
      e || Ln(t, !0));
  }
  function Lf(t) {
    for (ie = t.return; ie; )
      switch (ie.tag) {
        case 5:
        case 31:
        case 13:
          He = !1;
          return;
        case 27:
        case 3:
          He = !0;
          return;
        default:
          ie = ie.return;
      }
  }
  function el(t) {
    if (t !== ie) return !1;
    if (!gt) return (Lf(t), (gt = !0), !1);
    var e = t.tag,
      n;
    if (
      ((n = e !== 3 && e !== 27) &&
        ((n = e === 5) &&
          ((n = t.type),
          (n =
            !(n !== "form" && n !== "button") || Co(t.type, t.memoizedProps))),
        (n = !n)),
      n && Ut && Ln(t),
      Lf(t),
      e === 13)
    ) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
        throw Error(r(317));
      Ut = lm(t);
    } else if (e === 31) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
        throw Error(r(317));
      Ut = lm(t);
    } else
      e === 27
        ? ((e = Ut), Jn(t.type) ? ((t = No), (No = null), (Ut = t)) : (Ut = e))
        : (Ut = ie ? Ye(t.stateNode.nextSibling) : null);
    return !0;
  }
  function ga() {
    ((Ut = ie = null), (gt = !1));
  }
  function dr() {
    var t = Dn;
    return (
      t !== null &&
        (ve === null ? (ve = t) : ve.push.apply(ve, t), (Dn = null)),
      t
    );
  }
  function Fl(t) {
    Dn === null ? (Dn = [t]) : Dn.push(t);
  }
  var hr = M(null),
    va = null,
    on = null;
  function Un(t, e, n) {
    (k(hr, e._currentValue), (e._currentValue = n));
  }
  function cn(t) {
    ((t._currentValue = hr.current), X(hr));
  }
  function mr(t, e, n) {
    for (; t !== null; ) {
      var l = t.alternate;
      if (
        ((t.childLanes & e) !== e
          ? ((t.childLanes |= e), l !== null && (l.childLanes |= e))
          : l !== null && (l.childLanes & e) !== e && (l.childLanes |= e),
        t === n)
      )
        break;
      t = t.return;
    }
  }
  function yr(t, e, n, l) {
    var s = t.child;
    for (s !== null && (s.return = t); s !== null; ) {
      var o = s.dependencies;
      if (o !== null) {
        var d = s.child;
        o = o.firstContext;
        t: for (; o !== null; ) {
          var v = o;
          o = s;
          for (var R = 0; R < e.length; R++)
            if (v.context === e[R]) {
              ((o.lanes |= n),
                (v = o.alternate),
                v !== null && (v.lanes |= n),
                mr(o.return, n, t),
                l || (d = null));
              break t;
            }
          o = v.next;
        }
      } else if (s.tag === 18) {
        if (((d = s.return), d === null)) throw Error(r(341));
        ((d.lanes |= n),
          (o = d.alternate),
          o !== null && (o.lanes |= n),
          mr(d, n, t),
          (d = null));
      } else d = s.child;
      if (d !== null) d.return = s;
      else
        for (d = s; d !== null; ) {
          if (d === t) {
            d = null;
            break;
          }
          if (((s = d.sibling), s !== null)) {
            ((s.return = d.return), (d = s));
            break;
          }
          d = d.return;
        }
      s = d;
    }
  }
  function nl(t, e, n, l) {
    t = null;
    for (var s = e, o = !1; s !== null; ) {
      if (!o) {
        if ((s.flags & 524288) !== 0) o = !0;
        else if ((s.flags & 262144) !== 0) break;
      }
      if (s.tag === 10) {
        var d = s.alternate;
        if (d === null) throw Error(r(387));
        if (((d = d.memoizedProps), d !== null)) {
          var v = s.type;
          xe(s.pendingProps.value, d.value) ||
            (t !== null ? t.push(v) : (t = [v]));
        }
      } else if (s === vt.current) {
        if (((d = s.alternate), d === null)) throw Error(r(387));
        d.memoizedState.memoizedState !== s.memoizedState.memoizedState &&
          (t !== null ? t.push(Ei) : (t = [Ei]));
      }
      s = s.return;
    }
    (t !== null && yr(e, t, n, l), (e.flags |= 262144));
  }
  function cu(t) {
    for (t = t.firstContext; t !== null; ) {
      if (!xe(t.context._currentValue, t.memoizedValue)) return !0;
      t = t.next;
    }
    return !1;
  }
  function Sa(t) {
    ((va = t),
      (on = null),
      (t = t.dependencies),
      t !== null && (t.firstContext = null));
  }
  function ue(t) {
    return Uf(va, t);
  }
  function fu(t, e) {
    return (va === null && Sa(t), Uf(t, e));
  }
  function Uf(t, e) {
    var n = e._currentValue;
    if (((e = { context: e, memoizedValue: n, next: null }), on === null)) {
      if (t === null) throw Error(r(308));
      ((on = e),
        (t.dependencies = { lanes: 0, firstContext: e }),
        (t.flags |= 524288));
    } else on = on.next = e;
    return n;
  }
  var kg =
      typeof AbortController < "u"
        ? AbortController
        : function () {
            var t = [],
              e = (this.signal = {
                aborted: !1,
                addEventListener: function (n, l) {
                  t.push(l);
                },
              });
            this.abort = function () {
              ((e.aborted = !0),
                t.forEach(function (n) {
                  return n();
                }));
            };
          },
    Fg = a.unstable_scheduleCallback,
    Pg = a.unstable_NormalPriority,
    Zt = {
      $$typeof: Q,
      Consumer: null,
      Provider: null,
      _currentValue: null,
      _currentValue2: null,
      _threadCount: 0,
    };
  function pr() {
    return { controller: new kg(), data: new Map(), refCount: 0 };
  }
  function Pl(t) {
    (t.refCount--,
      t.refCount === 0 &&
        Fg(Pg, function () {
          t.controller.abort();
        }));
  }
  var Il = null,
    gr = 0,
    al = 0,
    ll = null;
  function Ig(t, e) {
    if (Il === null) {
      var n = (Il = []);
      ((gr = 0),
        (al = _o()),
        (ll = {
          status: "pending",
          value: void 0,
          then: function (l) {
            n.push(l);
          },
        }));
    }
    return (gr++, e.then(Nf, Nf), e);
  }
  function Nf() {
    if (--gr === 0 && Il !== null) {
      ll !== null && (ll.status = "fulfilled");
      var t = Il;
      ((Il = null), (al = 0), (ll = null));
      for (var e = 0; e < t.length; e++) (0, t[e])();
    }
  }
  function Wg(t, e) {
    var n = [],
      l = {
        status: "pending",
        value: null,
        reason: null,
        then: function (s) {
          n.push(s);
        },
      };
    return (
      t.then(
        function () {
          ((l.status = "fulfilled"), (l.value = e));
          for (var s = 0; s < n.length; s++) (0, n[s])(e);
        },
        function (s) {
          for (l.status = "rejected", l.reason = s, s = 0; s < n.length; s++)
            (0, n[s])(void 0);
        },
      ),
      l
    );
  }
  var Bf = B.S;
  B.S = function (t, e) {
    ((Sh = ce()),
      typeof e == "object" &&
        e !== null &&
        typeof e.then == "function" &&
        Ig(t, e),
      Bf !== null && Bf(t, e));
  };
  var ba = M(null);
  function vr() {
    var t = ba.current;
    return t !== null ? t : wt.pooledCache;
  }
  function du(t, e) {
    e === null ? k(ba, ba.current) : k(ba, e.pool);
  }
  function jf() {
    var t = vr();
    return t === null ? null : { parent: Zt._currentValue, pool: t };
  }
  var il = Error(r(460)),
    Sr = Error(r(474)),
    hu = Error(r(542)),
    mu = { then: function () {} };
  function Hf(t) {
    return ((t = t.status), t === "fulfilled" || t === "rejected");
  }
  function qf(t, e, n) {
    switch (
      ((n = t[n]),
      n === void 0 ? t.push(e) : n !== e && (e.then(ln, ln), (e = n)),
      e.status)
    ) {
      case "fulfilled":
        return e.value;
      case "rejected":
        throw ((t = e.reason), Gf(t), t);
      default:
        if (typeof e.status == "string") e.then(ln, ln);
        else {
          if (((t = wt), t !== null && 100 < t.shellSuspendCounter))
            throw Error(r(482));
          ((t = e),
            (t.status = "pending"),
            t.then(
              function (l) {
                if (e.status === "pending") {
                  var s = e;
                  ((s.status = "fulfilled"), (s.value = l));
                }
              },
              function (l) {
                if (e.status === "pending") {
                  var s = e;
                  ((s.status = "rejected"), (s.reason = l));
                }
              },
            ));
        }
        switch (e.status) {
          case "fulfilled":
            return e.value;
          case "rejected":
            throw ((t = e.reason), Gf(t), t);
        }
        throw ((Ea = e), il);
    }
  }
  function _a(t) {
    try {
      var e = t._init;
      return e(t._payload);
    } catch (n) {
      throw n !== null && typeof n == "object" && typeof n.then == "function"
        ? ((Ea = n), il)
        : n;
    }
  }
  var Ea = null;
  function Yf() {
    if (Ea === null) throw Error(r(459));
    var t = Ea;
    return ((Ea = null), t);
  }
  function Gf(t) {
    if (t === il || t === hu) throw Error(r(483));
  }
  var ul = null,
    Wl = 0;
  function yu(t) {
    var e = Wl;
    return ((Wl += 1), ul === null && (ul = []), qf(ul, t, e));
  }
  function $l(t, e) {
    ((e = e.props.ref), (t.ref = e !== void 0 ? e : null));
  }
  function pu(t, e) {
    throw e.$$typeof === E
      ? Error(r(525))
      : ((t = Object.prototype.toString.call(e)),
        Error(
          r(
            31,
            t === "[object Object]"
              ? "object with keys {" + Object.keys(e).join(", ") + "}"
              : t,
          ),
        ));
  }
  function Xf(t) {
    function e(O, A) {
      if (t) {
        var w = O.deletions;
        w === null ? ((O.deletions = [A]), (O.flags |= 16)) : w.push(A);
      }
    }
    function n(O, A) {
      if (!t) return null;
      for (; A !== null; ) (e(O, A), (A = A.sibling));
      return null;
    }
    function l(O) {
      for (var A = new Map(); O !== null; )
        (O.key !== null ? A.set(O.key, O) : A.set(O.index, O), (O = O.sibling));
      return A;
    }
    function s(O, A) {
      return ((O = sn(O, A)), (O.index = 0), (O.sibling = null), O);
    }
    function o(O, A, w) {
      return (
        (O.index = w),
        t
          ? ((w = O.alternate),
            w !== null
              ? ((w = w.index), w < A ? ((O.flags |= 67108866), A) : w)
              : ((O.flags |= 67108866), A))
          : ((O.flags |= 1048576), A)
      );
    }
    function d(O) {
      return (t && O.alternate === null && (O.flags |= 67108866), O);
    }
    function v(O, A, w, q) {
      return A === null || A.tag !== 6
        ? ((A = sr(w, O.mode, q)), (A.return = O), A)
        : ((A = s(A, w)), (A.return = O), A);
    }
    function R(O, A, w, q) {
      var at = w.type;
      return at === L
        ? j(O, A, w.props.children, q, w.key)
        : A !== null &&
            (A.elementType === at ||
              (typeof at == "object" &&
                at !== null &&
                at.$$typeof === Z &&
                _a(at) === A.type))
          ? ((A = s(A, w.props)), $l(A, w), (A.return = O), A)
          : ((A = ru(w.type, w.key, w.props, null, O.mode, q)),
            $l(A, w),
            (A.return = O),
            A);
    }
    function D(O, A, w, q) {
      return A === null ||
        A.tag !== 4 ||
        A.stateNode.containerInfo !== w.containerInfo ||
        A.stateNode.implementation !== w.implementation
        ? ((A = rr(w, O.mode, q)), (A.return = O), A)
        : ((A = s(A, w.children || [])), (A.return = O), A);
    }
    function j(O, A, w, q, at) {
      return A === null || A.tag !== 7
        ? ((A = pa(w, O.mode, q, at)), (A.return = O), A)
        : ((A = s(A, w)), (A.return = O), A);
    }
    function V(O, A, w) {
      if (
        (typeof A == "string" && A !== "") ||
        typeof A == "number" ||
        typeof A == "bigint"
      )
        return ((A = sr("" + A, O.mode, w)), (A.return = O), A);
      if (typeof A == "object" && A !== null) {
        switch (A.$$typeof) {
          case _:
            return (
              (w = ru(A.type, A.key, A.props, null, O.mode, w)),
              $l(w, A),
              (w.return = O),
              w
            );
          case z:
            return ((A = rr(A, O.mode, w)), (A.return = O), A);
          case Z:
            return ((A = _a(A)), V(O, A, w));
        }
        if (Gt(A) || it(A))
          return ((A = pa(A, O.mode, w, null)), (A.return = O), A);
        if (typeof A.then == "function") return V(O, yu(A), w);
        if (A.$$typeof === Q) return V(O, fu(O, A), w);
        pu(O, A);
      }
      return null;
    }
    function U(O, A, w, q) {
      var at = A !== null ? A.key : null;
      if (
        (typeof w == "string" && w !== "") ||
        typeof w == "number" ||
        typeof w == "bigint"
      )
        return at !== null ? null : v(O, A, "" + w, q);
      if (typeof w == "object" && w !== null) {
        switch (w.$$typeof) {
          case _:
            return w.key === at ? R(O, A, w, q) : null;
          case z:
            return w.key === at ? D(O, A, w, q) : null;
          case Z:
            return ((w = _a(w)), U(O, A, w, q));
        }
        if (Gt(w) || it(w)) return at !== null ? null : j(O, A, w, q, null);
        if (typeof w.then == "function") return U(O, A, yu(w), q);
        if (w.$$typeof === Q) return U(O, A, fu(O, w), q);
        pu(O, w);
      }
      return null;
    }
    function N(O, A, w, q, at) {
      if (
        (typeof q == "string" && q !== "") ||
        typeof q == "number" ||
        typeof q == "bigint"
      )
        return ((O = O.get(w) || null), v(A, O, "" + q, at));
      if (typeof q == "object" && q !== null) {
        switch (q.$$typeof) {
          case _:
            return (
              (O = O.get(q.key === null ? w : q.key) || null),
              R(A, O, q, at)
            );
          case z:
            return (
              (O = O.get(q.key === null ? w : q.key) || null),
              D(A, O, q, at)
            );
          case Z:
            return ((q = _a(q)), N(O, A, w, q, at));
        }
        if (Gt(q) || it(q))
          return ((O = O.get(w) || null), j(A, O, q, at, null));
        if (typeof q.then == "function") return N(O, A, w, yu(q), at);
        if (q.$$typeof === Q) return N(O, A, w, fu(A, q), at);
        pu(A, q);
      }
      return null;
    }
    function $(O, A, w, q) {
      for (
        var at = null, St = null, tt = A, ft = (A = 0), pt = null;
        tt !== null && ft < w.length;
        ft++
      ) {
        tt.index > ft ? ((pt = tt), (tt = null)) : (pt = tt.sibling);
        var bt = U(O, tt, w[ft], q);
        if (bt === null) {
          tt === null && (tt = pt);
          break;
        }
        (t && tt && bt.alternate === null && e(O, tt),
          (A = o(bt, A, ft)),
          St === null ? (at = bt) : (St.sibling = bt),
          (St = bt),
          (tt = pt));
      }
      if (ft === w.length) return (n(O, tt), gt && rn(O, ft), at);
      if (tt === null) {
        for (; ft < w.length; ft++)
          ((tt = V(O, w[ft], q)),
            tt !== null &&
              ((A = o(tt, A, ft)),
              St === null ? (at = tt) : (St.sibling = tt),
              (St = tt)));
        return (gt && rn(O, ft), at);
      }
      for (tt = l(tt); ft < w.length; ft++)
        ((pt = N(tt, O, ft, w[ft], q)),
          pt !== null &&
            (t &&
              pt.alternate !== null &&
              tt.delete(pt.key === null ? ft : pt.key),
            (A = o(pt, A, ft)),
            St === null ? (at = pt) : (St.sibling = pt),
            (St = pt)));
      return (
        t &&
          tt.forEach(function (Wn) {
            return e(O, Wn);
          }),
        gt && rn(O, ft),
        at
      );
    }
    function st(O, A, w, q) {
      if (w == null) throw Error(r(151));
      for (
        var at = null,
          St = null,
          tt = A,
          ft = (A = 0),
          pt = null,
          bt = w.next();
        tt !== null && !bt.done;
        ft++, bt = w.next()
      ) {
        tt.index > ft ? ((pt = tt), (tt = null)) : (pt = tt.sibling);
        var Wn = U(O, tt, bt.value, q);
        if (Wn === null) {
          tt === null && (tt = pt);
          break;
        }
        (t && tt && Wn.alternate === null && e(O, tt),
          (A = o(Wn, A, ft)),
          St === null ? (at = Wn) : (St.sibling = Wn),
          (St = Wn),
          (tt = pt));
      }
      if (bt.done) return (n(O, tt), gt && rn(O, ft), at);
      if (tt === null) {
        for (; !bt.done; ft++, bt = w.next())
          ((bt = V(O, bt.value, q)),
            bt !== null &&
              ((A = o(bt, A, ft)),
              St === null ? (at = bt) : (St.sibling = bt),
              (St = bt)));
        return (gt && rn(O, ft), at);
      }
      for (tt = l(tt); !bt.done; ft++, bt = w.next())
        ((bt = N(tt, O, ft, bt.value, q)),
          bt !== null &&
            (t &&
              bt.alternate !== null &&
              tt.delete(bt.key === null ? ft : bt.key),
            (A = o(bt, A, ft)),
            St === null ? (at = bt) : (St.sibling = bt),
            (St = bt)));
      return (
        t &&
          tt.forEach(function (o0) {
            return e(O, o0);
          }),
        gt && rn(O, ft),
        at
      );
    }
    function Ot(O, A, w, q) {
      if (
        (typeof w == "object" &&
          w !== null &&
          w.type === L &&
          w.key === null &&
          (w = w.props.children),
        typeof w == "object" && w !== null)
      ) {
        switch (w.$$typeof) {
          case _:
            t: {
              for (var at = w.key; A !== null; ) {
                if (A.key === at) {
                  if (((at = w.type), at === L)) {
                    if (A.tag === 7) {
                      (n(O, A.sibling),
                        (q = s(A, w.props.children)),
                        (q.return = O),
                        (O = q));
                      break t;
                    }
                  } else if (
                    A.elementType === at ||
                    (typeof at == "object" &&
                      at !== null &&
                      at.$$typeof === Z &&
                      _a(at) === A.type)
                  ) {
                    (n(O, A.sibling),
                      (q = s(A, w.props)),
                      $l(q, w),
                      (q.return = O),
                      (O = q));
                    break t;
                  }
                  n(O, A);
                  break;
                } else e(O, A);
                A = A.sibling;
              }
              w.type === L
                ? ((q = pa(w.props.children, O.mode, q, w.key)),
                  (q.return = O),
                  (O = q))
                : ((q = ru(w.type, w.key, w.props, null, O.mode, q)),
                  $l(q, w),
                  (q.return = O),
                  (O = q));
            }
            return d(O);
          case z:
            t: {
              for (at = w.key; A !== null; ) {
                if (A.key === at)
                  if (
                    A.tag === 4 &&
                    A.stateNode.containerInfo === w.containerInfo &&
                    A.stateNode.implementation === w.implementation
                  ) {
                    (n(O, A.sibling),
                      (q = s(A, w.children || [])),
                      (q.return = O),
                      (O = q));
                    break t;
                  } else {
                    n(O, A);
                    break;
                  }
                else e(O, A);
                A = A.sibling;
              }
              ((q = rr(w, O.mode, q)), (q.return = O), (O = q));
            }
            return d(O);
          case Z:
            return ((w = _a(w)), Ot(O, A, w, q));
        }
        if (Gt(w)) return $(O, A, w, q);
        if (it(w)) {
          if (((at = it(w)), typeof at != "function")) throw Error(r(150));
          return ((w = at.call(w)), st(O, A, w, q));
        }
        if (typeof w.then == "function") return Ot(O, A, yu(w), q);
        if (w.$$typeof === Q) return Ot(O, A, fu(O, w), q);
        pu(O, w);
      }
      return (typeof w == "string" && w !== "") ||
        typeof w == "number" ||
        typeof w == "bigint"
        ? ((w = "" + w),
          A !== null && A.tag === 6
            ? (n(O, A.sibling), (q = s(A, w)), (q.return = O), (O = q))
            : (n(O, A), (q = sr(w, O.mode, q)), (q.return = O), (O = q)),
          d(O))
        : n(O, A);
    }
    return function (O, A, w, q) {
      try {
        Wl = 0;
        var at = Ot(O, A, w, q);
        return ((ul = null), at);
      } catch (tt) {
        if (tt === il || tt === hu) throw tt;
        var St = Ae(29, tt, null, O.mode);
        return ((St.lanes = q), (St.return = O), St);
      }
    };
  }
  var Ra = Xf(!0),
    Vf = Xf(!1),
    Nn = !1;
  function br(t) {
    t.updateQueue = {
      baseState: t.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null,
    };
  }
  function _r(t, e) {
    ((t = t.updateQueue),
      e.updateQueue === t &&
        (e.updateQueue = {
          baseState: t.baseState,
          firstBaseUpdate: t.firstBaseUpdate,
          lastBaseUpdate: t.lastBaseUpdate,
          shared: t.shared,
          callbacks: null,
        }));
  }
  function Bn(t) {
    return { lane: t, tag: 0, payload: null, callback: null, next: null };
  }
  function jn(t, e, n) {
    var l = t.updateQueue;
    if (l === null) return null;
    if (((l = l.shared), (Rt & 2) !== 0)) {
      var s = l.pending;
      return (
        s === null ? (e.next = e) : ((e.next = s.next), (s.next = e)),
        (l.pending = e),
        (e = su(t)),
        Af(t, null, n),
        e
      );
    }
    return (uu(t, l, e, n), su(t));
  }
  function ti(t, e, n) {
    if (
      ((e = e.updateQueue), e !== null && ((e = e.shared), (n & 4194048) !== 0))
    ) {
      var l = e.lanes;
      ((l &= t.pendingLanes), (n |= l), (e.lanes = n), Lc(t, n));
    }
  }
  function Er(t, e) {
    var n = t.updateQueue,
      l = t.alternate;
    if (l !== null && ((l = l.updateQueue), n === l)) {
      var s = null,
        o = null;
      if (((n = n.firstBaseUpdate), n !== null)) {
        do {
          var d = {
            lane: n.lane,
            tag: n.tag,
            payload: n.payload,
            callback: null,
            next: null,
          };
          (o === null ? (s = o = d) : (o = o.next = d), (n = n.next));
        } while (n !== null);
        o === null ? (s = o = e) : (o = o.next = e);
      } else s = o = e;
      ((n = {
        baseState: l.baseState,
        firstBaseUpdate: s,
        lastBaseUpdate: o,
        shared: l.shared,
        callbacks: l.callbacks,
      }),
        (t.updateQueue = n));
      return;
    }
    ((t = n.lastBaseUpdate),
      t === null ? (n.firstBaseUpdate = e) : (t.next = e),
      (n.lastBaseUpdate = e));
  }
  var Rr = !1;
  function ei() {
    if (Rr) {
      var t = ll;
      if (t !== null) throw t;
    }
  }
  function ni(t, e, n, l) {
    Rr = !1;
    var s = t.updateQueue;
    Nn = !1;
    var o = s.firstBaseUpdate,
      d = s.lastBaseUpdate,
      v = s.shared.pending;
    if (v !== null) {
      s.shared.pending = null;
      var R = v,
        D = R.next;
      ((R.next = null), d === null ? (o = D) : (d.next = D), (d = R));
      var j = t.alternate;
      j !== null &&
        ((j = j.updateQueue),
        (v = j.lastBaseUpdate),
        v !== d &&
          (v === null ? (j.firstBaseUpdate = D) : (v.next = D),
          (j.lastBaseUpdate = R)));
    }
    if (o !== null) {
      var V = s.baseState;
      ((d = 0), (j = D = R = null), (v = o));
      do {
        var U = v.lane & -536870913,
          N = U !== v.lane;
        if (N ? (yt & U) === U : (l & U) === U) {
          (U !== 0 && U === al && (Rr = !0),
            j !== null &&
              (j = j.next =
                {
                  lane: 0,
                  tag: v.tag,
                  payload: v.payload,
                  callback: null,
                  next: null,
                }));
          t: {
            var $ = t,
              st = v;
            U = e;
            var Ot = n;
            switch (st.tag) {
              case 1:
                if ((($ = st.payload), typeof $ == "function")) {
                  V = $.call(Ot, V, U);
                  break t;
                }
                V = $;
                break t;
              case 3:
                $.flags = ($.flags & -65537) | 128;
              case 0:
                if (
                  (($ = st.payload),
                  (U = typeof $ == "function" ? $.call(Ot, V, U) : $),
                  U == null)
                )
                  break t;
                V = p({}, V, U);
                break t;
              case 2:
                Nn = !0;
            }
          }
          ((U = v.callback),
            U !== null &&
              ((t.flags |= 64),
              N && (t.flags |= 8192),
              (N = s.callbacks),
              N === null ? (s.callbacks = [U]) : N.push(U)));
        } else
          ((N = {
            lane: U,
            tag: v.tag,
            payload: v.payload,
            callback: v.callback,
            next: null,
          }),
            j === null ? ((D = j = N), (R = V)) : (j = j.next = N),
            (d |= U));
        if (((v = v.next), v === null)) {
          if (((v = s.shared.pending), v === null)) break;
          ((N = v),
            (v = N.next),
            (N.next = null),
            (s.lastBaseUpdate = N),
            (s.shared.pending = null));
        }
      } while (!0);
      (j === null && (R = V),
        (s.baseState = R),
        (s.firstBaseUpdate = D),
        (s.lastBaseUpdate = j),
        o === null && (s.shared.lanes = 0),
        (Xn |= d),
        (t.lanes = d),
        (t.memoizedState = V));
    }
  }
  function Qf(t, e) {
    if (typeof t != "function") throw Error(r(191, t));
    t.call(e);
  }
  function Zf(t, e) {
    var n = t.callbacks;
    if (n !== null)
      for (t.callbacks = null, t = 0; t < n.length; t++) Qf(n[t], e);
  }
  var sl = M(null),
    gu = M(0);
  function Kf(t, e) {
    ((t = Sn), k(gu, t), k(sl, e), (Sn = t | e.baseLanes));
  }
  function Tr() {
    (k(gu, Sn), k(sl, sl.current));
  }
  function xr() {
    ((Sn = gu.current), X(sl), X(gu));
  }
  var Me = M(null),
    qe = null;
  function Hn(t) {
    var e = t.alternate;
    (k(Vt, Vt.current & 1),
      k(Me, t),
      qe === null &&
        (e === null || sl.current !== null || e.memoizedState !== null) &&
        (qe = t));
  }
  function Ar(t) {
    (k(Vt, Vt.current), k(Me, t), qe === null && (qe = t));
  }
  function Jf(t) {
    t.tag === 22
      ? (k(Vt, Vt.current), k(Me, t), qe === null && (qe = t))
      : qn();
  }
  function qn() {
    (k(Vt, Vt.current), k(Me, Me.current));
  }
  function ze(t) {
    (X(Me), qe === t && (qe = null), X(Vt));
  }
  var Vt = M(0);
  function vu(t) {
    for (var e = t; e !== null; ) {
      if (e.tag === 13) {
        var n = e.memoizedState;
        if (n !== null && ((n = n.dehydrated), n === null || Lo(n) || Uo(n)))
          return e;
      } else if (
        e.tag === 19 &&
        (e.memoizedProps.revealOrder === "forwards" ||
          e.memoizedProps.revealOrder === "backwards" ||
          e.memoizedProps.revealOrder === "unstable_legacy-backwards" ||
          e.memoizedProps.revealOrder === "together")
      ) {
        if ((e.flags & 128) !== 0) return e;
      } else if (e.child !== null) {
        ((e.child.return = e), (e = e.child));
        continue;
      }
      if (e === t) break;
      for (; e.sibling === null; ) {
        if (e.return === null || e.return === t) return null;
        e = e.return;
      }
      ((e.sibling.return = e.return), (e = e.sibling));
    }
    return null;
  }
  var fn = 0,
    ct = null,
    Mt = null,
    Kt = null,
    Su = !1,
    rl = !1,
    Ta = !1,
    bu = 0,
    ai = 0,
    ol = null,
    $g = 0;
  function qt() {
    throw Error(r(321));
  }
  function Mr(t, e) {
    if (e === null) return !1;
    for (var n = 0; n < e.length && n < t.length; n++)
      if (!xe(t[n], e[n])) return !1;
    return !0;
  }
  function zr(t, e, n, l, s, o) {
    return (
      (fn = o),
      (ct = e),
      (e.memoizedState = null),
      (e.updateQueue = null),
      (e.lanes = 0),
      (B.H = t === null || t.memoizedState === null ? Cd : Vr),
      (Ta = !1),
      (o = n(l, s)),
      (Ta = !1),
      rl && (o = Ff(e, n, l, s)),
      kf(t),
      o
    );
  }
  function kf(t) {
    B.H = ui;
    var e = Mt !== null && Mt.next !== null;
    if (((fn = 0), (Kt = Mt = ct = null), (Su = !1), (ai = 0), (ol = null), e))
      throw Error(r(300));
    t === null ||
      Jt ||
      ((t = t.dependencies), t !== null && cu(t) && (Jt = !0));
  }
  function Ff(t, e, n, l) {
    ct = t;
    var s = 0;
    do {
      if ((rl && (ol = null), (ai = 0), (rl = !1), 25 <= s))
        throw Error(r(301));
      if (((s += 1), (Kt = Mt = null), t.updateQueue != null)) {
        var o = t.updateQueue;
        ((o.lastEffect = null),
          (o.events = null),
          (o.stores = null),
          o.memoCache != null && (o.memoCache.index = 0));
      }
      ((B.H = wd), (o = e(n, l)));
    } while (rl);
    return o;
  }
  function tv() {
    var t = B.H,
      e = t.useState()[0];
    return (
      (e = typeof e.then == "function" ? li(e) : e),
      (t = t.useState()[0]),
      (Mt !== null ? Mt.memoizedState : null) !== t && (ct.flags |= 1024),
      e
    );
  }
  function Or() {
    var t = bu !== 0;
    return ((bu = 0), t);
  }
  function Cr(t, e, n) {
    ((e.updateQueue = t.updateQueue), (e.flags &= -2053), (t.lanes &= ~n));
  }
  function wr(t) {
    if (Su) {
      for (t = t.memoizedState; t !== null; ) {
        var e = t.queue;
        (e !== null && (e.pending = null), (t = t.next));
      }
      Su = !1;
    }
    ((fn = 0), (Kt = Mt = ct = null), (rl = !1), (ai = bu = 0), (ol = null));
  }
  function de() {
    var t = {
      memoizedState: null,
      baseState: null,
      baseQueue: null,
      queue: null,
      next: null,
    };
    return (Kt === null ? (ct.memoizedState = Kt = t) : (Kt = Kt.next = t), Kt);
  }
  function Qt() {
    if (Mt === null) {
      var t = ct.alternate;
      t = t !== null ? t.memoizedState : null;
    } else t = Mt.next;
    var e = Kt === null ? ct.memoizedState : Kt.next;
    if (e !== null) ((Kt = e), (Mt = t));
    else {
      if (t === null)
        throw ct.alternate === null ? Error(r(467)) : Error(r(310));
      ((Mt = t),
        (t = {
          memoizedState: Mt.memoizedState,
          baseState: Mt.baseState,
          baseQueue: Mt.baseQueue,
          queue: Mt.queue,
          next: null,
        }),
        Kt === null ? (ct.memoizedState = Kt = t) : (Kt = Kt.next = t));
    }
    return Kt;
  }
  function _u() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function li(t) {
    var e = ai;
    return (
      (ai += 1),
      ol === null && (ol = []),
      (t = qf(ol, t, e)),
      (e = ct),
      (Kt === null ? e.memoizedState : Kt.next) === null &&
        ((e = e.alternate),
        (B.H = e === null || e.memoizedState === null ? Cd : Vr)),
      t
    );
  }
  function Eu(t) {
    if (t !== null && typeof t == "object") {
      if (typeof t.then == "function") return li(t);
      if (t.$$typeof === Q) return ue(t);
    }
    throw Error(r(438, String(t)));
  }
  function Dr(t) {
    var e = null,
      n = ct.updateQueue;
    if ((n !== null && (e = n.memoCache), e == null)) {
      var l = ct.alternate;
      l !== null &&
        ((l = l.updateQueue),
        l !== null &&
          ((l = l.memoCache),
          l != null &&
            (e = {
              data: l.data.map(function (s) {
                return s.slice();
              }),
              index: 0,
            })));
    }
    if (
      (e == null && (e = { data: [], index: 0 }),
      n === null && ((n = _u()), (ct.updateQueue = n)),
      (n.memoCache = e),
      (n = e.data[e.index]),
      n === void 0)
    )
      for (n = e.data[e.index] = Array(t), l = 0; l < t; l++) n[l] = rt;
    return (e.index++, n);
  }
  function dn(t, e) {
    return typeof e == "function" ? e(t) : e;
  }
  function Ru(t) {
    var e = Qt();
    return Lr(e, Mt, t);
  }
  function Lr(t, e, n) {
    var l = t.queue;
    if (l === null) throw Error(r(311));
    l.lastRenderedReducer = n;
    var s = t.baseQueue,
      o = l.pending;
    if (o !== null) {
      if (s !== null) {
        var d = s.next;
        ((s.next = o.next), (o.next = d));
      }
      ((e.baseQueue = s = o), (l.pending = null));
    }
    if (((o = t.baseState), s === null)) t.memoizedState = o;
    else {
      e = s.next;
      var v = (d = null),
        R = null,
        D = e,
        j = !1;
      do {
        var V = D.lane & -536870913;
        if (V !== D.lane ? (yt & V) === V : (fn & V) === V) {
          var U = D.revertLane;
          if (U === 0)
            (R !== null &&
              (R = R.next =
                {
                  lane: 0,
                  revertLane: 0,
                  gesture: null,
                  action: D.action,
                  hasEagerState: D.hasEagerState,
                  eagerState: D.eagerState,
                  next: null,
                }),
              V === al && (j = !0));
          else if ((fn & U) === U) {
            ((D = D.next), U === al && (j = !0));
            continue;
          } else
            ((V = {
              lane: 0,
              revertLane: D.revertLane,
              gesture: null,
              action: D.action,
              hasEagerState: D.hasEagerState,
              eagerState: D.eagerState,
              next: null,
            }),
              R === null ? ((v = R = V), (d = o)) : (R = R.next = V),
              (ct.lanes |= U),
              (Xn |= U));
          ((V = D.action),
            Ta && n(o, V),
            (o = D.hasEagerState ? D.eagerState : n(o, V)));
        } else
          ((U = {
            lane: V,
            revertLane: D.revertLane,
            gesture: D.gesture,
            action: D.action,
            hasEagerState: D.hasEagerState,
            eagerState: D.eagerState,
            next: null,
          }),
            R === null ? ((v = R = U), (d = o)) : (R = R.next = U),
            (ct.lanes |= V),
            (Xn |= V));
        D = D.next;
      } while (D !== null && D !== e);
      if (
        (R === null ? (d = o) : (R.next = v),
        !xe(o, t.memoizedState) && ((Jt = !0), j && ((n = ll), n !== null)))
      )
        throw n;
      ((t.memoizedState = o),
        (t.baseState = d),
        (t.baseQueue = R),
        (l.lastRenderedState = o));
    }
    return (s === null && (l.lanes = 0), [t.memoizedState, l.dispatch]);
  }
  function Ur(t) {
    var e = Qt(),
      n = e.queue;
    if (n === null) throw Error(r(311));
    n.lastRenderedReducer = t;
    var l = n.dispatch,
      s = n.pending,
      o = e.memoizedState;
    if (s !== null) {
      n.pending = null;
      var d = (s = s.next);
      do ((o = t(o, d.action)), (d = d.next));
      while (d !== s);
      (xe(o, e.memoizedState) || (Jt = !0),
        (e.memoizedState = o),
        e.baseQueue === null && (e.baseState = o),
        (n.lastRenderedState = o));
    }
    return [o, l];
  }
  function Pf(t, e, n) {
    var l = ct,
      s = Qt(),
      o = gt;
    if (o) {
      if (n === void 0) throw Error(r(407));
      n = n();
    } else n = e();
    var d = !xe((Mt || s).memoizedState, n);
    if (
      (d && ((s.memoizedState = n), (Jt = !0)),
      (s = s.queue),
      jr($f.bind(null, l, s, t), [t]),
      s.getSnapshot !== e || d || (Kt !== null && Kt.memoizedState.tag & 1))
    ) {
      if (
        ((l.flags |= 2048),
        cl(9, { destroy: void 0 }, Wf.bind(null, l, s, n, e), null),
        wt === null)
      )
        throw Error(r(349));
      o || (fn & 127) !== 0 || If(l, e, n);
    }
    return n;
  }
  function If(t, e, n) {
    ((t.flags |= 16384),
      (t = { getSnapshot: e, value: n }),
      (e = ct.updateQueue),
      e === null
        ? ((e = _u()), (ct.updateQueue = e), (e.stores = [t]))
        : ((n = e.stores), n === null ? (e.stores = [t]) : n.push(t)));
  }
  function Wf(t, e, n, l) {
    ((e.value = n), (e.getSnapshot = l), td(e) && ed(t));
  }
  function $f(t, e, n) {
    return n(function () {
      td(e) && ed(t);
    });
  }
  function td(t) {
    var e = t.getSnapshot;
    t = t.value;
    try {
      var n = e();
      return !xe(t, n);
    } catch {
      return !0;
    }
  }
  function ed(t) {
    var e = ya(t, 2);
    e !== null && Se(e, t, 2);
  }
  function Nr(t) {
    var e = de();
    if (typeof t == "function") {
      var n = t;
      if (((t = n()), Ta)) {
        zn(!0);
        try {
          n();
        } finally {
          zn(!1);
        }
      }
    }
    return (
      (e.memoizedState = e.baseState = t),
      (e.queue = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: dn,
        lastRenderedState: t,
      }),
      e
    );
  }
  function nd(t, e, n, l) {
    return ((t.baseState = n), Lr(t, Mt, typeof l == "function" ? l : dn));
  }
  function ev(t, e, n, l, s) {
    if (Au(t)) throw Error(r(485));
    if (((t = e.action), t !== null)) {
      var o = {
        payload: s,
        action: t,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function (d) {
          o.listeners.push(d);
        },
      };
      (B.T !== null ? n(!0) : (o.isTransition = !1),
        l(o),
        (n = e.pending),
        n === null
          ? ((o.next = e.pending = o), ad(e, o))
          : ((o.next = n.next), (e.pending = n.next = o)));
    }
  }
  function ad(t, e) {
    var n = e.action,
      l = e.payload,
      s = t.state;
    if (e.isTransition) {
      var o = B.T,
        d = {};
      B.T = d;
      try {
        var v = n(s, l),
          R = B.S;
        (R !== null && R(d, v), ld(t, e, v));
      } catch (D) {
        Br(t, e, D);
      } finally {
        (o !== null && d.types !== null && (o.types = d.types), (B.T = o));
      }
    } else
      try {
        ((o = n(s, l)), ld(t, e, o));
      } catch (D) {
        Br(t, e, D);
      }
  }
  function ld(t, e, n) {
    n !== null && typeof n == "object" && typeof n.then == "function"
      ? n.then(
          function (l) {
            id(t, e, l);
          },
          function (l) {
            return Br(t, e, l);
          },
        )
      : id(t, e, n);
  }
  function id(t, e, n) {
    ((e.status = "fulfilled"),
      (e.value = n),
      ud(e),
      (t.state = n),
      (e = t.pending),
      e !== null &&
        ((n = e.next),
        n === e ? (t.pending = null) : ((n = n.next), (e.next = n), ad(t, n))));
  }
  function Br(t, e, n) {
    var l = t.pending;
    if (((t.pending = null), l !== null)) {
      l = l.next;
      do ((e.status = "rejected"), (e.reason = n), ud(e), (e = e.next));
      while (e !== l);
    }
    t.action = null;
  }
  function ud(t) {
    t = t.listeners;
    for (var e = 0; e < t.length; e++) (0, t[e])();
  }
  function sd(t, e) {
    return e;
  }
  function rd(t, e) {
    if (gt) {
      var n = wt.formState;
      if (n !== null) {
        t: {
          var l = ct;
          if (gt) {
            if (Ut) {
              e: {
                for (var s = Ut, o = He; s.nodeType !== 8; ) {
                  if (!o) {
                    s = null;
                    break e;
                  }
                  if (((s = Ye(s.nextSibling)), s === null)) {
                    s = null;
                    break e;
                  }
                }
                ((o = s.data), (s = o === "F!" || o === "F" ? s : null));
              }
              if (s) {
                ((Ut = Ye(s.nextSibling)), (l = s.data === "F!"));
                break t;
              }
            }
            Ln(l);
          }
          l = !1;
        }
        l && (e = n[0]);
      }
    }
    return (
      (n = de()),
      (n.memoizedState = n.baseState = e),
      (l = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: sd,
        lastRenderedState: e,
      }),
      (n.queue = l),
      (n = Md.bind(null, ct, l)),
      (l.dispatch = n),
      (l = Nr(!1)),
      (o = Xr.bind(null, ct, !1, l.queue)),
      (l = de()),
      (s = { state: e, dispatch: null, action: t, pending: null }),
      (l.queue = s),
      (n = ev.bind(null, ct, s, o, n)),
      (s.dispatch = n),
      (l.memoizedState = t),
      [e, n, !1]
    );
  }
  function od(t) {
    var e = Qt();
    return cd(e, Mt, t);
  }
  function cd(t, e, n) {
    if (
      ((e = Lr(t, e, sd)[0]),
      (t = Ru(dn)[0]),
      typeof e == "object" && e !== null && typeof e.then == "function")
    )
      try {
        var l = li(e);
      } catch (d) {
        throw d === il ? hu : d;
      }
    else l = e;
    e = Qt();
    var s = e.queue,
      o = s.dispatch;
    return (
      n !== e.memoizedState &&
        ((ct.flags |= 2048),
        cl(9, { destroy: void 0 }, nv.bind(null, s, n), null)),
      [l, o, t]
    );
  }
  function nv(t, e) {
    t.action = e;
  }
  function fd(t) {
    var e = Qt(),
      n = Mt;
    if (n !== null) return cd(e, n, t);
    (Qt(), (e = e.memoizedState), (n = Qt()));
    var l = n.queue.dispatch;
    return ((n.memoizedState = t), [e, l, !1]);
  }
  function cl(t, e, n, l) {
    return (
      (t = { tag: t, create: n, deps: l, inst: e, next: null }),
      (e = ct.updateQueue),
      e === null && ((e = _u()), (ct.updateQueue = e)),
      (n = e.lastEffect),
      n === null
        ? (e.lastEffect = t.next = t)
        : ((l = n.next), (n.next = t), (t.next = l), (e.lastEffect = t)),
      t
    );
  }
  function dd() {
    return Qt().memoizedState;
  }
  function Tu(t, e, n, l) {
    var s = de();
    ((ct.flags |= t),
      (s.memoizedState = cl(
        1 | e,
        { destroy: void 0 },
        n,
        l === void 0 ? null : l,
      )));
  }
  function xu(t, e, n, l) {
    var s = Qt();
    l = l === void 0 ? null : l;
    var o = s.memoizedState.inst;
    Mt !== null && l !== null && Mr(l, Mt.memoizedState.deps)
      ? (s.memoizedState = cl(e, o, n, l))
      : ((ct.flags |= t), (s.memoizedState = cl(1 | e, o, n, l)));
  }
  function hd(t, e) {
    Tu(8390656, 8, t, e);
  }
  function jr(t, e) {
    xu(2048, 8, t, e);
  }
  function av(t) {
    ct.flags |= 4;
    var e = ct.updateQueue;
    if (e === null) ((e = _u()), (ct.updateQueue = e), (e.events = [t]));
    else {
      var n = e.events;
      n === null ? (e.events = [t]) : n.push(t);
    }
  }
  function md(t) {
    var e = Qt().memoizedState;
    return (
      av({ ref: e, nextImpl: t }),
      function () {
        if ((Rt & 2) !== 0) throw Error(r(440));
        return e.impl.apply(void 0, arguments);
      }
    );
  }
  function yd(t, e) {
    return xu(4, 2, t, e);
  }
  function pd(t, e) {
    return xu(4, 4, t, e);
  }
  function gd(t, e) {
    if (typeof e == "function") {
      t = t();
      var n = e(t);
      return function () {
        typeof n == "function" ? n() : e(null);
      };
    }
    if (e != null)
      return (
        (t = t()),
        (e.current = t),
        function () {
          e.current = null;
        }
      );
  }
  function vd(t, e, n) {
    ((n = n != null ? n.concat([t]) : null), xu(4, 4, gd.bind(null, e, t), n));
  }
  function Hr() {}
  function Sd(t, e) {
    var n = Qt();
    e = e === void 0 ? null : e;
    var l = n.memoizedState;
    return e !== null && Mr(e, l[1]) ? l[0] : ((n.memoizedState = [t, e]), t);
  }
  function bd(t, e) {
    var n = Qt();
    e = e === void 0 ? null : e;
    var l = n.memoizedState;
    if (e !== null && Mr(e, l[1])) return l[0];
    if (((l = t()), Ta)) {
      zn(!0);
      try {
        t();
      } finally {
        zn(!1);
      }
    }
    return ((n.memoizedState = [l, e]), l);
  }
  function qr(t, e, n) {
    return n === void 0 || ((fn & 1073741824) !== 0 && (yt & 261930) === 0)
      ? (t.memoizedState = e)
      : ((t.memoizedState = n), (t = _h()), (ct.lanes |= t), (Xn |= t), n);
  }
  function _d(t, e, n, l) {
    return xe(n, e)
      ? n
      : sl.current !== null
        ? ((t = qr(t, n, l)), xe(t, e) || (Jt = !0), t)
        : (fn & 42) === 0 || ((fn & 1073741824) !== 0 && (yt & 261930) === 0)
          ? ((Jt = !0), (t.memoizedState = n))
          : ((t = _h()), (ct.lanes |= t), (Xn |= t), e);
  }
  function Ed(t, e, n, l, s) {
    var o = J.p;
    J.p = o !== 0 && 8 > o ? o : 8;
    var d = B.T,
      v = {};
    ((B.T = v), Xr(t, !1, e, n));
    try {
      var R = s(),
        D = B.S;
      if (
        (D !== null && D(v, R),
        R !== null && typeof R == "object" && typeof R.then == "function")
      ) {
        var j = Wg(R, l);
        ii(t, e, j, we(t));
      } else ii(t, e, l, we(t));
    } catch (V) {
      ii(t, e, { then: function () {}, status: "rejected", reason: V }, we());
    } finally {
      ((J.p = o),
        d !== null && v.types !== null && (d.types = v.types),
        (B.T = d));
    }
  }
  function lv() {}
  function Yr(t, e, n, l) {
    if (t.tag !== 5) throw Error(r(476));
    var s = Rd(t).queue;
    Ed(
      t,
      s,
      e,
      lt,
      n === null
        ? lv
        : function () {
            return (Td(t), n(l));
          },
    );
  }
  function Rd(t) {
    var e = t.memoizedState;
    if (e !== null) return e;
    e = {
      memoizedState: lt,
      baseState: lt,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: dn,
        lastRenderedState: lt,
      },
      next: null,
    };
    var n = {};
    return (
      (e.next = {
        memoizedState: n,
        baseState: n,
        baseQueue: null,
        queue: {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: dn,
          lastRenderedState: n,
        },
        next: null,
      }),
      (t.memoizedState = e),
      (t = t.alternate),
      t !== null && (t.memoizedState = e),
      e
    );
  }
  function Td(t) {
    var e = Rd(t);
    (e.next === null && (e = t.alternate.memoizedState),
      ii(t, e.next.queue, {}, we()));
  }
  function Gr() {
    return ue(Ei);
  }
  function xd() {
    return Qt().memoizedState;
  }
  function Ad() {
    return Qt().memoizedState;
  }
  function iv(t) {
    for (var e = t.return; e !== null; ) {
      switch (e.tag) {
        case 24:
        case 3:
          var n = we();
          t = Bn(n);
          var l = jn(e, t, n);
          (l !== null && (Se(l, e, n), ti(l, e, n)),
            (e = { cache: pr() }),
            (t.payload = e));
          return;
      }
      e = e.return;
    }
  }
  function uv(t, e, n) {
    var l = we();
    ((n = {
      lane: l,
      revertLane: 0,
      gesture: null,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
      Au(t)
        ? zd(e, n)
        : ((n = ir(t, e, n, l)), n !== null && (Se(n, t, l), Od(n, e, l))));
  }
  function Md(t, e, n) {
    var l = we();
    ii(t, e, n, l);
  }
  function ii(t, e, n, l) {
    var s = {
      lane: l,
      revertLane: 0,
      gesture: null,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    };
    if (Au(t)) zd(e, s);
    else {
      var o = t.alternate;
      if (
        t.lanes === 0 &&
        (o === null || o.lanes === 0) &&
        ((o = e.lastRenderedReducer), o !== null)
      )
        try {
          var d = e.lastRenderedState,
            v = o(d, n);
          if (((s.hasEagerState = !0), (s.eagerState = v), xe(v, d)))
            return (uu(t, e, s, 0), wt === null && iu(), !1);
        } catch {}
      if (((n = ir(t, e, s, l)), n !== null))
        return (Se(n, t, l), Od(n, e, l), !0);
    }
    return !1;
  }
  function Xr(t, e, n, l) {
    if (
      ((l = {
        lane: 2,
        revertLane: _o(),
        gesture: null,
        action: l,
        hasEagerState: !1,
        eagerState: null,
        next: null,
      }),
      Au(t))
    ) {
      if (e) throw Error(r(479));
    } else ((e = ir(t, n, l, 2)), e !== null && Se(e, t, 2));
  }
  function Au(t) {
    var e = t.alternate;
    return t === ct || (e !== null && e === ct);
  }
  function zd(t, e) {
    rl = Su = !0;
    var n = t.pending;
    (n === null ? (e.next = e) : ((e.next = n.next), (n.next = e)),
      (t.pending = e));
  }
  function Od(t, e, n) {
    if ((n & 4194048) !== 0) {
      var l = e.lanes;
      ((l &= t.pendingLanes), (n |= l), (e.lanes = n), Lc(t, n));
    }
  }
  var ui = {
    readContext: ue,
    use: Eu,
    useCallback: qt,
    useContext: qt,
    useEffect: qt,
    useImperativeHandle: qt,
    useLayoutEffect: qt,
    useInsertionEffect: qt,
    useMemo: qt,
    useReducer: qt,
    useRef: qt,
    useState: qt,
    useDebugValue: qt,
    useDeferredValue: qt,
    useTransition: qt,
    useSyncExternalStore: qt,
    useId: qt,
    useHostTransitionStatus: qt,
    useFormState: qt,
    useActionState: qt,
    useOptimistic: qt,
    useMemoCache: qt,
    useCacheRefresh: qt,
  };
  ui.useEffectEvent = qt;
  var Cd = {
      readContext: ue,
      use: Eu,
      useCallback: function (t, e) {
        return ((de().memoizedState = [t, e === void 0 ? null : e]), t);
      },
      useContext: ue,
      useEffect: hd,
      useImperativeHandle: function (t, e, n) {
        ((n = n != null ? n.concat([t]) : null),
          Tu(4194308, 4, gd.bind(null, e, t), n));
      },
      useLayoutEffect: function (t, e) {
        return Tu(4194308, 4, t, e);
      },
      useInsertionEffect: function (t, e) {
        Tu(4, 2, t, e);
      },
      useMemo: function (t, e) {
        var n = de();
        e = e === void 0 ? null : e;
        var l = t();
        if (Ta) {
          zn(!0);
          try {
            t();
          } finally {
            zn(!1);
          }
        }
        return ((n.memoizedState = [l, e]), l);
      },
      useReducer: function (t, e, n) {
        var l = de();
        if (n !== void 0) {
          var s = n(e);
          if (Ta) {
            zn(!0);
            try {
              n(e);
            } finally {
              zn(!1);
            }
          }
        } else s = e;
        return (
          (l.memoizedState = l.baseState = s),
          (t = {
            pending: null,
            lanes: 0,
            dispatch: null,
            lastRenderedReducer: t,
            lastRenderedState: s,
          }),
          (l.queue = t),
          (t = t.dispatch = uv.bind(null, ct, t)),
          [l.memoizedState, t]
        );
      },
      useRef: function (t) {
        var e = de();
        return ((t = { current: t }), (e.memoizedState = t));
      },
      useState: function (t) {
        t = Nr(t);
        var e = t.queue,
          n = Md.bind(null, ct, e);
        return ((e.dispatch = n), [t.memoizedState, n]);
      },
      useDebugValue: Hr,
      useDeferredValue: function (t, e) {
        var n = de();
        return qr(n, t, e);
      },
      useTransition: function () {
        var t = Nr(!1);
        return (
          (t = Ed.bind(null, ct, t.queue, !0, !1)),
          (de().memoizedState = t),
          [!1, t]
        );
      },
      useSyncExternalStore: function (t, e, n) {
        var l = ct,
          s = de();
        if (gt) {
          if (n === void 0) throw Error(r(407));
          n = n();
        } else {
          if (((n = e()), wt === null)) throw Error(r(349));
          (yt & 127) !== 0 || If(l, e, n);
        }
        s.memoizedState = n;
        var o = { value: n, getSnapshot: e };
        return (
          (s.queue = o),
          hd($f.bind(null, l, o, t), [t]),
          (l.flags |= 2048),
          cl(9, { destroy: void 0 }, Wf.bind(null, l, o, n, e), null),
          n
        );
      },
      useId: function () {
        var t = de(),
          e = wt.identifierPrefix;
        if (gt) {
          var n = Fe,
            l = ke;
          ((n = (l & ~(1 << (32 - Te(l) - 1))).toString(32) + n),
            (e = "_" + e + "R_" + n),
            (n = bu++),
            0 < n && (e += "H" + n.toString(32)),
            (e += "_"));
        } else ((n = $g++), (e = "_" + e + "r_" + n.toString(32) + "_"));
        return (t.memoizedState = e);
      },
      useHostTransitionStatus: Gr,
      useFormState: rd,
      useActionState: rd,
      useOptimistic: function (t) {
        var e = de();
        e.memoizedState = e.baseState = t;
        var n = {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: null,
          lastRenderedState: null,
        };
        return (
          (e.queue = n),
          (e = Xr.bind(null, ct, !0, n)),
          (n.dispatch = e),
          [t, e]
        );
      },
      useMemoCache: Dr,
      useCacheRefresh: function () {
        return (de().memoizedState = iv.bind(null, ct));
      },
      useEffectEvent: function (t) {
        var e = de(),
          n = { impl: t };
        return (
          (e.memoizedState = n),
          function () {
            if ((Rt & 2) !== 0) throw Error(r(440));
            return n.impl.apply(void 0, arguments);
          }
        );
      },
    },
    Vr = {
      readContext: ue,
      use: Eu,
      useCallback: Sd,
      useContext: ue,
      useEffect: jr,
      useImperativeHandle: vd,
      useInsertionEffect: yd,
      useLayoutEffect: pd,
      useMemo: bd,
      useReducer: Ru,
      useRef: dd,
      useState: function () {
        return Ru(dn);
      },
      useDebugValue: Hr,
      useDeferredValue: function (t, e) {
        var n = Qt();
        return _d(n, Mt.memoizedState, t, e);
      },
      useTransition: function () {
        var t = Ru(dn)[0],
          e = Qt().memoizedState;
        return [typeof t == "boolean" ? t : li(t), e];
      },
      useSyncExternalStore: Pf,
      useId: xd,
      useHostTransitionStatus: Gr,
      useFormState: od,
      useActionState: od,
      useOptimistic: function (t, e) {
        var n = Qt();
        return nd(n, Mt, t, e);
      },
      useMemoCache: Dr,
      useCacheRefresh: Ad,
    };
  Vr.useEffectEvent = md;
  var wd = {
    readContext: ue,
    use: Eu,
    useCallback: Sd,
    useContext: ue,
    useEffect: jr,
    useImperativeHandle: vd,
    useInsertionEffect: yd,
    useLayoutEffect: pd,
    useMemo: bd,
    useReducer: Ur,
    useRef: dd,
    useState: function () {
      return Ur(dn);
    },
    useDebugValue: Hr,
    useDeferredValue: function (t, e) {
      var n = Qt();
      return Mt === null ? qr(n, t, e) : _d(n, Mt.memoizedState, t, e);
    },
    useTransition: function () {
      var t = Ur(dn)[0],
        e = Qt().memoizedState;
      return [typeof t == "boolean" ? t : li(t), e];
    },
    useSyncExternalStore: Pf,
    useId: xd,
    useHostTransitionStatus: Gr,
    useFormState: fd,
    useActionState: fd,
    useOptimistic: function (t, e) {
      var n = Qt();
      return Mt !== null
        ? nd(n, Mt, t, e)
        : ((n.baseState = t), [t, n.queue.dispatch]);
    },
    useMemoCache: Dr,
    useCacheRefresh: Ad,
  };
  wd.useEffectEvent = md;
  function Qr(t, e, n, l) {
    ((e = t.memoizedState),
      (n = n(l, e)),
      (n = n == null ? e : p({}, e, n)),
      (t.memoizedState = n),
      t.lanes === 0 && (t.updateQueue.baseState = n));
  }
  var Zr = {
    enqueueSetState: function (t, e, n) {
      t = t._reactInternals;
      var l = we(),
        s = Bn(l);
      ((s.payload = e),
        n != null && (s.callback = n),
        (e = jn(t, s, l)),
        e !== null && (Se(e, t, l), ti(e, t, l)));
    },
    enqueueReplaceState: function (t, e, n) {
      t = t._reactInternals;
      var l = we(),
        s = Bn(l);
      ((s.tag = 1),
        (s.payload = e),
        n != null && (s.callback = n),
        (e = jn(t, s, l)),
        e !== null && (Se(e, t, l), ti(e, t, l)));
    },
    enqueueForceUpdate: function (t, e) {
      t = t._reactInternals;
      var n = we(),
        l = Bn(n);
      ((l.tag = 2),
        e != null && (l.callback = e),
        (e = jn(t, l, n)),
        e !== null && (Se(e, t, n), ti(e, t, n)));
    },
  };
  function Dd(t, e, n, l, s, o, d) {
    return (
      (t = t.stateNode),
      typeof t.shouldComponentUpdate == "function"
        ? t.shouldComponentUpdate(l, o, d)
        : e.prototype && e.prototype.isPureReactComponent
          ? !Kl(n, l) || !Kl(s, o)
          : !0
    );
  }
  function Ld(t, e, n, l) {
    ((t = e.state),
      typeof e.componentWillReceiveProps == "function" &&
        e.componentWillReceiveProps(n, l),
      typeof e.UNSAFE_componentWillReceiveProps == "function" &&
        e.UNSAFE_componentWillReceiveProps(n, l),
      e.state !== t && Zr.enqueueReplaceState(e, e.state, null));
  }
  function xa(t, e) {
    var n = e;
    if ("ref" in e) {
      n = {};
      for (var l in e) l !== "ref" && (n[l] = e[l]);
    }
    if ((t = t.defaultProps)) {
      n === e && (n = p({}, n));
      for (var s in t) n[s] === void 0 && (n[s] = t[s]);
    }
    return n;
  }
  function Ud(t) {
    lu(t);
  }
  function Nd(t) {
    console.error(t);
  }
  function Bd(t) {
    lu(t);
  }
  function Mu(t, e) {
    try {
      var n = t.onUncaughtError;
      n(e.value, { componentStack: e.stack });
    } catch (l) {
      setTimeout(function () {
        throw l;
      });
    }
  }
  function jd(t, e, n) {
    try {
      var l = t.onCaughtError;
      l(n.value, {
        componentStack: n.stack,
        errorBoundary: e.tag === 1 ? e.stateNode : null,
      });
    } catch (s) {
      setTimeout(function () {
        throw s;
      });
    }
  }
  function Kr(t, e, n) {
    return (
      (n = Bn(n)),
      (n.tag = 3),
      (n.payload = { element: null }),
      (n.callback = function () {
        Mu(t, e);
      }),
      n
    );
  }
  function Hd(t) {
    return ((t = Bn(t)), (t.tag = 3), t);
  }
  function qd(t, e, n, l) {
    var s = n.type.getDerivedStateFromError;
    if (typeof s == "function") {
      var o = l.value;
      ((t.payload = function () {
        return s(o);
      }),
        (t.callback = function () {
          jd(e, n, l);
        }));
    }
    var d = n.stateNode;
    d !== null &&
      typeof d.componentDidCatch == "function" &&
      (t.callback = function () {
        (jd(e, n, l),
          typeof s != "function" &&
            (Vn === null ? (Vn = new Set([this])) : Vn.add(this)));
        var v = l.stack;
        this.componentDidCatch(l.value, {
          componentStack: v !== null ? v : "",
        });
      });
  }
  function sv(t, e, n, l, s) {
    if (
      ((n.flags |= 32768),
      l !== null && typeof l == "object" && typeof l.then == "function")
    ) {
      if (
        ((e = n.alternate),
        e !== null && nl(e, n, s, !0),
        (n = Me.current),
        n !== null)
      ) {
        switch (n.tag) {
          case 31:
          case 13:
            return (
              qe === null ? qu() : n.alternate === null && Yt === 0 && (Yt = 3),
              (n.flags &= -257),
              (n.flags |= 65536),
              (n.lanes = s),
              l === mu
                ? (n.flags |= 16384)
                : ((e = n.updateQueue),
                  e === null ? (n.updateQueue = new Set([l])) : e.add(l),
                  vo(t, l, s)),
              !1
            );
          case 22:
            return (
              (n.flags |= 65536),
              l === mu
                ? (n.flags |= 16384)
                : ((e = n.updateQueue),
                  e === null
                    ? ((e = {
                        transitions: null,
                        markerInstances: null,
                        retryQueue: new Set([l]),
                      }),
                      (n.updateQueue = e))
                    : ((n = e.retryQueue),
                      n === null ? (e.retryQueue = new Set([l])) : n.add(l)),
                  vo(t, l, s)),
              !1
            );
        }
        throw Error(r(435, n.tag));
      }
      return (vo(t, l, s), qu(), !1);
    }
    if (gt)
      return (
        (e = Me.current),
        e !== null
          ? ((e.flags & 65536) === 0 && (e.flags |= 256),
            (e.flags |= 65536),
            (e.lanes = s),
            l !== fr && ((t = Error(r(422), { cause: l })), Fl(Ne(t, n))))
          : (l !== fr && ((e = Error(r(423), { cause: l })), Fl(Ne(e, n))),
            (t = t.current.alternate),
            (t.flags |= 65536),
            (s &= -s),
            (t.lanes |= s),
            (l = Ne(l, n)),
            (s = Kr(t.stateNode, l, s)),
            Er(t, s),
            Yt !== 4 && (Yt = 2)),
        !1
      );
    var o = Error(r(520), { cause: l });
    if (
      ((o = Ne(o, n)),
      mi === null ? (mi = [o]) : mi.push(o),
      Yt !== 4 && (Yt = 2),
      e === null)
    )
      return !0;
    ((l = Ne(l, n)), (n = e));
    do {
      switch (n.tag) {
        case 3:
          return (
            (n.flags |= 65536),
            (t = s & -s),
            (n.lanes |= t),
            (t = Kr(n.stateNode, l, t)),
            Er(n, t),
            !1
          );
        case 1:
          if (
            ((e = n.type),
            (o = n.stateNode),
            (n.flags & 128) === 0 &&
              (typeof e.getDerivedStateFromError == "function" ||
                (o !== null &&
                  typeof o.componentDidCatch == "function" &&
                  (Vn === null || !Vn.has(o)))))
          )
            return (
              (n.flags |= 65536),
              (s &= -s),
              (n.lanes |= s),
              (s = Hd(s)),
              qd(s, t, n, l),
              Er(n, s),
              !1
            );
      }
      n = n.return;
    } while (n !== null);
    return !1;
  }
  var Jr = Error(r(461)),
    Jt = !1;
  function se(t, e, n, l) {
    e.child = t === null ? Vf(e, null, n, l) : Ra(e, t.child, n, l);
  }
  function Yd(t, e, n, l, s) {
    n = n.render;
    var o = e.ref;
    if ("ref" in l) {
      var d = {};
      for (var v in l) v !== "ref" && (d[v] = l[v]);
    } else d = l;
    return (
      Sa(e),
      (l = zr(t, e, n, d, o, s)),
      (v = Or()),
      t !== null && !Jt
        ? (Cr(t, e, s), hn(t, e, s))
        : (gt && v && or(e), (e.flags |= 1), se(t, e, l, s), e.child)
    );
  }
  function Gd(t, e, n, l, s) {
    if (t === null) {
      var o = n.type;
      return typeof o == "function" &&
        !ur(o) &&
        o.defaultProps === void 0 &&
        n.compare === null
        ? ((e.tag = 15), (e.type = o), Xd(t, e, o, l, s))
        : ((t = ru(n.type, null, l, e, e.mode, s)),
          (t.ref = e.ref),
          (t.return = e),
          (e.child = t));
    }
    if (((o = t.child), !eo(t, s))) {
      var d = o.memoizedProps;
      if (
        ((n = n.compare), (n = n !== null ? n : Kl), n(d, l) && t.ref === e.ref)
      )
        return hn(t, e, s);
    }
    return (
      (e.flags |= 1),
      (t = sn(o, l)),
      (t.ref = e.ref),
      (t.return = e),
      (e.child = t)
    );
  }
  function Xd(t, e, n, l, s) {
    if (t !== null) {
      var o = t.memoizedProps;
      if (Kl(o, l) && t.ref === e.ref)
        if (((Jt = !1), (e.pendingProps = l = o), eo(t, s)))
          (t.flags & 131072) !== 0 && (Jt = !0);
        else return ((e.lanes = t.lanes), hn(t, e, s));
    }
    return kr(t, e, n, l, s);
  }
  function Vd(t, e, n, l) {
    var s = l.children,
      o = t !== null ? t.memoizedState : null;
    if (
      (t === null &&
        e.stateNode === null &&
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      l.mode === "hidden")
    ) {
      if ((e.flags & 128) !== 0) {
        if (((o = o !== null ? o.baseLanes | n : n), t !== null)) {
          for (l = e.child = t.child, s = 0; l !== null; )
            ((s = s | l.lanes | l.childLanes), (l = l.sibling));
          l = s & ~o;
        } else ((l = 0), (e.child = null));
        return Qd(t, e, o, n, l);
      }
      if ((n & 536870912) !== 0)
        ((e.memoizedState = { baseLanes: 0, cachePool: null }),
          t !== null && du(e, o !== null ? o.cachePool : null),
          o !== null ? Kf(e, o) : Tr(),
          Jf(e));
      else
        return (
          (l = e.lanes = 536870912),
          Qd(t, e, o !== null ? o.baseLanes | n : n, n, l)
        );
    } else
      o !== null
        ? (du(e, o.cachePool), Kf(e, o), qn(), (e.memoizedState = null))
        : (t !== null && du(e, null), Tr(), qn());
    return (se(t, e, s, n), e.child);
  }
  function si(t, e) {
    return (
      (t !== null && t.tag === 22) ||
        e.stateNode !== null ||
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      e.sibling
    );
  }
  function Qd(t, e, n, l, s) {
    var o = vr();
    return (
      (o = o === null ? null : { parent: Zt._currentValue, pool: o }),
      (e.memoizedState = { baseLanes: n, cachePool: o }),
      t !== null && du(e, null),
      Tr(),
      Jf(e),
      t !== null && nl(t, e, l, !0),
      (e.childLanes = s),
      null
    );
  }
  function zu(t, e) {
    return (
      (e = Cu({ mode: e.mode, children: e.children }, t.mode)),
      (e.ref = t.ref),
      (t.child = e),
      (e.return = t),
      e
    );
  }
  function Zd(t, e, n) {
    return (
      Ra(e, t.child, null, n),
      (t = zu(e, e.pendingProps)),
      (t.flags |= 2),
      ze(e),
      (e.memoizedState = null),
      t
    );
  }
  function rv(t, e, n) {
    var l = e.pendingProps,
      s = (e.flags & 128) !== 0;
    if (((e.flags &= -129), t === null)) {
      if (gt) {
        if (l.mode === "hidden")
          return ((t = zu(e, l)), (e.lanes = 536870912), si(null, t));
        if (
          (Ar(e),
          (t = Ut)
            ? ((t = am(t, He)),
              (t = t !== null && t.data === "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: wn !== null ? { id: ke, overflow: Fe } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (n = zf(t)),
                (n.return = e),
                (e.child = n),
                (ie = e),
                (Ut = null)))
            : (t = null),
          t === null)
        )
          throw Ln(e);
        return ((e.lanes = 536870912), null);
      }
      return zu(e, l);
    }
    var o = t.memoizedState;
    if (o !== null) {
      var d = o.dehydrated;
      if ((Ar(e), s))
        if (e.flags & 256) ((e.flags &= -257), (e = Zd(t, e, n)));
        else if (e.memoizedState !== null)
          ((e.child = t.child), (e.flags |= 128), (e = null));
        else throw Error(r(558));
      else if (
        (Jt || nl(t, e, n, !1), (s = (n & t.childLanes) !== 0), Jt || s)
      ) {
        if (
          ((l = wt),
          l !== null && ((d = Uc(l, n)), d !== 0 && d !== o.retryLane))
        )
          throw ((o.retryLane = d), ya(t, d), Se(l, t, d), Jr);
        (qu(), (e = Zd(t, e, n)));
      } else
        ((t = o.treeContext),
          (Ut = Ye(d.nextSibling)),
          (ie = e),
          (gt = !0),
          (Dn = null),
          (He = !1),
          t !== null && wf(e, t),
          (e = zu(e, l)),
          (e.flags |= 4096));
      return e;
    }
    return (
      (t = sn(t.child, { mode: l.mode, children: l.children })),
      (t.ref = e.ref),
      (e.child = t),
      (t.return = e),
      t
    );
  }
  function Ou(t, e) {
    var n = e.ref;
    if (n === null) t !== null && t.ref !== null && (e.flags |= 4194816);
    else {
      if (typeof n != "function" && typeof n != "object") throw Error(r(284));
      (t === null || t.ref !== n) && (e.flags |= 4194816);
    }
  }
  function kr(t, e, n, l, s) {
    return (
      Sa(e),
      (n = zr(t, e, n, l, void 0, s)),
      (l = Or()),
      t !== null && !Jt
        ? (Cr(t, e, s), hn(t, e, s))
        : (gt && l && or(e), (e.flags |= 1), se(t, e, n, s), e.child)
    );
  }
  function Kd(t, e, n, l, s, o) {
    return (
      Sa(e),
      (e.updateQueue = null),
      (n = Ff(e, l, n, s)),
      kf(t),
      (l = Or()),
      t !== null && !Jt
        ? (Cr(t, e, o), hn(t, e, o))
        : (gt && l && or(e), (e.flags |= 1), se(t, e, n, o), e.child)
    );
  }
  function Jd(t, e, n, l, s) {
    if ((Sa(e), e.stateNode === null)) {
      var o = Wa,
        d = n.contextType;
      (typeof d == "object" && d !== null && (o = ue(d)),
        (o = new n(l, o)),
        (e.memoizedState =
          o.state !== null && o.state !== void 0 ? o.state : null),
        (o.updater = Zr),
        (e.stateNode = o),
        (o._reactInternals = e),
        (o = e.stateNode),
        (o.props = l),
        (o.state = e.memoizedState),
        (o.refs = {}),
        br(e),
        (d = n.contextType),
        (o.context = typeof d == "object" && d !== null ? ue(d) : Wa),
        (o.state = e.memoizedState),
        (d = n.getDerivedStateFromProps),
        typeof d == "function" && (Qr(e, n, d, l), (o.state = e.memoizedState)),
        typeof n.getDerivedStateFromProps == "function" ||
          typeof o.getSnapshotBeforeUpdate == "function" ||
          (typeof o.UNSAFE_componentWillMount != "function" &&
            typeof o.componentWillMount != "function") ||
          ((d = o.state),
          typeof o.componentWillMount == "function" && o.componentWillMount(),
          typeof o.UNSAFE_componentWillMount == "function" &&
            o.UNSAFE_componentWillMount(),
          d !== o.state && Zr.enqueueReplaceState(o, o.state, null),
          ni(e, l, o, s),
          ei(),
          (o.state = e.memoizedState)),
        typeof o.componentDidMount == "function" && (e.flags |= 4194308),
        (l = !0));
    } else if (t === null) {
      o = e.stateNode;
      var v = e.memoizedProps,
        R = xa(n, v);
      o.props = R;
      var D = o.context,
        j = n.contextType;
      ((d = Wa), typeof j == "object" && j !== null && (d = ue(j)));
      var V = n.getDerivedStateFromProps;
      ((j =
        typeof V == "function" ||
        typeof o.getSnapshotBeforeUpdate == "function"),
        (v = e.pendingProps !== v),
        j ||
          (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
            typeof o.componentWillReceiveProps != "function") ||
          ((v || D !== d) && Ld(e, o, l, d)),
        (Nn = !1));
      var U = e.memoizedState;
      ((o.state = U),
        ni(e, l, o, s),
        ei(),
        (D = e.memoizedState),
        v || U !== D || Nn
          ? (typeof V == "function" && (Qr(e, n, V, l), (D = e.memoizedState)),
            (R = Nn || Dd(e, n, R, l, U, D, d))
              ? (j ||
                  (typeof o.UNSAFE_componentWillMount != "function" &&
                    typeof o.componentWillMount != "function") ||
                  (typeof o.componentWillMount == "function" &&
                    o.componentWillMount(),
                  typeof o.UNSAFE_componentWillMount == "function" &&
                    o.UNSAFE_componentWillMount()),
                typeof o.componentDidMount == "function" &&
                  (e.flags |= 4194308))
              : (typeof o.componentDidMount == "function" &&
                  (e.flags |= 4194308),
                (e.memoizedProps = l),
                (e.memoizedState = D)),
            (o.props = l),
            (o.state = D),
            (o.context = d),
            (l = R))
          : (typeof o.componentDidMount == "function" && (e.flags |= 4194308),
            (l = !1)));
    } else {
      ((o = e.stateNode),
        _r(t, e),
        (d = e.memoizedProps),
        (j = xa(n, d)),
        (o.props = j),
        (V = e.pendingProps),
        (U = o.context),
        (D = n.contextType),
        (R = Wa),
        typeof D == "object" && D !== null && (R = ue(D)),
        (v = n.getDerivedStateFromProps),
        (D =
          typeof v == "function" ||
          typeof o.getSnapshotBeforeUpdate == "function") ||
          (typeof o.UNSAFE_componentWillReceiveProps != "function" &&
            typeof o.componentWillReceiveProps != "function") ||
          ((d !== V || U !== R) && Ld(e, o, l, R)),
        (Nn = !1),
        (U = e.memoizedState),
        (o.state = U),
        ni(e, l, o, s),
        ei());
      var N = e.memoizedState;
      d !== V ||
      U !== N ||
      Nn ||
      (t !== null && t.dependencies !== null && cu(t.dependencies))
        ? (typeof v == "function" && (Qr(e, n, v, l), (N = e.memoizedState)),
          (j =
            Nn ||
            Dd(e, n, j, l, U, N, R) ||
            (t !== null && t.dependencies !== null && cu(t.dependencies)))
            ? (D ||
                (typeof o.UNSAFE_componentWillUpdate != "function" &&
                  typeof o.componentWillUpdate != "function") ||
                (typeof o.componentWillUpdate == "function" &&
                  o.componentWillUpdate(l, N, R),
                typeof o.UNSAFE_componentWillUpdate == "function" &&
                  o.UNSAFE_componentWillUpdate(l, N, R)),
              typeof o.componentDidUpdate == "function" && (e.flags |= 4),
              typeof o.getSnapshotBeforeUpdate == "function" &&
                (e.flags |= 1024))
            : (typeof o.componentDidUpdate != "function" ||
                (d === t.memoizedProps && U === t.memoizedState) ||
                (e.flags |= 4),
              typeof o.getSnapshotBeforeUpdate != "function" ||
                (d === t.memoizedProps && U === t.memoizedState) ||
                (e.flags |= 1024),
              (e.memoizedProps = l),
              (e.memoizedState = N)),
          (o.props = l),
          (o.state = N),
          (o.context = R),
          (l = j))
        : (typeof o.componentDidUpdate != "function" ||
            (d === t.memoizedProps && U === t.memoizedState) ||
            (e.flags |= 4),
          typeof o.getSnapshotBeforeUpdate != "function" ||
            (d === t.memoizedProps && U === t.memoizedState) ||
            (e.flags |= 1024),
          (l = !1));
    }
    return (
      (o = l),
      Ou(t, e),
      (l = (e.flags & 128) !== 0),
      o || l
        ? ((o = e.stateNode),
          (n =
            l && typeof n.getDerivedStateFromError != "function"
              ? null
              : o.render()),
          (e.flags |= 1),
          t !== null && l
            ? ((e.child = Ra(e, t.child, null, s)),
              (e.child = Ra(e, null, n, s)))
            : se(t, e, n, s),
          (e.memoizedState = o.state),
          (t = e.child))
        : (t = hn(t, e, s)),
      t
    );
  }
  function kd(t, e, n, l) {
    return (ga(), (e.flags |= 256), se(t, e, n, l), e.child);
  }
  var Fr = {
    dehydrated: null,
    treeContext: null,
    retryLane: 0,
    hydrationErrors: null,
  };
  function Pr(t) {
    return { baseLanes: t, cachePool: jf() };
  }
  function Ir(t, e, n) {
    return ((t = t !== null ? t.childLanes & ~n : 0), e && (t |= Ce), t);
  }
  function Fd(t, e, n) {
    var l = e.pendingProps,
      s = !1,
      o = (e.flags & 128) !== 0,
      d;
    if (
      ((d = o) ||
        (d =
          t !== null && t.memoizedState === null ? !1 : (Vt.current & 2) !== 0),
      d && ((s = !0), (e.flags &= -129)),
      (d = (e.flags & 32) !== 0),
      (e.flags &= -33),
      t === null)
    ) {
      if (gt) {
        if (
          (s ? Hn(e) : qn(),
          (t = Ut)
            ? ((t = am(t, He)),
              (t = t !== null && t.data !== "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: wn !== null ? { id: ke, overflow: Fe } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (n = zf(t)),
                (n.return = e),
                (e.child = n),
                (ie = e),
                (Ut = null)))
            : (t = null),
          t === null)
        )
          throw Ln(e);
        return (Uo(t) ? (e.lanes = 32) : (e.lanes = 536870912), null);
      }
      var v = l.children;
      return (
        (l = l.fallback),
        s
          ? (qn(),
            (s = e.mode),
            (v = Cu({ mode: "hidden", children: v }, s)),
            (l = pa(l, s, n, null)),
            (v.return = e),
            (l.return = e),
            (v.sibling = l),
            (e.child = v),
            (l = e.child),
            (l.memoizedState = Pr(n)),
            (l.childLanes = Ir(t, d, n)),
            (e.memoizedState = Fr),
            si(null, l))
          : (Hn(e), Wr(e, v))
      );
    }
    var R = t.memoizedState;
    if (R !== null && ((v = R.dehydrated), v !== null)) {
      if (o)
        e.flags & 256
          ? (Hn(e), (e.flags &= -257), (e = $r(t, e, n)))
          : e.memoizedState !== null
            ? (qn(), (e.child = t.child), (e.flags |= 128), (e = null))
            : (qn(),
              (v = l.fallback),
              (s = e.mode),
              (l = Cu({ mode: "visible", children: l.children }, s)),
              (v = pa(v, s, n, null)),
              (v.flags |= 2),
              (l.return = e),
              (v.return = e),
              (l.sibling = v),
              (e.child = l),
              Ra(e, t.child, null, n),
              (l = e.child),
              (l.memoizedState = Pr(n)),
              (l.childLanes = Ir(t, d, n)),
              (e.memoizedState = Fr),
              (e = si(null, l)));
      else if ((Hn(e), Uo(v))) {
        if (((d = v.nextSibling && v.nextSibling.dataset), d)) var D = d.dgst;
        ((d = D),
          (l = Error(r(419))),
          (l.stack = ""),
          (l.digest = d),
          Fl({ value: l, source: null, stack: null }),
          (e = $r(t, e, n)));
      } else if (
        (Jt || nl(t, e, n, !1), (d = (n & t.childLanes) !== 0), Jt || d)
      ) {
        if (
          ((d = wt),
          d !== null && ((l = Uc(d, n)), l !== 0 && l !== R.retryLane))
        )
          throw ((R.retryLane = l), ya(t, l), Se(d, t, l), Jr);
        (Lo(v) || qu(), (e = $r(t, e, n)));
      } else
        Lo(v)
          ? ((e.flags |= 192), (e.child = t.child), (e = null))
          : ((t = R.treeContext),
            (Ut = Ye(v.nextSibling)),
            (ie = e),
            (gt = !0),
            (Dn = null),
            (He = !1),
            t !== null && wf(e, t),
            (e = Wr(e, l.children)),
            (e.flags |= 4096));
      return e;
    }
    return s
      ? (qn(),
        (v = l.fallback),
        (s = e.mode),
        (R = t.child),
        (D = R.sibling),
        (l = sn(R, { mode: "hidden", children: l.children })),
        (l.subtreeFlags = R.subtreeFlags & 65011712),
        D !== null ? (v = sn(D, v)) : ((v = pa(v, s, n, null)), (v.flags |= 2)),
        (v.return = e),
        (l.return = e),
        (l.sibling = v),
        (e.child = l),
        si(null, l),
        (l = e.child),
        (v = t.child.memoizedState),
        v === null
          ? (v = Pr(n))
          : ((s = v.cachePool),
            s !== null
              ? ((R = Zt._currentValue),
                (s = s.parent !== R ? { parent: R, pool: R } : s))
              : (s = jf()),
            (v = { baseLanes: v.baseLanes | n, cachePool: s })),
        (l.memoizedState = v),
        (l.childLanes = Ir(t, d, n)),
        (e.memoizedState = Fr),
        si(t.child, l))
      : (Hn(e),
        (n = t.child),
        (t = n.sibling),
        (n = sn(n, { mode: "visible", children: l.children })),
        (n.return = e),
        (n.sibling = null),
        t !== null &&
          ((d = e.deletions),
          d === null ? ((e.deletions = [t]), (e.flags |= 16)) : d.push(t)),
        (e.child = n),
        (e.memoizedState = null),
        n);
  }
  function Wr(t, e) {
    return (
      (e = Cu({ mode: "visible", children: e }, t.mode)),
      (e.return = t),
      (t.child = e)
    );
  }
  function Cu(t, e) {
    return ((t = Ae(22, t, null, e)), (t.lanes = 0), t);
  }
  function $r(t, e, n) {
    return (
      Ra(e, t.child, null, n),
      (t = Wr(e, e.pendingProps.children)),
      (t.flags |= 2),
      (e.memoizedState = null),
      t
    );
  }
  function Pd(t, e, n) {
    t.lanes |= e;
    var l = t.alternate;
    (l !== null && (l.lanes |= e), mr(t.return, e, n));
  }
  function to(t, e, n, l, s, o) {
    var d = t.memoizedState;
    d === null
      ? (t.memoizedState = {
          isBackwards: e,
          rendering: null,
          renderingStartTime: 0,
          last: l,
          tail: n,
          tailMode: s,
          treeForkCount: o,
        })
      : ((d.isBackwards = e),
        (d.rendering = null),
        (d.renderingStartTime = 0),
        (d.last = l),
        (d.tail = n),
        (d.tailMode = s),
        (d.treeForkCount = o));
  }
  function Id(t, e, n) {
    var l = e.pendingProps,
      s = l.revealOrder,
      o = l.tail;
    l = l.children;
    var d = Vt.current,
      v = (d & 2) !== 0;
    if (
      (v ? ((d = (d & 1) | 2), (e.flags |= 128)) : (d &= 1),
      k(Vt, d),
      se(t, e, l, n),
      (l = gt ? kl : 0),
      !v && t !== null && (t.flags & 128) !== 0)
    )
      t: for (t = e.child; t !== null; ) {
        if (t.tag === 13) t.memoizedState !== null && Pd(t, n, e);
        else if (t.tag === 19) Pd(t, n, e);
        else if (t.child !== null) {
          ((t.child.return = t), (t = t.child));
          continue;
        }
        if (t === e) break t;
        for (; t.sibling === null; ) {
          if (t.return === null || t.return === e) break t;
          t = t.return;
        }
        ((t.sibling.return = t.return), (t = t.sibling));
      }
    switch (s) {
      case "forwards":
        for (n = e.child, s = null; n !== null; )
          ((t = n.alternate),
            t !== null && vu(t) === null && (s = n),
            (n = n.sibling));
        ((n = s),
          n === null
            ? ((s = e.child), (e.child = null))
            : ((s = n.sibling), (n.sibling = null)),
          to(e, !1, s, n, o, l));
        break;
      case "backwards":
      case "unstable_legacy-backwards":
        for (n = null, s = e.child, e.child = null; s !== null; ) {
          if (((t = s.alternate), t !== null && vu(t) === null)) {
            e.child = s;
            break;
          }
          ((t = s.sibling), (s.sibling = n), (n = s), (s = t));
        }
        to(e, !0, n, null, o, l);
        break;
      case "together":
        to(e, !1, null, null, void 0, l);
        break;
      default:
        e.memoizedState = null;
    }
    return e.child;
  }
  function hn(t, e, n) {
    if (
      (t !== null && (e.dependencies = t.dependencies),
      (Xn |= e.lanes),
      (n & e.childLanes) === 0)
    )
      if (t !== null) {
        if ((nl(t, e, n, !1), (n & e.childLanes) === 0)) return null;
      } else return null;
    if (t !== null && e.child !== t.child) throw Error(r(153));
    if (e.child !== null) {
      for (
        t = e.child, n = sn(t, t.pendingProps), e.child = n, n.return = e;
        t.sibling !== null;
      )
        ((t = t.sibling),
          (n = n.sibling = sn(t, t.pendingProps)),
          (n.return = e));
      n.sibling = null;
    }
    return e.child;
  }
  function eo(t, e) {
    return (t.lanes & e) !== 0
      ? !0
      : ((t = t.dependencies), !!(t !== null && cu(t)));
  }
  function ov(t, e, n) {
    switch (e.tag) {
      case 3:
        (Xt(e, e.stateNode.containerInfo),
          Un(e, Zt, t.memoizedState.cache),
          ga());
        break;
      case 27:
      case 5:
        tn(e);
        break;
      case 4:
        Xt(e, e.stateNode.containerInfo);
        break;
      case 10:
        Un(e, e.type, e.memoizedProps.value);
        break;
      case 31:
        if (e.memoizedState !== null) return ((e.flags |= 128), Ar(e), null);
        break;
      case 13:
        var l = e.memoizedState;
        if (l !== null)
          return l.dehydrated !== null
            ? (Hn(e), (e.flags |= 128), null)
            : (n & e.child.childLanes) !== 0
              ? Fd(t, e, n)
              : (Hn(e), (t = hn(t, e, n)), t !== null ? t.sibling : null);
        Hn(e);
        break;
      case 19:
        var s = (t.flags & 128) !== 0;
        if (
          ((l = (n & e.childLanes) !== 0),
          l || (nl(t, e, n, !1), (l = (n & e.childLanes) !== 0)),
          s)
        ) {
          if (l) return Id(t, e, n);
          e.flags |= 128;
        }
        if (
          ((s = e.memoizedState),
          s !== null &&
            ((s.rendering = null), (s.tail = null), (s.lastEffect = null)),
          k(Vt, Vt.current),
          l)
        )
          break;
        return null;
      case 22:
        return ((e.lanes = 0), Vd(t, e, n, e.pendingProps));
      case 24:
        Un(e, Zt, t.memoizedState.cache);
    }
    return hn(t, e, n);
  }
  function Wd(t, e, n) {
    if (t !== null)
      if (t.memoizedProps !== e.pendingProps) Jt = !0;
      else {
        if (!eo(t, n) && (e.flags & 128) === 0) return ((Jt = !1), ov(t, e, n));
        Jt = (t.flags & 131072) !== 0;
      }
    else ((Jt = !1), gt && (e.flags & 1048576) !== 0 && Cf(e, kl, e.index));
    switch (((e.lanes = 0), e.tag)) {
      case 16:
        t: {
          var l = e.pendingProps;
          if (((t = _a(e.elementType)), (e.type = t), typeof t == "function"))
            ur(t)
              ? ((l = xa(t, l)), (e.tag = 1), (e = Jd(null, e, t, l, n)))
              : ((e.tag = 0), (e = kr(null, e, t, l, n)));
          else {
            if (t != null) {
              var s = t.$$typeof;
              if (s === H) {
                ((e.tag = 11), (e = Yd(null, e, t, l, n)));
                break t;
              } else if (s === G) {
                ((e.tag = 14), (e = Gd(null, e, t, l, n)));
                break t;
              }
            }
            throw ((e = Bt(t) || t), Error(r(306, e, "")));
          }
        }
        return e;
      case 0:
        return kr(t, e, e.type, e.pendingProps, n);
      case 1:
        return ((l = e.type), (s = xa(l, e.pendingProps)), Jd(t, e, l, s, n));
      case 3:
        t: {
          if ((Xt(e, e.stateNode.containerInfo), t === null))
            throw Error(r(387));
          l = e.pendingProps;
          var o = e.memoizedState;
          ((s = o.element), _r(t, e), ni(e, l, null, n));
          var d = e.memoizedState;
          if (
            ((l = d.cache),
            Un(e, Zt, l),
            l !== o.cache && yr(e, [Zt], n, !0),
            ei(),
            (l = d.element),
            o.isDehydrated)
          )
            if (
              ((o = { element: l, isDehydrated: !1, cache: d.cache }),
              (e.updateQueue.baseState = o),
              (e.memoizedState = o),
              e.flags & 256)
            ) {
              e = kd(t, e, l, n);
              break t;
            } else if (l !== s) {
              ((s = Ne(Error(r(424)), e)), Fl(s), (e = kd(t, e, l, n)));
              break t;
            } else
              for (
                t = e.stateNode.containerInfo,
                  t.nodeType === 9
                    ? (t = t.body)
                    : (t = t.nodeName === "HTML" ? t.ownerDocument.body : t),
                  Ut = Ye(t.firstChild),
                  ie = e,
                  gt = !0,
                  Dn = null,
                  He = !0,
                  n = Vf(e, null, l, n),
                  e.child = n;
                n;
              )
                ((n.flags = (n.flags & -3) | 4096), (n = n.sibling));
          else {
            if ((ga(), l === s)) {
              e = hn(t, e, n);
              break t;
            }
            se(t, e, l, n);
          }
          e = e.child;
        }
        return e;
      case 26:
        return (
          Ou(t, e),
          t === null
            ? (n = om(e.type, null, e.pendingProps, null))
              ? (e.memoizedState = n)
              : gt ||
                ((n = e.type),
                (t = e.pendingProps),
                (l = Ku(dt.current).createElement(n)),
                (l[le] = e),
                (l[he] = t),
                re(l, n, t),
                te(l),
                (e.stateNode = l))
            : (e.memoizedState = om(
                e.type,
                t.memoizedProps,
                e.pendingProps,
                t.memoizedState,
              )),
          null
        );
      case 27:
        return (
          tn(e),
          t === null &&
            gt &&
            ((l = e.stateNode = um(e.type, e.pendingProps, dt.current)),
            (ie = e),
            (He = !0),
            (s = Ut),
            Jn(e.type) ? ((No = s), (Ut = Ye(l.firstChild))) : (Ut = s)),
          se(t, e, e.pendingProps.children, n),
          Ou(t, e),
          t === null && (e.flags |= 4194304),
          e.child
        );
      case 5:
        return (
          t === null &&
            gt &&
            ((s = l = Ut) &&
              ((l = qv(l, e.type, e.pendingProps, He)),
              l !== null
                ? ((e.stateNode = l),
                  (ie = e),
                  (Ut = Ye(l.firstChild)),
                  (He = !1),
                  (s = !0))
                : (s = !1)),
            s || Ln(e)),
          tn(e),
          (s = e.type),
          (o = e.pendingProps),
          (d = t !== null ? t.memoizedProps : null),
          (l = o.children),
          Co(s, o) ? (l = null) : d !== null && Co(s, d) && (e.flags |= 32),
          e.memoizedState !== null &&
            ((s = zr(t, e, tv, null, null, n)), (Ei._currentValue = s)),
          Ou(t, e),
          se(t, e, l, n),
          e.child
        );
      case 6:
        return (
          t === null &&
            gt &&
            ((t = n = Ut) &&
              ((n = Yv(n, e.pendingProps, He)),
              n !== null
                ? ((e.stateNode = n), (ie = e), (Ut = null), (t = !0))
                : (t = !1)),
            t || Ln(e)),
          null
        );
      case 13:
        return Fd(t, e, n);
      case 4:
        return (
          Xt(e, e.stateNode.containerInfo),
          (l = e.pendingProps),
          t === null ? (e.child = Ra(e, null, l, n)) : se(t, e, l, n),
          e.child
        );
      case 11:
        return Yd(t, e, e.type, e.pendingProps, n);
      case 7:
        return (se(t, e, e.pendingProps, n), e.child);
      case 8:
        return (se(t, e, e.pendingProps.children, n), e.child);
      case 12:
        return (se(t, e, e.pendingProps.children, n), e.child);
      case 10:
        return (
          (l = e.pendingProps),
          Un(e, e.type, l.value),
          se(t, e, l.children, n),
          e.child
        );
      case 9:
        return (
          (s = e.type._context),
          (l = e.pendingProps.children),
          Sa(e),
          (s = ue(s)),
          (l = l(s)),
          (e.flags |= 1),
          se(t, e, l, n),
          e.child
        );
      case 14:
        return Gd(t, e, e.type, e.pendingProps, n);
      case 15:
        return Xd(t, e, e.type, e.pendingProps, n);
      case 19:
        return Id(t, e, n);
      case 31:
        return rv(t, e, n);
      case 22:
        return Vd(t, e, n, e.pendingProps);
      case 24:
        return (
          Sa(e),
          (l = ue(Zt)),
          t === null
            ? ((s = vr()),
              s === null &&
                ((s = wt),
                (o = pr()),
                (s.pooledCache = o),
                o.refCount++,
                o !== null && (s.pooledCacheLanes |= n),
                (s = o)),
              (e.memoizedState = { parent: l, cache: s }),
              br(e),
              Un(e, Zt, s))
            : ((t.lanes & n) !== 0 && (_r(t, e), ni(e, null, null, n), ei()),
              (s = t.memoizedState),
              (o = e.memoizedState),
              s.parent !== l
                ? ((s = { parent: l, cache: l }),
                  (e.memoizedState = s),
                  e.lanes === 0 &&
                    (e.memoizedState = e.updateQueue.baseState = s),
                  Un(e, Zt, l))
                : ((l = o.cache),
                  Un(e, Zt, l),
                  l !== s.cache && yr(e, [Zt], n, !0))),
          se(t, e, e.pendingProps.children, n),
          e.child
        );
      case 29:
        throw e.pendingProps;
    }
    throw Error(r(156, e.tag));
  }
  function mn(t) {
    t.flags |= 4;
  }
  function no(t, e, n, l, s) {
    if (((e = (t.mode & 32) !== 0) && (e = !1), e)) {
      if (((t.flags |= 16777216), (s & 335544128) === s))
        if (t.stateNode.complete) t.flags |= 8192;
        else if (xh()) t.flags |= 8192;
        else throw ((Ea = mu), Sr);
    } else t.flags &= -16777217;
  }
  function $d(t, e) {
    if (e.type !== "stylesheet" || (e.state.loading & 4) !== 0)
      t.flags &= -16777217;
    else if (((t.flags |= 16777216), !mm(e)))
      if (xh()) t.flags |= 8192;
      else throw ((Ea = mu), Sr);
  }
  function wu(t, e) {
    (e !== null && (t.flags |= 4),
      t.flags & 16384 &&
        ((e = t.tag !== 22 ? wc() : 536870912), (t.lanes |= e), (ml |= e)));
  }
  function ri(t, e) {
    if (!gt)
      switch (t.tailMode) {
        case "hidden":
          e = t.tail;
          for (var n = null; e !== null; )
            (e.alternate !== null && (n = e), (e = e.sibling));
          n === null ? (t.tail = null) : (n.sibling = null);
          break;
        case "collapsed":
          n = t.tail;
          for (var l = null; n !== null; )
            (n.alternate !== null && (l = n), (n = n.sibling));
          l === null
            ? e || t.tail === null
              ? (t.tail = null)
              : (t.tail.sibling = null)
            : (l.sibling = null);
      }
  }
  function Nt(t) {
    var e = t.alternate !== null && t.alternate.child === t.child,
      n = 0,
      l = 0;
    if (e)
      for (var s = t.child; s !== null; )
        ((n |= s.lanes | s.childLanes),
          (l |= s.subtreeFlags & 65011712),
          (l |= s.flags & 65011712),
          (s.return = t),
          (s = s.sibling));
    else
      for (s = t.child; s !== null; )
        ((n |= s.lanes | s.childLanes),
          (l |= s.subtreeFlags),
          (l |= s.flags),
          (s.return = t),
          (s = s.sibling));
    return ((t.subtreeFlags |= l), (t.childLanes = n), e);
  }
  function cv(t, e, n) {
    var l = e.pendingProps;
    switch ((cr(e), e.tag)) {
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return (Nt(e), null);
      case 1:
        return (Nt(e), null);
      case 3:
        return (
          (n = e.stateNode),
          (l = null),
          t !== null && (l = t.memoizedState.cache),
          e.memoizedState.cache !== l && (e.flags |= 2048),
          cn(Zt),
          Ct(),
          n.pendingContext &&
            ((n.context = n.pendingContext), (n.pendingContext = null)),
          (t === null || t.child === null) &&
            (el(e)
              ? mn(e)
              : t === null ||
                (t.memoizedState.isDehydrated && (e.flags & 256) === 0) ||
                ((e.flags |= 1024), dr())),
          Nt(e),
          null
        );
      case 26:
        var s = e.type,
          o = e.memoizedState;
        return (
          t === null
            ? (mn(e),
              o !== null ? (Nt(e), $d(e, o)) : (Nt(e), no(e, s, null, l, n)))
            : o
              ? o !== t.memoizedState
                ? (mn(e), Nt(e), $d(e, o))
                : (Nt(e), (e.flags &= -16777217))
              : ((t = t.memoizedProps),
                t !== l && mn(e),
                Nt(e),
                no(e, s, t, l, n)),
          null
        );
      case 27:
        if (
          (en(e),
          (n = dt.current),
          (s = e.type),
          t !== null && e.stateNode != null)
        )
          t.memoizedProps !== l && mn(e);
        else {
          if (!l) {
            if (e.stateNode === null) throw Error(r(166));
            return (Nt(e), null);
          }
          ((t = I.current),
            el(e) ? Df(e) : ((t = um(s, l, n)), (e.stateNode = t), mn(e)));
        }
        return (Nt(e), null);
      case 5:
        if ((en(e), (s = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== l && mn(e);
        else {
          if (!l) {
            if (e.stateNode === null) throw Error(r(166));
            return (Nt(e), null);
          }
          if (((o = I.current), el(e))) Df(e);
          else {
            var d = Ku(dt.current);
            switch (o) {
              case 1:
                o = d.createElementNS("http://www.w3.org/2000/svg", s);
                break;
              case 2:
                o = d.createElementNS("http://www.w3.org/1998/Math/MathML", s);
                break;
              default:
                switch (s) {
                  case "svg":
                    o = d.createElementNS("http://www.w3.org/2000/svg", s);
                    break;
                  case "math":
                    o = d.createElementNS(
                      "http://www.w3.org/1998/Math/MathML",
                      s,
                    );
                    break;
                  case "script":
                    ((o = d.createElement("div")),
                      (o.innerHTML = "<script><\/script>"),
                      (o = o.removeChild(o.firstChild)));
                    break;
                  case "select":
                    ((o =
                      typeof l.is == "string"
                        ? d.createElement("select", { is: l.is })
                        : d.createElement("select")),
                      l.multiple
                        ? (o.multiple = !0)
                        : l.size && (o.size = l.size));
                    break;
                  default:
                    o =
                      typeof l.is == "string"
                        ? d.createElement(s, { is: l.is })
                        : d.createElement(s);
                }
            }
            ((o[le] = e), (o[he] = l));
            t: for (d = e.child; d !== null; ) {
              if (d.tag === 5 || d.tag === 6) o.appendChild(d.stateNode);
              else if (d.tag !== 4 && d.tag !== 27 && d.child !== null) {
                ((d.child.return = d), (d = d.child));
                continue;
              }
              if (d === e) break t;
              for (; d.sibling === null; ) {
                if (d.return === null || d.return === e) break t;
                d = d.return;
              }
              ((d.sibling.return = d.return), (d = d.sibling));
            }
            e.stateNode = o;
            t: switch ((re(o, s, l), s)) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                l = !!l.autoFocus;
                break t;
              case "img":
                l = !0;
                break t;
              default:
                l = !1;
            }
            l && mn(e);
          }
        }
        return (
          Nt(e),
          no(e, e.type, t === null ? null : t.memoizedProps, e.pendingProps, n),
          null
        );
      case 6:
        if (t && e.stateNode != null) t.memoizedProps !== l && mn(e);
        else {
          if (typeof l != "string" && e.stateNode === null) throw Error(r(166));
          if (((t = dt.current), el(e))) {
            if (
              ((t = e.stateNode),
              (n = e.memoizedProps),
              (l = null),
              (s = ie),
              s !== null)
            )
              switch (s.tag) {
                case 27:
                case 5:
                  l = s.memoizedProps;
              }
            ((t[le] = e),
              (t = !!(
                t.nodeValue === n ||
                (l !== null && l.suppressHydrationWarning === !0) ||
                Fh(t.nodeValue, n)
              )),
              t || Ln(e, !0));
          } else
            ((t = Ku(t).createTextNode(l)), (t[le] = e), (e.stateNode = t));
        }
        return (Nt(e), null);
      case 31:
        if (((n = e.memoizedState), t === null || t.memoizedState !== null)) {
          if (((l = el(e)), n !== null)) {
            if (t === null) {
              if (!l) throw Error(r(318));
              if (
                ((t = e.memoizedState),
                (t = t !== null ? t.dehydrated : null),
                !t)
              )
                throw Error(r(557));
              t[le] = e;
            } else
              (ga(),
                (e.flags & 128) === 0 && (e.memoizedState = null),
                (e.flags |= 4));
            (Nt(e), (t = !1));
          } else
            ((n = dr()),
              t !== null &&
                t.memoizedState !== null &&
                (t.memoizedState.hydrationErrors = n),
              (t = !0));
          if (!t) return e.flags & 256 ? (ze(e), e) : (ze(e), null);
          if ((e.flags & 128) !== 0) throw Error(r(558));
        }
        return (Nt(e), null);
      case 13:
        if (
          ((l = e.memoizedState),
          t === null ||
            (t.memoizedState !== null && t.memoizedState.dehydrated !== null))
        ) {
          if (((s = el(e)), l !== null && l.dehydrated !== null)) {
            if (t === null) {
              if (!s) throw Error(r(318));
              if (
                ((s = e.memoizedState),
                (s = s !== null ? s.dehydrated : null),
                !s)
              )
                throw Error(r(317));
              s[le] = e;
            } else
              (ga(),
                (e.flags & 128) === 0 && (e.memoizedState = null),
                (e.flags |= 4));
            (Nt(e), (s = !1));
          } else
            ((s = dr()),
              t !== null &&
                t.memoizedState !== null &&
                (t.memoizedState.hydrationErrors = s),
              (s = !0));
          if (!s) return e.flags & 256 ? (ze(e), e) : (ze(e), null);
        }
        return (
          ze(e),
          (e.flags & 128) !== 0
            ? ((e.lanes = n), e)
            : ((n = l !== null),
              (t = t !== null && t.memoizedState !== null),
              n &&
                ((l = e.child),
                (s = null),
                l.alternate !== null &&
                  l.alternate.memoizedState !== null &&
                  l.alternate.memoizedState.cachePool !== null &&
                  (s = l.alternate.memoizedState.cachePool.pool),
                (o = null),
                l.memoizedState !== null &&
                  l.memoizedState.cachePool !== null &&
                  (o = l.memoizedState.cachePool.pool),
                o !== s && (l.flags |= 2048)),
              n !== t && n && (e.child.flags |= 8192),
              wu(e, e.updateQueue),
              Nt(e),
              null)
        );
      case 4:
        return (Ct(), t === null && xo(e.stateNode.containerInfo), Nt(e), null);
      case 10:
        return (cn(e.type), Nt(e), null);
      case 19:
        if ((X(Vt), (l = e.memoizedState), l === null)) return (Nt(e), null);
        if (((s = (e.flags & 128) !== 0), (o = l.rendering), o === null))
          if (s) ri(l, !1);
          else {
            if (Yt !== 0 || (t !== null && (t.flags & 128) !== 0))
              for (t = e.child; t !== null; ) {
                if (((o = vu(t)), o !== null)) {
                  for (
                    e.flags |= 128,
                      ri(l, !1),
                      t = o.updateQueue,
                      e.updateQueue = t,
                      wu(e, t),
                      e.subtreeFlags = 0,
                      t = n,
                      n = e.child;
                    n !== null;
                  )
                    (Mf(n, t), (n = n.sibling));
                  return (
                    k(Vt, (Vt.current & 1) | 2),
                    gt && rn(e, l.treeForkCount),
                    e.child
                  );
                }
                t = t.sibling;
              }
            l.tail !== null &&
              ce() > Bu &&
              ((e.flags |= 128), (s = !0), ri(l, !1), (e.lanes = 4194304));
          }
        else {
          if (!s)
            if (((t = vu(o)), t !== null)) {
              if (
                ((e.flags |= 128),
                (s = !0),
                (t = t.updateQueue),
                (e.updateQueue = t),
                wu(e, t),
                ri(l, !0),
                l.tail === null &&
                  l.tailMode === "hidden" &&
                  !o.alternate &&
                  !gt)
              )
                return (Nt(e), null);
            } else
              2 * ce() - l.renderingStartTime > Bu &&
                n !== 536870912 &&
                ((e.flags |= 128), (s = !0), ri(l, !1), (e.lanes = 4194304));
          l.isBackwards
            ? ((o.sibling = e.child), (e.child = o))
            : ((t = l.last),
              t !== null ? (t.sibling = o) : (e.child = o),
              (l.last = o));
        }
        return l.tail !== null
          ? ((t = l.tail),
            (l.rendering = t),
            (l.tail = t.sibling),
            (l.renderingStartTime = ce()),
            (t.sibling = null),
            (n = Vt.current),
            k(Vt, s ? (n & 1) | 2 : n & 1),
            gt && rn(e, l.treeForkCount),
            t)
          : (Nt(e), null);
      case 22:
      case 23:
        return (
          ze(e),
          xr(),
          (l = e.memoizedState !== null),
          t !== null
            ? (t.memoizedState !== null) !== l && (e.flags |= 8192)
            : l && (e.flags |= 8192),
          l
            ? (n & 536870912) !== 0 &&
              (e.flags & 128) === 0 &&
              (Nt(e), e.subtreeFlags & 6 && (e.flags |= 8192))
            : Nt(e),
          (n = e.updateQueue),
          n !== null && wu(e, n.retryQueue),
          (n = null),
          t !== null &&
            t.memoizedState !== null &&
            t.memoizedState.cachePool !== null &&
            (n = t.memoizedState.cachePool.pool),
          (l = null),
          e.memoizedState !== null &&
            e.memoizedState.cachePool !== null &&
            (l = e.memoizedState.cachePool.pool),
          l !== n && (e.flags |= 2048),
          t !== null && X(ba),
          null
        );
      case 24:
        return (
          (n = null),
          t !== null && (n = t.memoizedState.cache),
          e.memoizedState.cache !== n && (e.flags |= 2048),
          cn(Zt),
          Nt(e),
          null
        );
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(r(156, e.tag));
  }
  function fv(t, e) {
    switch ((cr(e), e.tag)) {
      case 1:
        return (
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 3:
        return (
          cn(Zt),
          Ct(),
          (t = e.flags),
          (t & 65536) !== 0 && (t & 128) === 0
            ? ((e.flags = (t & -65537) | 128), e)
            : null
        );
      case 26:
      case 27:
      case 5:
        return (en(e), null);
      case 31:
        if (e.memoizedState !== null) {
          if ((ze(e), e.alternate === null)) throw Error(r(340));
          ga();
        }
        return (
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 13:
        if (
          (ze(e), (t = e.memoizedState), t !== null && t.dehydrated !== null)
        ) {
          if (e.alternate === null) throw Error(r(340));
          ga();
        }
        return (
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 19:
        return (X(Vt), null);
      case 4:
        return (Ct(), null);
      case 10:
        return (cn(e.type), null);
      case 22:
      case 23:
        return (
          ze(e),
          xr(),
          t !== null && X(ba),
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 24:
        return (cn(Zt), null);
      case 25:
        return null;
      default:
        return null;
    }
  }
  function th(t, e) {
    switch ((cr(e), e.tag)) {
      case 3:
        (cn(Zt), Ct());
        break;
      case 26:
      case 27:
      case 5:
        en(e);
        break;
      case 4:
        Ct();
        break;
      case 31:
        e.memoizedState !== null && ze(e);
        break;
      case 13:
        ze(e);
        break;
      case 19:
        X(Vt);
        break;
      case 10:
        cn(e.type);
        break;
      case 22:
      case 23:
        (ze(e), xr(), t !== null && X(ba));
        break;
      case 24:
        cn(Zt);
    }
  }
  function oi(t, e) {
    try {
      var n = e.updateQueue,
        l = n !== null ? n.lastEffect : null;
      if (l !== null) {
        var s = l.next;
        n = s;
        do {
          if ((n.tag & t) === t) {
            l = void 0;
            var o = n.create,
              d = n.inst;
            ((l = o()), (d.destroy = l));
          }
          n = n.next;
        } while (n !== s);
      }
    } catch (v) {
      At(e, e.return, v);
    }
  }
  function Yn(t, e, n) {
    try {
      var l = e.updateQueue,
        s = l !== null ? l.lastEffect : null;
      if (s !== null) {
        var o = s.next;
        l = o;
        do {
          if ((l.tag & t) === t) {
            var d = l.inst,
              v = d.destroy;
            if (v !== void 0) {
              ((d.destroy = void 0), (s = e));
              var R = n,
                D = v;
              try {
                D();
              } catch (j) {
                At(s, R, j);
              }
            }
          }
          l = l.next;
        } while (l !== o);
      }
    } catch (j) {
      At(e, e.return, j);
    }
  }
  function eh(t) {
    var e = t.updateQueue;
    if (e !== null) {
      var n = t.stateNode;
      try {
        Zf(e, n);
      } catch (l) {
        At(t, t.return, l);
      }
    }
  }
  function nh(t, e, n) {
    ((n.props = xa(t.type, t.memoizedProps)), (n.state = t.memoizedState));
    try {
      n.componentWillUnmount();
    } catch (l) {
      At(t, e, l);
    }
  }
  function ci(t, e) {
    try {
      var n = t.ref;
      if (n !== null) {
        switch (t.tag) {
          case 26:
          case 27:
          case 5:
            var l = t.stateNode;
            break;
          case 30:
            l = t.stateNode;
            break;
          default:
            l = t.stateNode;
        }
        typeof n == "function" ? (t.refCleanup = n(l)) : (n.current = l);
      }
    } catch (s) {
      At(t, e, s);
    }
  }
  function Pe(t, e) {
    var n = t.ref,
      l = t.refCleanup;
    if (n !== null)
      if (typeof l == "function")
        try {
          l();
        } catch (s) {
          At(t, e, s);
        } finally {
          ((t.refCleanup = null),
            (t = t.alternate),
            t != null && (t.refCleanup = null));
        }
      else if (typeof n == "function")
        try {
          n(null);
        } catch (s) {
          At(t, e, s);
        }
      else n.current = null;
  }
  function ah(t) {
    var e = t.type,
      n = t.memoizedProps,
      l = t.stateNode;
    try {
      t: switch (e) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          n.autoFocus && l.focus();
          break t;
        case "img":
          n.src ? (l.src = n.src) : n.srcSet && (l.srcset = n.srcSet);
      }
    } catch (s) {
      At(t, t.return, s);
    }
  }
  function ao(t, e, n) {
    try {
      var l = t.stateNode;
      (Lv(l, t.type, n, e), (l[he] = e));
    } catch (s) {
      At(t, t.return, s);
    }
  }
  function lh(t) {
    return (
      t.tag === 5 ||
      t.tag === 3 ||
      t.tag === 26 ||
      (t.tag === 27 && Jn(t.type)) ||
      t.tag === 4
    );
  }
  function lo(t) {
    t: for (;;) {
      for (; t.sibling === null; ) {
        if (t.return === null || lh(t.return)) return null;
        t = t.return;
      }
      for (
        t.sibling.return = t.return, t = t.sibling;
        t.tag !== 5 && t.tag !== 6 && t.tag !== 18;
      ) {
        if (
          (t.tag === 27 && Jn(t.type)) ||
          t.flags & 2 ||
          t.child === null ||
          t.tag === 4
        )
          continue t;
        ((t.child.return = t), (t = t.child));
      }
      if (!(t.flags & 2)) return t.stateNode;
    }
  }
  function io(t, e, n) {
    var l = t.tag;
    if (l === 5 || l === 6)
      ((t = t.stateNode),
        e
          ? (n.nodeType === 9
              ? n.body
              : n.nodeName === "HTML"
                ? n.ownerDocument.body
                : n
            ).insertBefore(t, e)
          : ((e =
              n.nodeType === 9
                ? n.body
                : n.nodeName === "HTML"
                  ? n.ownerDocument.body
                  : n),
            e.appendChild(t),
            (n = n._reactRootContainer),
            n != null || e.onclick !== null || (e.onclick = ln)));
    else if (
      l !== 4 &&
      (l === 27 && Jn(t.type) && ((n = t.stateNode), (e = null)),
      (t = t.child),
      t !== null)
    )
      for (io(t, e, n), t = t.sibling; t !== null; )
        (io(t, e, n), (t = t.sibling));
  }
  function Du(t, e, n) {
    var l = t.tag;
    if (l === 5 || l === 6)
      ((t = t.stateNode), e ? n.insertBefore(t, e) : n.appendChild(t));
    else if (
      l !== 4 &&
      (l === 27 && Jn(t.type) && (n = t.stateNode), (t = t.child), t !== null)
    )
      for (Du(t, e, n), t = t.sibling; t !== null; )
        (Du(t, e, n), (t = t.sibling));
  }
  function ih(t) {
    var e = t.stateNode,
      n = t.memoizedProps;
    try {
      for (var l = t.type, s = e.attributes; s.length; )
        e.removeAttributeNode(s[0]);
      (re(e, l, n), (e[le] = t), (e[he] = n));
    } catch (o) {
      At(t, t.return, o);
    }
  }
  var yn = !1,
    kt = !1,
    uo = !1,
    uh = typeof WeakSet == "function" ? WeakSet : Set,
    ee = null;
  function dv(t, e) {
    if (((t = t.containerInfo), (zo = $u), (t = vf(t)), $s(t))) {
      if ("selectionStart" in t)
        var n = { start: t.selectionStart, end: t.selectionEnd };
      else
        t: {
          n = ((n = t.ownerDocument) && n.defaultView) || window;
          var l = n.getSelection && n.getSelection();
          if (l && l.rangeCount !== 0) {
            n = l.anchorNode;
            var s = l.anchorOffset,
              o = l.focusNode;
            l = l.focusOffset;
            try {
              (n.nodeType, o.nodeType);
            } catch {
              n = null;
              break t;
            }
            var d = 0,
              v = -1,
              R = -1,
              D = 0,
              j = 0,
              V = t,
              U = null;
            e: for (;;) {
              for (
                var N;
                V !== n || (s !== 0 && V.nodeType !== 3) || (v = d + s),
                  V !== o || (l !== 0 && V.nodeType !== 3) || (R = d + l),
                  V.nodeType === 3 && (d += V.nodeValue.length),
                  (N = V.firstChild) !== null;
              )
                ((U = V), (V = N));
              for (;;) {
                if (V === t) break e;
                if (
                  (U === n && ++D === s && (v = d),
                  U === o && ++j === l && (R = d),
                  (N = V.nextSibling) !== null)
                )
                  break;
                ((V = U), (U = V.parentNode));
              }
              V = N;
            }
            n = v === -1 || R === -1 ? null : { start: v, end: R };
          } else n = null;
        }
      n = n || { start: 0, end: 0 };
    } else n = null;
    for (
      Oo = { focusedElem: t, selectionRange: n }, $u = !1, ee = e;
      ee !== null;
    )
      if (
        ((e = ee), (t = e.child), (e.subtreeFlags & 1028) !== 0 && t !== null)
      )
        ((t.return = e), (ee = t));
      else
        for (; ee !== null; ) {
          switch (((e = ee), (o = e.alternate), (t = e.flags), e.tag)) {
            case 0:
              if (
                (t & 4) !== 0 &&
                ((t = e.updateQueue),
                (t = t !== null ? t.events : null),
                t !== null)
              )
                for (n = 0; n < t.length; n++)
                  ((s = t[n]), (s.ref.impl = s.nextImpl));
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((t & 1024) !== 0 && o !== null) {
                ((t = void 0),
                  (n = e),
                  (s = o.memoizedProps),
                  (o = o.memoizedState),
                  (l = n.stateNode));
                try {
                  var $ = xa(n.type, s);
                  ((t = l.getSnapshotBeforeUpdate($, o)),
                    (l.__reactInternalSnapshotBeforeUpdate = t));
                } catch (st) {
                  At(n, n.return, st);
                }
              }
              break;
            case 3:
              if ((t & 1024) !== 0) {
                if (
                  ((t = e.stateNode.containerInfo), (n = t.nodeType), n === 9)
                )
                  Do(t);
                else if (n === 1)
                  switch (t.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      Do(t);
                      break;
                    default:
                      t.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((t & 1024) !== 0) throw Error(r(163));
          }
          if (((t = e.sibling), t !== null)) {
            ((t.return = e.return), (ee = t));
            break;
          }
          ee = e.return;
        }
  }
  function sh(t, e, n) {
    var l = n.flags;
    switch (n.tag) {
      case 0:
      case 11:
      case 15:
        (gn(t, n), l & 4 && oi(5, n));
        break;
      case 1:
        if ((gn(t, n), l & 4))
          if (((t = n.stateNode), e === null))
            try {
              t.componentDidMount();
            } catch (d) {
              At(n, n.return, d);
            }
          else {
            var s = xa(n.type, e.memoizedProps);
            e = e.memoizedState;
            try {
              t.componentDidUpdate(s, e, t.__reactInternalSnapshotBeforeUpdate);
            } catch (d) {
              At(n, n.return, d);
            }
          }
        (l & 64 && eh(n), l & 512 && ci(n, n.return));
        break;
      case 3:
        if ((gn(t, n), l & 64 && ((t = n.updateQueue), t !== null))) {
          if (((e = null), n.child !== null))
            switch (n.child.tag) {
              case 27:
              case 5:
                e = n.child.stateNode;
                break;
              case 1:
                e = n.child.stateNode;
            }
          try {
            Zf(t, e);
          } catch (d) {
            At(n, n.return, d);
          }
        }
        break;
      case 27:
        e === null && l & 4 && ih(n);
      case 26:
      case 5:
        (gn(t, n), e === null && l & 4 && ah(n), l & 512 && ci(n, n.return));
        break;
      case 12:
        gn(t, n);
        break;
      case 31:
        (gn(t, n), l & 4 && ch(t, n));
        break;
      case 13:
        (gn(t, n),
          l & 4 && fh(t, n),
          l & 64 &&
            ((t = n.memoizedState),
            t !== null &&
              ((t = t.dehydrated),
              t !== null && ((n = _v.bind(null, n)), Gv(t, n)))));
        break;
      case 22:
        if (((l = n.memoizedState !== null || yn), !l)) {
          ((e = (e !== null && e.memoizedState !== null) || kt), (s = yn));
          var o = kt;
          ((yn = l),
            (kt = e) && !o ? vn(t, n, (n.subtreeFlags & 8772) !== 0) : gn(t, n),
            (yn = s),
            (kt = o));
        }
        break;
      case 30:
        break;
      default:
        gn(t, n);
    }
  }
  function rh(t) {
    var e = t.alternate;
    (e !== null && ((t.alternate = null), rh(e)),
      (t.child = null),
      (t.deletions = null),
      (t.sibling = null),
      t.tag === 5 && ((e = t.stateNode), e !== null && Bs(e)),
      (t.stateNode = null),
      (t.return = null),
      (t.dependencies = null),
      (t.memoizedProps = null),
      (t.memoizedState = null),
      (t.pendingProps = null),
      (t.stateNode = null),
      (t.updateQueue = null));
  }
  var jt = null,
    ye = !1;
  function pn(t, e, n) {
    for (n = n.child; n !== null; ) (oh(t, e, n), (n = n.sibling));
  }
  function oh(t, e, n) {
    if (Re && typeof Re.onCommitFiberUnmount == "function")
      try {
        Re.onCommitFiberUnmount(Ul, n);
      } catch {}
    switch (n.tag) {
      case 26:
        (kt || Pe(n, e),
          pn(t, e, n),
          n.memoizedState
            ? n.memoizedState.count--
            : n.stateNode && ((n = n.stateNode), n.parentNode.removeChild(n)));
        break;
      case 27:
        kt || Pe(n, e);
        var l = jt,
          s = ye;
        (Jn(n.type) && ((jt = n.stateNode), (ye = !1)),
          pn(t, e, n),
          Si(n.stateNode),
          (jt = l),
          (ye = s));
        break;
      case 5:
        kt || Pe(n, e);
      case 6:
        if (
          ((l = jt),
          (s = ye),
          (jt = null),
          pn(t, e, n),
          (jt = l),
          (ye = s),
          jt !== null)
        )
          if (ye)
            try {
              (jt.nodeType === 9
                ? jt.body
                : jt.nodeName === "HTML"
                  ? jt.ownerDocument.body
                  : jt
              ).removeChild(n.stateNode);
            } catch (o) {
              At(n, e, o);
            }
          else
            try {
              jt.removeChild(n.stateNode);
            } catch (o) {
              At(n, e, o);
            }
        break;
      case 18:
        jt !== null &&
          (ye
            ? ((t = jt),
              em(
                t.nodeType === 9
                  ? t.body
                  : t.nodeName === "HTML"
                    ? t.ownerDocument.body
                    : t,
                n.stateNode,
              ),
              El(t))
            : em(jt, n.stateNode));
        break;
      case 4:
        ((l = jt),
          (s = ye),
          (jt = n.stateNode.containerInfo),
          (ye = !0),
          pn(t, e, n),
          (jt = l),
          (ye = s));
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        (Yn(2, n, e), kt || Yn(4, n, e), pn(t, e, n));
        break;
      case 1:
        (kt ||
          (Pe(n, e),
          (l = n.stateNode),
          typeof l.componentWillUnmount == "function" && nh(n, e, l)),
          pn(t, e, n));
        break;
      case 21:
        pn(t, e, n);
        break;
      case 22:
        ((kt = (l = kt) || n.memoizedState !== null), pn(t, e, n), (kt = l));
        break;
      default:
        pn(t, e, n);
    }
  }
  function ch(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate), t !== null && ((t = t.memoizedState), t !== null))
    ) {
      t = t.dehydrated;
      try {
        El(t);
      } catch (n) {
        At(e, e.return, n);
      }
    }
  }
  function fh(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate),
      t !== null &&
        ((t = t.memoizedState), t !== null && ((t = t.dehydrated), t !== null)))
    )
      try {
        El(t);
      } catch (n) {
        At(e, e.return, n);
      }
  }
  function hv(t) {
    switch (t.tag) {
      case 31:
      case 13:
      case 19:
        var e = t.stateNode;
        return (e === null && (e = t.stateNode = new uh()), e);
      case 22:
        return (
          (t = t.stateNode),
          (e = t._retryCache),
          e === null && (e = t._retryCache = new uh()),
          e
        );
      default:
        throw Error(r(435, t.tag));
    }
  }
  function Lu(t, e) {
    var n = hv(t);
    e.forEach(function (l) {
      if (!n.has(l)) {
        n.add(l);
        var s = Ev.bind(null, t, l);
        l.then(s, s);
      }
    });
  }
  function pe(t, e) {
    var n = e.deletions;
    if (n !== null)
      for (var l = 0; l < n.length; l++) {
        var s = n[l],
          o = t,
          d = e,
          v = d;
        t: for (; v !== null; ) {
          switch (v.tag) {
            case 27:
              if (Jn(v.type)) {
                ((jt = v.stateNode), (ye = !1));
                break t;
              }
              break;
            case 5:
              ((jt = v.stateNode), (ye = !1));
              break t;
            case 3:
            case 4:
              ((jt = v.stateNode.containerInfo), (ye = !0));
              break t;
          }
          v = v.return;
        }
        if (jt === null) throw Error(r(160));
        (oh(o, d, s),
          (jt = null),
          (ye = !1),
          (o = s.alternate),
          o !== null && (o.return = null),
          (s.return = null));
      }
    if (e.subtreeFlags & 13886)
      for (e = e.child; e !== null; ) (dh(e, t), (e = e.sibling));
  }
  var Qe = null;
  function dh(t, e) {
    var n = t.alternate,
      l = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        (pe(e, t),
          ge(t),
          l & 4 && (Yn(3, t, t.return), oi(3, t), Yn(5, t, t.return)));
        break;
      case 1:
        (pe(e, t),
          ge(t),
          l & 512 && (kt || n === null || Pe(n, n.return)),
          l & 64 &&
            yn &&
            ((t = t.updateQueue),
            t !== null &&
              ((l = t.callbacks),
              l !== null &&
                ((n = t.shared.hiddenCallbacks),
                (t.shared.hiddenCallbacks = n === null ? l : n.concat(l))))));
        break;
      case 26:
        var s = Qe;
        if (
          (pe(e, t),
          ge(t),
          l & 512 && (kt || n === null || Pe(n, n.return)),
          l & 4)
        ) {
          var o = n !== null ? n.memoizedState : null;
          if (((l = t.memoizedState), n === null))
            if (l === null)
              if (t.stateNode === null) {
                t: {
                  ((l = t.type),
                    (n = t.memoizedProps),
                    (s = s.ownerDocument || s));
                  e: switch (l) {
                    case "title":
                      ((o = s.getElementsByTagName("title")[0]),
                        (!o ||
                          o[jl] ||
                          o[le] ||
                          o.namespaceURI === "http://www.w3.org/2000/svg" ||
                          o.hasAttribute("itemprop")) &&
                          ((o = s.createElement(l)),
                          s.head.insertBefore(
                            o,
                            s.querySelector("head > title"),
                          )),
                        re(o, l, n),
                        (o[le] = t),
                        te(o),
                        (l = o));
                      break t;
                    case "link":
                      var d = dm("link", "href", s).get(l + (n.href || ""));
                      if (d) {
                        for (var v = 0; v < d.length; v++)
                          if (
                            ((o = d[v]),
                            o.getAttribute("href") ===
                              (n.href == null || n.href === ""
                                ? null
                                : n.href) &&
                              o.getAttribute("rel") ===
                                (n.rel == null ? null : n.rel) &&
                              o.getAttribute("title") ===
                                (n.title == null ? null : n.title) &&
                              o.getAttribute("crossorigin") ===
                                (n.crossOrigin == null ? null : n.crossOrigin))
                          ) {
                            d.splice(v, 1);
                            break e;
                          }
                      }
                      ((o = s.createElement(l)),
                        re(o, l, n),
                        s.head.appendChild(o));
                      break;
                    case "meta":
                      if (
                        (d = dm("meta", "content", s).get(
                          l + (n.content || ""),
                        ))
                      ) {
                        for (v = 0; v < d.length; v++)
                          if (
                            ((o = d[v]),
                            o.getAttribute("content") ===
                              (n.content == null ? null : "" + n.content) &&
                              o.getAttribute("name") ===
                                (n.name == null ? null : n.name) &&
                              o.getAttribute("property") ===
                                (n.property == null ? null : n.property) &&
                              o.getAttribute("http-equiv") ===
                                (n.httpEquiv == null ? null : n.httpEquiv) &&
                              o.getAttribute("charset") ===
                                (n.charSet == null ? null : n.charSet))
                          ) {
                            d.splice(v, 1);
                            break e;
                          }
                      }
                      ((o = s.createElement(l)),
                        re(o, l, n),
                        s.head.appendChild(o));
                      break;
                    default:
                      throw Error(r(468, l));
                  }
                  ((o[le] = t), te(o), (l = o));
                }
                t.stateNode = l;
              } else hm(s, t.type, t.stateNode);
            else t.stateNode = fm(s, l, t.memoizedProps);
          else
            o !== l
              ? (o === null
                  ? n.stateNode !== null &&
                    ((n = n.stateNode), n.parentNode.removeChild(n))
                  : o.count--,
                l === null
                  ? hm(s, t.type, t.stateNode)
                  : fm(s, l, t.memoizedProps))
              : l === null &&
                t.stateNode !== null &&
                ao(t, t.memoizedProps, n.memoizedProps);
        }
        break;
      case 27:
        (pe(e, t),
          ge(t),
          l & 512 && (kt || n === null || Pe(n, n.return)),
          n !== null && l & 4 && ao(t, t.memoizedProps, n.memoizedProps));
        break;
      case 5:
        if (
          (pe(e, t),
          ge(t),
          l & 512 && (kt || n === null || Pe(n, n.return)),
          t.flags & 32)
        ) {
          s = t.stateNode;
          try {
            Za(s, "");
          } catch ($) {
            At(t, t.return, $);
          }
        }
        (l & 4 &&
          t.stateNode != null &&
          ((s = t.memoizedProps), ao(t, s, n !== null ? n.memoizedProps : s)),
          l & 1024 && (uo = !0));
        break;
      case 6:
        if ((pe(e, t), ge(t), l & 4)) {
          if (t.stateNode === null) throw Error(r(162));
          ((l = t.memoizedProps), (n = t.stateNode));
          try {
            n.nodeValue = l;
          } catch ($) {
            At(t, t.return, $);
          }
        }
        break;
      case 3:
        if (
          ((Fu = null),
          (s = Qe),
          (Qe = Ju(e.containerInfo)),
          pe(e, t),
          (Qe = s),
          ge(t),
          l & 4 && n !== null && n.memoizedState.isDehydrated)
        )
          try {
            El(e.containerInfo);
          } catch ($) {
            At(t, t.return, $);
          }
        uo && ((uo = !1), hh(t));
        break;
      case 4:
        ((l = Qe),
          (Qe = Ju(t.stateNode.containerInfo)),
          pe(e, t),
          ge(t),
          (Qe = l));
        break;
      case 12:
        (pe(e, t), ge(t));
        break;
      case 31:
        (pe(e, t),
          ge(t),
          l & 4 &&
            ((l = t.updateQueue),
            l !== null && ((t.updateQueue = null), Lu(t, l))));
        break;
      case 13:
        (pe(e, t),
          ge(t),
          t.child.flags & 8192 &&
            (t.memoizedState !== null) !=
              (n !== null && n.memoizedState !== null) &&
            (Nu = ce()),
          l & 4 &&
            ((l = t.updateQueue),
            l !== null && ((t.updateQueue = null), Lu(t, l))));
        break;
      case 22:
        s = t.memoizedState !== null;
        var R = n !== null && n.memoizedState !== null,
          D = yn,
          j = kt;
        if (
          ((yn = D || s),
          (kt = j || R),
          pe(e, t),
          (kt = j),
          (yn = D),
          ge(t),
          l & 8192)
        )
          t: for (
            e = t.stateNode,
              e._visibility = s ? e._visibility & -2 : e._visibility | 1,
              s && (n === null || R || yn || kt || Aa(t)),
              n = null,
              e = t;
            ;
          ) {
            if (e.tag === 5 || e.tag === 26) {
              if (n === null) {
                R = n = e;
                try {
                  if (((o = R.stateNode), s))
                    ((d = o.style),
                      typeof d.setProperty == "function"
                        ? d.setProperty("display", "none", "important")
                        : (d.display = "none"));
                  else {
                    v = R.stateNode;
                    var V = R.memoizedProps.style,
                      U =
                        V != null && V.hasOwnProperty("display")
                          ? V.display
                          : null;
                    v.style.display =
                      U == null || typeof U == "boolean" ? "" : ("" + U).trim();
                  }
                } catch ($) {
                  At(R, R.return, $);
                }
              }
            } else if (e.tag === 6) {
              if (n === null) {
                R = e;
                try {
                  R.stateNode.nodeValue = s ? "" : R.memoizedProps;
                } catch ($) {
                  At(R, R.return, $);
                }
              }
            } else if (e.tag === 18) {
              if (n === null) {
                R = e;
                try {
                  var N = R.stateNode;
                  s ? nm(N, !0) : nm(R.stateNode, !1);
                } catch ($) {
                  At(R, R.return, $);
                }
              }
            } else if (
              ((e.tag !== 22 && e.tag !== 23) ||
                e.memoizedState === null ||
                e === t) &&
              e.child !== null
            ) {
              ((e.child.return = e), (e = e.child));
              continue;
            }
            if (e === t) break t;
            for (; e.sibling === null; ) {
              if (e.return === null || e.return === t) break t;
              (n === e && (n = null), (e = e.return));
            }
            (n === e && (n = null),
              (e.sibling.return = e.return),
              (e = e.sibling));
          }
        l & 4 &&
          ((l = t.updateQueue),
          l !== null &&
            ((n = l.retryQueue),
            n !== null && ((l.retryQueue = null), Lu(t, n))));
        break;
      case 19:
        (pe(e, t),
          ge(t),
          l & 4 &&
            ((l = t.updateQueue),
            l !== null && ((t.updateQueue = null), Lu(t, l))));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        (pe(e, t), ge(t));
    }
  }
  function ge(t) {
    var e = t.flags;
    if (e & 2) {
      try {
        for (var n, l = t.return; l !== null; ) {
          if (lh(l)) {
            n = l;
            break;
          }
          l = l.return;
        }
        if (n == null) throw Error(r(160));
        switch (n.tag) {
          case 27:
            var s = n.stateNode,
              o = lo(t);
            Du(t, o, s);
            break;
          case 5:
            var d = n.stateNode;
            n.flags & 32 && (Za(d, ""), (n.flags &= -33));
            var v = lo(t);
            Du(t, v, d);
            break;
          case 3:
          case 4:
            var R = n.stateNode.containerInfo,
              D = lo(t);
            io(t, D, R);
            break;
          default:
            throw Error(r(161));
        }
      } catch (j) {
        At(t, t.return, j);
      }
      t.flags &= -3;
    }
    e & 4096 && (t.flags &= -4097);
  }
  function hh(t) {
    if (t.subtreeFlags & 1024)
      for (t = t.child; t !== null; ) {
        var e = t;
        (hh(e),
          e.tag === 5 && e.flags & 1024 && e.stateNode.reset(),
          (t = t.sibling));
      }
  }
  function gn(t, e) {
    if (e.subtreeFlags & 8772)
      for (e = e.child; e !== null; ) (sh(t, e.alternate, e), (e = e.sibling));
  }
  function Aa(t) {
    for (t = t.child; t !== null; ) {
      var e = t;
      switch (e.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          (Yn(4, e, e.return), Aa(e));
          break;
        case 1:
          Pe(e, e.return);
          var n = e.stateNode;
          (typeof n.componentWillUnmount == "function" && nh(e, e.return, n),
            Aa(e));
          break;
        case 27:
          Si(e.stateNode);
        case 26:
        case 5:
          (Pe(e, e.return), Aa(e));
          break;
        case 22:
          e.memoizedState === null && Aa(e);
          break;
        case 30:
          Aa(e);
          break;
        default:
          Aa(e);
      }
      t = t.sibling;
    }
  }
  function vn(t, e, n) {
    for (n = n && (e.subtreeFlags & 8772) !== 0, e = e.child; e !== null; ) {
      var l = e.alternate,
        s = t,
        o = e,
        d = o.flags;
      switch (o.tag) {
        case 0:
        case 11:
        case 15:
          (vn(s, o, n), oi(4, o));
          break;
        case 1:
          if (
            (vn(s, o, n),
            (l = o),
            (s = l.stateNode),
            typeof s.componentDidMount == "function")
          )
            try {
              s.componentDidMount();
            } catch (D) {
              At(l, l.return, D);
            }
          if (((l = o), (s = l.updateQueue), s !== null)) {
            var v = l.stateNode;
            try {
              var R = s.shared.hiddenCallbacks;
              if (R !== null)
                for (s.shared.hiddenCallbacks = null, s = 0; s < R.length; s++)
                  Qf(R[s], v);
            } catch (D) {
              At(l, l.return, D);
            }
          }
          (n && d & 64 && eh(o), ci(o, o.return));
          break;
        case 27:
          ih(o);
        case 26:
        case 5:
          (vn(s, o, n), n && l === null && d & 4 && ah(o), ci(o, o.return));
          break;
        case 12:
          vn(s, o, n);
          break;
        case 31:
          (vn(s, o, n), n && d & 4 && ch(s, o));
          break;
        case 13:
          (vn(s, o, n), n && d & 4 && fh(s, o));
          break;
        case 22:
          (o.memoizedState === null && vn(s, o, n), ci(o, o.return));
          break;
        case 30:
          break;
        default:
          vn(s, o, n);
      }
      e = e.sibling;
    }
  }
  function so(t, e) {
    var n = null;
    (t !== null &&
      t.memoizedState !== null &&
      t.memoizedState.cachePool !== null &&
      (n = t.memoizedState.cachePool.pool),
      (t = null),
      e.memoizedState !== null &&
        e.memoizedState.cachePool !== null &&
        (t = e.memoizedState.cachePool.pool),
      t !== n && (t != null && t.refCount++, n != null && Pl(n)));
  }
  function ro(t, e) {
    ((t = null),
      e.alternate !== null && (t = e.alternate.memoizedState.cache),
      (e = e.memoizedState.cache),
      e !== t && (e.refCount++, t != null && Pl(t)));
  }
  function Ze(t, e, n, l) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) (mh(t, e, n, l), (e = e.sibling));
  }
  function mh(t, e, n, l) {
    var s = e.flags;
    switch (e.tag) {
      case 0:
      case 11:
      case 15:
        (Ze(t, e, n, l), s & 2048 && oi(9, e));
        break;
      case 1:
        Ze(t, e, n, l);
        break;
      case 3:
        (Ze(t, e, n, l),
          s & 2048 &&
            ((t = null),
            e.alternate !== null && (t = e.alternate.memoizedState.cache),
            (e = e.memoizedState.cache),
            e !== t && (e.refCount++, t != null && Pl(t))));
        break;
      case 12:
        if (s & 2048) {
          (Ze(t, e, n, l), (t = e.stateNode));
          try {
            var o = e.memoizedProps,
              d = o.id,
              v = o.onPostCommit;
            typeof v == "function" &&
              v(
                d,
                e.alternate === null ? "mount" : "update",
                t.passiveEffectDuration,
                -0,
              );
          } catch (R) {
            At(e, e.return, R);
          }
        } else Ze(t, e, n, l);
        break;
      case 31:
        Ze(t, e, n, l);
        break;
      case 13:
        Ze(t, e, n, l);
        break;
      case 23:
        break;
      case 22:
        ((o = e.stateNode),
          (d = e.alternate),
          e.memoizedState !== null
            ? o._visibility & 2
              ? Ze(t, e, n, l)
              : fi(t, e)
            : o._visibility & 2
              ? Ze(t, e, n, l)
              : ((o._visibility |= 2),
                fl(t, e, n, l, (e.subtreeFlags & 10256) !== 0 || !1)),
          s & 2048 && so(d, e));
        break;
      case 24:
        (Ze(t, e, n, l), s & 2048 && ro(e.alternate, e));
        break;
      default:
        Ze(t, e, n, l);
    }
  }
  function fl(t, e, n, l, s) {
    for (
      s = s && ((e.subtreeFlags & 10256) !== 0 || !1), e = e.child;
      e !== null;
    ) {
      var o = t,
        d = e,
        v = n,
        R = l,
        D = d.flags;
      switch (d.tag) {
        case 0:
        case 11:
        case 15:
          (fl(o, d, v, R, s), oi(8, d));
          break;
        case 23:
          break;
        case 22:
          var j = d.stateNode;
          (d.memoizedState !== null
            ? j._visibility & 2
              ? fl(o, d, v, R, s)
              : fi(o, d)
            : ((j._visibility |= 2), fl(o, d, v, R, s)),
            s && D & 2048 && so(d.alternate, d));
          break;
        case 24:
          (fl(o, d, v, R, s), s && D & 2048 && ro(d.alternate, d));
          break;
        default:
          fl(o, d, v, R, s);
      }
      e = e.sibling;
    }
  }
  function fi(t, e) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) {
        var n = t,
          l = e,
          s = l.flags;
        switch (l.tag) {
          case 22:
            (fi(n, l), s & 2048 && so(l.alternate, l));
            break;
          case 24:
            (fi(n, l), s & 2048 && ro(l.alternate, l));
            break;
          default:
            fi(n, l);
        }
        e = e.sibling;
      }
  }
  var di = 8192;
  function dl(t, e, n) {
    if (t.subtreeFlags & di)
      for (t = t.child; t !== null; ) (yh(t, e, n), (t = t.sibling));
  }
  function yh(t, e, n) {
    switch (t.tag) {
      case 26:
        (dl(t, e, n),
          t.flags & di &&
            t.memoizedState !== null &&
            $v(n, Qe, t.memoizedState, t.memoizedProps));
        break;
      case 5:
        dl(t, e, n);
        break;
      case 3:
      case 4:
        var l = Qe;
        ((Qe = Ju(t.stateNode.containerInfo)), dl(t, e, n), (Qe = l));
        break;
      case 22:
        t.memoizedState === null &&
          ((l = t.alternate),
          l !== null && l.memoizedState !== null
            ? ((l = di), (di = 16777216), dl(t, e, n), (di = l))
            : dl(t, e, n));
        break;
      default:
        dl(t, e, n);
    }
  }
  function ph(t) {
    var e = t.alternate;
    if (e !== null && ((t = e.child), t !== null)) {
      e.child = null;
      do ((e = t.sibling), (t.sibling = null), (t = e));
      while (t !== null);
    }
  }
  function hi(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var n = 0; n < e.length; n++) {
          var l = e[n];
          ((ee = l), vh(l, t));
        }
      ph(t);
    }
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; ) (gh(t), (t = t.sibling));
  }
  function gh(t) {
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        (hi(t), t.flags & 2048 && Yn(9, t, t.return));
        break;
      case 3:
        hi(t);
        break;
      case 12:
        hi(t);
        break;
      case 22:
        var e = t.stateNode;
        t.memoizedState !== null &&
        e._visibility & 2 &&
        (t.return === null || t.return.tag !== 13)
          ? ((e._visibility &= -3), Uu(t))
          : hi(t);
        break;
      default:
        hi(t);
    }
  }
  function Uu(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var n = 0; n < e.length; n++) {
          var l = e[n];
          ((ee = l), vh(l, t));
        }
      ph(t);
    }
    for (t = t.child; t !== null; ) {
      switch (((e = t), e.tag)) {
        case 0:
        case 11:
        case 15:
          (Yn(8, e, e.return), Uu(e));
          break;
        case 22:
          ((n = e.stateNode),
            n._visibility & 2 && ((n._visibility &= -3), Uu(e)));
          break;
        default:
          Uu(e);
      }
      t = t.sibling;
    }
  }
  function vh(t, e) {
    for (; ee !== null; ) {
      var n = ee;
      switch (n.tag) {
        case 0:
        case 11:
        case 15:
          Yn(8, n, e);
          break;
        case 23:
        case 22:
          if (n.memoizedState !== null && n.memoizedState.cachePool !== null) {
            var l = n.memoizedState.cachePool.pool;
            l != null && l.refCount++;
          }
          break;
        case 24:
          Pl(n.memoizedState.cache);
      }
      if (((l = n.child), l !== null)) ((l.return = n), (ee = l));
      else
        t: for (n = t; ee !== null; ) {
          l = ee;
          var s = l.sibling,
            o = l.return;
          if ((rh(l), l === n)) {
            ee = null;
            break t;
          }
          if (s !== null) {
            ((s.return = o), (ee = s));
            break t;
          }
          ee = o;
        }
    }
  }
  var mv = {
      getCacheForType: function (t) {
        var e = ue(Zt),
          n = e.data.get(t);
        return (n === void 0 && ((n = t()), e.data.set(t, n)), n);
      },
      cacheSignal: function () {
        return ue(Zt).controller.signal;
      },
    },
    yv = typeof WeakMap == "function" ? WeakMap : Map,
    Rt = 0,
    wt = null,
    ht = null,
    yt = 0,
    xt = 0,
    Oe = null,
    Gn = !1,
    hl = !1,
    oo = !1,
    Sn = 0,
    Yt = 0,
    Xn = 0,
    Ma = 0,
    co = 0,
    Ce = 0,
    ml = 0,
    mi = null,
    ve = null,
    fo = !1,
    Nu = 0,
    Sh = 0,
    Bu = 1 / 0,
    ju = null,
    Vn = null,
    Pt = 0,
    Qn = null,
    yl = null,
    bn = 0,
    ho = 0,
    mo = null,
    bh = null,
    yi = 0,
    yo = null;
  function we() {
    return (Rt & 2) !== 0 && yt !== 0 ? yt & -yt : B.T !== null ? _o() : Nc();
  }
  function _h() {
    if (Ce === 0)
      if ((yt & 536870912) === 0 || gt) {
        var t = Zi;
        ((Zi <<= 1), (Zi & 3932160) === 0 && (Zi = 262144), (Ce = t));
      } else Ce = 536870912;
    return ((t = Me.current), t !== null && (t.flags |= 32), Ce);
  }
  function Se(t, e, n) {
    (((t === wt && (xt === 2 || xt === 9)) || t.cancelPendingCommit !== null) &&
      (pl(t, 0), Zn(t, yt, Ce, !1)),
      Bl(t, n),
      ((Rt & 2) === 0 || t !== wt) &&
        (t === wt &&
          ((Rt & 2) === 0 && (Ma |= n), Yt === 4 && Zn(t, yt, Ce, !1)),
        Ie(t)));
  }
  function Eh(t, e, n) {
    if ((Rt & 6) !== 0) throw Error(r(327));
    var l = (!n && (e & 127) === 0 && (e & t.expiredLanes) === 0) || Nl(t, e),
      s = l ? vv(t, e) : go(t, e, !0),
      o = l;
    do {
      if (s === 0) {
        hl && !l && Zn(t, e, 0, !1);
        break;
      } else {
        if (((n = t.current.alternate), o && !pv(n))) {
          ((s = go(t, e, !1)), (o = !1));
          continue;
        }
        if (s === 2) {
          if (((o = e), t.errorRecoveryDisabledLanes & o)) var d = 0;
          else
            ((d = t.pendingLanes & -536870913),
              (d = d !== 0 ? d : d & 536870912 ? 536870912 : 0));
          if (d !== 0) {
            e = d;
            t: {
              var v = t;
              s = mi;
              var R = v.current.memoizedState.isDehydrated;
              if ((R && (pl(v, d).flags |= 256), (d = go(v, d, !1)), d !== 2)) {
                if (oo && !R) {
                  ((v.errorRecoveryDisabledLanes |= o), (Ma |= o), (s = 4));
                  break t;
                }
                ((o = ve),
                  (ve = s),
                  o !== null &&
                    (ve === null ? (ve = o) : ve.push.apply(ve, o)));
              }
              s = d;
            }
            if (((o = !1), s !== 2)) continue;
          }
        }
        if (s === 1) {
          (pl(t, 0), Zn(t, e, 0, !0));
          break;
        }
        t: {
          switch (((l = t), (o = s), o)) {
            case 0:
            case 1:
              throw Error(r(345));
            case 4:
              if ((e & 4194048) !== e) break;
            case 6:
              Zn(l, e, Ce, !Gn);
              break t;
            case 2:
              ve = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(r(329));
          }
          if ((e & 62914560) === e && ((s = Nu + 300 - ce()), 10 < s)) {
            if ((Zn(l, e, Ce, !Gn), Ji(l, 0, !0) !== 0)) break t;
            ((bn = e),
              (l.timeoutHandle = $h(
                Rh.bind(
                  null,
                  l,
                  n,
                  ve,
                  ju,
                  fo,
                  e,
                  Ce,
                  Ma,
                  ml,
                  Gn,
                  o,
                  "Throttled",
                  -0,
                  0,
                ),
                s,
              )));
            break t;
          }
          Rh(l, n, ve, ju, fo, e, Ce, Ma, ml, Gn, o, null, -0, 0);
        }
      }
      break;
    } while (!0);
    Ie(t);
  }
  function Rh(t, e, n, l, s, o, d, v, R, D, j, V, U, N) {
    if (
      ((t.timeoutHandle = -1),
      (V = e.subtreeFlags),
      V & 8192 || (V & 16785408) === 16785408)
    ) {
      ((V = {
        stylesheets: null,
        count: 0,
        imgCount: 0,
        imgBytes: 0,
        suspenseyImages: [],
        waitingForImages: !0,
        waitingForViewTransition: !1,
        unsuspend: ln,
      }),
        yh(e, o, V));
      var $ =
        (o & 62914560) === o ? Nu - ce() : (o & 4194048) === o ? Sh - ce() : 0;
      if ((($ = t0(V, $)), $ !== null)) {
        ((bn = o),
          (t.cancelPendingCommit = $(
            wh.bind(null, t, e, o, n, l, s, d, v, R, j, V, null, U, N),
          )),
          Zn(t, o, d, !D));
        return;
      }
    }
    wh(t, e, o, n, l, s, d, v, R);
  }
  function pv(t) {
    for (var e = t; ; ) {
      var n = e.tag;
      if (
        (n === 0 || n === 11 || n === 15) &&
        e.flags & 16384 &&
        ((n = e.updateQueue), n !== null && ((n = n.stores), n !== null))
      )
        for (var l = 0; l < n.length; l++) {
          var s = n[l],
            o = s.getSnapshot;
          s = s.value;
          try {
            if (!xe(o(), s)) return !1;
          } catch {
            return !1;
          }
        }
      if (((n = e.child), e.subtreeFlags & 16384 && n !== null))
        ((n.return = e), (e = n));
      else {
        if (e === t) break;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t) return !0;
          e = e.return;
        }
        ((e.sibling.return = e.return), (e = e.sibling));
      }
    }
    return !0;
  }
  function Zn(t, e, n, l) {
    ((e &= ~co),
      (e &= ~Ma),
      (t.suspendedLanes |= e),
      (t.pingedLanes &= ~e),
      l && (t.warmLanes |= e),
      (l = t.expirationTimes));
    for (var s = e; 0 < s; ) {
      var o = 31 - Te(s),
        d = 1 << o;
      ((l[o] = -1), (s &= ~d));
    }
    n !== 0 && Dc(t, n, e);
  }
  function Hu() {
    return (Rt & 6) === 0 ? (pi(0), !1) : !0;
  }
  function po() {
    if (ht !== null) {
      if (xt === 0) var t = ht.return;
      else ((t = ht), (on = va = null), wr(t), (ul = null), (Wl = 0), (t = ht));
      for (; t !== null; ) (th(t.alternate, t), (t = t.return));
      ht = null;
    }
  }
  function pl(t, e) {
    var n = t.timeoutHandle;
    (n !== -1 && ((t.timeoutHandle = -1), Bv(n)),
      (n = t.cancelPendingCommit),
      n !== null && ((t.cancelPendingCommit = null), n()),
      (bn = 0),
      po(),
      (wt = t),
      (ht = n = sn(t.current, null)),
      (yt = e),
      (xt = 0),
      (Oe = null),
      (Gn = !1),
      (hl = Nl(t, e)),
      (oo = !1),
      (ml = Ce = co = Ma = Xn = Yt = 0),
      (ve = mi = null),
      (fo = !1),
      (e & 8) !== 0 && (e |= e & 32));
    var l = t.entangledLanes;
    if (l !== 0)
      for (t = t.entanglements, l &= e; 0 < l; ) {
        var s = 31 - Te(l),
          o = 1 << s;
        ((e |= t[s]), (l &= ~o));
      }
    return ((Sn = e), iu(), n);
  }
  function Th(t, e) {
    ((ct = null),
      (B.H = ui),
      e === il || e === hu
        ? ((e = Yf()), (xt = 3))
        : e === Sr
          ? ((e = Yf()), (xt = 4))
          : (xt =
              e === Jr
                ? 8
                : e !== null &&
                    typeof e == "object" &&
                    typeof e.then == "function"
                  ? 6
                  : 1),
      (Oe = e),
      ht === null && ((Yt = 1), Mu(t, Ne(e, t.current))));
  }
  function xh() {
    var t = Me.current;
    return t === null
      ? !0
      : (yt & 4194048) === yt
        ? qe === null
        : (yt & 62914560) === yt || (yt & 536870912) !== 0
          ? t === qe
          : !1;
  }
  function Ah() {
    var t = B.H;
    return ((B.H = ui), t === null ? ui : t);
  }
  function Mh() {
    var t = B.A;
    return ((B.A = mv), t);
  }
  function qu() {
    ((Yt = 4),
      Gn || ((yt & 4194048) !== yt && Me.current !== null) || (hl = !0),
      ((Xn & 134217727) === 0 && (Ma & 134217727) === 0) ||
        wt === null ||
        Zn(wt, yt, Ce, !1));
  }
  function go(t, e, n) {
    var l = Rt;
    Rt |= 2;
    var s = Ah(),
      o = Mh();
    ((wt !== t || yt !== e) && ((ju = null), pl(t, e)), (e = !1));
    var d = Yt;
    t: do
      try {
        if (xt !== 0 && ht !== null) {
          var v = ht,
            R = Oe;
          switch (xt) {
            case 8:
              (po(), (d = 6));
              break t;
            case 3:
            case 2:
            case 9:
            case 6:
              Me.current === null && (e = !0);
              var D = xt;
              if (((xt = 0), (Oe = null), gl(t, v, R, D), n && hl)) {
                d = 0;
                break t;
              }
              break;
            default:
              ((D = xt), (xt = 0), (Oe = null), gl(t, v, R, D));
          }
        }
        (gv(), (d = Yt));
        break;
      } catch (j) {
        Th(t, j);
      }
    while (!0);
    return (
      e && t.shellSuspendCounter++,
      (on = va = null),
      (Rt = l),
      (B.H = s),
      (B.A = o),
      ht === null && ((wt = null), (yt = 0), iu()),
      d
    );
  }
  function gv() {
    for (; ht !== null; ) zh(ht);
  }
  function vv(t, e) {
    var n = Rt;
    Rt |= 2;
    var l = Ah(),
      s = Mh();
    wt !== t || yt !== e
      ? ((ju = null), (Bu = ce() + 500), pl(t, e))
      : (hl = Nl(t, e));
    t: do
      try {
        if (xt !== 0 && ht !== null) {
          e = ht;
          var o = Oe;
          e: switch (xt) {
            case 1:
              ((xt = 0), (Oe = null), gl(t, e, o, 1));
              break;
            case 2:
            case 9:
              if (Hf(o)) {
                ((xt = 0), (Oe = null), Oh(e));
                break;
              }
              ((e = function () {
                ((xt !== 2 && xt !== 9) || wt !== t || (xt = 7), Ie(t));
              }),
                o.then(e, e));
              break t;
            case 3:
              xt = 7;
              break t;
            case 4:
              xt = 5;
              break t;
            case 7:
              Hf(o)
                ? ((xt = 0), (Oe = null), Oh(e))
                : ((xt = 0), (Oe = null), gl(t, e, o, 7));
              break;
            case 5:
              var d = null;
              switch (ht.tag) {
                case 26:
                  d = ht.memoizedState;
                case 5:
                case 27:
                  var v = ht;
                  if (d ? mm(d) : v.stateNode.complete) {
                    ((xt = 0), (Oe = null));
                    var R = v.sibling;
                    if (R !== null) ht = R;
                    else {
                      var D = v.return;
                      D !== null ? ((ht = D), Yu(D)) : (ht = null);
                    }
                    break e;
                  }
              }
              ((xt = 0), (Oe = null), gl(t, e, o, 5));
              break;
            case 6:
              ((xt = 0), (Oe = null), gl(t, e, o, 6));
              break;
            case 8:
              (po(), (Yt = 6));
              break t;
            default:
              throw Error(r(462));
          }
        }
        Sv();
        break;
      } catch (j) {
        Th(t, j);
      }
    while (!0);
    return (
      (on = va = null),
      (B.H = l),
      (B.A = s),
      (Rt = n),
      ht !== null ? 0 : ((wt = null), (yt = 0), iu(), Yt)
    );
  }
  function Sv() {
    for (; ht !== null && !Vi(); ) zh(ht);
  }
  function zh(t) {
    var e = Wd(t.alternate, t, Sn);
    ((t.memoizedProps = t.pendingProps), e === null ? Yu(t) : (ht = e));
  }
  function Oh(t) {
    var e = t,
      n = e.alternate;
    switch (e.tag) {
      case 15:
      case 0:
        e = Kd(n, e, e.pendingProps, e.type, void 0, yt);
        break;
      case 11:
        e = Kd(n, e, e.pendingProps, e.type.render, e.ref, yt);
        break;
      case 5:
        wr(e);
      default:
        (th(n, e), (e = ht = Mf(e, Sn)), (e = Wd(n, e, Sn)));
    }
    ((t.memoizedProps = t.pendingProps), e === null ? Yu(t) : (ht = e));
  }
  function gl(t, e, n, l) {
    ((on = va = null), wr(e), (ul = null), (Wl = 0));
    var s = e.return;
    try {
      if (sv(t, s, e, n, yt)) {
        ((Yt = 1), Mu(t, Ne(n, t.current)), (ht = null));
        return;
      }
    } catch (o) {
      if (s !== null) throw ((ht = s), o);
      ((Yt = 1), Mu(t, Ne(n, t.current)), (ht = null));
      return;
    }
    e.flags & 32768
      ? (gt || l === 1
          ? (t = !0)
          : hl || (yt & 536870912) !== 0
            ? (t = !1)
            : ((Gn = t = !0),
              (l === 2 || l === 9 || l === 3 || l === 6) &&
                ((l = Me.current),
                l !== null && l.tag === 13 && (l.flags |= 16384))),
        Ch(e, t))
      : Yu(e);
  }
  function Yu(t) {
    var e = t;
    do {
      if ((e.flags & 32768) !== 0) {
        Ch(e, Gn);
        return;
      }
      t = e.return;
      var n = cv(e.alternate, e, Sn);
      if (n !== null) {
        ht = n;
        return;
      }
      if (((e = e.sibling), e !== null)) {
        ht = e;
        return;
      }
      ht = e = t;
    } while (e !== null);
    Yt === 0 && (Yt = 5);
  }
  function Ch(t, e) {
    do {
      var n = fv(t.alternate, t);
      if (n !== null) {
        ((n.flags &= 32767), (ht = n));
        return;
      }
      if (
        ((n = t.return),
        n !== null &&
          ((n.flags |= 32768), (n.subtreeFlags = 0), (n.deletions = null)),
        !e && ((t = t.sibling), t !== null))
      ) {
        ht = t;
        return;
      }
      ht = t = n;
    } while (t !== null);
    ((Yt = 6), (ht = null));
  }
  function wh(t, e, n, l, s, o, d, v, R) {
    t.cancelPendingCommit = null;
    do Gu();
    while (Pt !== 0);
    if ((Rt & 6) !== 0) throw Error(r(327));
    if (e !== null) {
      if (e === t.current) throw Error(r(177));
      if (
        ((o = e.lanes | e.childLanes),
        (o |= lr),
        Wp(t, n, o, d, v, R),
        t === wt && ((ht = wt = null), (yt = 0)),
        (yl = e),
        (Qn = t),
        (bn = n),
        (ho = o),
        (mo = s),
        (bh = l),
        (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
          ? ((t.callbackNode = null),
            (t.callbackPriority = 0),
            Rv(Ha, function () {
              return (Bh(), null);
            }))
          : ((t.callbackNode = null), (t.callbackPriority = 0)),
        (l = (e.flags & 13878) !== 0),
        (e.subtreeFlags & 13878) !== 0 || l)
      ) {
        ((l = B.T), (B.T = null), (s = J.p), (J.p = 2), (d = Rt), (Rt |= 4));
        try {
          dv(t, e, n);
        } finally {
          ((Rt = d), (J.p = s), (B.T = l));
        }
      }
      ((Pt = 1), Dh(), Lh(), Uh());
    }
  }
  function Dh() {
    if (Pt === 1) {
      Pt = 0;
      var t = Qn,
        e = yl,
        n = (e.flags & 13878) !== 0;
      if ((e.subtreeFlags & 13878) !== 0 || n) {
        ((n = B.T), (B.T = null));
        var l = J.p;
        J.p = 2;
        var s = Rt;
        Rt |= 4;
        try {
          dh(e, t);
          var o = Oo,
            d = vf(t.containerInfo),
            v = o.focusedElem,
            R = o.selectionRange;
          if (
            d !== v &&
            v &&
            v.ownerDocument &&
            gf(v.ownerDocument.documentElement, v)
          ) {
            if (R !== null && $s(v)) {
              var D = R.start,
                j = R.end;
              if ((j === void 0 && (j = D), "selectionStart" in v))
                ((v.selectionStart = D),
                  (v.selectionEnd = Math.min(j, v.value.length)));
              else {
                var V = v.ownerDocument || document,
                  U = (V && V.defaultView) || window;
                if (U.getSelection) {
                  var N = U.getSelection(),
                    $ = v.textContent.length,
                    st = Math.min(R.start, $),
                    Ot = R.end === void 0 ? st : Math.min(R.end, $);
                  !N.extend && st > Ot && ((d = Ot), (Ot = st), (st = d));
                  var O = pf(v, st),
                    A = pf(v, Ot);
                  if (
                    O &&
                    A &&
                    (N.rangeCount !== 1 ||
                      N.anchorNode !== O.node ||
                      N.anchorOffset !== O.offset ||
                      N.focusNode !== A.node ||
                      N.focusOffset !== A.offset)
                  ) {
                    var w = V.createRange();
                    (w.setStart(O.node, O.offset),
                      N.removeAllRanges(),
                      st > Ot
                        ? (N.addRange(w), N.extend(A.node, A.offset))
                        : (w.setEnd(A.node, A.offset), N.addRange(w)));
                  }
                }
              }
            }
            for (V = [], N = v; (N = N.parentNode); )
              N.nodeType === 1 &&
                V.push({ element: N, left: N.scrollLeft, top: N.scrollTop });
            for (
              typeof v.focus == "function" && v.focus(), v = 0;
              v < V.length;
              v++
            ) {
              var q = V[v];
              ((q.element.scrollLeft = q.left), (q.element.scrollTop = q.top));
            }
          }
          (($u = !!zo), (Oo = zo = null));
        } finally {
          ((Rt = s), (J.p = l), (B.T = n));
        }
      }
      ((t.current = e), (Pt = 2));
    }
  }
  function Lh() {
    if (Pt === 2) {
      Pt = 0;
      var t = Qn,
        e = yl,
        n = (e.flags & 8772) !== 0;
      if ((e.subtreeFlags & 8772) !== 0 || n) {
        ((n = B.T), (B.T = null));
        var l = J.p;
        J.p = 2;
        var s = Rt;
        Rt |= 4;
        try {
          sh(t, e.alternate, e);
        } finally {
          ((Rt = s), (J.p = l), (B.T = n));
        }
      }
      Pt = 3;
    }
  }
  function Uh() {
    if (Pt === 4 || Pt === 3) {
      ((Pt = 0), ws());
      var t = Qn,
        e = yl,
        n = bn,
        l = bh;
      (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
        ? (Pt = 5)
        : ((Pt = 0), (yl = Qn = null), Nh(t, t.pendingLanes));
      var s = t.pendingLanes;
      if (
        (s === 0 && (Vn = null),
        Us(n),
        (e = e.stateNode),
        Re && typeof Re.onCommitFiberRoot == "function")
      )
        try {
          Re.onCommitFiberRoot(Ul, e, void 0, (e.current.flags & 128) === 128);
        } catch {}
      if (l !== null) {
        ((e = B.T), (s = J.p), (J.p = 2), (B.T = null));
        try {
          for (var o = t.onRecoverableError, d = 0; d < l.length; d++) {
            var v = l[d];
            o(v.value, { componentStack: v.stack });
          }
        } finally {
          ((B.T = e), (J.p = s));
        }
      }
      ((bn & 3) !== 0 && Gu(),
        Ie(t),
        (s = t.pendingLanes),
        (n & 261930) !== 0 && (s & 42) !== 0
          ? t === yo
            ? yi++
            : ((yi = 0), (yo = t))
          : (yi = 0),
        pi(0));
    }
  }
  function Nh(t, e) {
    (t.pooledCacheLanes &= e) === 0 &&
      ((e = t.pooledCache), e != null && ((t.pooledCache = null), Pl(e)));
  }
  function Gu() {
    return (Dh(), Lh(), Uh(), Bh());
  }
  function Bh() {
    if (Pt !== 5) return !1;
    var t = Qn,
      e = ho;
    ho = 0;
    var n = Us(bn),
      l = B.T,
      s = J.p;
    try {
      ((J.p = 32 > n ? 32 : n), (B.T = null), (n = mo), (mo = null));
      var o = Qn,
        d = bn;
      if (((Pt = 0), (yl = Qn = null), (bn = 0), (Rt & 6) !== 0))
        throw Error(r(331));
      var v = Rt;
      if (
        ((Rt |= 4),
        gh(o.current),
        mh(o, o.current, d, n),
        (Rt = v),
        pi(0, !1),
        Re && typeof Re.onPostCommitFiberRoot == "function")
      )
        try {
          Re.onPostCommitFiberRoot(Ul, o);
        } catch {}
      return !0;
    } finally {
      ((J.p = s), (B.T = l), Nh(t, e));
    }
  }
  function jh(t, e, n) {
    ((e = Ne(n, e)),
      (e = Kr(t.stateNode, e, 2)),
      (t = jn(t, e, 2)),
      t !== null && (Bl(t, 2), Ie(t)));
  }
  function At(t, e, n) {
    if (t.tag === 3) jh(t, t, n);
    else
      for (; e !== null; ) {
        if (e.tag === 3) {
          jh(e, t, n);
          break;
        } else if (e.tag === 1) {
          var l = e.stateNode;
          if (
            typeof e.type.getDerivedStateFromError == "function" ||
            (typeof l.componentDidCatch == "function" &&
              (Vn === null || !Vn.has(l)))
          ) {
            ((t = Ne(n, t)),
              (n = Hd(2)),
              (l = jn(e, n, 2)),
              l !== null && (qd(n, l, e, t), Bl(l, 2), Ie(l)));
            break;
          }
        }
        e = e.return;
      }
  }
  function vo(t, e, n) {
    var l = t.pingCache;
    if (l === null) {
      l = t.pingCache = new yv();
      var s = new Set();
      l.set(e, s);
    } else ((s = l.get(e)), s === void 0 && ((s = new Set()), l.set(e, s)));
    s.has(n) ||
      ((oo = !0), s.add(n), (t = bv.bind(null, t, e, n)), e.then(t, t));
  }
  function bv(t, e, n) {
    var l = t.pingCache;
    (l !== null && l.delete(e),
      (t.pingedLanes |= t.suspendedLanes & n),
      (t.warmLanes &= ~n),
      wt === t &&
        (yt & n) === n &&
        (Yt === 4 || (Yt === 3 && (yt & 62914560) === yt && 300 > ce() - Nu)
          ? (Rt & 2) === 0 && pl(t, 0)
          : (co |= n),
        ml === yt && (ml = 0)),
      Ie(t));
  }
  function Hh(t, e) {
    (e === 0 && (e = wc()), (t = ya(t, e)), t !== null && (Bl(t, e), Ie(t)));
  }
  function _v(t) {
    var e = t.memoizedState,
      n = 0;
    (e !== null && (n = e.retryLane), Hh(t, n));
  }
  function Ev(t, e) {
    var n = 0;
    switch (t.tag) {
      case 31:
      case 13:
        var l = t.stateNode,
          s = t.memoizedState;
        s !== null && (n = s.retryLane);
        break;
      case 19:
        l = t.stateNode;
        break;
      case 22:
        l = t.stateNode._retryCache;
        break;
      default:
        throw Error(r(314));
    }
    (l !== null && l.delete(e), Hh(t, n));
  }
  function Rv(t, e) {
    return Xe(t, e);
  }
  var Xu = null,
    vl = null,
    So = !1,
    Vu = !1,
    bo = !1,
    Kn = 0;
  function Ie(t) {
    (t !== vl &&
      t.next === null &&
      (vl === null ? (Xu = vl = t) : (vl = vl.next = t)),
      (Vu = !0),
      So || ((So = !0), xv()));
  }
  function pi(t, e) {
    if (!bo && Vu) {
      bo = !0;
      do
        for (var n = !1, l = Xu; l !== null; ) {
          if (t !== 0) {
            var s = l.pendingLanes;
            if (s === 0) var o = 0;
            else {
              var d = l.suspendedLanes,
                v = l.pingedLanes;
              ((o = (1 << (31 - Te(42 | t) + 1)) - 1),
                (o &= s & ~(d & ~v)),
                (o = o & 201326741 ? (o & 201326741) | 1 : o ? o | 2 : 0));
            }
            o !== 0 && ((n = !0), Xh(l, o));
          } else
            ((o = yt),
              (o = Ji(
                l,
                l === wt ? o : 0,
                l.cancelPendingCommit !== null || l.timeoutHandle !== -1,
              )),
              (o & 3) === 0 || Nl(l, o) || ((n = !0), Xh(l, o)));
          l = l.next;
        }
      while (n);
      bo = !1;
    }
  }
  function Tv() {
    qh();
  }
  function qh() {
    Vu = So = !1;
    var t = 0;
    Kn !== 0 && Nv() && (t = Kn);
    for (var e = ce(), n = null, l = Xu; l !== null; ) {
      var s = l.next,
        o = Yh(l, e);
      (o === 0
        ? ((l.next = null),
          n === null ? (Xu = s) : (n.next = s),
          s === null && (vl = n))
        : ((n = l), (t !== 0 || (o & 3) !== 0) && (Vu = !0)),
        (l = s));
    }
    ((Pt !== 0 && Pt !== 5) || pi(t), Kn !== 0 && (Kn = 0));
  }
  function Yh(t, e) {
    for (
      var n = t.suspendedLanes,
        l = t.pingedLanes,
        s = t.expirationTimes,
        o = t.pendingLanes & -62914561;
      0 < o;
    ) {
      var d = 31 - Te(o),
        v = 1 << d,
        R = s[d];
      (R === -1
        ? ((v & n) === 0 || (v & l) !== 0) && (s[d] = Ip(v, e))
        : R <= e && (t.expiredLanes |= v),
        (o &= ~v));
    }
    if (
      ((e = wt),
      (n = yt),
      (n = Ji(
        t,
        t === e ? n : 0,
        t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
      )),
      (l = t.callbackNode),
      n === 0 ||
        (t === e && (xt === 2 || xt === 9)) ||
        t.cancelPendingCommit !== null)
    )
      return (
        l !== null && l !== null && Ll(l),
        (t.callbackNode = null),
        (t.callbackPriority = 0)
      );
    if ((n & 3) === 0 || Nl(t, n)) {
      if (((e = n & -n), e === t.callbackPriority)) return e;
      switch ((l !== null && Ll(l), Us(n))) {
        case 2:
        case 8:
          n = Je;
          break;
        case 32:
          n = Ha;
          break;
        case 268435456:
          n = Cc;
          break;
        default:
          n = Ha;
      }
      return (
        (l = Gh.bind(null, t)),
        (n = Xe(n, l)),
        (t.callbackPriority = e),
        (t.callbackNode = n),
        e
      );
    }
    return (
      l !== null && l !== null && Ll(l),
      (t.callbackPriority = 2),
      (t.callbackNode = null),
      2
    );
  }
  function Gh(t, e) {
    if (Pt !== 0 && Pt !== 5)
      return ((t.callbackNode = null), (t.callbackPriority = 0), null);
    var n = t.callbackNode;
    if (Gu() && t.callbackNode !== n) return null;
    var l = yt;
    return (
      (l = Ji(
        t,
        t === wt ? l : 0,
        t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
      )),
      l === 0
        ? null
        : (Eh(t, l, e),
          Yh(t, ce()),
          t.callbackNode != null && t.callbackNode === n
            ? Gh.bind(null, t)
            : null)
    );
  }
  function Xh(t, e) {
    if (Gu()) return null;
    Eh(t, e, !0);
  }
  function xv() {
    jv(function () {
      (Rt & 6) !== 0 ? Xe(ae, Tv) : qh();
    });
  }
  function _o() {
    if (Kn === 0) {
      var t = al;
      (t === 0 && ((t = Qi), (Qi <<= 1), (Qi & 261888) === 0 && (Qi = 256)),
        (Kn = t));
    }
    return Kn;
  }
  function Vh(t) {
    return t == null || typeof t == "symbol" || typeof t == "boolean"
      ? null
      : typeof t == "function"
        ? t
        : Ii("" + t);
  }
  function Qh(t, e) {
    var n = e.ownerDocument.createElement("input");
    return (
      (n.name = e.name),
      (n.value = e.value),
      t.id && n.setAttribute("form", t.id),
      e.parentNode.insertBefore(n, e),
      (t = new FormData(t)),
      n.parentNode.removeChild(n),
      t
    );
  }
  function Av(t, e, n, l, s) {
    if (e === "submit" && n && n.stateNode === s) {
      var o = Vh((s[he] || null).action),
        d = l.submitter;
      d &&
        ((e = (e = d[he] || null)
          ? Vh(e.formAction)
          : d.getAttribute("formAction")),
        e !== null && ((o = e), (d = null)));
      var v = new eu("action", "action", null, l, s);
      t.push({
        event: v,
        listeners: [
          {
            instance: null,
            listener: function () {
              if (l.defaultPrevented) {
                if (Kn !== 0) {
                  var R = d ? Qh(s, d) : new FormData(s);
                  Yr(
                    n,
                    { pending: !0, data: R, method: s.method, action: o },
                    null,
                    R,
                  );
                }
              } else
                typeof o == "function" &&
                  (v.preventDefault(),
                  (R = d ? Qh(s, d) : new FormData(s)),
                  Yr(
                    n,
                    { pending: !0, data: R, method: s.method, action: o },
                    o,
                    R,
                  ));
            },
            currentTarget: s,
          },
        ],
      });
    }
  }
  for (var Eo = 0; Eo < ar.length; Eo++) {
    var Ro = ar[Eo],
      Mv = Ro.toLowerCase(),
      zv = Ro[0].toUpperCase() + Ro.slice(1);
    Ve(Mv, "on" + zv);
  }
  (Ve(_f, "onAnimationEnd"),
    Ve(Ef, "onAnimationIteration"),
    Ve(Rf, "onAnimationStart"),
    Ve("dblclick", "onDoubleClick"),
    Ve("focusin", "onFocus"),
    Ve("focusout", "onBlur"),
    Ve(Qg, "onTransitionRun"),
    Ve(Zg, "onTransitionStart"),
    Ve(Kg, "onTransitionCancel"),
    Ve(Tf, "onTransitionEnd"),
    Va("onMouseEnter", ["mouseout", "mouseover"]),
    Va("onMouseLeave", ["mouseout", "mouseover"]),
    Va("onPointerEnter", ["pointerout", "pointerover"]),
    Va("onPointerLeave", ["pointerout", "pointerover"]),
    fa(
      "onChange",
      "change click focusin focusout input keydown keyup selectionchange".split(
        " ",
      ),
    ),
    fa(
      "onSelect",
      "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
        " ",
      ),
    ),
    fa("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
    fa(
      "onCompositionEnd",
      "compositionend focusout keydown keypress keyup mousedown".split(" "),
    ),
    fa(
      "onCompositionStart",
      "compositionstart focusout keydown keypress keyup mousedown".split(" "),
    ),
    fa(
      "onCompositionUpdate",
      "compositionupdate focusout keydown keypress keyup mousedown".split(" "),
    ));
  var gi =
      "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
        " ",
      ),
    Ov = new Set(
      "beforetoggle cancel close invalid load scroll scrollend toggle"
        .split(" ")
        .concat(gi),
    );
  function Zh(t, e) {
    e = (e & 4) !== 0;
    for (var n = 0; n < t.length; n++) {
      var l = t[n],
        s = l.event;
      l = l.listeners;
      t: {
        var o = void 0;
        if (e)
          for (var d = l.length - 1; 0 <= d; d--) {
            var v = l[d],
              R = v.instance,
              D = v.currentTarget;
            if (((v = v.listener), R !== o && s.isPropagationStopped()))
              break t;
            ((o = v), (s.currentTarget = D));
            try {
              o(s);
            } catch (j) {
              lu(j);
            }
            ((s.currentTarget = null), (o = R));
          }
        else
          for (d = 0; d < l.length; d++) {
            if (
              ((v = l[d]),
              (R = v.instance),
              (D = v.currentTarget),
              (v = v.listener),
              R !== o && s.isPropagationStopped())
            )
              break t;
            ((o = v), (s.currentTarget = D));
            try {
              o(s);
            } catch (j) {
              lu(j);
            }
            ((s.currentTarget = null), (o = R));
          }
      }
    }
  }
  function mt(t, e) {
    var n = e[Ns];
    n === void 0 && (n = e[Ns] = new Set());
    var l = t + "__bubble";
    n.has(l) || (Kh(e, t, 2, !1), n.add(l));
  }
  function To(t, e, n) {
    var l = 0;
    (e && (l |= 4), Kh(n, t, l, e));
  }
  var Qu = "_reactListening" + Math.random().toString(36).slice(2);
  function xo(t) {
    if (!t[Qu]) {
      ((t[Qu] = !0),
        Hc.forEach(function (n) {
          n !== "selectionchange" && (Ov.has(n) || To(n, !1, t), To(n, !0, t));
        }));
      var e = t.nodeType === 9 ? t : t.ownerDocument;
      e === null || e[Qu] || ((e[Qu] = !0), To("selectionchange", !1, e));
    }
  }
  function Kh(t, e, n, l) {
    switch (_m(e)) {
      case 2:
        var s = a0;
        break;
      case 8:
        s = l0;
        break;
      default:
        s = Yo;
    }
    ((n = s.bind(null, e, n, t)),
      (s = void 0),
      !Qs ||
        (e !== "touchstart" && e !== "touchmove" && e !== "wheel") ||
        (s = !0),
      l
        ? s !== void 0
          ? t.addEventListener(e, n, { capture: !0, passive: s })
          : t.addEventListener(e, n, !0)
        : s !== void 0
          ? t.addEventListener(e, n, { passive: s })
          : t.addEventListener(e, n, !1));
  }
  function Ao(t, e, n, l, s) {
    var o = l;
    if ((e & 1) === 0 && (e & 2) === 0 && l !== null)
      t: for (;;) {
        if (l === null) return;
        var d = l.tag;
        if (d === 3 || d === 4) {
          var v = l.stateNode.containerInfo;
          if (v === s) break;
          if (d === 4)
            for (d = l.return; d !== null; ) {
              var R = d.tag;
              if ((R === 3 || R === 4) && d.stateNode.containerInfo === s)
                return;
              d = d.return;
            }
          for (; v !== null; ) {
            if (((d = Ya(v)), d === null)) return;
            if (((R = d.tag), R === 5 || R === 6 || R === 26 || R === 27)) {
              l = o = d;
              continue t;
            }
            v = v.parentNode;
          }
        }
        l = l.return;
      }
    Pc(function () {
      var D = o,
        j = Xs(n),
        V = [];
      t: {
        var U = xf.get(t);
        if (U !== void 0) {
          var N = eu,
            $ = t;
          switch (t) {
            case "keypress":
              if ($i(n) === 0) break t;
            case "keydown":
            case "keyup":
              N = Eg;
              break;
            case "focusin":
              (($ = "focus"), (N = ks));
              break;
            case "focusout":
              (($ = "blur"), (N = ks));
              break;
            case "beforeblur":
            case "afterblur":
              N = ks;
              break;
            case "click":
              if (n.button === 2) break t;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              N = $c;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              N = cg;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              N = xg;
              break;
            case _f:
            case Ef:
            case Rf:
              N = hg;
              break;
            case Tf:
              N = Mg;
              break;
            case "scroll":
            case "scrollend":
              N = rg;
              break;
            case "wheel":
              N = Og;
              break;
            case "copy":
            case "cut":
            case "paste":
              N = yg;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              N = ef;
              break;
            case "toggle":
            case "beforetoggle":
              N = wg;
          }
          var st = (e & 4) !== 0,
            Ot = !st && (t === "scroll" || t === "scrollend"),
            O = st ? (U !== null ? U + "Capture" : null) : U;
          st = [];
          for (var A = D, w; A !== null; ) {
            var q = A;
            if (
              ((w = q.stateNode),
              (q = q.tag),
              (q !== 5 && q !== 26 && q !== 27) ||
                w === null ||
                O === null ||
                ((q = ql(A, O)), q != null && st.push(vi(A, q, w))),
              Ot)
            )
              break;
            A = A.return;
          }
          0 < st.length &&
            ((U = new N(U, $, null, n, j)),
            V.push({ event: U, listeners: st }));
        }
      }
      if ((e & 7) === 0) {
        t: {
          if (
            ((U = t === "mouseover" || t === "pointerover"),
            (N = t === "mouseout" || t === "pointerout"),
            U &&
              n !== Gs &&
              ($ = n.relatedTarget || n.fromElement) &&
              (Ya($) || $[qa]))
          )
            break t;
          if (
            (N || U) &&
            ((U =
              j.window === j
                ? j
                : (U = j.ownerDocument)
                  ? U.defaultView || U.parentWindow
                  : window),
            N
              ? (($ = n.relatedTarget || n.toElement),
                (N = D),
                ($ = $ ? Ya($) : null),
                $ !== null &&
                  ((Ot = f($)),
                  (st = $.tag),
                  $ !== Ot || (st !== 5 && st !== 27 && st !== 6)) &&
                  ($ = null))
              : ((N = null), ($ = D)),
            N !== $)
          ) {
            if (
              ((st = $c),
              (q = "onMouseLeave"),
              (O = "onMouseEnter"),
              (A = "mouse"),
              (t === "pointerout" || t === "pointerover") &&
                ((st = ef),
                (q = "onPointerLeave"),
                (O = "onPointerEnter"),
                (A = "pointer")),
              (Ot = N == null ? U : Hl(N)),
              (w = $ == null ? U : Hl($)),
              (U = new st(q, A + "leave", N, n, j)),
              (U.target = Ot),
              (U.relatedTarget = w),
              (q = null),
              Ya(j) === D &&
                ((st = new st(O, A + "enter", $, n, j)),
                (st.target = w),
                (st.relatedTarget = Ot),
                (q = st)),
              (Ot = q),
              N && $)
            )
              e: {
                for (st = Cv, O = N, A = $, w = 0, q = O; q; q = st(q)) w++;
                q = 0;
                for (var at = A; at; at = st(at)) q++;
                for (; 0 < w - q; ) ((O = st(O)), w--);
                for (; 0 < q - w; ) ((A = st(A)), q--);
                for (; w--; ) {
                  if (O === A || (A !== null && O === A.alternate)) {
                    st = O;
                    break e;
                  }
                  ((O = st(O)), (A = st(A)));
                }
                st = null;
              }
            else st = null;
            (N !== null && Jh(V, U, N, st, !1),
              $ !== null && Ot !== null && Jh(V, Ot, $, st, !0));
          }
        }
        t: {
          if (
            ((U = D ? Hl(D) : window),
            (N = U.nodeName && U.nodeName.toLowerCase()),
            N === "select" || (N === "input" && U.type === "file"))
          )
            var St = cf;
          else if (rf(U))
            if (ff) St = Gg;
            else {
              St = qg;
              var tt = Hg;
            }
          else
            ((N = U.nodeName),
              !N ||
              N.toLowerCase() !== "input" ||
              (U.type !== "checkbox" && U.type !== "radio")
                ? D && Ys(D.elementType) && (St = cf)
                : (St = Yg));
          if (St && (St = St(t, D))) {
            of(V, St, n, j);
            break t;
          }
          (tt && tt(t, U, D),
            t === "focusout" &&
              D &&
              U.type === "number" &&
              D.memoizedProps.value != null &&
              qs(U, "number", U.value));
        }
        switch (((tt = D ? Hl(D) : window), t)) {
          case "focusin":
            (rf(tt) || tt.contentEditable === "true") &&
              ((Fa = tt), (tr = D), (Jl = null));
            break;
          case "focusout":
            Jl = tr = Fa = null;
            break;
          case "mousedown":
            er = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            ((er = !1), Sf(V, n, j));
            break;
          case "selectionchange":
            if (Vg) break;
          case "keydown":
          case "keyup":
            Sf(V, n, j);
        }
        var ft;
        if (Ps)
          t: {
            switch (t) {
              case "compositionstart":
                var pt = "onCompositionStart";
                break t;
              case "compositionend":
                pt = "onCompositionEnd";
                break t;
              case "compositionupdate":
                pt = "onCompositionUpdate";
                break t;
            }
            pt = void 0;
          }
        else
          ka
            ? uf(t, n) && (pt = "onCompositionEnd")
            : t === "keydown" &&
              n.keyCode === 229 &&
              (pt = "onCompositionStart");
        (pt &&
          (nf &&
            n.locale !== "ko" &&
            (ka || pt !== "onCompositionStart"
              ? pt === "onCompositionEnd" && ka && (ft = Ic())
              : ((Cn = j),
                (Zs = "value" in Cn ? Cn.value : Cn.textContent),
                (ka = !0))),
          (tt = Zu(D, pt)),
          0 < tt.length &&
            ((pt = new tf(pt, t, null, n, j)),
            V.push({ event: pt, listeners: tt }),
            ft
              ? (pt.data = ft)
              : ((ft = sf(n)), ft !== null && (pt.data = ft)))),
          (ft = Lg ? Ug(t, n) : Ng(t, n)) &&
            ((pt = Zu(D, "onBeforeInput")),
            0 < pt.length &&
              ((tt = new tf("onBeforeInput", "beforeinput", null, n, j)),
              V.push({ event: tt, listeners: pt }),
              (tt.data = ft))),
          Av(V, t, D, n, j));
      }
      Zh(V, e);
    });
  }
  function vi(t, e, n) {
    return { instance: t, listener: e, currentTarget: n };
  }
  function Zu(t, e) {
    for (var n = e + "Capture", l = []; t !== null; ) {
      var s = t,
        o = s.stateNode;
      if (
        ((s = s.tag),
        (s !== 5 && s !== 26 && s !== 27) ||
          o === null ||
          ((s = ql(t, n)),
          s != null && l.unshift(vi(t, s, o)),
          (s = ql(t, e)),
          s != null && l.push(vi(t, s, o))),
        t.tag === 3)
      )
        return l;
      t = t.return;
    }
    return [];
  }
  function Cv(t) {
    if (t === null) return null;
    do t = t.return;
    while (t && t.tag !== 5 && t.tag !== 27);
    return t || null;
  }
  function Jh(t, e, n, l, s) {
    for (var o = e._reactName, d = []; n !== null && n !== l; ) {
      var v = n,
        R = v.alternate,
        D = v.stateNode;
      if (((v = v.tag), R !== null && R === l)) break;
      ((v !== 5 && v !== 26 && v !== 27) ||
        D === null ||
        ((R = D),
        s
          ? ((D = ql(n, o)), D != null && d.unshift(vi(n, D, R)))
          : s || ((D = ql(n, o)), D != null && d.push(vi(n, D, R)))),
        (n = n.return));
    }
    d.length !== 0 && t.push({ event: e, listeners: d });
  }
  var wv = /\r\n?/g,
    Dv = /\u0000|\uFFFD/g;
  function kh(t) {
    return (typeof t == "string" ? t : "" + t)
      .replace(
        wv,
        `
`,
      )
      .replace(Dv, "");
  }
  function Fh(t, e) {
    return ((e = kh(e)), kh(t) === e);
  }
  function zt(t, e, n, l, s, o) {
    switch (n) {
      case "children":
        typeof l == "string"
          ? e === "body" || (e === "textarea" && l === "") || Za(t, l)
          : (typeof l == "number" || typeof l == "bigint") &&
            e !== "body" &&
            Za(t, "" + l);
        break;
      case "className":
        Fi(t, "class", l);
        break;
      case "tabIndex":
        Fi(t, "tabindex", l);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        Fi(t, n, l);
        break;
      case "style":
        kc(t, l, o);
        break;
      case "data":
        if (e !== "object") {
          Fi(t, "data", l);
          break;
        }
      case "src":
      case "href":
        if (l === "" && (e !== "a" || n !== "href")) {
          t.removeAttribute(n);
          break;
        }
        if (
          l == null ||
          typeof l == "function" ||
          typeof l == "symbol" ||
          typeof l == "boolean"
        ) {
          t.removeAttribute(n);
          break;
        }
        ((l = Ii("" + l)), t.setAttribute(n, l));
        break;
      case "action":
      case "formAction":
        if (typeof l == "function") {
          t.setAttribute(
            n,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
          );
          break;
        } else
          typeof o == "function" &&
            (n === "formAction"
              ? (e !== "input" && zt(t, e, "name", s.name, s, null),
                zt(t, e, "formEncType", s.formEncType, s, null),
                zt(t, e, "formMethod", s.formMethod, s, null),
                zt(t, e, "formTarget", s.formTarget, s, null))
              : (zt(t, e, "encType", s.encType, s, null),
                zt(t, e, "method", s.method, s, null),
                zt(t, e, "target", s.target, s, null)));
        if (l == null || typeof l == "symbol" || typeof l == "boolean") {
          t.removeAttribute(n);
          break;
        }
        ((l = Ii("" + l)), t.setAttribute(n, l));
        break;
      case "onClick":
        l != null && (t.onclick = ln);
        break;
      case "onScroll":
        l != null && mt("scroll", t);
        break;
      case "onScrollEnd":
        l != null && mt("scrollend", t);
        break;
      case "dangerouslySetInnerHTML":
        if (l != null) {
          if (typeof l != "object" || !("__html" in l)) throw Error(r(61));
          if (((n = l.__html), n != null)) {
            if (s.children != null) throw Error(r(60));
            t.innerHTML = n;
          }
        }
        break;
      case "multiple":
        t.multiple = l && typeof l != "function" && typeof l != "symbol";
        break;
      case "muted":
        t.muted = l && typeof l != "function" && typeof l != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (
          l == null ||
          typeof l == "function" ||
          typeof l == "boolean" ||
          typeof l == "symbol"
        ) {
          t.removeAttribute("xlink:href");
          break;
        }
        ((n = Ii("" + l)),
          t.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", n));
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        l != null && typeof l != "function" && typeof l != "symbol"
          ? t.setAttribute(n, "" + l)
          : t.removeAttribute(n);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        l && typeof l != "function" && typeof l != "symbol"
          ? t.setAttribute(n, "")
          : t.removeAttribute(n);
        break;
      case "capture":
      case "download":
        l === !0
          ? t.setAttribute(n, "")
          : l !== !1 &&
              l != null &&
              typeof l != "function" &&
              typeof l != "symbol"
            ? t.setAttribute(n, l)
            : t.removeAttribute(n);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        l != null &&
        typeof l != "function" &&
        typeof l != "symbol" &&
        !isNaN(l) &&
        1 <= l
          ? t.setAttribute(n, l)
          : t.removeAttribute(n);
        break;
      case "rowSpan":
      case "start":
        l == null || typeof l == "function" || typeof l == "symbol" || isNaN(l)
          ? t.removeAttribute(n)
          : t.setAttribute(n, l);
        break;
      case "popover":
        (mt("beforetoggle", t), mt("toggle", t), ki(t, "popover", l));
        break;
      case "xlinkActuate":
        an(t, "http://www.w3.org/1999/xlink", "xlink:actuate", l);
        break;
      case "xlinkArcrole":
        an(t, "http://www.w3.org/1999/xlink", "xlink:arcrole", l);
        break;
      case "xlinkRole":
        an(t, "http://www.w3.org/1999/xlink", "xlink:role", l);
        break;
      case "xlinkShow":
        an(t, "http://www.w3.org/1999/xlink", "xlink:show", l);
        break;
      case "xlinkTitle":
        an(t, "http://www.w3.org/1999/xlink", "xlink:title", l);
        break;
      case "xlinkType":
        an(t, "http://www.w3.org/1999/xlink", "xlink:type", l);
        break;
      case "xmlBase":
        an(t, "http://www.w3.org/XML/1998/namespace", "xml:base", l);
        break;
      case "xmlLang":
        an(t, "http://www.w3.org/XML/1998/namespace", "xml:lang", l);
        break;
      case "xmlSpace":
        an(t, "http://www.w3.org/XML/1998/namespace", "xml:space", l);
        break;
      case "is":
        ki(t, "is", l);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < n.length) ||
          (n[0] !== "o" && n[0] !== "O") ||
          (n[1] !== "n" && n[1] !== "N")) &&
          ((n = ug.get(n) || n), ki(t, n, l));
    }
  }
  function Mo(t, e, n, l, s, o) {
    switch (n) {
      case "style":
        kc(t, l, o);
        break;
      case "dangerouslySetInnerHTML":
        if (l != null) {
          if (typeof l != "object" || !("__html" in l)) throw Error(r(61));
          if (((n = l.__html), n != null)) {
            if (s.children != null) throw Error(r(60));
            t.innerHTML = n;
          }
        }
        break;
      case "children":
        typeof l == "string"
          ? Za(t, l)
          : (typeof l == "number" || typeof l == "bigint") && Za(t, "" + l);
        break;
      case "onScroll":
        l != null && mt("scroll", t);
        break;
      case "onScrollEnd":
        l != null && mt("scrollend", t);
        break;
      case "onClick":
        l != null && (t.onclick = ln);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!qc.hasOwnProperty(n))
          t: {
            if (
              n[0] === "o" &&
              n[1] === "n" &&
              ((s = n.endsWith("Capture")),
              (e = n.slice(2, s ? n.length - 7 : void 0)),
              (o = t[he] || null),
              (o = o != null ? o[n] : null),
              typeof o == "function" && t.removeEventListener(e, o, s),
              typeof l == "function")
            ) {
              (typeof o != "function" &&
                o !== null &&
                (n in t
                  ? (t[n] = null)
                  : t.hasAttribute(n) && t.removeAttribute(n)),
                t.addEventListener(e, l, s));
              break t;
            }
            n in t
              ? (t[n] = l)
              : l === !0
                ? t.setAttribute(n, "")
                : ki(t, n, l);
          }
    }
  }
  function re(t, e, n) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        (mt("error", t), mt("load", t));
        var l = !1,
          s = !1,
          o;
        for (o in n)
          if (n.hasOwnProperty(o)) {
            var d = n[o];
            if (d != null)
              switch (o) {
                case "src":
                  l = !0;
                  break;
                case "srcSet":
                  s = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(r(137, e));
                default:
                  zt(t, e, o, d, n, null);
              }
          }
        (s && zt(t, e, "srcSet", n.srcSet, n, null),
          l && zt(t, e, "src", n.src, n, null));
        return;
      case "input":
        mt("invalid", t);
        var v = (o = d = s = null),
          R = null,
          D = null;
        for (l in n)
          if (n.hasOwnProperty(l)) {
            var j = n[l];
            if (j != null)
              switch (l) {
                case "name":
                  s = j;
                  break;
                case "type":
                  d = j;
                  break;
                case "checked":
                  R = j;
                  break;
                case "defaultChecked":
                  D = j;
                  break;
                case "value":
                  o = j;
                  break;
                case "defaultValue":
                  v = j;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (j != null) throw Error(r(137, e));
                  break;
                default:
                  zt(t, e, l, j, n, null);
              }
          }
        Qc(t, o, v, R, D, d, s, !1);
        return;
      case "select":
        (mt("invalid", t), (l = d = o = null));
        for (s in n)
          if (n.hasOwnProperty(s) && ((v = n[s]), v != null))
            switch (s) {
              case "value":
                o = v;
                break;
              case "defaultValue":
                d = v;
                break;
              case "multiple":
                l = v;
              default:
                zt(t, e, s, v, n, null);
            }
        ((e = o),
          (n = d),
          (t.multiple = !!l),
          e != null ? Qa(t, !!l, e, !1) : n != null && Qa(t, !!l, n, !0));
        return;
      case "textarea":
        (mt("invalid", t), (o = s = l = null));
        for (d in n)
          if (n.hasOwnProperty(d) && ((v = n[d]), v != null))
            switch (d) {
              case "value":
                l = v;
                break;
              case "defaultValue":
                s = v;
                break;
              case "children":
                o = v;
                break;
              case "dangerouslySetInnerHTML":
                if (v != null) throw Error(r(91));
                break;
              default:
                zt(t, e, d, v, n, null);
            }
        Kc(t, l, s, o);
        return;
      case "option":
        for (R in n)
          n.hasOwnProperty(R) &&
            ((l = n[R]), l != null) &&
            (R === "selected"
              ? (t.selected =
                  l && typeof l != "function" && typeof l != "symbol")
              : zt(t, e, R, l, n, null));
        return;
      case "dialog":
        (mt("beforetoggle", t),
          mt("toggle", t),
          mt("cancel", t),
          mt("close", t));
        break;
      case "iframe":
      case "object":
        mt("load", t);
        break;
      case "video":
      case "audio":
        for (l = 0; l < gi.length; l++) mt(gi[l], t);
        break;
      case "image":
        (mt("error", t), mt("load", t));
        break;
      case "details":
        mt("toggle", t);
        break;
      case "embed":
      case "source":
      case "link":
        (mt("error", t), mt("load", t));
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (D in n)
          if (n.hasOwnProperty(D) && ((l = n[D]), l != null))
            switch (D) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(r(137, e));
              default:
                zt(t, e, D, l, n, null);
            }
        return;
      default:
        if (Ys(e)) {
          for (j in n)
            n.hasOwnProperty(j) &&
              ((l = n[j]), l !== void 0 && Mo(t, e, j, l, n, void 0));
          return;
        }
    }
    for (v in n)
      n.hasOwnProperty(v) && ((l = n[v]), l != null && zt(t, e, v, l, n, null));
  }
  function Lv(t, e, n, l) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var s = null,
          o = null,
          d = null,
          v = null,
          R = null,
          D = null,
          j = null;
        for (N in n) {
          var V = n[N];
          if (n.hasOwnProperty(N) && V != null)
            switch (N) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                R = V;
              default:
                l.hasOwnProperty(N) || zt(t, e, N, null, l, V);
            }
        }
        for (var U in l) {
          var N = l[U];
          if (((V = n[U]), l.hasOwnProperty(U) && (N != null || V != null)))
            switch (U) {
              case "type":
                o = N;
                break;
              case "name":
                s = N;
                break;
              case "checked":
                D = N;
                break;
              case "defaultChecked":
                j = N;
                break;
              case "value":
                d = N;
                break;
              case "defaultValue":
                v = N;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (N != null) throw Error(r(137, e));
                break;
              default:
                N !== V && zt(t, e, U, N, l, V);
            }
        }
        Hs(t, d, v, R, D, j, o, s);
        return;
      case "select":
        N = d = v = U = null;
        for (o in n)
          if (((R = n[o]), n.hasOwnProperty(o) && R != null))
            switch (o) {
              case "value":
                break;
              case "multiple":
                N = R;
              default:
                l.hasOwnProperty(o) || zt(t, e, o, null, l, R);
            }
        for (s in l)
          if (
            ((o = l[s]),
            (R = n[s]),
            l.hasOwnProperty(s) && (o != null || R != null))
          )
            switch (s) {
              case "value":
                U = o;
                break;
              case "defaultValue":
                v = o;
                break;
              case "multiple":
                d = o;
              default:
                o !== R && zt(t, e, s, o, l, R);
            }
        ((e = v),
          (n = d),
          (l = N),
          U != null
            ? Qa(t, !!n, U, !1)
            : !!l != !!n &&
              (e != null ? Qa(t, !!n, e, !0) : Qa(t, !!n, n ? [] : "", !1)));
        return;
      case "textarea":
        N = U = null;
        for (v in n)
          if (
            ((s = n[v]),
            n.hasOwnProperty(v) && s != null && !l.hasOwnProperty(v))
          )
            switch (v) {
              case "value":
                break;
              case "children":
                break;
              default:
                zt(t, e, v, null, l, s);
            }
        for (d in l)
          if (
            ((s = l[d]),
            (o = n[d]),
            l.hasOwnProperty(d) && (s != null || o != null))
          )
            switch (d) {
              case "value":
                U = s;
                break;
              case "defaultValue":
                N = s;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (s != null) throw Error(r(91));
                break;
              default:
                s !== o && zt(t, e, d, s, l, o);
            }
        Zc(t, U, N);
        return;
      case "option":
        for (var $ in n)
          ((U = n[$]),
            n.hasOwnProperty($) &&
              U != null &&
              !l.hasOwnProperty($) &&
              ($ === "selected" ? (t.selected = !1) : zt(t, e, $, null, l, U)));
        for (R in l)
          ((U = l[R]),
            (N = n[R]),
            l.hasOwnProperty(R) &&
              U !== N &&
              (U != null || N != null) &&
              (R === "selected"
                ? (t.selected =
                    U && typeof U != "function" && typeof U != "symbol")
                : zt(t, e, R, U, l, N)));
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var st in n)
          ((U = n[st]),
            n.hasOwnProperty(st) &&
              U != null &&
              !l.hasOwnProperty(st) &&
              zt(t, e, st, null, l, U));
        for (D in l)
          if (
            ((U = l[D]),
            (N = n[D]),
            l.hasOwnProperty(D) && U !== N && (U != null || N != null))
          )
            switch (D) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (U != null) throw Error(r(137, e));
                break;
              default:
                zt(t, e, D, U, l, N);
            }
        return;
      default:
        if (Ys(e)) {
          for (var Ot in n)
            ((U = n[Ot]),
              n.hasOwnProperty(Ot) &&
                U !== void 0 &&
                !l.hasOwnProperty(Ot) &&
                Mo(t, e, Ot, void 0, l, U));
          for (j in l)
            ((U = l[j]),
              (N = n[j]),
              !l.hasOwnProperty(j) ||
                U === N ||
                (U === void 0 && N === void 0) ||
                Mo(t, e, j, U, l, N));
          return;
        }
    }
    for (var O in n)
      ((U = n[O]),
        n.hasOwnProperty(O) &&
          U != null &&
          !l.hasOwnProperty(O) &&
          zt(t, e, O, null, l, U));
    for (V in l)
      ((U = l[V]),
        (N = n[V]),
        !l.hasOwnProperty(V) ||
          U === N ||
          (U == null && N == null) ||
          zt(t, e, V, U, l, N));
  }
  function Ph(t) {
    switch (t) {
      case "css":
      case "script":
      case "font":
      case "img":
      case "image":
      case "input":
      case "link":
        return !0;
      default:
        return !1;
    }
  }
  function Uv() {
    if (typeof performance.getEntriesByType == "function") {
      for (
        var t = 0, e = 0, n = performance.getEntriesByType("resource"), l = 0;
        l < n.length;
        l++
      ) {
        var s = n[l],
          o = s.transferSize,
          d = s.initiatorType,
          v = s.duration;
        if (o && v && Ph(d)) {
          for (d = 0, v = s.responseEnd, l += 1; l < n.length; l++) {
            var R = n[l],
              D = R.startTime;
            if (D > v) break;
            var j = R.transferSize,
              V = R.initiatorType;
            j &&
              Ph(V) &&
              ((R = R.responseEnd), (d += j * (R < v ? 1 : (v - D) / (R - D))));
          }
          if ((--l, (e += (8 * (o + d)) / (s.duration / 1e3)), t++, 10 < t))
            break;
        }
      }
      if (0 < t) return e / t / 1e6;
    }
    return navigator.connection &&
      ((t = navigator.connection.downlink), typeof t == "number")
      ? t
      : 5;
  }
  var zo = null,
    Oo = null;
  function Ku(t) {
    return t.nodeType === 9 ? t : t.ownerDocument;
  }
  function Ih(t) {
    switch (t) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function Wh(t, e) {
    if (t === 0)
      switch (e) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return t === 1 && e === "foreignObject" ? 0 : t;
  }
  function Co(t, e) {
    return (
      t === "textarea" ||
      t === "noscript" ||
      typeof e.children == "string" ||
      typeof e.children == "number" ||
      typeof e.children == "bigint" ||
      (typeof e.dangerouslySetInnerHTML == "object" &&
        e.dangerouslySetInnerHTML !== null &&
        e.dangerouslySetInnerHTML.__html != null)
    );
  }
  var wo = null;
  function Nv() {
    var t = window.event;
    return t && t.type === "popstate"
      ? t === wo
        ? !1
        : ((wo = t), !0)
      : ((wo = null), !1);
  }
  var $h = typeof setTimeout == "function" ? setTimeout : void 0,
    Bv = typeof clearTimeout == "function" ? clearTimeout : void 0,
    tm = typeof Promise == "function" ? Promise : void 0,
    jv =
      typeof queueMicrotask == "function"
        ? queueMicrotask
        : typeof tm < "u"
          ? function (t) {
              return tm.resolve(null).then(t).catch(Hv);
            }
          : $h;
  function Hv(t) {
    setTimeout(function () {
      throw t;
    });
  }
  function Jn(t) {
    return t === "head";
  }
  function em(t, e) {
    var n = e,
      l = 0;
    do {
      var s = n.nextSibling;
      if ((t.removeChild(n), s && s.nodeType === 8))
        if (((n = s.data), n === "/$" || n === "/&")) {
          if (l === 0) {
            (t.removeChild(s), El(e));
            return;
          }
          l--;
        } else if (
          n === "$" ||
          n === "$?" ||
          n === "$~" ||
          n === "$!" ||
          n === "&"
        )
          l++;
        else if (n === "html") Si(t.ownerDocument.documentElement);
        else if (n === "head") {
          ((n = t.ownerDocument.head), Si(n));
          for (var o = n.firstChild; o; ) {
            var d = o.nextSibling,
              v = o.nodeName;
            (o[jl] ||
              v === "SCRIPT" ||
              v === "STYLE" ||
              (v === "LINK" && o.rel.toLowerCase() === "stylesheet") ||
              n.removeChild(o),
              (o = d));
          }
        } else n === "body" && Si(t.ownerDocument.body);
      n = s;
    } while (n);
    El(e);
  }
  function nm(t, e) {
    var n = t;
    t = 0;
    do {
      var l = n.nextSibling;
      if (
        (n.nodeType === 1
          ? e
            ? ((n._stashedDisplay = n.style.display),
              (n.style.display = "none"))
            : ((n.style.display = n._stashedDisplay || ""),
              n.getAttribute("style") === "" && n.removeAttribute("style"))
          : n.nodeType === 3 &&
            (e
              ? ((n._stashedText = n.nodeValue), (n.nodeValue = ""))
              : (n.nodeValue = n._stashedText || "")),
        l && l.nodeType === 8)
      )
        if (((n = l.data), n === "/$")) {
          if (t === 0) break;
          t--;
        } else (n !== "$" && n !== "$?" && n !== "$~" && n !== "$!") || t++;
      n = l;
    } while (n);
  }
  function Do(t) {
    var e = t.firstChild;
    for (e && e.nodeType === 10 && (e = e.nextSibling); e; ) {
      var n = e;
      switch (((e = e.nextSibling), n.nodeName)) {
        case "HTML":
        case "HEAD":
        case "BODY":
          (Do(n), Bs(n));
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (n.rel.toLowerCase() === "stylesheet") continue;
      }
      t.removeChild(n);
    }
  }
  function qv(t, e, n, l) {
    for (; t.nodeType === 1; ) {
      var s = n;
      if (t.nodeName.toLowerCase() !== e.toLowerCase()) {
        if (!l && (t.nodeName !== "INPUT" || t.type !== "hidden")) break;
      } else if (l) {
        if (!t[jl])
          switch (e) {
            case "meta":
              if (!t.hasAttribute("itemprop")) break;
              return t;
            case "link":
              if (
                ((o = t.getAttribute("rel")),
                o === "stylesheet" && t.hasAttribute("data-precedence"))
              )
                break;
              if (
                o !== s.rel ||
                t.getAttribute("href") !==
                  (s.href == null || s.href === "" ? null : s.href) ||
                t.getAttribute("crossorigin") !==
                  (s.crossOrigin == null ? null : s.crossOrigin) ||
                t.getAttribute("title") !== (s.title == null ? null : s.title)
              )
                break;
              return t;
            case "style":
              if (t.hasAttribute("data-precedence")) break;
              return t;
            case "script":
              if (
                ((o = t.getAttribute("src")),
                (o !== (s.src == null ? null : s.src) ||
                  t.getAttribute("type") !== (s.type == null ? null : s.type) ||
                  t.getAttribute("crossorigin") !==
                    (s.crossOrigin == null ? null : s.crossOrigin)) &&
                  o &&
                  t.hasAttribute("async") &&
                  !t.hasAttribute("itemprop"))
              )
                break;
              return t;
            default:
              return t;
          }
      } else if (e === "input" && t.type === "hidden") {
        var o = s.name == null ? null : "" + s.name;
        if (s.type === "hidden" && t.getAttribute("name") === o) return t;
      } else return t;
      if (((t = Ye(t.nextSibling)), t === null)) break;
    }
    return null;
  }
  function Yv(t, e, n) {
    if (e === "") return null;
    for (; t.nodeType !== 3; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") &&
          !n) ||
        ((t = Ye(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function am(t, e) {
    for (; t.nodeType !== 8; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") &&
          !e) ||
        ((t = Ye(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function Lo(t) {
    return t.data === "$?" || t.data === "$~";
  }
  function Uo(t) {
    return (
      t.data === "$!" ||
      (t.data === "$?" && t.ownerDocument.readyState !== "loading")
    );
  }
  function Gv(t, e) {
    var n = t.ownerDocument;
    if (t.data === "$~") t._reactRetry = e;
    else if (t.data !== "$?" || n.readyState !== "loading") e();
    else {
      var l = function () {
        (e(), n.removeEventListener("DOMContentLoaded", l));
      };
      (n.addEventListener("DOMContentLoaded", l), (t._reactRetry = l));
    }
  }
  function Ye(t) {
    for (; t != null; t = t.nextSibling) {
      var e = t.nodeType;
      if (e === 1 || e === 3) break;
      if (e === 8) {
        if (
          ((e = t.data),
          e === "$" ||
            e === "$!" ||
            e === "$?" ||
            e === "$~" ||
            e === "&" ||
            e === "F!" ||
            e === "F")
        )
          break;
        if (e === "/$" || e === "/&") return null;
      }
    }
    return t;
  }
  var No = null;
  function lm(t) {
    t = t.nextSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var n = t.data;
        if (n === "/$" || n === "/&") {
          if (e === 0) return Ye(t.nextSibling);
          e--;
        } else
          (n !== "$" && n !== "$!" && n !== "$?" && n !== "$~" && n !== "&") ||
            e++;
      }
      t = t.nextSibling;
    }
    return null;
  }
  function im(t) {
    t = t.previousSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var n = t.data;
        if (n === "$" || n === "$!" || n === "$?" || n === "$~" || n === "&") {
          if (e === 0) return t;
          e--;
        } else (n !== "/$" && n !== "/&") || e++;
      }
      t = t.previousSibling;
    }
    return null;
  }
  function um(t, e, n) {
    switch (((e = Ku(n)), t)) {
      case "html":
        if (((t = e.documentElement), !t)) throw Error(r(452));
        return t;
      case "head":
        if (((t = e.head), !t)) throw Error(r(453));
        return t;
      case "body":
        if (((t = e.body), !t)) throw Error(r(454));
        return t;
      default:
        throw Error(r(451));
    }
  }
  function Si(t) {
    for (var e = t.attributes; e.length; ) t.removeAttributeNode(e[0]);
    Bs(t);
  }
  var Ge = new Map(),
    sm = new Set();
  function Ju(t) {
    return typeof t.getRootNode == "function"
      ? t.getRootNode()
      : t.nodeType === 9
        ? t
        : t.ownerDocument;
  }
  var _n = J.d;
  J.d = { f: Xv, r: Vv, D: Qv, C: Zv, L: Kv, m: Jv, X: Fv, S: kv, M: Pv };
  function Xv() {
    var t = _n.f(),
      e = Hu();
    return t || e;
  }
  function Vv(t) {
    var e = Ga(t);
    e !== null && e.tag === 5 && e.type === "form" ? Td(e) : _n.r(t);
  }
  var Sl = typeof document > "u" ? null : document;
  function rm(t, e, n) {
    var l = Sl;
    if (l && typeof e == "string" && e) {
      var s = Le(e);
      ((s = 'link[rel="' + t + '"][href="' + s + '"]'),
        typeof n == "string" && (s += '[crossorigin="' + n + '"]'),
        sm.has(s) ||
          (sm.add(s),
          (t = { rel: t, crossOrigin: n, href: e }),
          l.querySelector(s) === null &&
            ((e = l.createElement("link")),
            re(e, "link", t),
            te(e),
            l.head.appendChild(e))));
    }
  }
  function Qv(t) {
    (_n.D(t), rm("dns-prefetch", t, null));
  }
  function Zv(t, e) {
    (_n.C(t, e), rm("preconnect", t, e));
  }
  function Kv(t, e, n) {
    _n.L(t, e, n);
    var l = Sl;
    if (l && t && e) {
      var s = 'link[rel="preload"][as="' + Le(e) + '"]';
      e === "image" && n && n.imageSrcSet
        ? ((s += '[imagesrcset="' + Le(n.imageSrcSet) + '"]'),
          typeof n.imageSizes == "string" &&
            (s += '[imagesizes="' + Le(n.imageSizes) + '"]'))
        : (s += '[href="' + Le(t) + '"]');
      var o = s;
      switch (e) {
        case "style":
          o = bl(t);
          break;
        case "script":
          o = _l(t);
      }
      Ge.has(o) ||
        ((t = p(
          {
            rel: "preload",
            href: e === "image" && n && n.imageSrcSet ? void 0 : t,
            as: e,
          },
          n,
        )),
        Ge.set(o, t),
        l.querySelector(s) !== null ||
          (e === "style" && l.querySelector(bi(o))) ||
          (e === "script" && l.querySelector(_i(o))) ||
          ((e = l.createElement("link")),
          re(e, "link", t),
          te(e),
          l.head.appendChild(e)));
    }
  }
  function Jv(t, e) {
    _n.m(t, e);
    var n = Sl;
    if (n && t) {
      var l = e && typeof e.as == "string" ? e.as : "script",
        s =
          'link[rel="modulepreload"][as="' + Le(l) + '"][href="' + Le(t) + '"]',
        o = s;
      switch (l) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          o = _l(t);
      }
      if (
        !Ge.has(o) &&
        ((t = p({ rel: "modulepreload", href: t }, e)),
        Ge.set(o, t),
        n.querySelector(s) === null)
      ) {
        switch (l) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (n.querySelector(_i(o))) return;
        }
        ((l = n.createElement("link")),
          re(l, "link", t),
          te(l),
          n.head.appendChild(l));
      }
    }
  }
  function kv(t, e, n) {
    _n.S(t, e, n);
    var l = Sl;
    if (l && t) {
      var s = Xa(l).hoistableStyles,
        o = bl(t);
      e = e || "default";
      var d = s.get(o);
      if (!d) {
        var v = { loading: 0, preload: null };
        if ((d = l.querySelector(bi(o)))) v.loading = 5;
        else {
          ((t = p({ rel: "stylesheet", href: t, "data-precedence": e }, n)),
            (n = Ge.get(o)) && Bo(t, n));
          var R = (d = l.createElement("link"));
          (te(R),
            re(R, "link", t),
            (R._p = new Promise(function (D, j) {
              ((R.onload = D), (R.onerror = j));
            })),
            R.addEventListener("load", function () {
              v.loading |= 1;
            }),
            R.addEventListener("error", function () {
              v.loading |= 2;
            }),
            (v.loading |= 4),
            ku(d, e, l));
        }
        ((d = { type: "stylesheet", instance: d, count: 1, state: v }),
          s.set(o, d));
      }
    }
  }
  function Fv(t, e) {
    _n.X(t, e);
    var n = Sl;
    if (n && t) {
      var l = Xa(n).hoistableScripts,
        s = _l(t),
        o = l.get(s);
      o ||
        ((o = n.querySelector(_i(s))),
        o ||
          ((t = p({ src: t, async: !0 }, e)),
          (e = Ge.get(s)) && jo(t, e),
          (o = n.createElement("script")),
          te(o),
          re(o, "link", t),
          n.head.appendChild(o)),
        (o = { type: "script", instance: o, count: 1, state: null }),
        l.set(s, o));
    }
  }
  function Pv(t, e) {
    _n.M(t, e);
    var n = Sl;
    if (n && t) {
      var l = Xa(n).hoistableScripts,
        s = _l(t),
        o = l.get(s);
      o ||
        ((o = n.querySelector(_i(s))),
        o ||
          ((t = p({ src: t, async: !0, type: "module" }, e)),
          (e = Ge.get(s)) && jo(t, e),
          (o = n.createElement("script")),
          te(o),
          re(o, "link", t),
          n.head.appendChild(o)),
        (o = { type: "script", instance: o, count: 1, state: null }),
        l.set(s, o));
    }
  }
  function om(t, e, n, l) {
    var s = (s = dt.current) ? Ju(s) : null;
    if (!s) throw Error(r(446));
    switch (t) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof n.precedence == "string" && typeof n.href == "string"
          ? ((e = bl(n.href)),
            (n = Xa(s).hoistableStyles),
            (l = n.get(e)),
            l ||
              ((l = { type: "style", instance: null, count: 0, state: null }),
              n.set(e, l)),
            l)
          : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (
          n.rel === "stylesheet" &&
          typeof n.href == "string" &&
          typeof n.precedence == "string"
        ) {
          t = bl(n.href);
          var o = Xa(s).hoistableStyles,
            d = o.get(t);
          if (
            (d ||
              ((s = s.ownerDocument || s),
              (d = {
                type: "stylesheet",
                instance: null,
                count: 0,
                state: { loading: 0, preload: null },
              }),
              o.set(t, d),
              (o = s.querySelector(bi(t))) &&
                !o._p &&
                ((d.instance = o), (d.state.loading = 5)),
              Ge.has(t) ||
                ((n = {
                  rel: "preload",
                  as: "style",
                  href: n.href,
                  crossOrigin: n.crossOrigin,
                  integrity: n.integrity,
                  media: n.media,
                  hrefLang: n.hrefLang,
                  referrerPolicy: n.referrerPolicy,
                }),
                Ge.set(t, n),
                o || Iv(s, t, n, d.state))),
            e && l === null)
          )
            throw Error(r(528, ""));
          return d;
        }
        if (e && l !== null) throw Error(r(529, ""));
        return null;
      case "script":
        return (
          (e = n.async),
          (n = n.src),
          typeof n == "string" &&
          e &&
          typeof e != "function" &&
          typeof e != "symbol"
            ? ((e = _l(n)),
              (n = Xa(s).hoistableScripts),
              (l = n.get(e)),
              l ||
                ((l = {
                  type: "script",
                  instance: null,
                  count: 0,
                  state: null,
                }),
                n.set(e, l)),
              l)
            : { type: "void", instance: null, count: 0, state: null }
        );
      default:
        throw Error(r(444, t));
    }
  }
  function bl(t) {
    return 'href="' + Le(t) + '"';
  }
  function bi(t) {
    return 'link[rel="stylesheet"][' + t + "]";
  }
  function cm(t) {
    return p({}, t, { "data-precedence": t.precedence, precedence: null });
  }
  function Iv(t, e, n, l) {
    t.querySelector('link[rel="preload"][as="style"][' + e + "]")
      ? (l.loading = 1)
      : ((e = t.createElement("link")),
        (l.preload = e),
        e.addEventListener("load", function () {
          return (l.loading |= 1);
        }),
        e.addEventListener("error", function () {
          return (l.loading |= 2);
        }),
        re(e, "link", n),
        te(e),
        t.head.appendChild(e));
  }
  function _l(t) {
    return '[src="' + Le(t) + '"]';
  }
  function _i(t) {
    return "script[async]" + t;
  }
  function fm(t, e, n) {
    if ((e.count++, e.instance === null))
      switch (e.type) {
        case "style":
          var l = t.querySelector('style[data-href~="' + Le(n.href) + '"]');
          if (l) return ((e.instance = l), te(l), l);
          var s = p({}, n, {
            "data-href": n.href,
            "data-precedence": n.precedence,
            href: null,
            precedence: null,
          });
          return (
            (l = (t.ownerDocument || t).createElement("style")),
            te(l),
            re(l, "style", s),
            ku(l, n.precedence, t),
            (e.instance = l)
          );
        case "stylesheet":
          s = bl(n.href);
          var o = t.querySelector(bi(s));
          if (o) return ((e.state.loading |= 4), (e.instance = o), te(o), o);
          ((l = cm(n)),
            (s = Ge.get(s)) && Bo(l, s),
            (o = (t.ownerDocument || t).createElement("link")),
            te(o));
          var d = o;
          return (
            (d._p = new Promise(function (v, R) {
              ((d.onload = v), (d.onerror = R));
            })),
            re(o, "link", l),
            (e.state.loading |= 4),
            ku(o, n.precedence, t),
            (e.instance = o)
          );
        case "script":
          return (
            (o = _l(n.src)),
            (s = t.querySelector(_i(o)))
              ? ((e.instance = s), te(s), s)
              : ((l = n),
                (s = Ge.get(o)) && ((l = p({}, n)), jo(l, s)),
                (t = t.ownerDocument || t),
                (s = t.createElement("script")),
                te(s),
                re(s, "link", l),
                t.head.appendChild(s),
                (e.instance = s))
          );
        case "void":
          return null;
        default:
          throw Error(r(443, e.type));
      }
    else
      e.type === "stylesheet" &&
        (e.state.loading & 4) === 0 &&
        ((l = e.instance), (e.state.loading |= 4), ku(l, n.precedence, t));
    return e.instance;
  }
  function ku(t, e, n) {
    for (
      var l = n.querySelectorAll(
          'link[rel="stylesheet"][data-precedence],style[data-precedence]',
        ),
        s = l.length ? l[l.length - 1] : null,
        o = s,
        d = 0;
      d < l.length;
      d++
    ) {
      var v = l[d];
      if (v.dataset.precedence === e) o = v;
      else if (o !== s) break;
    }
    o
      ? o.parentNode.insertBefore(t, o.nextSibling)
      : ((e = n.nodeType === 9 ? n.head : n), e.insertBefore(t, e.firstChild));
  }
  function Bo(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.title == null && (t.title = e.title));
  }
  function jo(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.integrity == null && (t.integrity = e.integrity));
  }
  var Fu = null;
  function dm(t, e, n) {
    if (Fu === null) {
      var l = new Map(),
        s = (Fu = new Map());
      s.set(n, l);
    } else ((s = Fu), (l = s.get(n)), l || ((l = new Map()), s.set(n, l)));
    if (l.has(t)) return l;
    for (
      l.set(t, null), n = n.getElementsByTagName(t), s = 0;
      s < n.length;
      s++
    ) {
      var o = n[s];
      if (
        !(
          o[jl] ||
          o[le] ||
          (t === "link" && o.getAttribute("rel") === "stylesheet")
        ) &&
        o.namespaceURI !== "http://www.w3.org/2000/svg"
      ) {
        var d = o.getAttribute(e) || "";
        d = t + d;
        var v = l.get(d);
        v ? v.push(o) : l.set(d, [o]);
      }
    }
    return l;
  }
  function hm(t, e, n) {
    ((t = t.ownerDocument || t),
      t.head.insertBefore(
        n,
        e === "title" ? t.querySelector("head > title") : null,
      ));
  }
  function Wv(t, e, n) {
    if (n === 1 || e.itemProp != null) return !1;
    switch (t) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (
          typeof e.precedence != "string" ||
          typeof e.href != "string" ||
          e.href === ""
        )
          break;
        return !0;
      case "link":
        if (
          typeof e.rel != "string" ||
          typeof e.href != "string" ||
          e.href === "" ||
          e.onLoad ||
          e.onError
        )
          break;
        return e.rel === "stylesheet"
          ? ((t = e.disabled), typeof e.precedence == "string" && t == null)
          : !0;
      case "script":
        if (
          e.async &&
          typeof e.async != "function" &&
          typeof e.async != "symbol" &&
          !e.onLoad &&
          !e.onError &&
          e.src &&
          typeof e.src == "string"
        )
          return !0;
    }
    return !1;
  }
  function mm(t) {
    return !(t.type === "stylesheet" && (t.state.loading & 3) === 0);
  }
  function $v(t, e, n, l) {
    if (
      n.type === "stylesheet" &&
      (typeof l.media != "string" || matchMedia(l.media).matches !== !1) &&
      (n.state.loading & 4) === 0
    ) {
      if (n.instance === null) {
        var s = bl(l.href),
          o = e.querySelector(bi(s));
        if (o) {
          ((e = o._p),
            e !== null &&
              typeof e == "object" &&
              typeof e.then == "function" &&
              (t.count++, (t = Pu.bind(t)), e.then(t, t)),
            (n.state.loading |= 4),
            (n.instance = o),
            te(o));
          return;
        }
        ((o = e.ownerDocument || e),
          (l = cm(l)),
          (s = Ge.get(s)) && Bo(l, s),
          (o = o.createElement("link")),
          te(o));
        var d = o;
        ((d._p = new Promise(function (v, R) {
          ((d.onload = v), (d.onerror = R));
        })),
          re(o, "link", l),
          (n.instance = o));
      }
      (t.stylesheets === null && (t.stylesheets = new Map()),
        t.stylesheets.set(n, e),
        (e = n.state.preload) &&
          (n.state.loading & 3) === 0 &&
          (t.count++,
          (n = Pu.bind(t)),
          e.addEventListener("load", n),
          e.addEventListener("error", n)));
    }
  }
  var Ho = 0;
  function t0(t, e) {
    return (
      t.stylesheets && t.count === 0 && Wu(t, t.stylesheets),
      0 < t.count || 0 < t.imgCount
        ? function (n) {
            var l = setTimeout(function () {
              if ((t.stylesheets && Wu(t, t.stylesheets), t.unsuspend)) {
                var o = t.unsuspend;
                ((t.unsuspend = null), o());
              }
            }, 6e4 + e);
            0 < t.imgBytes && Ho === 0 && (Ho = 62500 * Uv());
            var s = setTimeout(
              function () {
                if (
                  ((t.waitingForImages = !1),
                  t.count === 0 &&
                    (t.stylesheets && Wu(t, t.stylesheets), t.unsuspend))
                ) {
                  var o = t.unsuspend;
                  ((t.unsuspend = null), o());
                }
              },
              (t.imgBytes > Ho ? 50 : 800) + e,
            );
            return (
              (t.unsuspend = n),
              function () {
                ((t.unsuspend = null), clearTimeout(l), clearTimeout(s));
              }
            );
          }
        : null
    );
  }
  function Pu() {
    if (
      (this.count--,
      this.count === 0 && (this.imgCount === 0 || !this.waitingForImages))
    ) {
      if (this.stylesheets) Wu(this, this.stylesheets);
      else if (this.unsuspend) {
        var t = this.unsuspend;
        ((this.unsuspend = null), t());
      }
    }
  }
  var Iu = null;
  function Wu(t, e) {
    ((t.stylesheets = null),
      t.unsuspend !== null &&
        (t.count++,
        (Iu = new Map()),
        e.forEach(e0, t),
        (Iu = null),
        Pu.call(t)));
  }
  function e0(t, e) {
    if (!(e.state.loading & 4)) {
      var n = Iu.get(t);
      if (n) var l = n.get(null);
      else {
        ((n = new Map()), Iu.set(t, n));
        for (
          var s = t.querySelectorAll(
              "link[data-precedence],style[data-precedence]",
            ),
            o = 0;
          o < s.length;
          o++
        ) {
          var d = s[o];
          (d.nodeName === "LINK" || d.getAttribute("media") !== "not all") &&
            (n.set(d.dataset.precedence, d), (l = d));
        }
        l && n.set(null, l);
      }
      ((s = e.instance),
        (d = s.getAttribute("data-precedence")),
        (o = n.get(d) || l),
        o === l && n.set(null, s),
        n.set(d, s),
        this.count++,
        (l = Pu.bind(this)),
        s.addEventListener("load", l),
        s.addEventListener("error", l),
        o
          ? o.parentNode.insertBefore(s, o.nextSibling)
          : ((t = t.nodeType === 9 ? t.head : t),
            t.insertBefore(s, t.firstChild)),
        (e.state.loading |= 4));
    }
  }
  var Ei = {
    $$typeof: Q,
    Provider: null,
    Consumer: null,
    _currentValue: lt,
    _currentValue2: lt,
    _threadCount: 0,
  };
  function n0(t, e, n, l, s, o, d, v, R) {
    ((this.tag = 1),
      (this.containerInfo = t),
      (this.pingCache = this.current = this.pendingChildren = null),
      (this.timeoutHandle = -1),
      (this.callbackNode =
        this.next =
        this.pendingContext =
        this.context =
        this.cancelPendingCommit =
          null),
      (this.callbackPriority = 0),
      (this.expirationTimes = Ds(-1)),
      (this.entangledLanes =
        this.shellSuspendCounter =
        this.errorRecoveryDisabledLanes =
        this.expiredLanes =
        this.warmLanes =
        this.pingedLanes =
        this.suspendedLanes =
        this.pendingLanes =
          0),
      (this.entanglements = Ds(0)),
      (this.hiddenUpdates = Ds(null)),
      (this.identifierPrefix = l),
      (this.onUncaughtError = s),
      (this.onCaughtError = o),
      (this.onRecoverableError = d),
      (this.pooledCache = null),
      (this.pooledCacheLanes = 0),
      (this.formState = R),
      (this.incompleteTransitions = new Map()));
  }
  function ym(t, e, n, l, s, o, d, v, R, D, j, V) {
    return (
      (t = new n0(t, e, n, d, R, D, j, V, v)),
      (e = 1),
      o === !0 && (e |= 24),
      (o = Ae(3, null, null, e)),
      (t.current = o),
      (o.stateNode = t),
      (e = pr()),
      e.refCount++,
      (t.pooledCache = e),
      e.refCount++,
      (o.memoizedState = { element: l, isDehydrated: n, cache: e }),
      br(o),
      t
    );
  }
  function pm(t) {
    return t ? ((t = Wa), t) : Wa;
  }
  function gm(t, e, n, l, s, o) {
    ((s = pm(s)),
      l.context === null ? (l.context = s) : (l.pendingContext = s),
      (l = Bn(e)),
      (l.payload = { element: n }),
      (o = o === void 0 ? null : o),
      o !== null && (l.callback = o),
      (n = jn(t, l, e)),
      n !== null && (Se(n, t, e), ti(n, t, e)));
  }
  function vm(t, e) {
    if (((t = t.memoizedState), t !== null && t.dehydrated !== null)) {
      var n = t.retryLane;
      t.retryLane = n !== 0 && n < e ? n : e;
    }
  }
  function qo(t, e) {
    (vm(t, e), (t = t.alternate) && vm(t, e));
  }
  function Sm(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = ya(t, 67108864);
      (e !== null && Se(e, t, 67108864), qo(t, 67108864));
    }
  }
  function bm(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = we();
      e = Ls(e);
      var n = ya(t, e);
      (n !== null && Se(n, t, e), qo(t, e));
    }
  }
  var $u = !0;
  function a0(t, e, n, l) {
    var s = B.T;
    B.T = null;
    var o = J.p;
    try {
      ((J.p = 2), Yo(t, e, n, l));
    } finally {
      ((J.p = o), (B.T = s));
    }
  }
  function l0(t, e, n, l) {
    var s = B.T;
    B.T = null;
    var o = J.p;
    try {
      ((J.p = 8), Yo(t, e, n, l));
    } finally {
      ((J.p = o), (B.T = s));
    }
  }
  function Yo(t, e, n, l) {
    if ($u) {
      var s = Go(l);
      if (s === null) (Ao(t, e, l, ts, n), Em(t, l));
      else if (u0(s, t, e, n, l)) l.stopPropagation();
      else if ((Em(t, l), e & 4 && -1 < i0.indexOf(t))) {
        for (; s !== null; ) {
          var o = Ga(s);
          if (o !== null)
            switch (o.tag) {
              case 3:
                if (((o = o.stateNode), o.current.memoizedState.isDehydrated)) {
                  var d = ca(o.pendingLanes);
                  if (d !== 0) {
                    var v = o;
                    for (v.pendingLanes |= 2, v.entangledLanes |= 2; d; ) {
                      var R = 1 << (31 - Te(d));
                      ((v.entanglements[1] |= R), (d &= ~R));
                    }
                    (Ie(o), (Rt & 6) === 0 && ((Bu = ce() + 500), pi(0)));
                  }
                }
                break;
              case 31:
              case 13:
                ((v = ya(o, 2)), v !== null && Se(v, o, 2), Hu(), qo(o, 2));
            }
          if (((o = Go(l)), o === null && Ao(t, e, l, ts, n), o === s)) break;
          s = o;
        }
        s !== null && l.stopPropagation();
      } else Ao(t, e, l, null, n);
    }
  }
  function Go(t) {
    return ((t = Xs(t)), Xo(t));
  }
  var ts = null;
  function Xo(t) {
    if (((ts = null), (t = Ya(t)), t !== null)) {
      var e = f(t);
      if (e === null) t = null;
      else {
        var n = e.tag;
        if (n === 13) {
          if (((t = h(e)), t !== null)) return t;
          t = null;
        } else if (n === 31) {
          if (((t = g(e)), t !== null)) return t;
          t = null;
        } else if (n === 3) {
          if (e.stateNode.current.memoizedState.isDehydrated)
            return e.tag === 3 ? e.stateNode.containerInfo : null;
          t = null;
        } else e !== t && (t = null);
      }
    }
    return ((ts = t), null);
  }
  function _m(t) {
    switch (t) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (Lt()) {
          case ae:
            return 2;
          case Je:
            return 8;
          case Ha:
          case Zp:
            return 32;
          case Cc:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var Vo = !1,
    kn = null,
    Fn = null,
    Pn = null,
    Ri = new Map(),
    Ti = new Map(),
    In = [],
    i0 =
      "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
        " ",
      );
  function Em(t, e) {
    switch (t) {
      case "focusin":
      case "focusout":
        kn = null;
        break;
      case "dragenter":
      case "dragleave":
        Fn = null;
        break;
      case "mouseover":
      case "mouseout":
        Pn = null;
        break;
      case "pointerover":
      case "pointerout":
        Ri.delete(e.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        Ti.delete(e.pointerId);
    }
  }
  function xi(t, e, n, l, s, o) {
    return t === null || t.nativeEvent !== o
      ? ((t = {
          blockedOn: e,
          domEventName: n,
          eventSystemFlags: l,
          nativeEvent: o,
          targetContainers: [s],
        }),
        e !== null && ((e = Ga(e)), e !== null && Sm(e)),
        t)
      : ((t.eventSystemFlags |= l),
        (e = t.targetContainers),
        s !== null && e.indexOf(s) === -1 && e.push(s),
        t);
  }
  function u0(t, e, n, l, s) {
    switch (e) {
      case "focusin":
        return ((kn = xi(kn, t, e, n, l, s)), !0);
      case "dragenter":
        return ((Fn = xi(Fn, t, e, n, l, s)), !0);
      case "mouseover":
        return ((Pn = xi(Pn, t, e, n, l, s)), !0);
      case "pointerover":
        var o = s.pointerId;
        return (Ri.set(o, xi(Ri.get(o) || null, t, e, n, l, s)), !0);
      case "gotpointercapture":
        return (
          (o = s.pointerId),
          Ti.set(o, xi(Ti.get(o) || null, t, e, n, l, s)),
          !0
        );
    }
    return !1;
  }
  function Rm(t) {
    var e = Ya(t.target);
    if (e !== null) {
      var n = f(e);
      if (n !== null) {
        if (((e = n.tag), e === 13)) {
          if (((e = h(n)), e !== null)) {
            ((t.blockedOn = e),
              Bc(t.priority, function () {
                bm(n);
              }));
            return;
          }
        } else if (e === 31) {
          if (((e = g(n)), e !== null)) {
            ((t.blockedOn = e),
              Bc(t.priority, function () {
                bm(n);
              }));
            return;
          }
        } else if (e === 3 && n.stateNode.current.memoizedState.isDehydrated) {
          t.blockedOn = n.tag === 3 ? n.stateNode.containerInfo : null;
          return;
        }
      }
    }
    t.blockedOn = null;
  }
  function es(t) {
    if (t.blockedOn !== null) return !1;
    for (var e = t.targetContainers; 0 < e.length; ) {
      var n = Go(t.nativeEvent);
      if (n === null) {
        n = t.nativeEvent;
        var l = new n.constructor(n.type, n);
        ((Gs = l), n.target.dispatchEvent(l), (Gs = null));
      } else return ((e = Ga(n)), e !== null && Sm(e), (t.blockedOn = n), !1);
      e.shift();
    }
    return !0;
  }
  function Tm(t, e, n) {
    es(t) && n.delete(e);
  }
  function s0() {
    ((Vo = !1),
      kn !== null && es(kn) && (kn = null),
      Fn !== null && es(Fn) && (Fn = null),
      Pn !== null && es(Pn) && (Pn = null),
      Ri.forEach(Tm),
      Ti.forEach(Tm));
  }
  function ns(t, e) {
    t.blockedOn === e &&
      ((t.blockedOn = null),
      Vo ||
        ((Vo = !0),
        a.unstable_scheduleCallback(a.unstable_NormalPriority, s0)));
  }
  var as = null;
  function xm(t) {
    as !== t &&
      ((as = t),
      a.unstable_scheduleCallback(a.unstable_NormalPriority, function () {
        as === t && (as = null);
        for (var e = 0; e < t.length; e += 3) {
          var n = t[e],
            l = t[e + 1],
            s = t[e + 2];
          if (typeof l != "function") {
            if (Xo(l || n) === null) continue;
            break;
          }
          var o = Ga(n);
          o !== null &&
            (t.splice(e, 3),
            (e -= 3),
            Yr(o, { pending: !0, data: s, method: n.method, action: l }, l, s));
        }
      }));
  }
  function El(t) {
    function e(R) {
      return ns(R, t);
    }
    (kn !== null && ns(kn, t),
      Fn !== null && ns(Fn, t),
      Pn !== null && ns(Pn, t),
      Ri.forEach(e),
      Ti.forEach(e));
    for (var n = 0; n < In.length; n++) {
      var l = In[n];
      l.blockedOn === t && (l.blockedOn = null);
    }
    for (; 0 < In.length && ((n = In[0]), n.blockedOn === null); )
      (Rm(n), n.blockedOn === null && In.shift());
    if (((n = (t.ownerDocument || t).$$reactFormReplay), n != null))
      for (l = 0; l < n.length; l += 3) {
        var s = n[l],
          o = n[l + 1],
          d = s[he] || null;
        if (typeof o == "function") d || xm(n);
        else if (d) {
          var v = null;
          if (o && o.hasAttribute("formAction")) {
            if (((s = o), (d = o[he] || null))) v = d.formAction;
            else if (Xo(s) !== null) continue;
          } else v = d.action;
          (typeof v == "function" ? (n[l + 1] = v) : (n.splice(l, 3), (l -= 3)),
            xm(n));
        }
      }
  }
  function Am() {
    function t(o) {
      o.canIntercept &&
        o.info === "react-transition" &&
        o.intercept({
          handler: function () {
            return new Promise(function (d) {
              return (s = d);
            });
          },
          focusReset: "manual",
          scroll: "manual",
        });
    }
    function e() {
      (s !== null && (s(), (s = null)), l || setTimeout(n, 20));
    }
    function n() {
      if (!l && !navigation.transition) {
        var o = navigation.currentEntry;
        o &&
          o.url != null &&
          navigation.navigate(o.url, {
            state: o.getState(),
            info: "react-transition",
            history: "replace",
          });
      }
    }
    if (typeof navigation == "object") {
      var l = !1,
        s = null;
      return (
        navigation.addEventListener("navigate", t),
        navigation.addEventListener("navigatesuccess", e),
        navigation.addEventListener("navigateerror", e),
        setTimeout(n, 100),
        function () {
          ((l = !0),
            navigation.removeEventListener("navigate", t),
            navigation.removeEventListener("navigatesuccess", e),
            navigation.removeEventListener("navigateerror", e),
            s !== null && (s(), (s = null)));
        }
      );
    }
  }
  function Qo(t) {
    this._internalRoot = t;
  }
  ((ls.prototype.render = Qo.prototype.render =
    function (t) {
      var e = this._internalRoot;
      if (e === null) throw Error(r(409));
      var n = e.current,
        l = we();
      gm(n, l, t, e, null, null);
    }),
    (ls.prototype.unmount = Qo.prototype.unmount =
      function () {
        var t = this._internalRoot;
        if (t !== null) {
          this._internalRoot = null;
          var e = t.containerInfo;
          (gm(t.current, 2, null, t, null, null), Hu(), (e[qa] = null));
        }
      }));
  function ls(t) {
    this._internalRoot = t;
  }
  ls.prototype.unstable_scheduleHydration = function (t) {
    if (t) {
      var e = Nc();
      t = { blockedOn: null, target: t, priority: e };
      for (var n = 0; n < In.length && e !== 0 && e < In[n].priority; n++);
      (In.splice(n, 0, t), n === 0 && Rm(t));
    }
  };
  var Mm = i.version;
  if (Mm !== "19.2.5") throw Error(r(527, Mm, "19.2.5"));
  J.findDOMNode = function (t) {
    var e = t._reactInternals;
    if (e === void 0)
      throw typeof t.render == "function"
        ? Error(r(188))
        : ((t = Object.keys(t).join(",")), Error(r(268, t)));
    return (
      (t = m(e)),
      (t = t !== null ? S(t) : null),
      (t = t === null ? null : t.stateNode),
      t
    );
  };
  var r0 = {
    bundleType: 0,
    version: "19.2.5",
    rendererPackageName: "react-dom",
    currentDispatcherRef: B,
    reconcilerVersion: "19.2.5",
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var is = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!is.isDisabled && is.supportsFiber)
      try {
        ((Ul = is.inject(r0)), (Re = is));
      } catch {}
  }
  return (
    (Mi.createRoot = function (t, e) {
      if (!c(t)) throw Error(r(299));
      var n = !1,
        l = "",
        s = Ud,
        o = Nd,
        d = Bd;
      return (
        e != null &&
          (e.unstable_strictMode === !0 && (n = !0),
          e.identifierPrefix !== void 0 && (l = e.identifierPrefix),
          e.onUncaughtError !== void 0 && (s = e.onUncaughtError),
          e.onCaughtError !== void 0 && (o = e.onCaughtError),
          e.onRecoverableError !== void 0 && (d = e.onRecoverableError)),
        (e = ym(t, 1, !1, null, null, n, l, null, s, o, d, Am)),
        (t[qa] = e.current),
        xo(t),
        new Qo(e)
      );
    }),
    (Mi.hydrateRoot = function (t, e, n) {
      if (!c(t)) throw Error(r(299));
      var l = !1,
        s = "",
        o = Ud,
        d = Nd,
        v = Bd,
        R = null;
      return (
        n != null &&
          (n.unstable_strictMode === !0 && (l = !0),
          n.identifierPrefix !== void 0 && (s = n.identifierPrefix),
          n.onUncaughtError !== void 0 && (o = n.onUncaughtError),
          n.onCaughtError !== void 0 && (d = n.onCaughtError),
          n.onRecoverableError !== void 0 && (v = n.onRecoverableError),
          n.formState !== void 0 && (R = n.formState)),
        (e = ym(t, 1, !0, e, n ?? null, l, s, R, o, d, v, Am)),
        (e.context = pm(null)),
        (n = e.current),
        (l = we()),
        (l = Ls(l)),
        (s = Bn(l)),
        (s.callback = null),
        jn(n, s, l),
        (n = l),
        (e.current.lanes = n),
        Bl(e, n),
        Ie(e),
        (t[qa] = e.current),
        xo(t),
        new ls(e)
      );
    }),
    (Mi.version = "19.2.5"),
    Mi
  );
}
var jm;
function v0() {
  if (jm) return Jo.exports;
  jm = 1;
  function a() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(a);
      } catch (i) {
        console.error(i);
      }
  }
  return (a(), (Jo.exports = g0()), Jo.exports);
}
var S0 = v0(),
  b0 = "__TSS_CONTEXT",
  fc = Symbol.for("TSS_SERVER_FUNCTION"),
  _0 = "application/x-tss-framed",
  En = { JSON: 0, CHUNK: 1, END: 2, ERROR: 3 },
  E0 = /;\s*v=(\d+)/;
function R0(a) {
  const i = a.match(E0);
  return i ? parseInt(i[1], 10) : void 0;
}
function T0(a) {
  const i = R0(a);
  if (i !== void 0 && i !== 1)
    throw new Error(
      `Incompatible framed protocol version: server=${i}, client=1. Please ensure client and server are using compatible versions.`,
    );
}
var zy = () => window.__TSS_START_OPTIONS__;
const Oy = !1;
function Ni(a) {
  return a[a.length - 1];
}
function x0(a) {
  return typeof a == "function";
}
function ea(a, i) {
  return x0(a) ? a(i) : a;
}
const Cy = Object.prototype.hasOwnProperty,
  Hm = Object.prototype.propertyIsEnumerable;
function wy(a) {
  for (const i in a) if (Cy.call(a, i)) return !0;
  return !1;
}
const A0 = () => Object.create(null),
  za = (a, i) => Oa(a, i, A0);
function Oa(a, i, u = () => ({}), r = 0) {
  if (a === i) return a;
  if (r > 500) return i;
  const c = i,
    f = Gm(a) && Gm(c);
  if (!f && !(Cl(a) && Cl(c))) return c;
  const h = f ? a : qm(a);
  if (!h) return c;
  const g = f ? c : qm(c);
  if (!g) return c;
  const y = h.length,
    m = g.length,
    S = f ? new Array(m) : u();
  let p = 0;
  for (let E = 0; E < m; E++) {
    const _ = f ? E : g[E],
      z = a[_],
      L = c[_];
    if (z === L) {
      ((S[_] = z), (f ? E < y : Cy.call(a, _)) && p++);
      continue;
    }
    if (
      z === null ||
      L === null ||
      typeof z != "object" ||
      typeof L != "object"
    ) {
      S[_] = L;
      continue;
    }
    const x = Oa(z, L, u, r + 1);
    ((S[_] = x), x === z && p++);
  }
  return y === m && p === y ? a : S;
}
function qm(a) {
  const i = Object.getOwnPropertyNames(a);
  for (const c of i) if (!Hm.call(a, c)) return !1;
  const u = Object.getOwnPropertySymbols(a);
  if (u.length === 0) return i;
  const r = i;
  for (const c of u) {
    if (!Hm.call(a, c)) return !1;
    r.push(c);
  }
  return r;
}
function Cl(a) {
  if (!Ym(a)) return !1;
  const i = a.constructor;
  if (typeof i > "u") return !0;
  const u = i.prototype;
  return !(!Ym(u) || !u.hasOwnProperty("isPrototypeOf"));
}
function Ym(a) {
  return Object.prototype.toString.call(a) === "[object Object]";
}
function Gm(a) {
  return Array.isArray(a) && a.length === Object.keys(a).length;
}
function be(a, i, u) {
  if (a === i) return !0;
  if (typeof a != typeof i) return !1;
  if (Array.isArray(a) && Array.isArray(i)) {
    if (a.length !== i.length) return !1;
    for (let r = 0, c = a.length; r < c; r++) if (!be(a[r], i[r], u)) return !1;
    return !0;
  }
  if (Cl(a) && Cl(i)) {
    const r = u?.ignoreUndefined ?? !0;
    if (u?.partial) {
      for (const h in i)
        if ((!r || i[h] !== void 0) && !be(a[h], i[h], u)) return !1;
      return !0;
    }
    let c = 0;
    if (!r) c = Object.keys(a).length;
    else for (const h in a) a[h] !== void 0 && c++;
    let f = 0;
    for (const h in i)
      if ((!r || i[h] !== void 0) && (f++, f > c || !be(a[h], i[h], u)))
        return !1;
    return c === f;
  }
  return !1;
}
function Ua(a) {
  let i, u;
  const r = new Promise((c, f) => {
    ((i = c), (u = f));
  });
  return (
    (r.status = "pending"),
    (r.resolve = (c) => {
      ((r.status = "resolved"), (r.value = c), i(c), a?.(c));
    }),
    (r.reject = (c) => {
      ((r.status = "rejected"), u(c));
    }),
    r
  );
}
function M0(a) {
  return typeof a?.message != "string"
    ? !1
    : a.message.startsWith("Failed to fetch dynamically imported module") ||
        a.message.startsWith("error loading dynamically imported module") ||
        a.message.startsWith("Importing a module script failed");
}
function Bi(a) {
  return !!(a && typeof a == "object" && typeof a.then == "function");
}
function z0(a) {
  return a.replace(/[\x00-\x1f\x7f]/g, "");
}
function Xm(a) {
  let i;
  try {
    i = decodeURI(a);
  } catch {
    i = a.replaceAll(/%[0-9A-F]{2}/gi, (u) => {
      try {
        return decodeURI(u);
      } catch {
        return u;
      }
    });
  }
  return z0(i);
}
const O0 = ["http:", "https:", "mailto:", "tel:"];
function ps(a, i) {
  if (!a) return !1;
  try {
    const u = new URL(a);
    return !i.has(u.protocol);
  } catch {
    return !1;
  }
}
const C0 = {
    "&": "\\u0026",
    ">": "\\u003e",
    "<": "\\u003c",
    "\u2028": "\\u2028",
    "\u2029": "\\u2029",
  },
  w0 = /[&><\u2028\u2029]/g;
function D0(a) {
  return a.replace(w0, (i) => C0[i]);
}
function zi(a) {
  if (!a) return { path: a, handledProtocolRelativeURL: !1 };
  if (!/[%\\\x00-\x1f\x7f]/.test(a) && !a.startsWith("//"))
    return { path: a, handledProtocolRelativeURL: !1 };
  const i = /%25|%5C/gi;
  let u = 0,
    r = "",
    c;
  for (; (c = i.exec(a)) !== null; )
    ((r += Xm(a.slice(u, c.index)) + c[0]), (u = i.lastIndex));
  r = r + Xm(u ? a.slice(u) : a);
  let f = !1;
  return (
    r.startsWith("//") && ((f = !0), (r = "/" + r.replace(/^\/+/, ""))),
    { path: r, handledProtocolRelativeURL: f }
  );
}
function L0(a) {
  return /\s|[^\u0000-\u007F]/.test(a)
    ? a.replace(/\s|[^\u0000-\u007F]/gu, encodeURIComponent)
    : a;
}
function U0(a, i) {
  if (a === i) return !0;
  if (a.length !== i.length) return !1;
  for (let u = 0; u < a.length; u++) if (a[u] !== i[u]) return !1;
  return !0;
}
function Ee() {
  throw new Error("Invariant failed");
}
function ji(a) {
  const i = new Map();
  let u, r;
  const c = (f) => {
    f.next &&
      (f.prev
        ? ((f.prev.next = f.next),
          (f.next.prev = f.prev),
          (f.next = void 0),
          r && ((r.next = f), (f.prev = r)))
        : ((f.next.prev = void 0),
          (u = f.next),
          (f.next = void 0),
          r && ((f.prev = r), (r.next = f))),
      (r = f));
  };
  return {
    get(f) {
      const h = i.get(f);
      if (h) return (c(h), h.value);
    },
    set(f, h) {
      if (i.size >= a && u) {
        const y = u;
        (i.delete(y.key),
          y.next && ((u = y.next), (y.next.prev = void 0)),
          y === r && (r = void 0));
      }
      const g = i.get(f);
      if (g) ((g.value = h), c(g));
      else {
        const y = { key: f, value: h, prev: r };
        (r && (r.next = y), (r = y), u || (u = y), i.set(f, y));
      }
    },
    clear() {
      (i.clear(), (u = void 0), (r = void 0));
    },
  };
}
const na = 4,
  Dy = 5;
function N0(a) {
  const i = a.indexOf("{");
  if (i === -1) return null;
  const u = a.indexOf("}", i);
  return u === -1 || i + 1 >= a.length ? null : [i, u];
}
function Ly(a, i, u = new Uint16Array(6)) {
  const r = a.indexOf("/", i),
    c = r === -1 ? a.length : r,
    f = a.substring(i, c);
  if (!f || !f.includes("$"))
    return (
      (u[0] = 0),
      (u[1] = i),
      (u[2] = i),
      (u[3] = c),
      (u[4] = c),
      (u[5] = c),
      u
    );
  if (f === "$") {
    const g = a.length;
    return (
      (u[0] = 2),
      (u[1] = i),
      (u[2] = i),
      (u[3] = g),
      (u[4] = g),
      (u[5] = g),
      u
    );
  }
  if (f.charCodeAt(0) === 36)
    return (
      (u[0] = 1),
      (u[1] = i),
      (u[2] = i + 1),
      (u[3] = c),
      (u[4] = c),
      (u[5] = c),
      u
    );
  const h = N0(f);
  if (h) {
    const [g, y] = h,
      m = f.charCodeAt(g + 1);
    if (m === 45) {
      if (g + 2 < f.length && f.charCodeAt(g + 2) === 36) {
        const S = g + 3,
          p = y;
        if (S < p)
          return (
            (u[0] = 3),
            (u[1] = i + g),
            (u[2] = i + S),
            (u[3] = i + p),
            (u[4] = i + y + 1),
            (u[5] = c),
            u
          );
      }
    } else if (m === 36) {
      const S = g + 1,
        p = g + 2;
      return p === y
        ? ((u[0] = 2),
          (u[1] = i + g),
          (u[2] = i + S),
          (u[3] = i + p),
          (u[4] = i + y + 1),
          (u[5] = a.length),
          u)
        : ((u[0] = 1),
          (u[1] = i + g),
          (u[2] = i + p),
          (u[3] = i + y),
          (u[4] = i + y + 1),
          (u[5] = c),
          u);
    }
  }
  return (
    (u[0] = 0),
    (u[1] = i),
    (u[2] = i),
    (u[3] = c),
    (u[4] = c),
    (u[5] = c),
    u
  );
}
function Rs(a, i, u, r, c, f, h) {
  h?.(u);
  let g = r;
  {
    const y = u.fullPath ?? u.from,
      m = y.length,
      S = u.options?.caseSensitive ?? a,
      p = u.options?.params?.parse ?? u.options?.parseParams;
    for (; g < m; ) {
      const _ = Ly(y, g, i);
      let z;
      const L = g,
        x = _[5];
      switch (((g = x + 1), f++, _[0])) {
        case 0: {
          const C = y.substring(_[2], _[3]);
          if (S) {
            const Y = c.static?.get(C);
            if (Y) z = Y;
            else {
              c.static ??= new Map();
              const Q = Ca(u.fullPath ?? u.from);
              ((Q.parent = c), (Q.depth = f), (z = Q), c.static.set(C, Q));
            }
          } else {
            const Y = C.toLowerCase(),
              Q = c.staticInsensitive?.get(Y);
            if (Q) z = Q;
            else {
              c.staticInsensitive ??= new Map();
              const H = Ca(u.fullPath ?? u.from);
              ((H.parent = c),
                (H.depth = f),
                (z = H),
                c.staticInsensitive.set(Y, H));
            }
          }
          break;
        }
        case 1: {
          const C = y.substring(L, _[1]),
            Y = y.substring(_[4], x),
            Q = S && !!(C || Y),
            H = C ? (Q ? C : C.toLowerCase()) : void 0,
            W = Y ? (Q ? Y : Y.toLowerCase()) : void 0,
            F =
              !p &&
              c.dynamic?.find(
                (G) =>
                  !G.parse &&
                  G.caseSensitive === Q &&
                  G.prefix === H &&
                  G.suffix === W,
              );
          if (F) z = F;
          else {
            const G = Wo(1, u.fullPath ?? u.from, Q, H, W);
            ((z = G),
              (G.depth = f),
              (G.parent = c),
              (c.dynamic ??= []),
              c.dynamic.push(G));
          }
          break;
        }
        case 3: {
          const C = y.substring(L, _[1]),
            Y = y.substring(_[4], x),
            Q = S && !!(C || Y),
            H = C ? (Q ? C : C.toLowerCase()) : void 0,
            W = Y ? (Q ? Y : Y.toLowerCase()) : void 0,
            F =
              !p &&
              c.optional?.find(
                (G) =>
                  !G.parse &&
                  G.caseSensitive === Q &&
                  G.prefix === H &&
                  G.suffix === W,
              );
          if (F) z = F;
          else {
            const G = Wo(3, u.fullPath ?? u.from, Q, H, W);
            ((z = G),
              (G.parent = c),
              (G.depth = f),
              (c.optional ??= []),
              c.optional.push(G));
          }
          break;
        }
        case 2: {
          const C = y.substring(L, _[1]),
            Y = y.substring(_[4], x),
            Q = S && !!(C || Y),
            H = C ? (Q ? C : C.toLowerCase()) : void 0,
            W = Y ? (Q ? Y : Y.toLowerCase()) : void 0,
            F = Wo(2, u.fullPath ?? u.from, Q, H, W);
          ((z = F),
            (F.parent = c),
            (F.depth = f),
            (c.wildcard ??= []),
            c.wildcard.push(F));
        }
      }
      c = z;
    }
    if (
      p &&
      u.children &&
      !u.isRoot &&
      u.id &&
      u.id.charCodeAt(u.id.lastIndexOf("/") + 1) === 95
    ) {
      const _ = Ca(u.fullPath ?? u.from);
      ((_.kind = Dy),
        (_.parent = c),
        f++,
        (_.depth = f),
        (c.pathless ??= []),
        c.pathless.push(_),
        (c = _));
    }
    const E = (u.path || !u.children) && !u.isRoot;
    if (E && y.endsWith("/")) {
      const _ = Ca(u.fullPath ?? u.from);
      ((_.kind = na),
        (_.parent = c),
        f++,
        (_.depth = f),
        (c.index = _),
        (c = _));
    }
    ((c.parse = p ?? null),
      E && !c.route && ((c.route = u), (c.fullPath = u.fullPath ?? u.from)));
  }
  if (u.children) for (const y of u.children) Rs(a, i, y, g, c, f, h);
}
function Io(a, i) {
  if (a.parse && !i.parse) return -1;
  if (!a.parse && i.parse) return 1;
  if (a.prefix && i.prefix && a.prefix !== i.prefix) {
    if (a.prefix.startsWith(i.prefix)) return -1;
    if (i.prefix.startsWith(a.prefix)) return 1;
  }
  if (a.suffix && i.suffix && a.suffix !== i.suffix) {
    if (a.suffix.endsWith(i.suffix)) return -1;
    if (i.suffix.endsWith(a.suffix)) return 1;
  }
  return a.prefix && !i.prefix
    ? -1
    : !a.prefix && i.prefix
      ? 1
      : a.suffix && !i.suffix
        ? -1
        : !a.suffix && i.suffix
          ? 1
          : a.caseSensitive && !i.caseSensitive
            ? -1
            : !a.caseSensitive && i.caseSensitive
              ? 1
              : 0;
}
function $n(a) {
  if (a.pathless) for (const i of a.pathless) $n(i);
  if (a.static) for (const i of a.static.values()) $n(i);
  if (a.staticInsensitive) for (const i of a.staticInsensitive.values()) $n(i);
  if (a.dynamic?.length) {
    a.dynamic.sort(Io);
    for (const i of a.dynamic) $n(i);
  }
  if (a.optional?.length) {
    a.optional.sort(Io);
    for (const i of a.optional) $n(i);
  }
  if (a.wildcard?.length) {
    a.wildcard.sort(Io);
    for (const i of a.wildcard) $n(i);
  }
}
function Ca(a) {
  return {
    kind: 0,
    depth: 0,
    pathless: null,
    index: null,
    static: null,
    staticInsensitive: null,
    dynamic: null,
    optional: null,
    wildcard: null,
    route: null,
    fullPath: a,
    parent: null,
    parse: null,
  };
}
function Wo(a, i, u, r, c) {
  return {
    kind: a,
    depth: 0,
    pathless: null,
    index: null,
    static: null,
    staticInsensitive: null,
    dynamic: null,
    optional: null,
    wildcard: null,
    route: null,
    fullPath: i,
    parent: null,
    parse: null,
    caseSensitive: u,
    prefix: r,
    suffix: c,
  };
}
function B0(a, i) {
  const u = Ca("/"),
    r = new Uint16Array(6);
  for (const c of a) Rs(!1, r, c, 1, u, 0);
  ($n(u), (i.masksTree = u), (i.flatCache = ji(1e3)));
}
function j0(a, i) {
  a ||= "/";
  const u = i.flatCache.get(a);
  if (u) return u;
  const r = Sc(a, i.masksTree);
  return (i.flatCache.set(a, r), r);
}
function H0(a, i, u, r, c) {
  ((a ||= "/"), (r ||= "/"));
  const f = i ? `case\0${a}` : a;
  let h = c.singleCache.get(f);
  return (
    h ||
      ((h = Ca("/")),
      Rs(i, new Uint16Array(6), { from: a }, 1, h, 0),
      c.singleCache.set(f, h)),
    Sc(r, h, u)
  );
}
function q0(a, i, u = !1) {
  const r = u ? a : `nofuzz\0${a}`,
    c = i.matchCache.get(r);
  if (c !== void 0) return c;
  a ||= "/";
  let f;
  try {
    f = Sc(a, i.segmentTree, u);
  } catch (h) {
    if (h instanceof URIError) f = null;
    else throw h;
  }
  return (f && (f.branch = Ny(f.route)), i.matchCache.set(r, f), f);
}
function Y0(a) {
  return a === "/" ? a : a.replace(/\/{1,}$/, "");
}
function G0(a, i = !1, u) {
  const r = Ca(a.fullPath),
    c = new Uint16Array(6),
    f = {},
    h = {};
  let g = 0;
  return (
    Rs(i, c, a, 1, r, 0, (y) => {
      if ((u?.(y, g), y.id in f && Ee(), (f[y.id] = y), g !== 0 && y.path)) {
        const m = Y0(y.fullPath);
        (!h[m] || y.fullPath.endsWith("/")) && (h[m] = y);
      }
      g++;
    }),
    $n(r),
    {
      processedTree: {
        segmentTree: r,
        singleCache: ji(1e3),
        matchCache: ji(1e3),
        flatCache: null,
        masksTree: null,
      },
      routesById: f,
      routesByPath: h,
    }
  );
}
function Sc(a, i, u = !1) {
  const r = a.split("/"),
    c = V0(a, r, i, u);
  if (!c) return null;
  const [f] = Uy(a, r, c);
  return { route: c.node.route, rawParams: f };
}
function Uy(a, i, u) {
  const r = X0(u.node);
  let c = null;
  const f = Object.create(null);
  let h = u.extract?.part ?? 0,
    g = u.extract?.node ?? 0,
    y = u.extract?.path ?? 0,
    m = u.extract?.segment ?? 0;
  for (; g < r.length; h++, g++, y++, m++) {
    const S = r[g];
    if (S.kind === na) break;
    if (S.kind === Dy) {
      (m--, h--, y--);
      continue;
    }
    const p = i[h],
      E = y;
    if ((p && (y += p.length), S.kind === 1)) {
      c ??= u.node.fullPath.split("/");
      const _ = c[m],
        z = S.prefix?.length ?? 0;
      if (_.charCodeAt(z) === 123) {
        const L = S.suffix?.length ?? 0,
          x = _.substring(z + 2, _.length - L - 1),
          C = p.substring(z, p.length - L);
        f[x] = decodeURIComponent(C);
      } else {
        const L = _.substring(1);
        f[L] = decodeURIComponent(p);
      }
    } else if (S.kind === 3) {
      if (u.skipped & (1 << g)) {
        (h--, (y = E - 1));
        continue;
      }
      c ??= u.node.fullPath.split("/");
      const _ = c[m],
        z = S.prefix?.length ?? 0,
        L = S.suffix?.length ?? 0,
        x = _.substring(z + 3, _.length - L - 1),
        C = S.suffix || S.prefix ? p.substring(z, p.length - L) : p;
      C && (f[x] = decodeURIComponent(C));
    } else if (S.kind === 2) {
      const _ = S,
        z = a.substring(
          E + (_.prefix?.length ?? 0),
          a.length - (_.suffix?.length ?? 0),
        ),
        L = decodeURIComponent(z);
      ((f["*"] = L), (f._splat = L));
      break;
    }
  }
  return (
    u.rawParams && Object.assign(f, u.rawParams),
    [f, { part: h, node: g, path: y, segment: m }]
  );
}
function Ny(a) {
  const i = [a];
  for (; a.parentRoute; ) ((a = a.parentRoute), i.push(a));
  return (i.reverse(), i);
}
function X0(a) {
  const i = Array(a.depth + 1);
  do ((i[a.depth] = a), (a = a.parent));
  while (a);
  return i;
}
function V0(a, i, u, r) {
  if (a === "/" && u.index) return { node: u.index, skipped: 0 };
  const c = !Ni(i),
    f = c && a !== "/",
    h = i.length - (c ? 1 : 0),
    g = [
      {
        node: u,
        index: 1,
        skipped: 0,
        depth: 1,
        statics: 0,
        dynamics: 0,
        optionals: 0,
      },
    ];
  let y = null,
    m = null;
  for (; g.length; ) {
    const S = g.pop(),
      {
        node: p,
        index: E,
        skipped: _,
        depth: z,
        statics: L,
        dynamics: x,
        optionals: C,
      } = S;
    let { extract: Y, rawParams: Q } = S;
    if (p.kind === 2 && p.route && !ss(m, S)) continue;
    if (p.parse) {
      if (!Vm(a, i, S)) continue;
      ((Q = S.rawParams), (Y = S.extract));
    }
    r && p.route && p.kind !== na && ss(y, S) && (y = S);
    const H = E === h;
    if (
      H &&
      (p.route && (!f || p.kind === na || p.kind === 2) && ss(m, S) && (m = S),
      !p.optional && !p.wildcard && !p.index && !p.pathless)
    )
      continue;
    const W = H ? void 0 : i[E];
    let F;
    if (H && p.index) {
      const G = {
        node: p.index,
        index: E,
        skipped: _,
        depth: z + 1,
        statics: L,
        dynamics: x,
        optionals: C,
        extract: Y,
        rawParams: Q,
      };
      let Z = !0;
      if ((p.index.parse && (Vm(a, i, G) || (Z = !1)), Z)) {
        if (!x && !C && !_ && Q0(L, h)) return G;
        ss(m, G) && (m = G);
      }
    }
    if (p.wildcard)
      for (let G = p.wildcard.length - 1; G >= 0; G--) {
        const Z = p.wildcard[G],
          { prefix: P, suffix: rt } = Z;
        if (
          !(
            P &&
            (H ||
              !(Z.caseSensitive ? W : (F ??= W.toLowerCase())).startsWith(P))
          )
        ) {
          if (rt) {
            if (H) continue;
            const ut = i.slice(E).join("/").slice(-rt.length);
            if ((Z.caseSensitive ? ut : ut.toLowerCase()) !== rt) continue;
          }
          g.push({
            node: Z,
            index: h,
            skipped: _,
            depth: z + 1,
            statics: L,
            dynamics: x,
            optionals: C,
            extract: Y,
            rawParams: Q,
          });
        }
      }
    if (p.optional) {
      const G = _ | (1 << z),
        Z = z + 1;
      for (let P = p.optional.length - 1; P >= 0; P--) {
        const rt = p.optional[P];
        g.push({
          node: rt,
          index: E,
          skipped: G,
          depth: Z,
          statics: L,
          dynamics: x,
          optionals: C,
          extract: Y,
          rawParams: Q,
        });
      }
      if (!H)
        for (let P = p.optional.length - 1; P >= 0; P--) {
          const rt = p.optional[P],
            { prefix: ut, suffix: it } = rt;
          if (ut || it) {
            const $t = rt.caseSensitive ? W : (F ??= W.toLowerCase());
            if ((ut && !$t.startsWith(ut)) || (it && !$t.endsWith(it)))
              continue;
          }
          g.push({
            node: rt,
            index: E + 1,
            skipped: _,
            depth: Z,
            statics: L,
            dynamics: x,
            optionals: C + us(h, E),
            extract: Y,
            rawParams: Q,
          });
        }
    }
    if (!H && p.dynamic && W)
      for (let G = p.dynamic.length - 1; G >= 0; G--) {
        const Z = p.dynamic[G],
          { prefix: P, suffix: rt } = Z;
        if (P || rt) {
          const ut = Z.caseSensitive ? W : (F ??= W.toLowerCase());
          if ((P && !ut.startsWith(P)) || (rt && !ut.endsWith(rt))) continue;
        }
        g.push({
          node: Z,
          index: E + 1,
          skipped: _,
          depth: z + 1,
          statics: L,
          dynamics: x + us(h, E),
          optionals: C,
          extract: Y,
          rawParams: Q,
        });
      }
    if (!H && p.staticInsensitive) {
      const G = p.staticInsensitive.get((F ??= W.toLowerCase()));
      G &&
        g.push({
          node: G,
          index: E + 1,
          skipped: _,
          depth: z + 1,
          statics: L + us(h, E),
          dynamics: x,
          optionals: C,
          extract: Y,
          rawParams: Q,
        });
    }
    if (!H && p.static) {
      const G = p.static.get(W);
      G &&
        g.push({
          node: G,
          index: E + 1,
          skipped: _,
          depth: z + 1,
          statics: L + us(h, E),
          dynamics: x,
          optionals: C,
          extract: Y,
          rawParams: Q,
        });
    }
    if (p.pathless) {
      const G = z + 1;
      for (let Z = p.pathless.length - 1; Z >= 0; Z--) {
        const P = p.pathless[Z];
        g.push({
          node: P,
          index: E,
          skipped: _,
          depth: G,
          statics: L,
          dynamics: x,
          optionals: C,
          extract: Y,
          rawParams: Q,
        });
      }
    }
  }
  if (m) return m;
  if (r && y) {
    let S = y.index;
    for (let E = 0; E < y.index; E++) S += i[E].length;
    const p = S === a.length ? "/" : a.slice(S);
    return (
      (y.rawParams ??= Object.create(null)),
      (y.rawParams["**"] = decodeURIComponent(p)),
      y
    );
  }
  return null;
}
function us(a, i) {
  return 2 ** (a - i - 1);
}
function Q0(a, i) {
  return a === 2 ** (i - 1) - 1;
}
function Vm(a, i, u) {
  let r, c;
  try {
    [r, c] = Uy(a, i, u);
  } catch {
    return null;
  }
  if (((u.rawParams = r), (u.extract = c), !u.node.parse)) return !0;
  try {
    if (u.node.parse(r) === !1) return null;
  } catch {}
  return !0;
}
function ss(a, i) {
  return a
    ? i.statics > a.statics ||
        (i.statics === a.statics &&
          (i.dynamics > a.dynamics ||
            (i.dynamics === a.dynamics &&
              (i.optionals > a.optionals ||
                (i.optionals === a.optionals &&
                  ((i.node.kind === na) > (a.node.kind === na) ||
                    ((i.node.kind === na) == (a.node.kind === na) &&
                      i.depth > a.depth)))))))
    : !0;
}
function fs(a) {
  return bc(a.filter((i) => i !== void 0).join("/"));
}
function bc(a) {
  return a.replace(/\/{2,}/g, "/");
}
function By(a) {
  return a === "/" ? a : a.replace(/^\/{1,}/, "");
}
function Rn(a) {
  const i = a.length;
  return i > 1 && a[i - 1] === "/" ? a.replace(/\/{1,}$/, "") : a;
}
function jy(a) {
  return Rn(By(a));
}
function gs(a, i) {
  return a?.endsWith("/") && a !== "/" && a !== `${i}/` ? a.slice(0, -1) : a;
}
function Z0(a, i, u) {
  return gs(a, u) === gs(i, u);
}
function K0({ base: a, to: i, trailingSlash: u = "never", cache: r }) {
  const c = i.startsWith("/"),
    f = !c && i === ".";
  let h;
  if (r) {
    h = c ? i : f ? a : a + "\0" + i;
    const m = r.get(h);
    if (m) return m;
  }
  let g;
  if (f) g = a.split("/");
  else if (c) g = i.split("/");
  else {
    for (g = a.split("/"); g.length > 1 && Ni(g) === ""; ) g.pop();
    const m = i.split("/");
    for (let S = 0, p = m.length; S < p; S++) {
      const E = m[S];
      E === ""
        ? S
          ? S === p - 1 && g.push(E)
          : (g = [E])
        : E === ".."
          ? g.pop()
          : E === "." || g.push(E);
    }
  }
  g.length > 1 &&
    (Ni(g) === "" ? u === "never" && g.pop() : u === "always" && g.push(""));
  const y = bc(g.join("/")) || "/";
  return (h && r && r.set(h, y), y);
}
function J0(a) {
  const i = new Map(a.map((c) => [encodeURIComponent(c), c])),
    u = Array.from(i.keys())
      .map((c) => c.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
      .join("|"),
    r = new RegExp(u, "g");
  return (c) => c.replace(r, (f) => i.get(f) ?? f);
}
function $o(a, i, u) {
  const r = i[a];
  return typeof r != "string"
    ? r
    : a === "_splat"
      ? /^[a-zA-Z0-9\-._~!/]*$/.test(r)
        ? r
        : r
            .split("/")
            .map((c) => Zm(c, u))
            .join("/")
      : Zm(r, u);
}
function Qm({ path: a, params: i, decoder: u, ...r }) {
  let c = !1;
  const f = Object.create(null);
  if (!a || a === "/")
    return { interpolatedPath: "/", usedParams: f, isMissingParams: c };
  if (!a.includes("$"))
    return { interpolatedPath: a, usedParams: f, isMissingParams: c };
  const h = a.length;
  let g = 0,
    y,
    m = "";
  for (; g < h; ) {
    const S = g;
    y = Ly(a, S, y);
    const p = y[5];
    if (((g = p + 1), S === p)) continue;
    const E = y[0];
    if (E === 0) {
      m += "/" + a.substring(S, p);
      continue;
    }
    if (E === 2) {
      const _ = i._splat;
      ((f._splat = _), (f["*"] = _));
      const z = a.substring(S, y[1]),
        L = a.substring(y[4], p);
      if (!_) {
        ((c = !0), (z || L) && (m += "/" + z + L));
        continue;
      }
      const x = $o("_splat", i, u);
      m += "/" + z + x + L;
      continue;
    }
    if (E === 1) {
      const _ = a.substring(y[2], y[3]);
      (!c && !(_ in i) && (c = !0), (f[_] = i[_]));
      const z = a.substring(S, y[1]),
        L = a.substring(y[4], p),
        x = $o(_, i, u) ?? "undefined";
      m += "/" + z + x + L;
      continue;
    }
    if (E === 3) {
      const _ = a.substring(y[2], y[3]),
        z = i[_];
      if (z == null) continue;
      f[_] = z;
      const L = a.substring(S, y[1]),
        x = a.substring(y[4], p),
        C = $o(_, i, u) ?? "";
      m += "/" + L + C + x;
      continue;
    }
  }
  return (
    a.endsWith("/") && (m += "/"),
    { usedParams: f, interpolatedPath: m || "/", isMissingParams: c }
  );
}
function Zm(a, i) {
  const u = encodeURIComponent(a);
  return i?.(u) ?? u;
}
function ne(a) {
  return a?.isNotFound === !0;
}
function k0() {
  try {
    return typeof window < "u" && typeof window.sessionStorage == "object"
      ? window.sessionStorage
      : void 0;
  } catch {
    return;
  }
}
const F0 = "tsr-scroll-restoration-v1_3";
function P0() {
  const a = k0();
  if (!a) return null;
  let i = {};
  try {
    const r = JSON.parse(a.getItem("tsr-scroll-restoration-v1_3") || "{}");
    Cl(r) && (i = r);
  } catch {}
  return {
    get state() {
      return i;
    },
    set: (r) => {
      i = ea(r, i) || i;
    },
    persist: () => {
      try {
        a.setItem(F0, JSON.stringify(i));
      } catch {}
    },
  };
}
const Km = P0(),
  I0 = (a) => a.state.__TSR_key || a.href;
function W0(a) {
  const i = [];
  let u;
  for (; (u = a.parentNode); )
    (i.push(
      `${a.tagName}:nth-child(${Array.prototype.indexOf.call(u.children, a) + 1})`,
    ),
      (a = u));
  return `${i.reverse().join(" > ")}`.toLowerCase();
}
let rs = !1;
const Oi = "window",
  Jm = "data-scroll-restoration-id";
function $0(a, i) {
  if (!Km) return;
  const u = Km;
  if (
    ((a.options.scrollRestoration ?? !1) && (a.isScrollRestoring = !0),
    a.isScrollRestorationSetup || !u)
  )
    return;
  ((a.isScrollRestorationSetup = !0), (rs = !1));
  const r = a.options.getScrollRestorationKey || I0,
    c = new Map();
  window.history.scrollRestoration = "manual";
  const f = (g) => {
      if (!(rs || !a.isScrollRestoring))
        if (g.target === document || g.target === window)
          c.set(Oi, {
            scrollX: window.scrollX || 0,
            scrollY: window.scrollY || 0,
          });
        else {
          const y = g.target;
          c.set(y, { scrollX: y.scrollLeft || 0, scrollY: y.scrollTop || 0 });
        }
    },
    h = (g) => {
      if (!a.isScrollRestoring || !g || c.size === 0 || !u) return;
      const y = (u.state[g] ||= {});
      for (const [m, S] of c) {
        let p;
        if (m === Oi) p = Oi;
        else if (m.isConnected) {
          const E = m.getAttribute(Jm);
          p = E ? `[${Jm}="${E}"]` : W0(m);
        }
        p && (y[p] = S);
      }
    };
  (document.addEventListener("scroll", f, !0),
    a.subscribe("onBeforeLoad", (g) => {
      (h(g.fromLocation ? r(g.fromLocation) : void 0), c.clear());
    }),
    window.addEventListener("pagehide", () => {
      (h(r(a.stores.resolvedLocation.get() ?? a.stores.location.get())),
        u.persist());
    }),
    a.subscribe("onRendered", (g) => {
      const y = r(g.toLocation),
        m = a.options.scrollRestorationBehavior,
        S = a.options.scrollToTopSelectors;
      if ((c.clear(), !a.resetNextScroll)) {
        a.resetNextScroll = !0;
        return;
      }
      if (
        !(
          typeof a.options.scrollRestoration == "function" &&
          !a.options.scrollRestoration({ location: a.latestLocation })
        )
      ) {
        rs = !0;
        try {
          const p = a.isScrollRestoring ? u.state[y] : void 0;
          let E = !1;
          if (p)
            for (const _ in p) {
              const z = p[_];
              if (!Cl(z)) continue;
              const { scrollX: L, scrollY: x } = z;
              if (!(!Number.isFinite(L) || !Number.isFinite(x))) {
                if (_ === Oi)
                  (window.scrollTo({ top: x, left: L, behavior: m }), (E = !0));
                else if (_) {
                  let C;
                  try {
                    C = document.querySelector(_);
                  } catch {
                    continue;
                  }
                  C && ((C.scrollLeft = L), (C.scrollTop = x), (E = !0));
                }
              }
            }
          if (!E) {
            const _ = a.history.location.hash.slice(1);
            if (_) {
              const z = window.history.state?.__hashScrollIntoViewOptions ?? !0;
              if (z) {
                const L = document.getElementById(_);
                L && L.scrollIntoView(z);
              }
            } else {
              const z = { top: 0, left: 0, behavior: m };
              if ((window.scrollTo(z), S))
                for (const L of S) {
                  if (L === Oi) continue;
                  const x =
                    typeof L == "function" ? L() : document.querySelector(L);
                  x && x.scrollTo(z);
                }
            }
          }
        } finally {
          rs = !1;
        }
        a.isScrollRestoring && u.set((p) => ((p[y] ||= {}), p));
      }
    }));
}
function Hy(a, i = String) {
  const u = new URLSearchParams();
  for (const r in a) {
    const c = a[r];
    c !== void 0 && u.set(r, i(c));
  }
  return u.toString();
}
function tc(a) {
  return a
    ? a === "false"
      ? !1
      : a === "true"
        ? !0
        : +a * 0 === 0 && +a + "" === a
          ? +a
          : a
    : "";
}
function tS(a) {
  const i = new URLSearchParams(a),
    u = Object.create(null);
  for (const [r, c] of i.entries()) {
    const f = u[r];
    f == null
      ? (u[r] = tc(c))
      : Array.isArray(f)
        ? f.push(tc(c))
        : (u[r] = [f, tc(c)]);
  }
  return u;
}
const eS = aS(JSON.parse),
  nS = lS(JSON.stringify, JSON.parse);
function aS(a) {
  return (i) => {
    i[0] === "?" && (i = i.substring(1));
    const u = tS(i);
    for (const r in u) {
      const c = u[r];
      if (typeof c == "string")
        try {
          u[r] = a(c);
        } catch {}
    }
    return u;
  };
}
function lS(a, i) {
  const u = typeof i == "function";
  function r(c) {
    if (typeof c == "object" && c !== null)
      try {
        return a(c);
      } catch {}
    else if (u && typeof c == "string")
      try {
        return (i(c), a(c));
      } catch {}
    return c;
  }
  return (c) => {
    const f = Hy(c, r);
    return f ? `?${f}` : "";
  };
}
const Da = "__root__";
function qy(a) {
  if (
    ((a.statusCode = a.statusCode || a.code || 307),
    !a._builtLocation && !a.reloadDocument && typeof a.href == "string")
  )
    try {
      (new URL(a.href), (a.reloadDocument = !0));
    } catch {}
  const i = new Headers(a.headers);
  a.href && i.get("Location") === null && i.set("Location", a.href);
  const u = new Response(null, { status: a.statusCode, headers: i });
  if (((u.options = a), a.throw)) throw u;
  return u;
}
function _e(a) {
  return a instanceof Response && !!a.options;
}
function iS(a) {
  if (a !== null && typeof a == "object" && a.isSerializedRedirect)
    return qy(a);
}
function uS(a) {
  return {
    input: ({ url: i }) => {
      for (const u of a) i = dc(u, i);
      return i;
    },
    output: ({ url: i }) => {
      for (let u = a.length - 1; u >= 0; u--) i = Yy(a[u], i);
      return i;
    },
  };
}
function sS(a) {
  const i = jy(a.basepath),
    u = `/${i}`,
    r = `${u}/`,
    c = a.caseSensitive ? u : u.toLowerCase(),
    f = a.caseSensitive ? r : r.toLowerCase();
  return {
    input: ({ url: h }) => {
      const g = a.caseSensitive ? h.pathname : h.pathname.toLowerCase();
      return (
        g === c
          ? (h.pathname = "/")
          : g.startsWith(f) && (h.pathname = h.pathname.slice(u.length)),
        h
      );
    },
    output: ({ url: h }) => ((h.pathname = fs(["/", i, h.pathname])), h),
  };
}
function dc(a, i) {
  const u = a?.input?.({ url: i });
  if (u) {
    if (typeof u == "string") return new URL(u);
    if (u instanceof URL) return u;
  }
  return i;
}
function Yy(a, i) {
  const u = a?.output?.({ url: i });
  if (u) {
    if (typeof u == "string") return new URL(u);
    if (u instanceof URL) return u;
  }
  return i;
}
function rS(a, i) {
  const {
      createMutableStore: u,
      createReadonlyStore: r,
      batch: c,
      init: f,
    } = i,
    h = new Map(),
    g = new Map(),
    y = new Map(),
    m = u(a.status),
    S = u(a.loadedAt),
    p = u(a.isLoading),
    E = u(a.isTransitioning),
    _ = u(a.location),
    z = u(a.resolvedLocation),
    L = u(a.statusCode),
    x = u(a.redirect),
    C = u([]),
    Y = u([]),
    Q = u([]),
    H = r(() => ec(h, C.get())),
    W = r(() => ec(g, Y.get())),
    F = r(() => ec(y, Q.get())),
    G = r(() => C.get()[0]),
    Z = r(() => C.get().some((J) => h.get(J)?.get().status === "pending")),
    P = r(() => ({
      locationHref: _.get().href,
      resolvedLocationHref: z.get()?.href,
      status: m.get(),
    })),
    rt = r(() => ({
      status: m.get(),
      loadedAt: S.get(),
      isLoading: p.get(),
      isTransitioning: E.get(),
      matches: H.get(),
      location: _.get(),
      resolvedLocation: z.get(),
      statusCode: L.get(),
      redirect: x.get(),
    })),
    ut = ji(64);
  function it(J) {
    let lt = ut.get(J);
    return (
      lt ||
        ((lt = r(() => {
          const Et = C.get();
          for (const Tt of Et) {
            const M = h.get(Tt);
            if (M && M.routeId === J) return M.get();
          }
        })),
        ut.set(J, lt)),
      lt
    );
  }
  const $t = {
    status: m,
    loadedAt: S,
    isLoading: p,
    isTransitioning: E,
    location: _,
    resolvedLocation: z,
    statusCode: L,
    redirect: x,
    matchesId: C,
    pendingIds: Y,
    cachedIds: Q,
    matches: H,
    pendingMatches: W,
    cachedMatches: F,
    firstId: G,
    hasPending: Z,
    matchRouteDeps: P,
    matchStores: h,
    pendingMatchStores: g,
    cachedMatchStores: y,
    __store: rt,
    getRouteMatchStore: it,
    setMatches: Bt,
    setPending: Gt,
    setCached: B,
  };
  (Bt(a.matches), f?.($t));
  function Bt(J) {
    nc(J, h, C, u, c);
  }
  function Gt(J) {
    nc(J, g, Y, u, c);
  }
  function B(J) {
    nc(J, y, Q, u, c);
  }
  return $t;
}
function ec(a, i) {
  const u = [];
  for (const r of i) {
    const c = a.get(r);
    c && u.push(c.get());
  }
  return u;
}
function nc(a, i, u, r, c) {
  const f = a.map((g) => g.id),
    h = new Set(f);
  c(() => {
    for (const g of i.keys()) h.has(g) || i.delete(g);
    for (const g of a) {
      const y = i.get(g.id);
      if (!y) {
        const m = r(g);
        ((m.routeId = g.routeId), i.set(g.id, m));
        continue;
      }
      ((y.routeId = g.routeId), y.get() !== g && y.set(g));
    }
    U0(u.get(), f) || u.set(f);
  });
}
const hc = (a) => {
    if (!a.rendered) return ((a.rendered = !0), a.onReady?.());
  },
  oS = (a) =>
    a.stores.matchesId
      .get()
      .some((i) => a.stores.matchStores.get(i)?.get()._forcePending),
  Ts = (a, i) => !!(a.preload && !a.router.stores.matchStores.has(i)),
  La = (a, i, u = !0) => {
    const r = { ...(a.router.options.context ?? {}) },
      c = u ? i : i - 1;
    for (let f = 0; f <= c; f++) {
      const h = a.matches[f];
      if (!h) continue;
      const g = a.router.getMatch(h.id);
      g && Object.assign(r, g.__routeContext, g.__beforeLoadContext);
    }
    return r;
  },
  km = (a, i) => {
    if (!a.matches.length) return;
    const u = i.routeId,
      r = a.matches.findIndex((h) => h.routeId === a.router.routeTree.id),
      c = r >= 0 ? r : 0;
    let f = u
      ? a.matches.findIndex((h) => h.routeId === u)
      : (a.firstBadMatchIndex ?? a.matches.length - 1);
    f < 0 && (f = c);
    for (let h = f; h >= 0; h--) {
      const g = a.matches[h];
      if (a.router.looseRoutesById[g.routeId].options.notFoundComponent)
        return h;
    }
    return u ? f : c;
  },
  aa = (a, i, u) => {
    if (!(!_e(u) && !ne(u)))
      throw (
        (_e(u) && u.redirectHandled && !u.options.reloadDocument) ||
          (i &&
            (i._nonReactive.beforeLoadPromise?.resolve(),
            i._nonReactive.loaderPromise?.resolve(),
            (i._nonReactive.beforeLoadPromise = void 0),
            (i._nonReactive.loaderPromise = void 0),
            (i._nonReactive.error = u),
            a.updateMatch(i.id, (r) => ({
              ...r,
              status: _e(u)
                ? "redirected"
                : ne(u)
                  ? "notFound"
                  : r.status === "pending"
                    ? "success"
                    : r.status,
              context: La(a, i.index),
              isFetching: !1,
              error: u,
            })),
            ne(u) && !u.routeId && (u.routeId = i.routeId),
            i._nonReactive.loadPromise?.resolve()),
          _e(u) &&
            ((a.rendered = !0),
            (u.options._fromLocation = a.location),
            (u.redirectHandled = !0),
            (u = a.router.resolveRedirect(u)))),
        u
      );
  },
  Gy = (a, i) => {
    const u = a.router.getMatch(i);
    return !!(!u || u._nonReactive.dehydrated);
  },
  Fm = (a, i, u) => {
    const r = La(a, u);
    a.updateMatch(i, (c) => ({ ...c, context: r }));
  },
  Ci = (a, i, u, r) => {
    const { id: c, routeId: f } = a.matches[i],
      h = a.router.looseRoutesById[f];
    if (u instanceof Promise) throw u;
    ((u.routerCode = r),
      (a.firstBadMatchIndex ??= i),
      aa(a, a.router.getMatch(c), u));
    try {
      h.options.onError?.(u);
    } catch (g) {
      ((u = g), aa(a, a.router.getMatch(c), u));
    }
    (a.updateMatch(
      c,
      (g) => (
        g._nonReactive.beforeLoadPromise?.resolve(),
        (g._nonReactive.beforeLoadPromise = void 0),
        g._nonReactive.loadPromise?.resolve(),
        {
          ...g,
          error: u,
          status: "error",
          isFetching: !1,
          updatedAt: Date.now(),
          abortController: new AbortController(),
        }
      ),
    ),
      !a.preload && !_e(u) && !ne(u) && (a.serialError ??= u));
  },
  Xy = (a, i, u, r) => {
    if (r._nonReactive.pendingTimeout !== void 0) return;
    const c = u.options.pendingMs ?? a.router.options.defaultPendingMs;
    if (
      a.onReady &&
      !Ts(a, i) &&
      (u.options.loader || u.options.beforeLoad || Qy(u)) &&
      typeof c == "number" &&
      c !== 1 / 0 &&
      (u.options.pendingComponent ?? a.router.options?.defaultPendingComponent)
    ) {
      const f = setTimeout(() => {
        hc(a);
      }, c);
      r._nonReactive.pendingTimeout = f;
    }
  },
  cS = (a, i, u) => {
    const r = a.router.getMatch(i);
    if (!r._nonReactive.beforeLoadPromise && !r._nonReactive.loaderPromise)
      return;
    Xy(a, i, u, r);
    const c = () => {
      const f = a.router.getMatch(i);
      f.preload &&
        (f.status === "redirected" || f.status === "notFound") &&
        aa(a, f, f.error);
    };
    return r._nonReactive.beforeLoadPromise
      ? r._nonReactive.beforeLoadPromise.then(c)
      : c();
  },
  fS = (a, i, u, r) => {
    const c = a.router.getMatch(i);
    let f = c._nonReactive.loadPromise;
    c._nonReactive.loadPromise = Ua(() => {
      (f?.resolve(), (f = void 0));
    });
    const { paramsError: h, searchError: g } = c;
    (h && Ci(a, u, h, "PARSE_PARAMS"),
      g && Ci(a, u, g, "VALIDATE_SEARCH"),
      Xy(a, i, r, c));
    const y = new AbortController();
    let m = !1;
    const S = () => {
        m ||
          ((m = !0),
          a.updateMatch(i, (H) => ({
            ...H,
            isFetching: "beforeLoad",
            fetchCount: H.fetchCount + 1,
            abortController: y,
          })));
      },
      p = () => {
        (c._nonReactive.beforeLoadPromise?.resolve(),
          (c._nonReactive.beforeLoadPromise = void 0),
          a.updateMatch(i, (H) => ({ ...H, isFetching: !1 })));
      };
    if (!r.options.beforeLoad) {
      a.router.batch(() => {
        (S(), p());
      });
      return;
    }
    c._nonReactive.beforeLoadPromise = Ua();
    const E = { ...La(a, u, !1), ...c.__routeContext },
      { search: _, params: z, cause: L } = c,
      x = Ts(a, i),
      C = {
        search: _,
        abortController: y,
        params: z,
        preload: x,
        context: E,
        location: a.location,
        navigate: (H) => a.router.navigate({ ...H, _fromLocation: a.location }),
        buildLocation: a.router.buildLocation,
        cause: x ? "preload" : L,
        matches: a.matches,
        routeId: r.id,
        ...a.router.options.additionalContext,
      },
      Y = (H) => {
        if (H === void 0) {
          a.router.batch(() => {
            (S(), p());
          });
          return;
        }
        ((_e(H) || ne(H)) && (S(), Ci(a, u, H, "BEFORE_LOAD")),
          a.router.batch(() => {
            (S(),
              a.updateMatch(i, (W) => ({ ...W, __beforeLoadContext: H })),
              p());
          }));
      };
    let Q;
    try {
      if (((Q = r.options.beforeLoad(C)), Bi(Q)))
        return (
          S(),
          Q.catch((H) => {
            Ci(a, u, H, "BEFORE_LOAD");
          }).then(Y)
        );
    } catch (H) {
      (S(), Ci(a, u, H, "BEFORE_LOAD"));
    }
    Y(Q);
  },
  dS = (a, i) => {
    const { id: u, routeId: r } = a.matches[i],
      c = a.router.looseRoutesById[r],
      f = () => g(),
      h = () => fS(a, u, i, c),
      g = () => {
        if (Gy(a, u)) return;
        const y = cS(a, u, c);
        return Bi(y) ? y.then(h) : h();
      };
    return f();
  },
  hS = (a, i, u) => {
    const r = a.router.getMatch(i);
    if (!r || (!u.options.head && !u.options.scripts && !u.options.headers))
      return;
    const c = {
      ssr: a.router.options.ssr,
      matches: a.matches,
      match: r,
      params: r.params,
      loaderData: r.loaderData,
    };
    return Promise.all([
      u.options.head?.(c),
      u.options.scripts?.(c),
      u.options.headers?.(c),
    ]).then(([f, h, g]) => ({
      meta: f?.meta,
      links: f?.links,
      headScripts: f?.scripts,
      headers: g,
      scripts: h,
      styles: f?.styles,
    }));
  },
  Vy = (a, i, u, r, c) => {
    const f = i[r - 1],
      {
        params: h,
        loaderDeps: g,
        abortController: y,
        cause: m,
      } = a.router.getMatch(u),
      S = La(a, r),
      p = Ts(a, u);
    return {
      params: h,
      deps: g,
      preload: !!p,
      parentMatchPromise: f,
      abortController: y,
      context: S,
      location: a.location,
      navigate: (E) => a.router.navigate({ ...E, _fromLocation: a.location }),
      cause: p ? "preload" : m,
      route: c,
      ...a.router.options.additionalContext,
    };
  },
  Pm = async (a, i, u, r, c) => {
    try {
      const f = a.router.getMatch(u);
      try {
        (!(Oy ?? a.router.isServer) || f.ssr === !0) && Hi(c);
        const h = c.options.loader,
          g = typeof h == "function" ? h : h?.handler,
          y = g?.(Vy(a, i, u, r, c)),
          m = !!g && Bi(y);
        if (
          ((m ||
            c._lazyPromise ||
            c._componentsPromise ||
            c.options.head ||
            c.options.scripts ||
            c.options.headers ||
            f._nonReactive.minPendingPromise) &&
            a.updateMatch(u, (p) => ({ ...p, isFetching: "loader" })),
          g)
        ) {
          const p = m ? await y : y;
          (aa(a, a.router.getMatch(u), p),
            p !== void 0 && a.updateMatch(u, (E) => ({ ...E, loaderData: p })));
        }
        c._lazyPromise && (await c._lazyPromise);
        const S = f._nonReactive.minPendingPromise;
        (S && (await S),
          c._componentsPromise && (await c._componentsPromise),
          a.updateMatch(u, (p) => ({
            ...p,
            error: void 0,
            context: La(a, r),
            status: "success",
            isFetching: !1,
            updatedAt: Date.now(),
          })));
      } catch (h) {
        let g = h;
        if (g?.name === "AbortError") {
          if (f.abortController.signal.aborted) {
            (f._nonReactive.loaderPromise?.resolve(),
              (f._nonReactive.loaderPromise = void 0));
            return;
          }
          a.updateMatch(u, (m) => ({
            ...m,
            status: m.status === "pending" ? "success" : m.status,
            isFetching: !1,
            context: La(a, r),
          }));
          return;
        }
        const y = f._nonReactive.minPendingPromise;
        (y && (await y),
          ne(h) && (await c.options.notFoundComponent?.preload?.()),
          aa(a, a.router.getMatch(u), h));
        try {
          c.options.onError?.(h);
        } catch (m) {
          ((g = m), aa(a, a.router.getMatch(u), m));
        }
        (!_e(g) && !ne(g) && (await Hi(c, ["errorComponent"])),
          a.updateMatch(u, (m) => ({
            ...m,
            error: g,
            context: La(a, r),
            status: "error",
            isFetching: !1,
          })));
      }
    } catch (f) {
      const h = a.router.getMatch(u);
      (h && (h._nonReactive.loaderPromise = void 0), aa(a, h, f));
    }
  },
  mS = async (a, i, u) => {
    async function r(_, z, L, x, C) {
      const Y = Date.now() - z.updatedAt,
        Q = _
          ? (C.options.preloadStaleTime ??
            a.router.options.defaultPreloadStaleTime ??
            3e4)
          : (C.options.staleTime ?? a.router.options.defaultStaleTime ?? 0),
        H = C.options.shouldReload,
        W = typeof H == "function" ? H(Vy(a, i, c, u, C)) : H,
        { status: F, invalid: G } = x,
        Z =
          Y >= Q &&
          (!!a.forceStaleReload ||
            x.cause === "enter" ||
            (L !== void 0 && L !== x.id));
      ((h = F === "success" && (G || (W ?? Z))),
        (_ && C.options.preload === !1) ||
          (h && !a.sync && S
            ? ((g = !0),
              (async () => {
                try {
                  await Pm(a, i, c, u, C);
                  const P = a.router.getMatch(c);
                  (P._nonReactive.loaderPromise?.resolve(),
                    P._nonReactive.loadPromise?.resolve(),
                    (P._nonReactive.loaderPromise = void 0),
                    (P._nonReactive.loadPromise = void 0));
                } catch (P) {
                  _e(P) && (await a.router.navigate(P.options));
                }
              })())
            : F !== "success" || h
              ? await Pm(a, i, c, u, C)
              : Fm(a, c, u)));
    }
    const { id: c, routeId: f } = a.matches[u];
    let h = !1,
      g = !1;
    const y = a.router.looseRoutesById[f],
      m = y.options.loader,
      S =
        ((typeof m == "function" ? void 0 : m?.staleReloadMode) ??
          a.router.options.defaultStaleReloadMode) !== "blocking";
    if (Gy(a, c)) {
      if (!a.router.getMatch(c)) return a.matches[u];
      Fm(a, c, u);
    } else {
      const _ = a.router.getMatch(c),
        z = a.router.stores.matchesId.get()[u],
        L =
          ((z && a.router.stores.matchStores.get(z)) || null)?.routeId === f
            ? z
            : a.router.stores.matches.get().find((C) => C.routeId === f)?.id,
        x = Ts(a, c);
      if (_._nonReactive.loaderPromise) {
        if (_.status === "success" && !a.sync && !_.preload && S) return _;
        await _._nonReactive.loaderPromise;
        const C = a.router.getMatch(c),
          Y = C._nonReactive.error || C.error;
        (Y && aa(a, C, Y), C.status === "pending" && (await r(x, _, L, C, y)));
      } else {
        const C = x && !a.router.stores.matchStores.has(c),
          Y = a.router.getMatch(c);
        ((Y._nonReactive.loaderPromise = Ua()),
          C !== Y.preload && a.updateMatch(c, (Q) => ({ ...Q, preload: C })),
          await r(x, _, L, Y, y));
      }
    }
    const p = a.router.getMatch(c);
    (g ||
      (p._nonReactive.loaderPromise?.resolve(),
      p._nonReactive.loadPromise?.resolve(),
      (p._nonReactive.loadPromise = void 0)),
      clearTimeout(p._nonReactive.pendingTimeout),
      (p._nonReactive.pendingTimeout = void 0),
      g || (p._nonReactive.loaderPromise = void 0),
      (p._nonReactive.dehydrated = void 0));
    const E = g ? p.isFetching : !1;
    return E !== p.isFetching || p.invalid !== !1
      ? (a.updateMatch(c, (_) => ({ ..._, isFetching: E, invalid: !1 })),
        a.router.getMatch(c))
      : p;
  };
async function Im(a) {
  const i = a,
    u = [];
  oS(i.router) && hc(i);
  let r;
  for (let E = 0; E < i.matches.length; E++) {
    try {
      const _ = dS(i, E);
      Bi(_) && (await _);
    } catch (_) {
      if (_e(_)) throw _;
      if (ne(_)) r = _;
      else if (!i.preload) throw _;
      break;
    }
    if (i.serialError || i.firstBadMatchIndex != null) break;
  }
  const c = i.firstBadMatchIndex ?? i.matches.length,
    f = r && !i.preload ? km(i, r) : void 0,
    h = r && i.preload ? 0 : f !== void 0 ? Math.min(f + 1, c) : c;
  let g, y;
  for (let E = 0; E < h; E++) u.push(mS(i, u, E));
  try {
    await Promise.all(u);
  } catch {
    const E = await Promise.allSettled(u);
    for (const _ of E) {
      if (_.status !== "rejected") continue;
      const z = _.reason;
      if (_e(z)) throw z;
      ne(z) ? (g ??= z) : (y ??= z);
    }
    if (y !== void 0) throw y;
  }
  const m = g ?? (r && !i.preload ? r : void 0);
  let S =
    i.firstBadMatchIndex !== void 0
      ? i.firstBadMatchIndex
      : i.matches.length - 1;
  if (!m && r && i.preload) return i.matches;
  if (m) {
    const E = km(i, m);
    E === void 0 && Ee();
    const _ = i.matches[E],
      z = i.router.looseRoutesById[_.routeId],
      L = i.router.options?.defaultNotFoundComponent;
    (!z.options.notFoundComponent && L && (z.options.notFoundComponent = L),
      (m.routeId = _.routeId));
    const x = _.routeId === i.router.routeTree.id;
    (i.updateMatch(_.id, (C) => ({
      ...C,
      ...(x
        ? { status: "success", globalNotFound: !0, error: void 0 }
        : { status: "notFound", error: m }),
      isFetching: !1,
    })),
      (S = E),
      await Hi(z, ["notFoundComponent"]));
  } else if (!i.preload) {
    const E = i.matches[0];
    E.globalNotFound ||
      (i.router.getMatch(E.id)?.globalNotFound &&
        i.updateMatch(E.id, (_) => ({
          ..._,
          globalNotFound: !1,
          error: void 0,
        })));
  }
  if (i.serialError && i.firstBadMatchIndex !== void 0) {
    const E = i.router.looseRoutesById[i.matches[i.firstBadMatchIndex].routeId];
    await Hi(E, ["errorComponent"]);
  }
  for (let E = 0; E <= S; E++) {
    const { id: _, routeId: z } = i.matches[E],
      L = i.router.looseRoutesById[z];
    try {
      const x = hS(i, _, L);
      if (x) {
        const C = await x;
        i.updateMatch(_, (Y) => ({ ...Y, ...C }));
      }
    } catch (x) {
      console.error(`Error executing head for route ${z}:`, x);
    }
  }
  const p = hc(i);
  if ((Bi(p) && (await p), m)) throw m;
  if (i.serialError && !i.preload && !i.onReady) throw i.serialError;
  return i.matches;
}
function Wm(a, i) {
  const u = i.map((r) => a.options[r]?.preload?.()).filter(Boolean);
  if (u.length !== 0) return Promise.all(u);
}
function Hi(a, i = ds) {
  !a._lazyLoaded &&
    a._lazyPromise === void 0 &&
    (a.lazyFn
      ? (a._lazyPromise = a.lazyFn().then((r) => {
          const { id: c, ...f } = r.options;
          (Object.assign(a.options, f),
            (a._lazyLoaded = !0),
            (a._lazyPromise = void 0));
        }))
      : (a._lazyLoaded = !0));
  const u = () =>
    a._componentsLoaded
      ? void 0
      : i === ds
        ? (() => {
            if (a._componentsPromise === void 0) {
              const r = Wm(a, ds);
              r
                ? (a._componentsPromise = r.then(() => {
                    ((a._componentsLoaded = !0),
                      (a._componentsPromise = void 0));
                  }))
                : (a._componentsLoaded = !0);
            }
            return a._componentsPromise;
          })()
        : Wm(a, i);
  return a._lazyPromise ? a._lazyPromise.then(u) : u();
}
function Qy(a) {
  for (const i of ds) if (a.options[i]?.preload) return !0;
  return !1;
}
const ds = [
  "component",
  "errorComponent",
  "pendingComponent",
  "notFoundComponent",
];
var la = "__TSR_index",
  $m = "popstate",
  ty = "beforeunload";
function yS(a) {
  let i = a.getLocation();
  const u = new Set(),
    r = (h) => {
      ((i = a.getLocation()), u.forEach((g) => g({ location: i, action: h })));
    },
    c = (h) => {
      (a.notifyOnIndexChange ?? !0) ? r(h) : (i = a.getLocation());
    },
    f = async ({ task: h, navigateOpts: g, ...y }) => {
      if (g?.ignoreBlocker ?? !1) {
        h();
        return;
      }
      const m = a.getBlockers?.() ?? [],
        S = y.type === "PUSH" || y.type === "REPLACE";
      if (typeof document < "u" && m.length && S)
        for (const p of m) {
          const E = vs(y.path, y.state);
          if (
            await p.blockerFn({
              currentLocation: i,
              nextLocation: E,
              action: y.type,
            })
          ) {
            a.onBlocked?.();
            return;
          }
        }
      h();
    };
  return {
    get location() {
      return i;
    },
    get length() {
      return a.getLength();
    },
    subscribers: u,
    subscribe: (h) => (
      u.add(h),
      () => {
        u.delete(h);
      }
    ),
    push: (h, g, y) => {
      const m = i.state[la];
      ((g = ey(m + 1, g)),
        f({
          task: () => {
            (a.pushState(h, g), r({ type: "PUSH" }));
          },
          navigateOpts: y,
          type: "PUSH",
          path: h,
          state: g,
        }));
    },
    replace: (h, g, y) => {
      const m = i.state[la];
      ((g = ey(m, g)),
        f({
          task: () => {
            (a.replaceState(h, g), r({ type: "REPLACE" }));
          },
          navigateOpts: y,
          type: "REPLACE",
          path: h,
          state: g,
        }));
    },
    go: (h, g) => {
      f({
        task: () => {
          (a.go(h), c({ type: "GO", index: h }));
        },
        navigateOpts: g,
        type: "GO",
      });
    },
    back: (h) => {
      f({
        task: () => {
          (a.back(h?.ignoreBlocker ?? !1), c({ type: "BACK" }));
        },
        navigateOpts: h,
        type: "BACK",
      });
    },
    forward: (h) => {
      f({
        task: () => {
          (a.forward(h?.ignoreBlocker ?? !1), c({ type: "FORWARD" }));
        },
        navigateOpts: h,
        type: "FORWARD",
      });
    },
    canGoBack: () => i.state[la] !== 0,
    createHref: (h) => a.createHref(h),
    block: (h) => {
      if (!a.setBlockers) return () => {};
      const g = a.getBlockers?.() ?? [];
      return (
        a.setBlockers([...g, h]),
        () => {
          const y = a.getBlockers?.() ?? [];
          a.setBlockers?.(y.filter((m) => m !== h));
        }
      );
    },
    flush: () => a.flush?.(),
    destroy: () => a.destroy?.(),
    notify: r,
  };
}
function ey(a, i) {
  i || (i = {});
  const u = _c();
  return { ...i, key: u, __TSR_key: u, [la]: a };
}
function pS(a) {
  const i = typeof document < "u" ? window : void 0,
    u = i.history.pushState,
    r = i.history.replaceState;
  let c = [];
  const f = () => c,
    h = (Z) => (c = Z),
    g = (Z) => Z,
    y = () =>
      vs(
        `${i.location.pathname}${i.location.search}${i.location.hash}`,
        i.history.state,
      );
  if (!i.history.state?.__TSR_key && !i.history.state?.key) {
    const Z = _c();
    i.history.replaceState({ [la]: 0, key: Z, __TSR_key: Z }, "");
  }
  let m = y(),
    S,
    p = !1,
    E = !1,
    _ = !1,
    z = !1;
  const L = () => m;
  let x, C;
  const Y = () => {
      x &&
        ((G._ignoreSubscribers = !0),
        (x.isPush ? i.history.pushState : i.history.replaceState)(
          x.state,
          "",
          x.href,
        ),
        (G._ignoreSubscribers = !1),
        (x = void 0),
        (C = void 0),
        (S = void 0));
    },
    Q = (Z, P, rt) => {
      const ut = g(P);
      (C || (S = m),
        (m = vs(P, rt)),
        (x = { href: ut, state: rt, isPush: x?.isPush || Z === "push" }),
        C || (C = Promise.resolve().then(() => Y())));
    },
    H = (Z) => {
      ((m = y()), G.notify({ type: Z }));
    },
    W = async () => {
      if (E) {
        E = !1;
        return;
      }
      const Z = y(),
        P = Z.state[la] - m.state[la],
        rt = P === 1,
        ut = P === -1,
        it = (!rt && !ut) || p;
      p = !1;
      const $t = it ? "GO" : ut ? "BACK" : "FORWARD",
        Bt = it ? { type: "GO", index: P } : { type: ut ? "BACK" : "FORWARD" };
      if (_) _ = !1;
      else {
        const Gt = f();
        if (typeof document < "u" && Gt.length) {
          for (const B of Gt)
            if (
              await B.blockerFn({
                currentLocation: m,
                nextLocation: Z,
                action: $t,
              })
            ) {
              ((E = !0), i.history.go(1), G.notify(Bt));
              return;
            }
        }
      }
      ((m = y()), G.notify(Bt));
    },
    F = (Z) => {
      if (z) {
        z = !1;
        return;
      }
      let P = !1;
      const rt = f();
      if (typeof document < "u" && rt.length)
        for (const ut of rt) {
          const it = ut.enableBeforeUnload ?? !0;
          if (it === !0) {
            P = !0;
            break;
          }
          if (typeof it == "function" && it() === !0) {
            P = !0;
            break;
          }
        }
      if (P) return (Z.preventDefault(), (Z.returnValue = ""));
    },
    G = yS({
      getLocation: L,
      getLength: () => i.history.length,
      pushState: (Z, P) => Q("push", Z, P),
      replaceState: (Z, P) => Q("replace", Z, P),
      back: (Z) => (Z && (_ = !0), (z = !0), i.history.back()),
      forward: (Z) => {
        (Z && (_ = !0), (z = !0), i.history.forward());
      },
      go: (Z) => {
        ((p = !0), i.history.go(Z));
      },
      createHref: (Z) => g(Z),
      flush: Y,
      destroy: () => {
        ((i.history.pushState = u),
          (i.history.replaceState = r),
          i.removeEventListener(ty, F, { capture: !0 }),
          i.removeEventListener($m, W));
      },
      onBlocked: () => {
        S && m !== S && (m = S);
      },
      getBlockers: f,
      setBlockers: h,
      notifyOnIndexChange: !1,
    });
  return (
    i.addEventListener(ty, F, { capture: !0 }),
    i.addEventListener($m, W),
    (i.history.pushState = function (...Z) {
      const P = u.apply(i.history, Z);
      return (G._ignoreSubscribers || H("PUSH"), P);
    }),
    (i.history.replaceState = function (...Z) {
      const P = r.apply(i.history, Z);
      return (G._ignoreSubscribers || H("REPLACE"), P);
    }),
    G
  );
}
function gS(a) {
  let i = a.replace(/[\x00-\x1f\x7f]/g, "");
  return (i.startsWith("//") && (i = "/" + i.replace(/^\/+/, "")), i);
}
function vs(a, i) {
  const u = gS(a),
    r = u.indexOf("#"),
    c = u.indexOf("?"),
    f = _c();
  return {
    href: u,
    pathname: u.substring(
      0,
      r > 0 ? (c > 0 ? Math.min(r, c) : r) : c > 0 ? c : u.length,
    ),
    hash: r > -1 ? u.substring(r) : "",
    search: c > -1 ? u.slice(c, r === -1 ? void 0 : r) : "",
    state: i || { [la]: 0, key: f, __TSR_key: f },
  };
}
function _c() {
  return (Math.random() + 1).toString(36).substring(7);
}
function vS(a) {
  return a instanceof Error
    ? { name: a.name, message: a.message }
    : { data: a };
}
function Al(a, i) {
  const u = i,
    r = a;
  return {
    fromLocation: u,
    toLocation: r,
    pathChanged: u?.pathname !== r.pathname,
    hrefChanged: u?.href !== r.href,
    hashChanged: u?.hash !== r.hash,
  };
}
var SS = class {
    constructor(a, i) {
      ((this.tempLocationKey = `${Math.round(Math.random() * 1e7)}`),
        (this.resetNextScroll = !0),
        (this.shouldViewTransition = void 0),
        (this.isViewTransitionTypesSupported = void 0),
        (this.subscribers = new Set()),
        (this.isScrollRestoring = !1),
        (this.isScrollRestorationSetup = !1),
        (this.routeBranchCache = new WeakMap()),
        (this.startTransition = (u) => u()),
        (this.update = (u) => {
          const r = this.options,
            c = this.basepath ?? r?.basepath ?? "/",
            f = this.basepath === void 0,
            h = r?.rewrite;
          if (
            ((this.options = { ...r, ...u }),
            (this.isServer = this.options.isServer ?? typeof document > "u"),
            (this.protocolAllowlist = new Set(this.options.protocolAllowlist)),
            this.options.pathParamsAllowedCharacters &&
              (this.pathParamsDecoder = J0(
                this.options.pathParamsAllowedCharacters,
              )),
            (!this.history ||
              (this.options.history &&
                this.options.history !== this.history)) &&
              (this.options.history
                ? (this.history = this.options.history)
                : (this.history = pS())),
            (this.origin = this.options.origin),
            this.origin ||
              (window?.origin && window.origin !== "null"
                ? (this.origin = window.origin)
                : (this.origin = "http://localhost")),
            this.history && this.updateLatestLocation(),
            this.options.routeTree !== this.routeTree)
          ) {
            this.routeTree = this.options.routeTree;
            let S;
            ((this.resolvePathCache = ji(1e3)),
              (S = this.buildRouteTree()),
              this.setRoutes(S));
          }
          if (!this.stores && this.latestLocation) {
            const S = this.getStoreConfig(this);
            ((this.batch = S.batch),
              (this.stores = rS(_S(this.latestLocation), S)),
              $0(this));
          }
          let g = !1;
          const y = this.options.basepath ?? "/",
            m = this.options.rewrite;
          if (f || c !== y || h !== m) {
            this.basepath = y;
            const S = [],
              p = jy(y);
            (p && p !== "/" && S.push(sS({ basepath: y })),
              m && S.push(m),
              (this.rewrite =
                S.length === 0 ? void 0 : S.length === 1 ? S[0] : uS(S)),
              this.history && this.updateLatestLocation(),
              (g = !0));
          }
          (g && this.stores && this.stores.location.set(this.latestLocation),
            typeof window < "u" &&
              "CSS" in window &&
              typeof window.CSS?.supports == "function" &&
              (this.isViewTransitionTypesSupported = window.CSS.supports(
                "selector(:active-view-transition-type(a)",
              )));
        }),
        (this.updateLatestLocation = () => {
          this.latestLocation = this.parseLocation(
            this.history.location,
            this.latestLocation,
          );
        }),
        (this.buildRouteTree = () => {
          const u = G0(this.routeTree, this.options.caseSensitive, (r, c) => {
            r.init({ originalIndex: c });
          });
          return (
            this.options.routeMasks &&
              B0(this.options.routeMasks, u.processedTree),
            u
          );
        }),
        (this.subscribe = (u, r) => {
          const c = { eventType: u, fn: r };
          return (
            this.subscribers.add(c),
            () => {
              this.subscribers.delete(c);
            }
          );
        }),
        (this.emit = (u) => {
          this.subscribers.forEach((r) => {
            r.eventType === u.type && r.fn(u);
          });
        }),
        (this.parseLocation = (u, r) => {
          const c = ({
              pathname: y,
              search: m,
              hash: S,
              href: p,
              state: E,
            }) => {
              if (!this.rewrite && !/[ \x00-\x1f\x7f\u0080-\uffff]/.test(y)) {
                const C = this.options.parseSearch(m),
                  Y = this.options.stringifySearch(C);
                return {
                  href: y + Y + S,
                  publicHref: y + Y + S,
                  pathname: zi(y).path,
                  external: !1,
                  searchStr: Y,
                  search: za(r?.search, C),
                  hash: zi(S.slice(1)).path,
                  state: Oa(r?.state, E),
                };
              }
              const _ = new URL(p, this.origin),
                z = dc(this.rewrite, _),
                L = this.options.parseSearch(z.search),
                x = this.options.stringifySearch(L);
              return (
                (z.search = x),
                {
                  href: z.href.replace(z.origin, ""),
                  publicHref: p,
                  pathname: zi(z.pathname).path,
                  external: !!this.rewrite && z.origin !== this.origin,
                  searchStr: x,
                  search: za(r?.search, L),
                  hash: zi(z.hash.slice(1)).path,
                  state: Oa(r?.state, E),
                }
              );
            },
            f = c(u),
            { __tempLocation: h, __tempKey: g } = f.state;
          if (h && (!g || g === this.tempLocationKey)) {
            const y = c(h);
            return (
              (y.state.key = f.state.key),
              (y.state.__TSR_key = f.state.__TSR_key),
              delete y.state.__tempLocation,
              { ...y, maskedLocation: f }
            );
          }
          return f;
        }),
        (this.resolvePathWithBase = (u, r) =>
          K0({
            base: u,
            to: r.includes("//") ? bc(r) : r,
            trailingSlash: this.options.trailingSlash,
            cache: this.resolvePathCache,
          })),
        (this.matchRoutes = (u, r, c) =>
          typeof u == "string"
            ? this.matchRoutesInternal({ pathname: u, search: r }, c)
            : this.matchRoutesInternal(u, r)),
        (this.getMatchedRoutes = (u) =>
          ES({
            pathname: u,
            routesById: this.routesById,
            processedTree: this.processedTree,
          })),
        (this.cancelMatch = (u) => {
          const r = this.getMatch(u);
          r &&
            (r.abortController.abort(),
            clearTimeout(r._nonReactive.pendingTimeout),
            (r._nonReactive.pendingTimeout = void 0));
        }),
        (this.cancelMatches = () => {
          (this.stores.pendingIds.get().forEach((u) => {
            this.cancelMatch(u);
          }),
            this.stores.matchesId.get().forEach((u) => {
              if (this.stores.pendingMatchStores.has(u)) return;
              const r = this.stores.matchStores.get(u)?.get();
              r &&
                (r.status === "pending" || r.isFetching === "loader") &&
                this.cancelMatch(u);
            }));
        }),
        (this.buildLocation = (u) => {
          const r = (f = {}) => {
              const h =
                  f._fromLocation ||
                  this.pendingBuiltLocation ||
                  this.latestLocation,
                g = this.matchRoutesLightweight(h);
              f.from;
              const y =
                  f.unsafeRelative === "path"
                    ? h.pathname
                    : (f.from ?? g.fullPath),
                m = f.to ? `${f.to}` : void 0,
                S = g.search,
                p = Object.assign(Object.create(null), g.params),
                E =
                  m?.charCodeAt(0) === 47
                    ? "/"
                    : this.resolvePathWithBase(y, "."),
                _ = m ? this.resolvePathWithBase(E, m) : E,
                z =
                  f.params === !1 || f.params === null
                    ? Object.create(null)
                    : (f.params ?? !0) === !0
                      ? p
                      : Object.assign(p, ea(f.params, p)),
                L = this.routesByPath[Rn(_)];
              let x;
              if (L) x = this.getRouteBranch(L);
              else if (_.includes("$")) x = [];
              else {
                const ut = this.getMatchedRoutes(_);
                ((x = ut.matchedRoutes),
                  this.options.notFoundRoute &&
                    (!ut.foundRoute ||
                      (ut.foundRoute.path !== "/" && ut.routeParams["**"])) &&
                    (x = [...x, this.options.notFoundRoute]));
              }
              if (x.length && wy(z))
                for (const ut of x) {
                  const it =
                    ut.options.params?.stringify ?? ut.options.stringifyParams;
                  if (it)
                    try {
                      Object.assign(z, it(z));
                    } catch {}
                }
              const C = u.leaveParams
                ? _
                : zi(
                    Qm({
                      path: _,
                      params: z,
                      decoder: this.pathParamsDecoder,
                      server: this.isServer,
                    }).interpolatedPath,
                  ).path;
              let Y = S;
              if (u._includeValidateSearch && this.options.search?.strict) {
                const ut = {};
                (x.forEach((it) => {
                  if (it.options.validateSearch)
                    try {
                      Object.assign(
                        ut,
                        hs(it.options.validateSearch, { ...ut, ...Y }),
                      );
                    } catch {}
                }),
                  (Y = ut));
              }
              ((Y = RS({
                search: Y,
                dest: f,
                destRoutes: x,
                _includeValidateSearch: u._includeValidateSearch,
              })),
                (Y = za(S, Y)));
              const Q = this.options.stringifySearch(Y),
                H =
                  f.hash === !0 ? h.hash : f.hash ? ea(f.hash, h.hash) : void 0,
                W = H ? `#${H}` : "";
              let F =
                f.state === !0 ? h.state : f.state ? ea(f.state, h.state) : {};
              F = Oa(h.state, F);
              const G = `${C}${Q}${W}`;
              let Z,
                P,
                rt = !1;
              if (this.rewrite) {
                const ut = new URL(G, this.origin),
                  it = Yy(this.rewrite, ut);
                ((Z = ut.href.replace(ut.origin, "")),
                  it.origin !== this.origin
                    ? ((P = it.href), (rt = !0))
                    : (P = it.pathname + it.search + it.hash));
              } else ((Z = L0(G)), (P = Z));
              return {
                publicHref: P,
                href: Z,
                pathname: C,
                search: Y,
                searchStr: Q,
                state: F,
                hash: H ?? "",
                external: rt,
                unmaskOnReload: f.unmaskOnReload,
              };
            },
            c = (f = {}, h) => {
              const g = r(f);
              let y = h ? r(h) : void 0;
              if (!y) {
                const m = Object.create(null);
                if (this.options.routeMasks) {
                  const S = j0(g.pathname, this.processedTree);
                  if (S) {
                    Object.assign(m, S.rawParams);
                    const { from: p, params: E, ..._ } = S.route,
                      z =
                        E === !1 || E === null
                          ? Object.create(null)
                          : (E ?? !0) === !0
                            ? m
                            : Object.assign(m, ea(E, m));
                    ((h = { from: u.from, ..._, params: z }), (y = r(h)));
                  }
                }
              }
              return (y && (g.maskedLocation = y), g);
            };
          return u.mask ? c(u, { from: u.from, ...u.mask }) : c(u);
        }),
        (this.commitLocation = async ({
          viewTransition: u,
          ignoreBlocker: r,
          ...c
        }) => {
          const f = () => {
              const y = [
                "key",
                "__TSR_key",
                "__TSR_index",
                "__hashScrollIntoViewOptions",
              ];
              y.forEach((S) => {
                c.state[S] = this.latestLocation.state[S];
              });
              const m = be(c.state, this.latestLocation.state);
              return (
                y.forEach((S) => {
                  delete c.state[S];
                }),
                m
              );
            },
            h = Rn(this.latestLocation.href) === Rn(c.href);
          let g = this.commitLocationPromise;
          if (
            ((this.commitLocationPromise = Ua(() => {
              (g?.resolve(), (g = void 0));
            })),
            h && f())
          )
            this.load();
          else {
            let { maskedLocation: y, hashScrollIntoView: m, ...S } = c;
            (y &&
              ((S = {
                ...y,
                state: {
                  ...y.state,
                  __tempKey: void 0,
                  __tempLocation: {
                    ...S,
                    search: S.searchStr,
                    state: {
                      ...S.state,
                      __tempKey: void 0,
                      __tempLocation: void 0,
                      __TSR_key: void 0,
                      key: void 0,
                    },
                  },
                },
              }),
              (S.unmaskOnReload ?? this.options.unmaskOnReload ?? !1) &&
                (S.state.__tempKey = this.tempLocationKey)),
              (S.state.__hashScrollIntoViewOptions =
                m ?? this.options.defaultHashScrollIntoView ?? !0),
              (this.shouldViewTransition = u),
              this.history[c.replace ? "replace" : "push"](
                S.publicHref,
                S.state,
                { ignoreBlocker: r },
              ));
          }
          return (
            (this.resetNextScroll = c.resetScroll ?? !0),
            this.history.subscribers.size || this.load(),
            this.commitLocationPromise
          );
        }),
        (this.buildAndCommitLocation = ({
          replace: u,
          resetScroll: r,
          hashScrollIntoView: c,
          viewTransition: f,
          ignoreBlocker: h,
          href: g,
          ...y
        } = {}) => {
          if (g) {
            const p = this.history.location.state.__TSR_index,
              E = vs(g, { __TSR_index: u ? p : p + 1 }),
              _ = new URL(E.pathname, this.origin);
            ((y.to = dc(this.rewrite, _).pathname),
              (y.search = this.options.parseSearch(E.search)),
              (y.hash = E.hash.slice(1)));
          }
          const m = this.buildLocation({ ...y, _includeValidateSearch: !0 });
          this.pendingBuiltLocation = m;
          const S = this.commitLocation({
            ...m,
            viewTransition: f,
            replace: u,
            resetScroll: r,
            hashScrollIntoView: c,
            ignoreBlocker: h,
          });
          return (
            Promise.resolve().then(() => {
              this.pendingBuiltLocation === m &&
                (this.pendingBuiltLocation = void 0);
            }),
            S
          );
        }),
        (this.navigate = async ({
          to: u,
          reloadDocument: r,
          href: c,
          publicHref: f,
          ...h
        }) => {
          let g = !1;
          if (c)
            try {
              (new URL(`${c}`), (g = !0));
            } catch {}
          if ((g && !r && (r = !0), r)) {
            if (u !== void 0 || !c) {
              const m = this.buildLocation({ to: u, ...h });
              ((c = c ?? m.publicHref), (f = f ?? m.publicHref));
            }
            const y = !g && f ? f : c;
            if (ps(y, this.protocolAllowlist)) return Promise.resolve();
            if (!h.ignoreBlocker) {
              const m = this.history.getBlockers?.() ?? [];
              for (const S of m)
                if (
                  S?.blockerFn &&
                  (await S.blockerFn({
                    currentLocation: this.latestLocation,
                    nextLocation: this.latestLocation,
                    action: "PUSH",
                  }))
                )
                  return Promise.resolve();
            }
            return (
              h.replace
                ? window.location.replace(y)
                : (window.location.href = y),
              Promise.resolve()
            );
          }
          return this.buildAndCommitLocation({
            ...h,
            href: c,
            to: u,
            _isNavigate: !0,
          });
        }),
        (this.beforeLoad = () => {
          (this.cancelMatches(), this.updateLatestLocation());
          const u = this.matchRoutes(this.latestLocation),
            r = this.stores.cachedMatches
              .get()
              .filter((c) => !u.some((f) => f.id === c.id));
          this.batch(() => {
            (this.stores.status.set("pending"),
              this.stores.statusCode.set(200),
              this.stores.isLoading.set(!0),
              this.stores.location.set(this.latestLocation),
              this.stores.setPending(u),
              this.stores.setCached(r));
          });
        }),
        (this.load = async (u) => {
          let r, c, f;
          const h =
            this.stores.resolvedLocation.get() ?? this.stores.location.get();
          for (
            f = new Promise((y) => {
              this.startTransition(async () => {
                try {
                  this.beforeLoad();
                  const m = this.latestLocation,
                    S = Al(m, this.stores.resolvedLocation.get());
                  (this.stores.redirect.get() ||
                    this.emit({ type: "onBeforeNavigate", ...S }),
                    this.emit({ type: "onBeforeLoad", ...S }),
                    await Im({
                      router: this,
                      sync: u?.sync,
                      forceStaleReload: h.href === m.href,
                      matches: this.stores.pendingMatches.get(),
                      location: m,
                      updateMatch: this.updateMatch,
                      onReady: async () => {
                        this.startTransition(() => {
                          this.startViewTransition(async () => {
                            let p = null,
                              E = null,
                              _ = null,
                              z = null;
                            this.batch(() => {
                              const L = this.stores.pendingMatches.get(),
                                x = L.length,
                                C = this.stores.matches.get();
                              p = x
                                ? C.filter(
                                    (H) =>
                                      !this.stores.pendingMatchStores.has(H.id),
                                  )
                                : null;
                              const Y = new Set();
                              for (const H of this.stores.pendingMatchStores.values())
                                H.routeId && Y.add(H.routeId);
                              const Q = new Set();
                              for (const H of this.stores.matchStores.values())
                                H.routeId && Q.add(H.routeId);
                              ((E = x
                                ? C.filter((H) => !Y.has(H.routeId))
                                : null),
                                (_ = x
                                  ? L.filter((H) => !Q.has(H.routeId))
                                  : null),
                                (z = x ? L.filter((H) => Q.has(H.routeId)) : C),
                                this.stores.isLoading.set(!1),
                                this.stores.loadedAt.set(Date.now()),
                                x &&
                                  (this.stores.setMatches(L),
                                  this.stores.setPending([]),
                                  this.stores.setCached([
                                    ...this.stores.cachedMatches.get(),
                                    ...p.filter(
                                      (H) =>
                                        H.status !== "error" &&
                                        H.status !== "notFound" &&
                                        H.status !== "redirected",
                                    ),
                                  ]),
                                  this.clearExpiredCache()));
                            });
                            for (const [L, x] of [
                              [E, "onLeave"],
                              [_, "onEnter"],
                              [z, "onStay"],
                            ])
                              if (L)
                                for (const C of L)
                                  this.looseRoutesById[C.routeId].options[x]?.(
                                    C,
                                  );
                          });
                        });
                      },
                    }));
                } catch (m) {
                  _e(m)
                    ? ((r = m),
                      this.navigate({
                        ...r.options,
                        replace: !0,
                        ignoreBlocker: !0,
                      }))
                    : ne(m) && (c = m);
                  const S = r
                    ? r.status
                    : c
                      ? 404
                      : this.stores.matches
                            .get()
                            .some((p) => p.status === "error")
                        ? 500
                        : 200;
                  this.batch(() => {
                    (this.stores.statusCode.set(S),
                      this.stores.redirect.set(r));
                  });
                }
                (this.latestLoadPromise === f &&
                  (this.commitLocationPromise?.resolve(),
                  (this.latestLoadPromise = void 0),
                  (this.commitLocationPromise = void 0)),
                  y());
              });
            }),
              this.latestLoadPromise = f,
              await f;
            this.latestLoadPromise && f !== this.latestLoadPromise;
          )
            await this.latestLoadPromise;
          let g;
          (this.hasNotFoundMatch()
            ? (g = 404)
            : this.stores.matches.get().some((y) => y.status === "error") &&
              (g = 500),
            g !== void 0 && this.stores.statusCode.set(g));
        }),
        (this.startViewTransition = (u) => {
          const r =
            this.shouldViewTransition ?? this.options.defaultViewTransition;
          if (
            ((this.shouldViewTransition = void 0),
            r &&
              typeof document < "u" &&
              "startViewTransition" in document &&
              typeof document.startViewTransition == "function")
          ) {
            let c;
            if (typeof r == "object" && this.isViewTransitionTypesSupported) {
              const f = this.latestLocation,
                h = this.stores.resolvedLocation.get(),
                g = typeof r.types == "function" ? r.types(Al(f, h)) : r.types;
              if (g === !1) {
                u();
                return;
              }
              c = { update: u, types: g };
            } else c = u;
            document.startViewTransition(c);
          } else u();
        }),
        (this.updateMatch = (u, r) => {
          this.startTransition(() => {
            const c = this.stores.pendingMatchStores.get(u);
            if (c) {
              c.set(r);
              return;
            }
            const f = this.stores.matchStores.get(u);
            if (f) {
              f.set(r);
              return;
            }
            const h = this.stores.cachedMatchStores.get(u);
            if (h) {
              const g = r(h.get());
              g.status === "redirected"
                ? this.stores.cachedMatchStores.delete(u) &&
                  this.stores.cachedIds.set((y) => y.filter((m) => m !== u))
                : h.set(g);
            }
          });
        }),
        (this.getMatch = (u) =>
          this.stores.cachedMatchStores.get(u)?.get() ??
          this.stores.pendingMatchStores.get(u)?.get() ??
          this.stores.matchStores.get(u)?.get()),
        (this.invalidate = (u) => {
          const r = (c) =>
            (u?.filter?.(c) ?? !0)
              ? {
                  ...c,
                  invalid: !0,
                  ...(u?.forcePending ||
                  c.status === "error" ||
                  c.status === "notFound"
                    ? { status: "pending", error: void 0 }
                    : void 0),
                }
              : c;
          return (
            this.batch(() => {
              (this.stores.setMatches(this.stores.matches.get().map(r)),
                this.stores.setCached(this.stores.cachedMatches.get().map(r)),
                this.stores.setPending(
                  this.stores.pendingMatches.get().map(r),
                ));
            }),
            (this.shouldViewTransition = !1),
            this.load({ sync: u?.sync })
          );
        }),
        (this.getParsedLocationHref = (u) => u.publicHref || "/"),
        (this.resolveRedirect = (u) => {
          const r = u.headers.get("Location");
          if (!u.options.href || u.options._builtLocation) {
            const c = u.options._builtLocation ?? this.buildLocation(u.options),
              f = this.getParsedLocationHref(c);
            ((u.options.href = f), u.headers.set("Location", f));
          } else if (r)
            try {
              const c = new URL(r);
              if (this.origin && c.origin === this.origin) {
                const f = c.pathname + c.search + c.hash;
                ((u.options.href = f), u.headers.set("Location", f));
              }
            } catch {}
          if (
            u.options.href &&
            !u.options._builtLocation &&
            ps(u.options.href, this.protocolAllowlist)
          )
            throw new Error("Redirect blocked: unsafe protocol");
          return (
            u.headers.get("Location") ||
              u.headers.set("Location", u.options.href),
            u
          );
        }),
        (this.clearCache = (u) => {
          const r = u?.filter;
          r !== void 0
            ? this.stores.setCached(
                this.stores.cachedMatches.get().filter((c) => !r(c)),
              )
            : this.stores.setCached([]);
        }),
        (this.clearExpiredCache = () => {
          const u = Date.now(),
            r = (c) => {
              const f = this.looseRoutesById[c.routeId];
              if (!f.options.loader) return !0;
              const h =
                (c.preload
                  ? (f.options.preloadGcTime ??
                    this.options.defaultPreloadGcTime)
                  : (f.options.gcTime ?? this.options.defaultGcTime)) ??
                300 * 1e3;
              return c.status === "error" ? !0 : u - c.updatedAt >= h;
            };
          this.clearCache({ filter: r });
        }),
        (this.loadRouteChunk = Hi),
        (this.preloadRoute = async (u) => {
          const r = u._builtLocation ?? this.buildLocation(u);
          let c = this.matchRoutes(r, {
            throwOnError: !0,
            preload: !0,
            dest: u,
          });
          const f = new Set([
              ...this.stores.matchesId.get(),
              ...this.stores.pendingIds.get(),
            ]),
            h = new Set([...f, ...this.stores.cachedIds.get()]),
            g = c.filter((y) => !h.has(y.id));
          if (g.length) {
            const y = this.stores.cachedMatches.get();
            this.stores.setCached([...y, ...g]);
          }
          try {
            return (
              (c = await Im({
                router: this,
                matches: c,
                location: r,
                preload: !0,
                updateMatch: (y, m) => {
                  f.has(y)
                    ? (c = c.map((S) => (S.id === y ? m(S) : S)))
                    : this.updateMatch(y, m);
                },
              })),
              c
            );
          } catch (y) {
            if (_e(y))
              return y.options.reloadDocument
                ? void 0
                : await this.preloadRoute({ ...y.options, _fromLocation: r });
            ne(y) || console.error(y);
            return;
          }
        }),
        (this.matchRoute = (u, r) => {
          const c = {
              ...u,
              to: u.to ? this.resolvePathWithBase(u.from || "", u.to) : void 0,
              params: u.params || {},
              leaveParams: !0,
            },
            f = this.buildLocation(c);
          if (r?.pending && this.stores.status.get() !== "pending") return !1;
          const h = (
              r?.pending === void 0 ? !this.stores.isLoading.get() : r.pending
            )
              ? this.latestLocation
              : this.stores.resolvedLocation.get() ||
                this.stores.location.get(),
            g = H0(
              f.pathname,
              r?.caseSensitive ?? !1,
              r?.fuzzy ?? !1,
              h.pathname,
              this.processedTree,
            );
          return !g || (u.params && !be(g.rawParams, u.params, { partial: !0 }))
            ? !1
            : (r?.includeSearch ?? !0)
              ? be(h.search, f.search, { partial: !0 })
                ? g.rawParams
                : !1
              : g.rawParams;
        }),
        (this.hasNotFoundMatch = () =>
          this.stores.matches
            .get()
            .some((u) => u.status === "notFound" || u.globalNotFound)),
        (this.getStoreConfig = i),
        this.update({
          defaultPreloadDelay: 50,
          defaultPendingMs: 1e3,
          defaultPendingMinMs: 500,
          context: void 0,
          ...a,
          caseSensitive: a.caseSensitive ?? !1,
          notFoundMode: a.notFoundMode ?? "fuzzy",
          stringifySearch: a.stringifySearch ?? nS,
          parseSearch: a.parseSearch ?? eS,
          protocolAllowlist: a.protocolAllowlist ?? O0,
        }),
        typeof document < "u" && (self.__TSR_ROUTER__ = this));
    }
    isShell() {
      return !!this.options.isShell;
    }
    isPrerendering() {
      return !!this.options.isPrerendering;
    }
    get state() {
      return this.stores.__store.get();
    }
    setRoutes({ routesById: a, routesByPath: i, processedTree: u }) {
      ((this.routesById = a),
        (this.routesByPath = i),
        (this.processedTree = u));
      const r = this.options.notFoundRoute;
      r &&
        (r.init({ originalIndex: 99999999999 }), (this.routesById[r.id] = r));
    }
    getRouteBranch(a) {
      let i = this.routeBranchCache.get(a);
      return (i || ((i = Ny(a)), this.routeBranchCache.set(a, i)), i);
    }
    get looseRoutesById() {
      return this.routesById;
    }
    getParentContext(a) {
      return a?.id
        ? (a.context ?? this.options.context ?? void 0)
        : (this.options.context ?? void 0);
    }
    matchRoutesInternal(a, i) {
      const u = this.getMatchedRoutes(a.pathname),
        { foundRoute: r, routeParams: c } = u;
      let { matchedRoutes: f } = u,
        h = !1;
      (r ? r.path !== "/" && c["**"] : Rn(a.pathname)) &&
        (this.options.notFoundRoute
          ? (f = [...f, this.options.notFoundRoute])
          : (h = !0));
      const g = h ? xS(this.options.notFoundMode, f) : void 0,
        y = new Array(f.length),
        m = new Map();
      for (const S of this.stores.matchStores.values())
        S.routeId && m.set(S.routeId, S.get());
      for (let S = 0; S < f.length; S++) {
        const p = f[S],
          E = y[S - 1];
        let _, z, L;
        {
          const it = E?.search ?? a.search,
            $t = E?._strictSearch ?? void 0;
          try {
            const Bt = hs(p.options.validateSearch, { ...it }) ?? void 0;
            ((_ = { ...it, ...Bt }), (z = { ...$t, ...Bt }), (L = void 0));
          } catch (Bt) {
            let Gt = Bt;
            if (
              (Bt instanceof Ss || (Gt = new Ss(Bt.message, { cause: Bt })),
              i?.throwOnError)
            )
              throw Gt;
            ((_ = it), (z = {}), (L = Gt));
          }
        }
        const x = p.options.loaderDeps?.({ search: _ }) ?? "",
          C = x ? JSON.stringify(x) : "",
          { interpolatedPath: Y, usedParams: Q } = Qm({
            path: p.fullPath,
            params: c,
            decoder: this.pathParamsDecoder,
            server: this.isServer,
          }),
          H = p.id + Y + C,
          W = this.getMatch(H),
          F = m.get(p.id),
          G = W?._strictParams ?? Q;
        let Z;
        if (!W)
          try {
            ny(p, G);
          } catch (it) {
            if (
              (ne(it) || _e(it)
                ? (Z = it)
                : (Z = new bS(it.message, { cause: it })),
              i?.throwOnError)
            )
              throw Z;
          }
        Object.assign(c, G);
        const P = F ? "stay" : "enter";
        let rt;
        if (W)
          rt = {
            ...W,
            cause: P,
            params: F?.params ?? c,
            _strictParams: G,
            search: za(F ? F.search : W.search, _),
            _strictSearch: z,
          };
        else {
          const it =
            p.options.loader || p.options.beforeLoad || p.lazyFn || Qy(p)
              ? "pending"
              : "success";
          rt = {
            id: H,
            ssr: p.options.ssr,
            index: S,
            routeId: p.id,
            params: F?.params ?? c,
            _strictParams: G,
            pathname: Y,
            updatedAt: Date.now(),
            search: F ? za(F.search, _) : _,
            _strictSearch: z,
            searchError: void 0,
            status: it,
            isFetching: !1,
            error: void 0,
            paramsError: Z,
            __routeContext: void 0,
            _nonReactive: { loadPromise: Ua() },
            __beforeLoadContext: void 0,
            context: {},
            abortController: new AbortController(),
            fetchCount: 0,
            cause: P,
            loaderDeps: F ? Oa(F.loaderDeps, x) : x,
            invalid: !1,
            preload: !1,
            links: void 0,
            scripts: void 0,
            headScripts: void 0,
            meta: void 0,
            staticData: p.options.staticData || {},
            fullPath: p.fullPath,
          };
        }
        (i?.preload || (rt.globalNotFound = g === p.id), (rt.searchError = L));
        const ut = this.getParentContext(E);
        ((rt.context = {
          ...ut,
          ...rt.__routeContext,
          ...rt.__beforeLoadContext,
        }),
          (y[S] = rt));
      }
      for (let S = 0; S < y.length; S++) {
        const p = y[S],
          E = this.looseRoutesById[p.routeId],
          _ = this.getMatch(p.id),
          z = m.get(p.routeId);
        if (((p.params = z ? za(z.params, c) : c), !_)) {
          const L = y[S - 1],
            x = this.getParentContext(L);
          if (E.options.context) {
            const C = {
              deps: p.loaderDeps,
              params: p.params,
              context: x ?? {},
              location: a,
              navigate: (Y) => this.navigate({ ...Y, _fromLocation: a }),
              buildLocation: this.buildLocation,
              cause: p.cause,
              abortController: p.abortController,
              preload: !!p.preload,
              matches: y,
              routeId: E.id,
            };
            p.__routeContext = E.options.context(C) ?? void 0;
          }
          p.context = { ...x, ...p.__routeContext, ...p.__beforeLoadContext };
        }
      }
      return y;
    }
    matchRoutesLightweight(a) {
      const { matchedRoutes: i, routeParams: u } = this.getMatchedRoutes(
          a.pathname,
        ),
        r = Ni(i),
        c = { ...a.search };
      for (const m of i)
        try {
          Object.assign(c, hs(m.options.validateSearch, c));
        } catch {}
      const f = Ni(this.stores.matchesId.get()),
        h = f && this.stores.matchStores.get(f)?.get(),
        g = h && h.routeId === r.id && h.pathname === a.pathname;
      let y;
      if (g) y = h.params;
      else {
        const m = Object.assign(Object.create(null), u);
        for (const S of i)
          try {
            ny(S, m);
          } catch {}
        y = m;
      }
      return { matchedRoutes: i, fullPath: r.fullPath, search: c, params: y };
    }
  },
  Ss = class extends Error {},
  bS = class extends Error {};
function _S(a) {
  return {
    loadedAt: 0,
    isLoading: !1,
    isTransitioning: !1,
    status: "idle",
    resolvedLocation: void 0,
    location: a,
    matches: [],
    statusCode: 200,
  };
}
function hs(a, i) {
  if (a == null) return {};
  if ("~standard" in a) {
    const u = a["~standard"].validate(i);
    if (u instanceof Promise) throw new Ss("Async validation not supported");
    if (u.issues)
      throw new Ss(JSON.stringify(u.issues, void 0, 2), { cause: u });
    return u.value;
  }
  return "parse" in a ? a.parse(i) : typeof a == "function" ? a(i) : {};
}
function ES({ pathname: a, routesById: i, processedTree: u }) {
  const r = Object.create(null),
    c = Rn(a);
  let f;
  const h = q0(c, u, !0);
  return (
    h && ((f = h.route), Object.assign(r, h.rawParams)),
    { matchedRoutes: h?.branch || [i.__root__], routeParams: r, foundRoute: f }
  );
}
function RS({ search: a, dest: i, destRoutes: u, _includeValidateSearch: r }) {
  return TS(u)(a, i, r ?? !1);
}
function TS(a) {
  const i = { dest: null, _includeValidateSearch: !1, middlewares: [] };
  for (const c of a) {
    if ("search" in c.options)
      c.options.search?.middlewares &&
        i.middlewares.push(...c.options.search.middlewares);
    else if (c.options.preSearchFilters || c.options.postSearchFilters) {
      const f = ({ search: h, next: g }) => {
        let y = h;
        "preSearchFilters" in c.options &&
          c.options.preSearchFilters &&
          (y = c.options.preSearchFilters.reduce((S, p) => p(S), h));
        const m = g(y);
        return "postSearchFilters" in c.options && c.options.postSearchFilters
          ? c.options.postSearchFilters.reduce((S, p) => p(S), m)
          : m;
      };
      i.middlewares.push(f);
    }
    if (c.options.validateSearch) {
      const f = ({ search: h, next: g }) => {
        const y = g(h);
        if (!i._includeValidateSearch) return y;
        try {
          return { ...y, ...(hs(c.options.validateSearch, y) ?? void 0) };
        } catch {
          return y;
        }
      };
      i.middlewares.push(f);
    }
  }
  const u = ({ search: c }) => {
    const f = i.dest;
    return f.search ? (f.search === !0 ? c : ea(f.search, c)) : {};
  };
  i.middlewares.push(u);
  const r = (c, f, h) => {
    if (c >= h.length) return f;
    const g = h[c];
    return g({ search: f, next: (m) => r(c + 1, m, h) });
  };
  return function (f, h, g) {
    return (
      (i.dest = h),
      (i._includeValidateSearch = g),
      r(0, f, i.middlewares)
    );
  };
}
function xS(a, i) {
  if (a !== "root")
    for (let u = i.length - 1; u >= 0; u--) {
      const r = i[u];
      if (r.children) return r.id;
    }
  return Da;
}
function ny(a, i) {
  const u = a.options.params?.parse ?? a.options.parseParams;
  if (u) {
    const r = u(i);
    if (r === !1)
      throw new Error("Route params.parse returned false for a matched route");
    Object.assign(i, r);
  }
}
const $e = Symbol.for("TSR_DEFERRED_PROMISE");
function AS(a, i) {
  const u = a;
  return (
    u[$e] ||
      ((u[$e] = { status: "pending" }),
      u
        .then((r) => {
          ((u[$e].status = "success"), (u[$e].data = r));
        })
        .catch((r) => {
          ((u[$e].status = "error"),
            (u[$e].error = { data: vS(r), __isServerError: !0 }));
        })),
    u
  );
}
const MS = "Error preloading route! ☝️";
function ay(a, i) {
  if (a) return typeof a == "string" ? a : a[i];
}
function zS(a) {
  return typeof a == "string" ? { href: a, crossOrigin: void 0 } : a;
}
function OS(a) {
  if (a.tag !== "link") return;
  const i = a.attrs?.rel,
    u = a.attrs?.href;
  if (
    typeof u == "string" &&
    (typeof i == "string" ? i.split(/\s+/) : []).includes("stylesheet")
  )
    return u;
}
function CS(a, i) {
  const u = OS(i);
  return !!u && a?.inlineCss?.styles[u] !== void 0;
}
var Zy = class {
    get to() {
      return this._to;
    }
    get id() {
      return this._id;
    }
    get path() {
      return this._path;
    }
    get fullPath() {
      return this._fullPath;
    }
    constructor(a) {
      if (
        ((this.init = (i) => {
          this.originalIndex = i.originalIndex;
          const u = this.options,
            r = !u?.path && !u?.id;
          ((this.parentRoute = this.options.getParentRoute?.()),
            r ? (this._path = Da) : this.parentRoute || Ee());
          let c = r ? Da : u?.path;
          c && c !== "/" && (c = By(c));
          const f = u?.id || c;
          let h = r
            ? Da
            : fs([
                this.parentRoute.id === "__root__" ? "" : this.parentRoute.id,
                f,
              ]);
          (c === "__root__" && (c = "/"),
            h !== "__root__" && (h = fs(["/", h])));
          const g = h === "__root__" ? "/" : fs([this.parentRoute.fullPath, c]);
          ((this._path = c),
            (this._id = h),
            (this._fullPath = g),
            (this._to = Rn(g)));
        }),
        (this.addChildren = (i) => this._addFileChildren(i)),
        (this._addFileChildren = (i) => (
          Array.isArray(i) && (this.children = i),
          typeof i == "object" &&
            i !== null &&
            (this.children = Object.values(i)),
          this
        )),
        (this._addFileTypes = () => this),
        (this.updateLoader = (i) => (Object.assign(this.options, i), this)),
        (this.update = (i) => (Object.assign(this.options, i), this)),
        (this.lazy = (i) => ((this.lazyFn = i), this)),
        (this.redirect = (i) => qy({ from: this.fullPath, ...i })),
        (this.options = a || {}),
        (this.isRoot = !a?.getParentRoute),
        a?.id && a?.path)
      )
        throw new Error("Route cannot have both an 'id' and a 'path' option.");
    }
  },
  wS = class extends Zy {
    constructor(a) {
      super(a);
    }
  };
function DS(a) {
  if (typeof document < "u" && document.querySelector) {
    const i = a.stores.location.get(),
      u = i.state.__hashScrollIntoViewOptions ?? !0;
    if (u && i.hash !== "") {
      const r = document.getElementById(i.hash);
      r && r.scrollIntoView(u);
    }
  }
}
var LS = ((a) => (
    (a[(a.AggregateError = 1)] = "AggregateError"),
    (a[(a.ArrowFunction = 2)] = "ArrowFunction"),
    (a[(a.ErrorPrototypeStack = 4)] = "ErrorPrototypeStack"),
    (a[(a.ObjectAssign = 8)] = "ObjectAssign"),
    (a[(a.BigIntTypedArray = 16)] = "BigIntTypedArray"),
    (a[(a.RegExp = 32)] = "RegExp"),
    a
  ))(LS || {}),
  Tn = Symbol.asyncIterator,
  Ky = Symbol.hasInstance,
  Ml = Symbol.isConcatSpreadable,
  xn = Symbol.iterator,
  Jy = Symbol.match,
  ky = Symbol.matchAll,
  Fy = Symbol.replace,
  Py = Symbol.search,
  Iy = Symbol.species,
  Wy = Symbol.split,
  $y = Symbol.toPrimitive,
  zl = Symbol.toStringTag,
  tp = Symbol.unscopables,
  ep = {
    [Tn]: 0,
    [Ky]: 1,
    [Ml]: 2,
    [xn]: 3,
    [Jy]: 4,
    [ky]: 5,
    [Fy]: 6,
    [Py]: 7,
    [Iy]: 8,
    [Wy]: 9,
    [$y]: 10,
    [zl]: 11,
    [tp]: 12,
  },
  US = {
    0: Tn,
    1: Ky,
    2: Ml,
    3: xn,
    4: Jy,
    5: ky,
    6: Fy,
    7: Py,
    8: Iy,
    9: Wy,
    10: $y,
    11: zl,
    12: tp,
  },
  b = void 0,
  NS = {
    2: !0,
    3: !1,
    1: b,
    0: null,
    4: -0,
    5: Number.POSITIVE_INFINITY,
    6: Number.NEGATIVE_INFINITY,
    7: Number.NaN,
  },
  BS = {
    0: "Error",
    1: "EvalError",
    2: "RangeError",
    3: "ReferenceError",
    4: "SyntaxError",
    5: "TypeError",
    6: "URIError",
  },
  jS = {
    0: Error,
    1: EvalError,
    2: RangeError,
    3: ReferenceError,
    4: SyntaxError,
    5: TypeError,
    6: URIError,
  };
function _t(a, i, u, r, c, f, h, g, y, m, S, p) {
  return {
    t: a,
    i,
    s: u,
    c: r,
    m: c,
    p: f,
    e: h,
    a: g,
    f: y,
    b: m,
    o: S,
    l: p,
  };
}
function ia(a) {
  return _t(2, b, a, b, b, b, b, b, b, b, b, b);
}
var np = ia(2),
  ap = ia(3),
  HS = ia(1),
  qS = ia(0),
  YS = ia(4),
  GS = ia(5),
  XS = ia(6),
  VS = ia(7);
function QS(a) {
  switch (a) {
    case '"':
      return '\\"';
    case "\\":
      return "\\\\";
    case `
`:
      return "\\n";
    case "\r":
      return "\\r";
    case "\b":
      return "\\b";
    case "	":
      return "\\t";
    case "\f":
      return "\\f";
    case "<":
      return "\\x3C";
    case "\u2028":
      return "\\u2028";
    case "\u2029":
      return "\\u2029";
    default:
      return b;
  }
}
function ua(a) {
  let i = "",
    u = 0,
    r;
  for (let c = 0, f = a.length; c < f; c++)
    ((r = QS(a[c])), r && ((i += a.slice(u, c) + r), (u = c + 1)));
  return (u === 0 ? (i = a) : (i += a.slice(u)), i);
}
function ZS(a) {
  switch (a) {
    case "\\\\":
      return "\\";
    case '\\"':
      return '"';
    case "\\n":
      return `
`;
    case "\\r":
      return "\r";
    case "\\b":
      return "\b";
    case "\\t":
      return "	";
    case "\\f":
      return "\f";
    case "\\x3C":
      return "<";
    case "\\u2028":
      return "\u2028";
    case "\\u2029":
      return "\u2029";
    default:
      return a;
  }
}
function sa(a) {
  return a.replace(/(\\\\|\\"|\\n|\\r|\\b|\\t|\\f|\\u2028|\\u2029|\\x3C)/g, ZS);
}
var os = "__SEROVAL_REFS__",
  lp = new Map(),
  xl = new Map();
function ip(a) {
  return lp.has(a);
}
function KS(a) {
  return xl.has(a);
}
function JS(a) {
  if (ip(a)) return lp.get(a);
  throw new T1(a);
}
function kS(a) {
  if (KS(a)) return xl.get(a);
  throw new x1(a);
}
typeof globalThis < "u"
  ? Object.defineProperty(globalThis, os, {
      value: xl,
      configurable: !0,
      writable: !1,
      enumerable: !1,
    })
  : typeof window < "u"
    ? Object.defineProperty(window, os, {
        value: xl,
        configurable: !0,
        writable: !1,
        enumerable: !1,
      })
    : typeof self < "u"
      ? Object.defineProperty(self, os, {
          value: xl,
          configurable: !0,
          writable: !1,
          enumerable: !1,
        })
      : typeof global < "u" &&
        Object.defineProperty(global, os, {
          value: xl,
          configurable: !0,
          writable: !1,
          enumerable: !1,
        });
function Ec(a) {
  return a instanceof EvalError
    ? 1
    : a instanceof RangeError
      ? 2
      : a instanceof ReferenceError
        ? 3
        : a instanceof SyntaxError
          ? 4
          : a instanceof TypeError
            ? 5
            : a instanceof URIError
              ? 6
              : 0;
}
function FS(a) {
  let i = BS[Ec(a)];
  return a.name !== i
    ? { name: a.name }
    : a.constructor.name !== i
      ? { name: a.constructor.name }
      : {};
}
function up(a, i) {
  let u = FS(a),
    r = Object.getOwnPropertyNames(a);
  for (let c = 0, f = r.length, h; c < f; c++)
    ((h = r[c]),
      h !== "name" &&
        h !== "message" &&
        (h === "stack"
          ? i & 4 && ((u = u || {}), (u[h] = a[h]))
          : ((u = u || {}), (u[h] = a[h]))));
  return u;
}
function sp(a) {
  return Object.isFrozen(a)
    ? 3
    : Object.isSealed(a)
      ? 2
      : Object.isExtensible(a)
        ? 0
        : 1;
}
function PS(a) {
  switch (a) {
    case Number.POSITIVE_INFINITY:
      return GS;
    case Number.NEGATIVE_INFINITY:
      return XS;
  }
  return a !== a
    ? VS
    : Object.is(a, -0)
      ? YS
      : _t(0, b, a, b, b, b, b, b, b, b, b, b);
}
function rp(a) {
  return _t(1, b, ua(a), b, b, b, b, b, b, b, b, b);
}
function IS(a) {
  return _t(3, b, "" + a, b, b, b, b, b, b, b, b, b);
}
function WS(a) {
  return _t(4, a, b, b, b, b, b, b, b, b, b, b);
}
function $S(a, i) {
  let u = i.valueOf();
  return _t(5, a, u !== u ? "" : i.toISOString(), b, b, b, b, b, b, b, b, b);
}
function t1(a, i) {
  return _t(6, a, b, ua(i.source), i.flags, b, b, b, b, b, b, b);
}
function e1(a, i) {
  return _t(17, a, ep[i], b, b, b, b, b, b, b, b, b);
}
function n1(a, i) {
  return _t(18, a, ua(JS(i)), b, b, b, b, b, b, b, b, b);
}
function a1(a, i, u) {
  return _t(25, a, u, ua(i), b, b, b, b, b, b, b, b);
}
function l1(a, i, u) {
  return _t(9, a, b, b, b, b, b, u, b, b, sp(i), b);
}
function i1(a, i) {
  return _t(21, a, b, b, b, b, b, b, i, b, b, b);
}
function u1(a, i, u) {
  return _t(
    15,
    a,
    b,
    i.constructor.name,
    b,
    b,
    b,
    b,
    u,
    i.byteOffset,
    b,
    i.length,
  );
}
function s1(a, i, u) {
  return _t(
    16,
    a,
    b,
    i.constructor.name,
    b,
    b,
    b,
    b,
    u,
    i.byteOffset,
    b,
    i.byteLength,
  );
}
function r1(a, i, u) {
  return _t(20, a, b, b, b, b, b, b, u, i.byteOffset, b, i.byteLength);
}
function o1(a, i, u) {
  return _t(13, a, Ec(i), b, ua(i.message), u, b, b, b, b, b, b);
}
function c1(a, i, u) {
  return _t(14, a, Ec(i), b, ua(i.message), u, b, b, b, b, b, b);
}
function f1(a, i) {
  return _t(7, a, b, b, b, b, b, i, b, b, b, b);
}
function d1(a, i) {
  return _t(28, b, b, b, b, b, b, [a, i], b, b, b, b);
}
function h1(a, i) {
  return _t(30, b, b, b, b, b, b, [a, i], b, b, b, b);
}
function m1(a, i, u) {
  return _t(31, a, b, b, b, b, b, u, i, b, b, b);
}
function y1(a, i) {
  return _t(32, a, b, b, b, b, b, b, i, b, b, b);
}
function p1(a, i) {
  return _t(33, a, b, b, b, b, b, b, i, b, b, b);
}
function g1(a, i) {
  return _t(34, a, b, b, b, b, b, b, i, b, b, b);
}
function v1(a, i, u, r) {
  return _t(35, a, u, b, b, b, b, i, b, b, b, r);
}
var S1 = { parsing: 1, serialization: 2, deserialization: 3 };
function b1(a) {
  return `Seroval Error (step: ${S1[a]})`;
}
var _1 = (a, i) => b1(a),
  op = class extends Error {
    constructor(a, i) {
      (super(_1(a)), (this.cause = i));
    }
  },
  ly = class extends op {
    constructor(a) {
      super("parsing", a);
    }
  },
  E1 = class extends op {
    constructor(a) {
      super("deserialization", a);
    }
  };
function An(a) {
  return `Seroval Error (specific: ${a})`;
}
var xs = class extends Error {
    constructor(i) {
      (super(An(1)), (this.value = i));
    }
  },
  cp = class extends Error {
    constructor(i) {
      super(An(2));
    }
  },
  R1 = class extends Error {
    constructor(a) {
      super(An(3));
    }
  },
  Gi = class extends Error {
    constructor(a) {
      super(An(4));
    }
  },
  T1 = class extends Error {
    constructor(a) {
      (super(An(5)), (this.value = a));
    }
  },
  x1 = class extends Error {
    constructor(a) {
      super(An(6));
    }
  },
  A1 = class extends Error {
    constructor(a) {
      super(An(7));
    }
  },
  ra = class extends Error {
    constructor(a) {
      super(An(8));
    }
  },
  M1 = class extends Error {
    constructor(i) {
      super(An(9));
    }
  },
  z1 = class {
    constructor(a, i) {
      ((this.value = a), (this.replacement = i));
    }
  },
  As = () => {
    let a = { p: 0, s: 0, f: 0 };
    return (
      (a.p = new Promise((i, u) => {
        ((a.s = i), (a.f = u));
      })),
      a
    );
  },
  O1 = (a, i) => {
    (a.s(i), (a.p.s = 1), (a.p.v = i));
  },
  C1 = (a, i) => {
    (a.f(i), (a.p.s = 2), (a.p.v = i));
  };
As.toString();
O1.toString();
C1.toString();
var w1 = () => {
    let a = [],
      i = [],
      u = !0,
      r = !1,
      c = 0,
      f = (y, m, S) => {
        for (S = 0; S < c; S++) i[S] && i[S][m](y);
      },
      h = (y, m, S, p) => {
        for (m = 0, S = a.length; m < S; m++)
          ((p = a[m]),
            !u && m === S - 1 ? y[r ? "return" : "throw"](p) : y.next(p));
      },
      g = (y, m) => (
        u && ((m = c++), (i[m] = y)),
        h(y),
        () => {
          u && ((i[m] = i[c]), (i[c--] = void 0));
        }
      );
    return {
      __SEROVAL_STREAM__: !0,
      on: (y) => g(y),
      next: (y) => {
        u && (a.push(y), f(y, "next"));
      },
      throw: (y) => {
        u && (a.push(y), f(y, "throw"), (u = !1), (r = !1), (i.length = 0));
      },
      return: (y) => {
        u && (a.push(y), f(y, "return"), (u = !1), (r = !0), (i.length = 0));
      },
    };
  },
  D1 = (a) => (i) => () => {
    let u = 0,
      r = {
        [a]: () => r,
        next: () => {
          if (u > i.d) return { done: !0, value: void 0 };
          let c = u++,
            f = i.v[c];
          if (c === i.t) throw f;
          return { done: c === i.d, value: f };
        },
      };
    return r;
  },
  L1 = (a, i) => (u) => () => {
    let r = 0,
      c = -1,
      f = !1,
      h = [],
      g = [],
      y = (S = 0, p = g.length) => {
        for (; S < p; S++) g[S].s({ done: !0, value: void 0 });
      };
    u.on({
      next: (S) => {
        let p = g.shift();
        (p && p.s({ done: !1, value: S }), h.push(S));
      },
      throw: (S) => {
        let p = g.shift();
        (p && p.f(S), y(), (c = h.length), (f = !0), h.push(S));
      },
      return: (S) => {
        let p = g.shift();
        (p && p.s({ done: !0, value: S }), y(), (c = h.length), h.push(S));
      },
    });
    let m = {
      [a]: () => m,
      next: () => {
        if (c === -1) {
          let E = r++;
          if (E >= h.length) {
            let _ = i();
            return (g.push(_), _.p);
          }
          return { done: !1, value: h[E] };
        }
        if (r > c) return { done: !0, value: void 0 };
        let S = r++,
          p = h[S];
        if (S !== c) return { done: !1, value: p };
        if (f) throw p;
        return { done: !0, value: p };
      },
    };
    return m;
  },
  fp = (a) => {
    let i = atob(a),
      u = i.length,
      r = new Uint8Array(u);
    for (let c = 0; c < u; c++) r[c] = i.charCodeAt(c);
    return r.buffer;
  };
fp.toString();
function U1(a) {
  return "__SEROVAL_SEQUENCE__" in a;
}
function dp(a, i, u) {
  return { __SEROVAL_SEQUENCE__: !0, v: a, t: i, d: u };
}
function N1(a) {
  let i = [],
    u = -1,
    r = -1,
    c = a[xn]();
  for (;;)
    try {
      let f = c.next();
      if ((i.push(f.value), f.done)) {
        r = i.length - 1;
        break;
      }
    } catch (f) {
      ((u = i.length), i.push(f));
    }
  return dp(i, u, r);
}
var B1 = D1(xn);
function j1(a) {
  return B1(a);
}
var H1 = {},
  q1 = {},
  Y1 = { 0: {}, 1: {}, 2: {}, 3: {}, 4: {}, 5: {} };
function Ms(a) {
  return "__SEROVAL_STREAM__" in a;
}
function Na() {
  return w1();
}
function G1(a) {
  let i = Na(),
    u = a[Tn]();
  async function r() {
    try {
      let c = await u.next();
      c.done ? i.return(c.value) : (i.next(c.value), await r());
    } catch (c) {
      i.throw(c);
    }
  }
  return (r().catch(() => {}), i);
}
var X1 = L1(Tn, As);
function V1(a) {
  return X1(a);
}
async function Q1(a) {
  try {
    return [1, await a];
  } catch (i) {
    return [0, i];
  }
}
function Z1(a, i) {
  return {
    plugins: i.plugins,
    mode: a,
    marked: new Set(),
    features: 63 ^ (i.disabledFeatures || 0),
    refs: i.refs || new Map(),
    depthLimit: i.depthLimit || 1e3,
  };
}
function ms(a, i) {
  a.marked.add(i);
}
function K1(a, i) {
  let u = a.refs.size;
  return (a.refs.set(i, u), u);
}
function zs(a, i) {
  let u = a.refs.get(i);
  return u != null
    ? (ms(a, u), { type: 1, value: WS(u) })
    : { type: 0, value: K1(a, i) };
}
function Rc(a, i) {
  let u = zs(a, i);
  return u.type === 1 ? u : ip(i) ? { type: 2, value: n1(u.value, i) } : u;
}
function wa(a, i) {
  let u = Rc(a, i);
  if (u.type !== 0) return u.value;
  if (i in ep) return e1(u.value, i);
  throw new xs(i);
}
function Os(a, i) {
  let u = zs(a, Y1[i]);
  return u.type === 1 ? u.value : _t(26, u.value, i, b, b, b, b, b, b, b, b, b);
}
function J1(a) {
  let i = zs(a, H1);
  return i.type === 1
    ? i.value
    : _t(27, i.value, b, b, b, b, b, b, wa(a, xn), b, b, b);
}
function k1(a) {
  let i = zs(a, q1);
  return i.type === 1
    ? i.value
    : _t(29, i.value, b, b, b, b, b, [Os(a, 1), wa(a, Tn)], b, b, b, b);
}
function F1(a, i, u, r) {
  return _t(u ? 11 : 10, a, b, b, b, r, b, b, b, b, sp(i), b);
}
function P1(a, i, u, r) {
  return _t(8, i, b, b, b, b, { k: u, v: r }, b, Os(a, 0), b, b, b);
}
function I1(a, i, u) {
  let r = new Uint8Array(u),
    c = "";
  for (let f = 0, h = r.length; f < h; f++) c += String.fromCharCode(r[f]);
  return _t(19, i, ua(btoa(c)), b, b, b, b, b, Os(a, 5), b, b, b);
}
function W1(a, i) {
  return { base: Z1(a, i), child: void 0 };
}
var $1 = class {
  constructor(a, i) {
    ((this._p = a), (this.depth = i));
  }
  parse(a) {
    return It(this._p, this.depth, a);
  }
};
async function tb(a, i, u) {
  let r = [];
  for (let c = 0, f = u.length; c < f; c++)
    c in u ? (r[c] = await It(a, i, u[c])) : (r[c] = 0);
  return r;
}
async function eb(a, i, u, r) {
  return l1(u, r, await tb(a, i, r));
}
async function Tc(a, i, u) {
  let r = Object.entries(u),
    c = [],
    f = [];
  for (let h = 0, g = r.length; h < g; h++)
    (c.push(ua(r[h][0])), f.push(await It(a, i, r[h][1])));
  return (
    xn in u &&
      (c.push(wa(a.base, xn)), f.push(d1(J1(a.base), await It(a, i, N1(u))))),
    Tn in u &&
      (c.push(wa(a.base, Tn)), f.push(h1(k1(a.base), await It(a, i, G1(u))))),
    zl in u && (c.push(wa(a.base, zl)), f.push(rp(u[zl]))),
    Ml in u && (c.push(wa(a.base, Ml)), f.push(u[Ml] ? np : ap)),
    { k: c, v: f }
  );
}
async function ac(a, i, u, r, c) {
  return F1(u, r, c, await Tc(a, i, r));
}
async function nb(a, i, u, r) {
  return i1(u, await It(a, i, r.valueOf()));
}
async function ab(a, i, u, r) {
  return u1(u, r, await It(a, i, r.buffer));
}
async function lb(a, i, u, r) {
  return s1(u, r, await It(a, i, r.buffer));
}
async function ib(a, i, u, r) {
  return r1(u, r, await It(a, i, r.buffer));
}
async function iy(a, i, u, r) {
  let c = up(r, a.base.features);
  return o1(u, r, c ? await Tc(a, i, c) : b);
}
async function ub(a, i, u, r) {
  let c = up(r, a.base.features);
  return c1(u, r, c ? await Tc(a, i, c) : b);
}
async function sb(a, i, u, r) {
  let c = [],
    f = [];
  for (let [h, g] of r.entries())
    (c.push(await It(a, i, h)), f.push(await It(a, i, g)));
  return P1(a.base, u, c, f);
}
async function rb(a, i, u, r) {
  let c = [];
  for (let f of r.keys()) c.push(await It(a, i, f));
  return f1(u, c);
}
async function hp(a, i, u, r) {
  let c = a.base.plugins;
  if (c)
    for (let f = 0, h = c.length; f < h; f++) {
      let g = c[f];
      if (g.parse.async && g.test(r))
        return a1(u, g.tag, await g.parse.async(r, new $1(a, i), { id: u }));
    }
  return b;
}
async function ob(a, i, u, r) {
  let [c, f] = await Q1(r);
  return _t(12, u, c, b, b, b, b, b, await It(a, i, f), b, b, b);
}
function cb(a, i, u, r, c) {
  let f = [],
    h = u.on({
      next: (g) => {
        (ms(this.base, i),
          It(this, a, g).then(
            (y) => {
              f.push(y1(i, y));
            },
            (y) => {
              (c(y), h());
            },
          ));
      },
      throw: (g) => {
        (ms(this.base, i),
          It(this, a, g).then(
            (y) => {
              (f.push(p1(i, y)), r(f), h());
            },
            (y) => {
              (c(y), h());
            },
          ));
      },
      return: (g) => {
        (ms(this.base, i),
          It(this, a, g).then(
            (y) => {
              (f.push(g1(i, y)), r(f), h());
            },
            (y) => {
              (c(y), h());
            },
          ));
      },
    });
}
async function fb(a, i, u, r) {
  return m1(u, Os(a.base, 4), await new Promise(cb.bind(a, i, u, r)));
}
async function db(a, i, u, r) {
  let c = [];
  for (let f = 0, h = r.v.length; f < h; f++) c[f] = await It(a, i, r.v[f]);
  return v1(u, c, r.t, r.d);
}
async function hb(a, i, u, r) {
  if (Array.isArray(r)) return eb(a, i, u, r);
  if (Ms(r)) return fb(a, i, u, r);
  if (U1(r)) return db(a, i, u, r);
  let c = r.constructor;
  if (c === z1) return It(a, i, r.replacement);
  let f = await hp(a, i, u, r);
  if (f) return f;
  switch (c) {
    case Object:
      return ac(a, i, u, r, !1);
    case b:
      return ac(a, i, u, r, !0);
    case Date:
      return $S(u, r);
    case Error:
    case EvalError:
    case RangeError:
    case ReferenceError:
    case SyntaxError:
    case TypeError:
    case URIError:
      return iy(a, i, u, r);
    case Number:
    case Boolean:
    case String:
    case BigInt:
      return nb(a, i, u, r);
    case ArrayBuffer:
      return I1(a.base, u, r);
    case Int8Array:
    case Int16Array:
    case Int32Array:
    case Uint8Array:
    case Uint16Array:
    case Uint32Array:
    case Uint8ClampedArray:
    case Float32Array:
    case Float64Array:
      return ab(a, i, u, r);
    case DataView:
      return ib(a, i, u, r);
    case Map:
      return sb(a, i, u, r);
    case Set:
      return rb(a, i, u, r);
  }
  if (c === Promise || r instanceof Promise) return ob(a, i, u, r);
  let h = a.base.features;
  if (h & 32 && c === RegExp) return t1(u, r);
  if (h & 16)
    switch (c) {
      case BigInt64Array:
      case BigUint64Array:
        return lb(a, i, u, r);
    }
  if (
    h & 1 &&
    typeof AggregateError < "u" &&
    (c === AggregateError || r instanceof AggregateError)
  )
    return ub(a, i, u, r);
  if (r instanceof Error) return iy(a, i, u, r);
  if (xn in r || Tn in r) return ac(a, i, u, r, !!c);
  throw new xs(r);
}
async function mb(a, i, u) {
  let r = Rc(a.base, u);
  if (r.type !== 0) return r.value;
  let c = await hp(a, i, r.value, u);
  if (c) return c;
  throw new xs(u);
}
async function It(a, i, u) {
  switch (typeof u) {
    case "boolean":
      return u ? np : ap;
    case "undefined":
      return HS;
    case "string":
      return rp(u);
    case "number":
      return PS(u);
    case "bigint":
      return IS(u);
    case "object": {
      if (u) {
        let r = Rc(a.base, u);
        return r.type === 0 ? await hb(a, i + 1, r.value, u) : r.value;
      }
      return qS;
    }
    case "symbol":
      return wa(a.base, u);
    case "function":
      return mb(a, i, u);
    default:
      throw new xs(u);
  }
}
async function yb(a, i) {
  try {
    return await It(a, 0, i);
  } catch (u) {
    throw u instanceof ly ? u : new ly(u);
  }
}
var pb = ((a) => (
  (a[(a.Vanilla = 1)] = "Vanilla"),
  (a[(a.Cross = 2)] = "Cross"),
  a
))(pb || {});
function mp(a, i) {
  for (let u = 0, r = i.length; u < r; u++) {
    let c = i[u];
    a.has(c) || (a.add(c), c.extends && mp(a, c.extends));
  }
}
function yp(a) {
  if (a) {
    let i = new Set();
    return (mp(i, a), [...i]);
  }
}
function gb(a) {
  switch (a) {
    case "Int8Array":
      return Int8Array;
    case "Int16Array":
      return Int16Array;
    case "Int32Array":
      return Int32Array;
    case "Uint8Array":
      return Uint8Array;
    case "Uint16Array":
      return Uint16Array;
    case "Uint32Array":
      return Uint32Array;
    case "Uint8ClampedArray":
      return Uint8ClampedArray;
    case "Float32Array":
      return Float32Array;
    case "Float64Array":
      return Float64Array;
    case "BigInt64Array":
      return BigInt64Array;
    case "BigUint64Array":
      return BigUint64Array;
    default:
      throw new A1(a);
  }
}
var vb = 1e6,
  Sb = 1e4,
  bb = 2e4;
function pp(a, i) {
  switch (i) {
    case 3:
      return Object.freeze(a);
    case 1:
      return Object.preventExtensions(a);
    case 2:
      return Object.seal(a);
    default:
      return a;
  }
}
var _b = 1e3;
function Eb(a, i) {
  var u;
  return {
    mode: a,
    plugins: i.plugins,
    refs: i.refs || new Map(),
    features: (u = i.features) != null ? u : 63 ^ (i.disabledFeatures || 0),
    depthLimit: i.depthLimit || _b,
  };
}
function Rb(a) {
  return { mode: 2, base: Eb(2, a), child: b };
}
var Tb = class {
  constructor(a, i) {
    ((this._p = a), (this.depth = i));
  }
  deserialize(a) {
    return Dt(this._p, this.depth, a);
  }
};
function gp(a, i) {
  if (i < 0 || !Number.isFinite(i) || !Number.isInteger(i))
    throw new ra({ t: 4, i });
  if (a.refs.has(i)) throw new Error("Conflicted ref id: " + i);
}
function xb(a, i, u) {
  return (gp(a.base, i), a.state.marked.has(i) && a.base.refs.set(i, u), u);
}
function Ab(a, i, u) {
  return (gp(a.base, i), a.base.refs.set(i, u), u);
}
function Wt(a, i, u) {
  return a.mode === 1 ? xb(a, i, u) : Ab(a, i, u);
}
function mc(a, i, u) {
  if (Object.hasOwn(i, u)) return i[u];
  throw new ra(a);
}
function Mb(a, i) {
  return Wt(a, i.i, kS(sa(i.s)));
}
function zb(a, i, u) {
  let r = u.a,
    c = r.length,
    f = Wt(a, u.i, new Array(c));
  for (let h = 0, g; h < c; h++) ((g = r[h]), g && (f[h] = Dt(a, i, g)));
  return (pp(f, u.o), f);
}
function Ob(a) {
  switch (a) {
    case "constructor":
    case "__proto__":
    case "prototype":
    case "__defineGetter__":
    case "__defineSetter__":
    case "__lookupGetter__":
    case "__lookupSetter__":
      return !1;
    default:
      return !0;
  }
}
function Cb(a) {
  switch (a) {
    case Tn:
    case Ml:
    case zl:
    case xn:
      return !0;
    default:
      return !1;
  }
}
function uy(a, i, u) {
  Ob(i)
    ? (a[i] = u)
    : Object.defineProperty(a, i, {
        value: u,
        configurable: !0,
        enumerable: !0,
        writable: !0,
      });
}
function wb(a, i, u, r, c) {
  if (typeof r == "string") uy(u, sa(r), Dt(a, i, c));
  else {
    let f = Dt(a, i, r);
    switch (typeof f) {
      case "string":
        uy(u, f, Dt(a, i, c));
        break;
      case "symbol":
        Cb(f) && (u[f] = Dt(a, i, c));
        break;
      default:
        throw new ra(r);
    }
  }
}
function vp(a, i, u, r) {
  let c = u.k;
  if (c.length > 0)
    for (let f = 0, h = u.v, g = c.length; f < g; f++) wb(a, i, r, c[f], h[f]);
  return r;
}
function Db(a, i, u) {
  let r = Wt(a, u.i, u.t === 10 ? {} : Object.create(null));
  return (vp(a, i, u.p, r), pp(r, u.o), r);
}
function Lb(a, i) {
  return Wt(a, i.i, new Date(i.s));
}
function Ub(a, i) {
  if (a.base.features & 32) {
    let u = sa(i.c);
    if (u.length > bb) throw new ra(i);
    return Wt(a, i.i, new RegExp(u, i.m));
  }
  throw new cp(i);
}
function Nb(a, i, u) {
  let r = Wt(a, u.i, new Set());
  for (let c = 0, f = u.a, h = f.length; c < h; c++) r.add(Dt(a, i, f[c]));
  return r;
}
function Bb(a, i, u) {
  let r = Wt(a, u.i, new Map());
  for (let c = 0, f = u.e.k, h = u.e.v, g = f.length; c < g; c++)
    r.set(Dt(a, i, f[c]), Dt(a, i, h[c]));
  return r;
}
function jb(a, i) {
  if (i.s.length > vb) throw new ra(i);
  return Wt(a, i.i, fp(sa(i.s)));
}
function Hb(a, i, u) {
  var r;
  let c = gb(u.c),
    f = Dt(a, i, u.f),
    h = (r = u.b) != null ? r : 0;
  if (h < 0 || h > f.byteLength) throw new ra(u);
  return Wt(a, u.i, new c(f, h, u.l));
}
function qb(a, i, u) {
  var r;
  let c = Dt(a, i, u.f),
    f = (r = u.b) != null ? r : 0;
  if (f < 0 || f > c.byteLength) throw new ra(u);
  return Wt(a, u.i, new DataView(c, f, u.l));
}
function Sp(a, i, u, r) {
  if (u.p) {
    let c = vp(a, i, u.p, {});
    Object.defineProperties(r, Object.getOwnPropertyDescriptors(c));
  }
  return r;
}
function Yb(a, i, u) {
  let r = Wt(a, u.i, new AggregateError([], sa(u.m)));
  return Sp(a, i, u, r);
}
function Gb(a, i, u) {
  let r = mc(u, jS, u.s),
    c = Wt(a, u.i, new r(sa(u.m)));
  return Sp(a, i, u, c);
}
function Xb(a, i, u) {
  let r = As(),
    c = Wt(a, u.i, r.p),
    f = Dt(a, i, u.f);
  return (u.s ? r.s(f) : r.f(f), c);
}
function Vb(a, i, u) {
  return Wt(a, u.i, Object(Dt(a, i, u.f)));
}
function Qb(a, i, u) {
  let r = a.base.plugins;
  if (r) {
    let c = sa(u.c);
    for (let f = 0, h = r.length; f < h; f++) {
      let g = r[f];
      if (g.tag === c)
        return Wt(a, u.i, g.deserialize(u.s, new Tb(a, i), { id: u.i }));
    }
  }
  throw new R1(u.c);
}
function Zb(a, i) {
  return Wt(a, i.i, Wt(a, i.s, As()).p);
}
function Kb(a, i, u) {
  let r = a.base.refs.get(u.i);
  if (r) return (r.s(Dt(a, i, u.a[1])), b);
  throw new Gi("Promise");
}
function Jb(a, i, u) {
  let r = a.base.refs.get(u.i);
  if (r) return (r.f(Dt(a, i, u.a[1])), b);
  throw new Gi("Promise");
}
function kb(a, i, u) {
  Dt(a, i, u.a[0]);
  let r = Dt(a, i, u.a[1]);
  return j1(r);
}
function Fb(a, i, u) {
  Dt(a, i, u.a[0]);
  let r = Dt(a, i, u.a[1]);
  return V1(r);
}
function Pb(a, i, u) {
  let r = Wt(a, u.i, Na()),
    c = u.a,
    f = c.length;
  if (f) for (let h = 0; h < f; h++) Dt(a, i, c[h]);
  return r;
}
function Ib(a, i, u) {
  let r = a.base.refs.get(u.i);
  if (r && Ms(r)) return (r.next(Dt(a, i, u.f)), b);
  throw new Gi("Stream");
}
function Wb(a, i, u) {
  let r = a.base.refs.get(u.i);
  if (r && Ms(r)) return (r.throw(Dt(a, i, u.f)), b);
  throw new Gi("Stream");
}
function $b(a, i, u) {
  let r = a.base.refs.get(u.i);
  if (r && Ms(r)) return (r.return(Dt(a, i, u.f)), b);
  throw new Gi("Stream");
}
function t_(a, i, u) {
  return (Dt(a, i, u.f), b);
}
function e_(a, i, u) {
  return (Dt(a, i, u.a[1]), b);
}
function n_(a, i, u) {
  let r = Wt(a, u.i, dp([], u.s, u.l));
  for (let c = 0, f = u.a.length; c < f; c++) r.v[c] = Dt(a, i, u.a[c]);
  return r;
}
function Dt(a, i, u) {
  if (i > a.base.depthLimit) throw new M1(a.base.depthLimit);
  switch (((i += 1), u.t)) {
    case 2:
      return mc(u, NS, u.s);
    case 0:
      return Number(u.s);
    case 1:
      return sa(String(u.s));
    case 3:
      if (String(u.s).length > Sb) throw new ra(u);
      return BigInt(u.s);
    case 4:
      return a.base.refs.get(u.i);
    case 18:
      return Mb(a, u);
    case 9:
      return zb(a, i, u);
    case 10:
    case 11:
      return Db(a, i, u);
    case 5:
      return Lb(a, u);
    case 6:
      return Ub(a, u);
    case 7:
      return Nb(a, i, u);
    case 8:
      return Bb(a, i, u);
    case 19:
      return jb(a, u);
    case 16:
    case 15:
      return Hb(a, i, u);
    case 20:
      return qb(a, i, u);
    case 14:
      return Yb(a, i, u);
    case 13:
      return Gb(a, i, u);
    case 12:
      return Xb(a, i, u);
    case 17:
      return mc(u, US, u.s);
    case 21:
      return Vb(a, i, u);
    case 25:
      return Qb(a, i, u);
    case 22:
      return Zb(a, u);
    case 23:
      return Kb(a, i, u);
    case 24:
      return Jb(a, i, u);
    case 28:
      return kb(a, i, u);
    case 30:
      return Fb(a, i, u);
    case 31:
      return Pb(a, i, u);
    case 32:
      return Ib(a, i, u);
    case 33:
      return Wb(a, i, u);
    case 34:
      return $b(a, i, u);
    case 27:
      return t_(a, i, u);
    case 29:
      return e_(a, i, u);
    case 35:
      return n_(a, i, u);
    default:
      throw new cp(u);
  }
}
function a_(a, i) {
  try {
    return Dt(a, 0, i);
  } catch (u) {
    throw new E1(u);
  }
}
var l_ = () => T;
l_.toString();
function sy(a, i) {
  let u = yp(i.plugins),
    r = Rb({
      plugins: u,
      refs: i.refs,
      features: i.features,
      disabledFeatures: i.disabledFeatures,
      depthLimit: i.depthLimit,
    });
  return a_(r, a);
}
async function i_(a, i = {}) {
  let u = yp(i.plugins),
    r = W1(1, { plugins: u, disabledFeatures: i.disabledFeatures });
  return {
    t: await yb(r, a),
    f: r.base.features,
    m: Array.from(r.base.marked),
  };
}
function u_(a) {
  return {
    tag: "$TSR/t/" + a.key,
    test: a.test,
    parse: {
      sync(i, u, r) {
        return { v: u.parse(a.toSerializable(i)) };
      },
      async async(i, u, r) {
        return { v: await u.parse(a.toSerializable(i)) };
      },
      stream(i, u, r) {
        return { v: u.parse(a.toSerializable(i)) };
      },
    },
    serialize: void 0,
    deserialize(i, u, r) {
      return a.fromSerializable(u.deserialize(i.v));
    },
  };
}
var s_ = class {
  constructor(a, i) {
    ((this.stream = a), (this.hint = i?.hint ?? "binary"));
  }
};
const bs = globalThis.Buffer,
  bp = !!bs && typeof bs.from == "function";
function _p(a) {
  if (a.length === 0) return "";
  if (bp) return bs.from(a).toString("base64");
  const i = 32768,
    u = [];
  for (let r = 0; r < a.length; r += i) {
    const c = a.subarray(r, r + i);
    u.push(String.fromCharCode.apply(null, c));
  }
  return btoa(u.join(""));
}
function Ep(a) {
  if (a.length === 0) return new Uint8Array(0);
  if (bp) {
    const r = bs.from(a, "base64");
    return new Uint8Array(r.buffer, r.byteOffset, r.byteLength);
  }
  const i = atob(a),
    u = new Uint8Array(i.length);
  for (let r = 0; r < i.length; r++) u[r] = i.charCodeAt(r);
  return u;
}
const wi = Object.create(null),
  Di = Object.create(null),
  r_ = (a) =>
    new ReadableStream({
      start(i) {
        a.on({
          next(u) {
            try {
              i.enqueue(Ep(u));
            } catch {}
          },
          throw(u) {
            i.error(u);
          },
          return() {
            try {
              i.close();
            } catch {}
          },
        });
      },
    }),
  o_ = new TextEncoder(),
  c_ = (a) =>
    new ReadableStream({
      start(i) {
        a.on({
          next(u) {
            try {
              typeof u == "string"
                ? i.enqueue(o_.encode(u))
                : i.enqueue(Ep(u.$b64));
            } catch {}
          },
          throw(u) {
            i.error(u);
          },
          return() {
            try {
              i.close();
            } catch {}
          },
        });
      },
    }),
  f_ =
    "(s=>new ReadableStream({start(c){s.on({next(b){try{const d=atob(b),a=new Uint8Array(d.length);for(let i=0;i<d.length;i++)a[i]=d.charCodeAt(i);c.enqueue(a)}catch(_){}},throw(e){c.error(e)},return(){try{c.close()}catch(_){}}})}}))",
  d_ =
    "(s=>{const e=new TextEncoder();return new ReadableStream({start(c){s.on({next(v){try{if(typeof v==='string'){c.enqueue(e.encode(v))}else{const d=atob(v.$b64),a=new Uint8Array(d.length);for(let i=0;i<d.length;i++)a[i]=d.charCodeAt(i);c.enqueue(a)}}catch(_){}},throw(x){c.error(x)},return(){try{c.close()}catch(_){}}})}})})";
function ry(a) {
  const i = Na(),
    u = a.getReader();
  return (
    (async () => {
      try {
        for (;;) {
          const { done: r, value: c } = await u.read();
          if (r) {
            i.return(void 0);
            break;
          }
          i.next(_p(c));
        }
      } catch (r) {
        i.throw(r);
      } finally {
        u.releaseLock();
      }
    })(),
    i
  );
}
function oy(a) {
  const i = Na(),
    u = a.getReader(),
    r = new TextDecoder("utf-8", { fatal: !0 });
  return (
    (async () => {
      try {
        for (;;) {
          const { done: c, value: f } = await u.read();
          if (c) {
            try {
              const h = r.decode();
              h.length > 0 && i.next(h);
            } catch {}
            i.return(void 0);
            break;
          }
          try {
            const h = r.decode(f, { stream: !0 });
            h.length > 0 && i.next(h);
          } catch {
            i.next({ $b64: _p(f) });
          }
        }
      } catch (c) {
        i.throw(c);
      } finally {
        u.releaseLock();
      }
    })(),
    i
  );
}
const h_ = {
  tag: "tss/RawStream",
  extends: [
    {
      tag: "tss/RawStreamFactory",
      test(a) {
        return a === wi;
      },
      parse: {
        sync(a, i, u) {
          return {};
        },
        async async(a, i, u) {
          return {};
        },
        stream(a, i, u) {
          return {};
        },
      },
      serialize(a, i, u) {
        return f_;
      },
      deserialize(a, i, u) {
        return wi;
      },
    },
    {
      tag: "tss/RawStreamFactoryText",
      test(a) {
        return a === Di;
      },
      parse: {
        sync(a, i, u) {
          return {};
        },
        async async(a, i, u) {
          return {};
        },
        stream(a, i, u) {
          return {};
        },
      },
      serialize(a, i, u) {
        return d_;
      },
      deserialize(a, i, u) {
        return Di;
      },
    },
  ],
  test(a) {
    return a instanceof s_;
  },
  parse: {
    sync(a, i, u) {
      const r = a.hint === "text" ? Di : wi;
      return {
        hint: i.parse(a.hint),
        factory: i.parse(r),
        stream: i.parse(Na()),
      };
    },
    async async(a, i, u) {
      const r = a.hint === "text" ? Di : wi,
        c = a.hint === "text" ? oy(a.stream) : ry(a.stream);
      return {
        hint: await i.parse(a.hint),
        factory: await i.parse(r),
        stream: await i.parse(c),
      };
    },
    stream(a, i, u) {
      const r = a.hint === "text" ? Di : wi,
        c = a.hint === "text" ? oy(a.stream) : ry(a.stream);
      return { hint: i.parse(a.hint), factory: i.parse(r), stream: i.parse(c) };
    },
  },
  serialize(a, i, u) {
    return "(" + i.serialize(a.factory) + ")(" + i.serialize(a.stream) + ")";
  },
  deserialize(a, i, u) {
    const r = i.deserialize(a.stream);
    return i.deserialize(a.hint) === "text" ? c_(r) : r_(r);
  },
};
function m_(a) {
  return {
    tag: "tss/RawStream",
    test: () => !1,
    parse: {},
    serialize() {
      throw new Error(
        "RawStreamDeserializePlugin.serialize should not be called. Client only deserializes.",
      );
    },
    deserialize(i, u, r) {
      return a(
        typeof u?.deserialize == "function"
          ? u.deserialize(i.streamId)
          : i.streamId,
      );
    },
  };
}
const y_ = {
  tag: "$TSR/Error",
  test(a) {
    return a instanceof Error;
  },
  parse: {
    sync(a, i) {
      return { message: i.parse(a.message) };
    },
    async async(a, i) {
      return { message: await i.parse(a.message) };
    },
    stream(a, i) {
      return { message: i.parse(a.message) };
    },
  },
  serialize(a, i) {
    return "new Error(" + i.serialize(a.message) + ")";
  },
  deserialize(a, i) {
    return new Error(i.deserialize(a.message));
  },
};
var ta = {},
  Rp = (a) =>
    new ReadableStream({
      start: (i) => {
        a.on({
          next: (u) => {
            try {
              i.enqueue(u);
            } catch {}
          },
          throw: (u) => {
            i.error(u);
          },
          return: () => {
            try {
              i.close();
            } catch {}
          },
        });
      },
    }),
  p_ = {
    tag: "seroval-plugins/web/ReadableStreamFactory",
    test(a) {
      return a === ta;
    },
    parse: {
      sync() {
        return ta;
      },
      async async() {
        return await Promise.resolve(ta);
      },
      stream() {
        return ta;
      },
    },
    serialize() {
      return Rp.toString();
    },
    deserialize() {
      return ta;
    },
  };
function cy(a) {
  let i = Na(),
    u = a.getReader();
  async function r() {
    try {
      let c = await u.read();
      c.done ? i.return(c.value) : (i.next(c.value), await r());
    } catch (c) {
      i.throw(c);
    }
  }
  return (r().catch(() => {}), i);
}
var g_ = {
    tag: "seroval/plugins/web/ReadableStream",
    extends: [p_],
    test(a) {
      return typeof ReadableStream > "u" ? !1 : a instanceof ReadableStream;
    },
    parse: {
      sync(a, i) {
        return { factory: i.parse(ta), stream: i.parse(Na()) };
      },
      async async(a, i) {
        return { factory: await i.parse(ta), stream: await i.parse(cy(a)) };
      },
      stream(a, i) {
        return { factory: i.parse(ta), stream: i.parse(cy(a)) };
      },
    },
    serialize(a, i) {
      return "(" + i.serialize(a.factory) + ")(" + i.serialize(a.stream) + ")";
    },
    deserialize(a, i) {
      let u = i.deserialize(a.stream);
      return Rp(u);
    },
  },
  v_ = g_;
const S_ = [y_, h_, v_];
function b_() {
  return [...(zy()?.serializationAdapters?.map(u_) ?? []), ...S_];
}
var fy = new TextDecoder(),
  __ = new Uint8Array(0),
  dy = 16 * 1024 * 1024,
  hy = 32 * 1024 * 1024,
  my = 1024,
  yy = 1e5;
function E_(a) {
  const i = new Map(),
    u = new Map(),
    r = new Set();
  let c = !1,
    f = null,
    h = 0,
    g;
  const y = new ReadableStream({
    start(p) {
      g = p;
    },
    cancel() {
      c = !0;
      try {
        f?.cancel();
      } catch {}
      (i.forEach((p) => {
        try {
          p.error(new Error("Framed response cancelled"));
        } catch {}
      }),
        i.clear(),
        u.clear(),
        r.clear());
    },
  });
  function m(p) {
    const E = u.get(p);
    if (E) return E;
    if (r.has(p))
      return new ReadableStream({
        start(z) {
          z.close();
        },
      });
    if (u.size >= my)
      throw new Error(`Too many raw streams in framed response (max ${my})`);
    const _ = new ReadableStream({
      start(z) {
        i.set(p, z);
      },
      cancel() {
        (r.add(p), i.delete(p), u.delete(p));
      },
    });
    return (u.set(p, _), _);
  }
  function S(p) {
    return (m(p), i.get(p));
  }
  return (
    (async () => {
      const p = a.getReader();
      f = p;
      const E = [];
      let _ = 0;
      function z() {
        if (_ < 9) return null;
        const x = E[0];
        if (x.length >= 9)
          return {
            type: x[0],
            streamId: ((x[1] << 24) | (x[2] << 16) | (x[3] << 8) | x[4]) >>> 0,
            length: ((x[5] << 24) | (x[6] << 16) | (x[7] << 8) | x[8]) >>> 0,
          };
        const C = new Uint8Array(9);
        let Y = 0,
          Q = 9;
        for (let H = 0; H < E.length && Q > 0; H++) {
          const W = E[H],
            F = Math.min(W.length, Q);
          (C.set(W.subarray(0, F), Y), (Y += F), (Q -= F));
        }
        return {
          type: C[0],
          streamId: ((C[1] << 24) | (C[2] << 16) | (C[3] << 8) | C[4]) >>> 0,
          length: ((C[5] << 24) | (C[6] << 16) | (C[7] << 8) | C[8]) >>> 0,
        };
      }
      function L(x) {
        if (x === 0) return __;
        const C = new Uint8Array(x);
        let Y = 0,
          Q = x;
        for (; Q > 0 && E.length > 0; ) {
          const H = E[0];
          if (!H) break;
          const W = Math.min(H.length, Q);
          (C.set(H.subarray(0, W), Y),
            (Y += W),
            (Q -= W),
            W === H.length ? E.shift() : (E[0] = H.subarray(W)));
        }
        return ((_ -= x), C);
      }
      try {
        for (;;) {
          const { done: x, value: C } = await p.read();
          if (c || x) break;
          if (C) {
            if (_ + C.length > hy)
              throw new Error(`Framed response buffer exceeded ${hy} bytes`);
            for (E.push(C), _ += C.length; ; ) {
              const Y = z();
              if (!Y) break;
              const { type: Q, streamId: H, length: W } = Y;
              if (
                Q !== En.JSON &&
                Q !== En.CHUNK &&
                Q !== En.END &&
                Q !== En.ERROR
              )
                throw new Error(`Unknown frame type: ${Q}`);
              if (Q === En.JSON) {
                if (H !== 0)
                  throw new Error("Invalid JSON frame streamId (expected 0)");
              } else if (H === 0)
                throw new Error(
                  "Invalid raw frame streamId (expected non-zero)",
                );
              if (W > dy)
                throw new Error(
                  `Frame payload too large: ${W} bytes (max ${dy})`,
                );
              const F = 9 + W;
              if (_ < F) break;
              if (++h > yy)
                throw new Error(
                  `Too many frames in framed response (max ${yy})`,
                );
              L(9);
              const G = L(W);
              switch (Q) {
                case En.JSON:
                  try {
                    g.enqueue(fy.decode(G));
                  } catch {}
                  break;
                case En.CHUNK: {
                  const Z = S(H);
                  Z && Z.enqueue(G);
                  break;
                }
                case En.END: {
                  const Z = S(H);
                  if ((r.add(H), Z)) {
                    try {
                      Z.close();
                    } catch {}
                    i.delete(H);
                  }
                  break;
                }
                case En.ERROR: {
                  const Z = S(H);
                  if ((r.add(H), Z)) {
                    const P = fy.decode(G);
                    (Z.error(new Error(P)), i.delete(H));
                  }
                  break;
                }
              }
            }
          }
        }
        if (_ !== 0)
          throw new Error("Incomplete frame at end of framed response");
        try {
          g.close();
        } catch {}
        (i.forEach((x) => {
          try {
            x.close();
          } catch {}
        }),
          i.clear());
      } catch (x) {
        try {
          g.error(x);
        } catch {}
        (i.forEach((C) => {
          try {
            C.error(x);
          } catch {}
        }),
          i.clear());
      } finally {
        try {
          p.releaseLock();
        } catch {}
        f = null;
      }
    })(),
    { getOrCreateStream: m, jsonChunks: y }
  );
}
var qi = null;
async function yc(a) {
  a.length > 0 && (await Promise.allSettled(a));
}
var R_ = Object.prototype.hasOwnProperty;
function Tp(a) {
  for (const i in a) if (R_.call(a, i)) return !0;
  return !1;
}
async function T_(a, i, u) {
  qi || (qi = b_());
  const r = i[0],
    c = r.fetch ?? u,
    f = r.data instanceof FormData ? "formData" : "payload",
    h = r.headers ? new Headers(r.headers) : new Headers();
  if (
    (h.set("x-tsr-serverFn", "true"),
    f === "payload" &&
      h.set("accept", `${_0}, application/x-ndjson, application/json`),
    r.method === "GET")
  ) {
    if (f === "formData")
      throw new Error("FormData is not supported with GET requests");
    const y = await xp(r);
    if (y !== void 0) {
      const m = Hy({ payload: y });
      a.includes("?") ? (a += `&${m}`) : (a += `?${m}`);
    }
  }
  let g;
  if (r.method === "POST") {
    const y = await x_(r);
    (y?.contentType && h.set("content-type", y.contentType), (g = y?.body));
  }
  return await A_(async () =>
    c(a, { method: r.method, headers: h, signal: r.signal, body: g }),
  );
}
async function xp(a) {
  let i = !1;
  const u = {};
  if (
    (a.data !== void 0 && ((i = !0), (u.data = a.data)),
    a.context && Tp(a.context) && ((i = !0), (u.context = a.context)),
    i)
  )
    return Ap(u);
}
async function Ap(a) {
  return JSON.stringify(await Promise.resolve(i_(a, { plugins: qi })));
}
async function x_(a) {
  if (a.data instanceof FormData) {
    let u;
    return (
      a.context && Tp(a.context) && (u = await Ap(a.context)),
      u !== void 0 && a.data.set(b0, u),
      { body: a.data }
    );
  }
  const i = await xp(a);
  if (i) return { body: i, contentType: "application/json" };
}
async function A_(a) {
  let i;
  try {
    i = await a();
  } catch (r) {
    if (r instanceof Response) i = r;
    else throw (console.log(r), r);
  }
  if (i.headers.get("x-tss-raw") === "true") return i;
  const u = i.headers.get("content-type");
  if ((u || Ee(), i.headers.get("x-tss-serialized"))) {
    let r;
    if (u.includes("application/x-tss-framed")) {
      if ((T0(u), !i.body))
        throw new Error("No response body for framed response");
      const { getOrCreateStream: c, jsonChunks: f } = E_(i.body),
        h = [m_(c), ...(qi || [])],
        g = new Map();
      r = await M_({
        jsonStream: f,
        onMessage: (y) => sy(y, { refs: g, plugins: h }),
        onError(y, m) {
          console.error(y, m);
        },
      });
    } else if (u.includes("application/json")) {
      const c = await i.json(),
        f = [];
      ((r = sy(c, { plugins: qi })), await yc(f));
    }
    if ((r || Ee(), r instanceof Error)) throw r;
    return r;
  }
  if (u.includes("application/json")) {
    const r = await i.json(),
      c = iS(r);
    if (c) throw c;
    if (ne(r)) throw r;
    return r;
  }
  if (!i.ok) throw new Error(await i.text());
  return i;
}
async function M_({ jsonStream: a, onMessage: i, onError: u }) {
  const r = a.getReader(),
    { value: c, done: f } = await r.read();
  if (f || !c) throw new Error("Stream ended before first object");
  const h = JSON.parse(c);
  let g = !1;
  const y = (async () => {
    try {
      for (;;) {
        const { value: p, done: E } = await r.read();
        if (E) break;
        if (p)
          try {
            const _ = [];
            try {
              i(JSON.parse(p));
            } finally {
            }
            await yc(_);
          } catch (_) {
            u?.(`Invalid JSON: ${p}`, _);
          }
      }
    } catch (p) {
      g || u?.("Stream processing error:", p);
    }
  })();
  let m;
  const S = [];
  try {
    m = i(h);
  } catch (p) {
    throw ((g = !0), r.cancel().catch(() => {}), p);
  }
  return (
    await yc(S),
    Promise.resolve(m).catch(() => {
      ((g = !0), r.cancel().catch(() => {}));
    }),
    y.finally(() => {
      try {
        r.releaseLock();
      } catch {}
    }),
    m
  );
}
function z_(a) {
  const i = "/_serverFn/" + a;
  return Object.assign(
    (...c) => {
      const f = zy()?.serverFns?.fetch;
      return T_(i, c, f ?? fetch);
    },
    { url: i, serverFnMeta: { id: a }, [fc]: !0 },
  );
}
var O_ = {
  key: "$TSS/serverfn",
  test: (a) => (typeof a != "function" || !(fc in a) ? !1 : !!a[fc]),
  toSerializable: ({ serverFnMeta: a }) => ({ functionId: a.id }),
  fromSerializable: ({ functionId: a }) => z_(a),
};
function py(a) {
  return a.replaceAll("\0", "/").replaceAll("�", "/");
}
function C_(a, i) {
  ((a.id = i.i),
    (a.__beforeLoadContext = i.b),
    (a.loaderData = i.l),
    (a.status = i.s),
    (a.ssr = i.ssr),
    (a.updatedAt = i.u),
    (a.error = i.e),
    i.g !== void 0 && (a.globalNotFound = i.g));
}
async function w_(a) {
  window.$_TSR || Ee();
  const i = a.options.serializationAdapters;
  if (i?.length) {
    const x = new Map();
    (i.forEach((C) => {
      x.set(C.key, C.fromSerializable);
    }),
      (window.$_TSR.t = x),
      window.$_TSR.buffer.forEach((C) => C()));
  }
  ((window.$_TSR.initialized = !0), window.$_TSR.router || Ee());
  const u = window.$_TSR.router;
  (u.matches.forEach((x) => {
    x.i = py(x.i);
  }),
    u.lastMatchId && (u.lastMatchId = py(u.lastMatchId)));
  const { manifest: r, dehydratedData: c, lastMatchId: f } = u;
  a.ssr = { manifest: r };
  const h = document.querySelector('meta[property="csp-nonce"]')?.content;
  a.options.ssr = { nonce: h };
  const g = a.matchRoutes(a.stores.location.get()),
    y = Promise.all(
      g.map((x) => a.loadRouteChunk(a.looseRoutesById[x.routeId])),
    );
  function m(x) {
    const C =
      a.looseRoutesById[x.routeId].options.pendingMinMs ??
      a.options.defaultPendingMinMs;
    if (C) {
      const Y = Ua();
      ((x._nonReactive.minPendingPromise = Y),
        (x._forcePending = !0),
        setTimeout(() => {
          (Y.resolve(),
            a.updateMatch(
              x.id,
              (Q) => (
                (Q._nonReactive.minPendingPromise = void 0),
                { ...Q, _forcePending: void 0 }
              ),
            ));
        }, C));
    }
  }
  function S(x) {
    const C = a.looseRoutesById[x.routeId];
    C && (C.options.ssr = x.ssr);
  }
  let p;
  (g.forEach((x) => {
    const C = u.matches.find((Y) => Y.i === x.id);
    if (!C) {
      ((x._nonReactive.dehydrated = !1), (x.ssr = !1), S(x));
      return;
    }
    (C_(x, C),
      S(x),
      (x._nonReactive.dehydrated = x.ssr !== !1),
      (x.ssr === "data-only" || x.ssr === !1) &&
        p === void 0 &&
        ((p = x.index), m(x)));
  }),
    a.stores.setMatches(g),
    await a.options.hydrate?.(c));
  const E = a.stores.matches.get(),
    _ = a.stores.location.get();
  await Promise.all(
    E.map(async (x) => {
      try {
        const C = a.looseRoutesById[x.routeId],
          Y = E[x.index - 1]?.context ?? a.options.context;
        if (C.options.context) {
          const F = {
            deps: x.loaderDeps,
            params: x.params,
            context: Y ?? {},
            location: _,
            navigate: (G) => a.navigate({ ...G, _fromLocation: _ }),
            buildLocation: a.buildLocation,
            cause: x.cause,
            abortController: x.abortController,
            preload: !1,
            matches: g,
            routeId: C.id,
          };
          x.__routeContext = C.options.context(F) ?? void 0;
        }
        x.context = { ...Y, ...x.__routeContext, ...x.__beforeLoadContext };
        const Q = {
            ssr: a.options.ssr,
            matches: E,
            match: x,
            params: x.params,
            loaderData: x.loaderData,
          },
          H = await C.options.head?.(Q),
          W = await C.options.scripts?.(Q);
        ((x.meta = H?.meta),
          (x.links = H?.links),
          (x.headScripts = H?.scripts),
          (x.styles = H?.styles),
          (x.scripts = W));
      } catch (C) {
        if (ne(C))
          ((x.error = { isNotFound: !0 }),
            console.error(
              `NotFound error during hydration for routeId: ${x.routeId}`,
              C,
            ));
        else
          throw (
            (x.error = C),
            console.error(`Error during hydration for route ${x.routeId}:`, C),
            C
          );
      }
    }),
  );
  const z = g[g.length - 1].id !== f;
  if (!g.some((x) => x.ssr === !1) && !z)
    return (
      g.forEach((x) => {
        x._nonReactive.dehydrated = void 0;
      }),
      a.stores.resolvedLocation.set(a.stores.location.get()),
      y
    );
  const L = Promise.resolve()
    .then(() => a.load())
    .catch((x) => {
      console.error("Error during router hydration:", x);
    });
  if (z) {
    const x = g[1];
    (x || Ee(),
      m(x),
      (x._displayPending = !0),
      (x._nonReactive.displayPendingPromise = L),
      L.then(() => {
        a.batch(() => {
          (a.stores.status.get() === "pending" &&
            (a.stores.status.set("idle"),
            a.stores.resolvedLocation.set(a.stores.location.get())),
            a.updateMatch(x.id, (C) => ({
              ...C,
              _displayPending: void 0,
              displayPendingPromise: void 0,
            })));
        });
      }));
  }
  return y;
}
var _s = nt.use,
  Ui = typeof window < "u" ? nt.useLayoutEffect : nt.useEffect;
function lc(a) {
  const i = nt.useRef({ value: a, prev: null }),
    u = i.current.value;
  return (a !== u && (i.current = { value: a, prev: u }), i.current.prev);
}
function D_(a, i, u = {}, r = {}) {
  nt.useEffect(() => {
    if (!a.current || r.disabled || typeof IntersectionObserver != "function")
      return;
    const c = new IntersectionObserver(([f]) => {
      i(f);
    }, u);
    return (
      c.observe(a.current),
      () => {
        c.disconnect();
      }
    );
  }, [i, u, r.disabled, a]);
}
function L_(a) {
  const i = nt.useRef(null);
  return (nt.useImperativeHandle(a, () => i.current, []), i);
}
function U_({ promise: a }) {
  if (_s) return _s(a);
  const i = AS(a);
  if (i[$e].status === "pending") throw i;
  if (i[$e].status === "error") throw i[$e].error;
  return i[$e].data;
}
function N_(a) {
  const i = K.jsx(B_, { ...a });
  return a.fallback
    ? K.jsx(nt.Suspense, { fallback: a.fallback, children: i })
    : i;
}
function B_(a) {
  const i = U_(a);
  return a.children(i);
}
function xc(a) {
  const i = a.errorComponent ?? Ac;
  return K.jsx(j_, {
    getResetKey: a.getResetKey,
    onCatch: a.onCatch,
    children: ({ error: u, reset: r }) =>
      u ? nt.createElement(i, { error: u, reset: r }) : a.children,
  });
}
var j_ = class extends nt.Component {
  constructor(...a) {
    (super(...a), (this.state = { error: null }));
  }
  static getDerivedStateFromProps(a, i) {
    const u = a.getResetKey();
    return i.error && i.resetKey !== u
      ? { resetKey: u, error: null }
      : { resetKey: u };
  }
  static getDerivedStateFromError(a) {
    return { error: a };
  }
  reset() {
    this.setState({ error: null });
  }
  componentDidCatch(a, i) {
    this.props.onCatch && this.props.onCatch(a, i);
  }
  render() {
    return this.props.children({
      error: this.state.error,
      reset: () => {
        this.reset();
      },
    });
  }
};
function Ac({ error: a }) {
  const [i, u] = nt.useState(!1);
  return K.jsxs("div", {
    style: { padding: ".5rem", maxWidth: "100%" },
    children: [
      K.jsxs("div", {
        style: { display: "flex", alignItems: "center", gap: ".5rem" },
        children: [
          K.jsx("strong", {
            style: { fontSize: "1rem" },
            children: "Something went wrong!",
          }),
          K.jsx("button", {
            style: {
              appearance: "none",
              fontSize: ".6em",
              border: "1px solid currentColor",
              padding: ".1rem .2rem",
              fontWeight: "bold",
              borderRadius: ".25rem",
            },
            onClick: () => u((r) => !r),
            children: i ? "Hide Error" : "Show Error",
          }),
        ],
      }),
      K.jsx("div", { style: { height: ".25rem" } }),
      i
        ? K.jsx("div", {
            children: K.jsx("pre", {
              style: {
                fontSize: ".7em",
                border: "1px solid red",
                borderRadius: ".25rem",
                padding: ".3rem",
                color: "red",
                overflow: "auto",
              },
              children: a.message
                ? K.jsx("code", { children: a.message })
                : null,
            }),
          })
        : null,
    ],
  });
}
function H_({ children: a, fallback: i = null }) {
  return Mc()
    ? K.jsx(Ol.Fragment, { children: a })
    : K.jsx(Ol.Fragment, { children: i });
}
function Mc() {
  return Ol.useSyncExternalStore(
    q_,
    () => !0,
    () => !1,
  );
}
function q_() {
  return () => {};
}
var Mp = nt.createContext(null);
function oe(a) {
  return nt.useContext(Mp);
}
var Cs = nt.createContext(void 0),
  Y_ = nt.createContext(void 0),
  Ht = ((a) => (
    (a[(a.None = 0)] = "None"),
    (a[(a.Mutable = 1)] = "Mutable"),
    (a[(a.Watching = 2)] = "Watching"),
    (a[(a.RecursedCheck = 4)] = "RecursedCheck"),
    (a[(a.Recursed = 8)] = "Recursed"),
    (a[(a.Dirty = 16)] = "Dirty"),
    (a[(a.Pending = 32)] = "Pending"),
    a
  ))(Ht || {});
function G_({ update: a, notify: i, unwatched: u }) {
  return {
    link: r,
    unlink: c,
    propagate: f,
    checkDirty: h,
    shallowPropagate: g,
  };
  function r(m, S, p) {
    const E = S.depsTail;
    if (E !== void 0 && E.dep === m) return;
    const _ = E !== void 0 ? E.nextDep : S.deps;
    if (_ !== void 0 && _.dep === m) {
      ((_.version = p), (S.depsTail = _));
      return;
    }
    const z = m.subsTail;
    if (z !== void 0 && z.version === p && z.sub === S) return;
    const L =
      (S.depsTail =
      m.subsTail =
        {
          version: p,
          dep: m,
          sub: S,
          prevDep: E,
          nextDep: _,
          prevSub: z,
          nextSub: void 0,
        });
    (_ !== void 0 && (_.prevDep = L),
      E !== void 0 ? (E.nextDep = L) : (S.deps = L),
      z !== void 0 ? (z.nextSub = L) : (m.subs = L));
  }
  function c(m, S = m.sub) {
    const p = m.dep,
      E = m.prevDep,
      _ = m.nextDep,
      z = m.nextSub,
      L = m.prevSub;
    return (
      _ !== void 0 ? (_.prevDep = E) : (S.depsTail = E),
      E !== void 0 ? (E.nextDep = _) : (S.deps = _),
      z !== void 0 ? (z.prevSub = L) : (p.subsTail = L),
      L !== void 0 ? (L.nextSub = z) : (p.subs = z) === void 0 && u(p),
      _
    );
  }
  function f(m) {
    let S = m.nextSub,
      p;
    t: do {
      const E = m.sub;
      let _ = E.flags;
      if (
        (_ & 60
          ? _ & 12
            ? _ & 4
              ? !(_ & 48) && y(m, E)
                ? ((E.flags = _ | 40), (_ &= 1))
                : (_ = 0)
              : (E.flags = (_ & -9) | 32)
            : (_ = 0)
          : (E.flags = _ | 32),
        _ & 2 && i(E),
        _ & 1)
      ) {
        const z = E.subs;
        if (z !== void 0) {
          const L = (m = z).nextSub;
          L !== void 0 && ((p = { value: S, prev: p }), (S = L));
          continue;
        }
      }
      if ((m = S) !== void 0) {
        S = m.nextSub;
        continue;
      }
      for (; p !== void 0; )
        if (((m = p.value), (p = p.prev), m !== void 0)) {
          S = m.nextSub;
          continue t;
        }
      break;
    } while (!0);
  }
  function h(m, S) {
    let p,
      E = 0,
      _ = !1;
    t: do {
      const z = m.dep,
        L = z.flags;
      if (S.flags & 16) _ = !0;
      else if ((L & 17) === 17) {
        if (a(z)) {
          const x = z.subs;
          (x.nextSub !== void 0 && g(x), (_ = !0));
        }
      } else if ((L & 33) === 33) {
        ((m.nextSub !== void 0 || m.prevSub !== void 0) &&
          (p = { value: m, prev: p }),
          (m = z.deps),
          (S = z),
          ++E);
        continue;
      }
      if (!_) {
        const x = m.nextDep;
        if (x !== void 0) {
          m = x;
          continue;
        }
      }
      for (; E--; ) {
        const x = S.subs,
          C = x.nextSub !== void 0;
        if ((C ? ((m = p.value), (p = p.prev)) : (m = x), _)) {
          if (a(S)) {
            (C && g(x), (S = m.sub));
            continue;
          }
          _ = !1;
        } else S.flags &= -33;
        S = m.sub;
        const Y = m.nextDep;
        if (Y !== void 0) {
          m = Y;
          continue t;
        }
      }
      return _;
    } while (!0);
  }
  function g(m) {
    do {
      const S = m.sub,
        p = S.flags;
      (p & 48) === 32 && ((S.flags = p | 16), (p & 6) === 2 && i(S));
    } while ((m = m.nextSub) !== void 0);
  }
  function y(m, S) {
    let p = S.depsTail;
    for (; p !== void 0; ) {
      if (p === m) return !0;
      p = p.prevDep;
    }
    return !1;
  }
}
function X_(a, i, u) {
  const r = typeof a == "object",
    c = r ? a : void 0;
  return {
    next: (r ? a.next : a)?.bind(c),
    error: (r ? a.error : i)?.bind(c),
    complete: (r ? a.complete : u)?.bind(c),
  };
}
const pc = [];
let ys = 0;
const {
  link: gy,
  unlink: V_,
  propagate: Q_,
  checkDirty: zp,
  shallowPropagate: vy,
} = G_({
  update(a) {
    return a._update();
  },
  notify(a) {
    ((pc[gc++] = a), (a.flags &= ~Ht.Watching));
  },
  unwatched(a) {
    a.depsTail !== void 0 &&
      ((a.depsTail = void 0), (a.flags = Ht.Mutable | Ht.Dirty), Es(a));
  },
});
let cs = 0,
  gc = 0,
  We,
  vc = 0;
function Op(a) {
  try {
    (++vc, a());
  } finally {
    --vc || Cp();
  }
}
function Es(a) {
  const i = a.depsTail;
  let u = i !== void 0 ? i.nextDep : a.deps;
  for (; u !== void 0; ) u = V_(u, a);
}
function Cp() {
  if (!(vc > 0)) {
    for (; cs < gc; ) {
      const a = pc[cs];
      ((pc[cs++] = void 0), a.notify());
    }
    ((cs = 0), (gc = 0));
  }
}
function Sy(a, i) {
  const u = typeof a == "function",
    r = a,
    c = {
      _snapshot: u ? void 0 : a,
      subs: void 0,
      subsTail: void 0,
      deps: void 0,
      depsTail: void 0,
      flags: u ? Ht.None : Ht.Mutable,
      get() {
        return (We !== void 0 && gy(c, We, ys), c._snapshot);
      },
      subscribe(f) {
        const h = X_(f),
          g = { current: !1 },
          y = Z_(() => {
            (c.get(), g.current ? h.next?.(c._snapshot) : (g.current = !0));
          });
        return {
          unsubscribe: () => {
            y.stop();
          },
        };
      },
      _update(f) {
        const h = We,
          g = i?.compare ?? Object.is;
        if (u) ((We = c), ++ys, (c.depsTail = void 0));
        else if (f === void 0) return !1;
        u && (c.flags = Ht.Mutable | Ht.RecursedCheck);
        try {
          const y = c._snapshot,
            m = typeof f == "function" ? f(y) : f === void 0 && u ? r(y) : f;
          return y === void 0 || !g(y, m) ? ((c._snapshot = m), !0) : !1;
        } finally {
          ((We = h), u && (c.flags &= ~Ht.RecursedCheck), Es(c));
        }
      },
    };
  return (
    u
      ? ((c.flags = Ht.Mutable | Ht.Dirty),
        (c.get = function () {
          const f = c.flags;
          if (f & Ht.Dirty || (f & Ht.Pending && zp(c.deps, c))) {
            if (c._update()) {
              const h = c.subs;
              h !== void 0 && vy(h);
            }
          } else f & Ht.Pending && (c.flags = f & ~Ht.Pending);
          return (We !== void 0 && gy(c, We, ys), c._snapshot);
        }))
      : (c.set = function (f) {
          if (c._update(f)) {
            const h = c.subs;
            h !== void 0 && (Q_(h), vy(h), Cp());
          }
        }),
    c
  );
}
function Z_(a) {
  const i = () => {
      const r = We;
      ((We = u),
        ++ys,
        (u.depsTail = void 0),
        (u.flags = Ht.Watching | Ht.RecursedCheck));
      try {
        return a();
      } finally {
        ((We = r), (u.flags &= ~Ht.RecursedCheck), Es(u));
      }
    },
    u = {
      deps: void 0,
      depsTail: void 0,
      subs: void 0,
      subsTail: void 0,
      flags: Ht.Watching | Ht.RecursedCheck,
      notify() {
        const r = this.flags;
        r & Ht.Dirty || (r & Ht.Pending && zp(this.deps, this))
          ? i()
          : (this.flags = Ht.Watching);
      },
      stop() {
        ((this.flags = Ht.None), (this.depsTail = void 0), Es(this));
      },
    };
  return (i(), u);
}
var ic = { exports: {} },
  uc = {},
  sc = { exports: {} },
  rc = {};
var by;
function K_() {
  if (by) return rc;
  by = 1;
  var a = Yi();
  function i(p, E) {
    return (p === E && (p !== 0 || 1 / p === 1 / E)) || (p !== p && E !== E);
  }
  var u = typeof Object.is == "function" ? Object.is : i,
    r = a.useState,
    c = a.useEffect,
    f = a.useLayoutEffect,
    h = a.useDebugValue;
  function g(p, E) {
    var _ = E(),
      z = r({ inst: { value: _, getSnapshot: E } }),
      L = z[0].inst,
      x = z[1];
    return (
      f(
        function () {
          ((L.value = _), (L.getSnapshot = E), y(L) && x({ inst: L }));
        },
        [p, _, E],
      ),
      c(
        function () {
          return (
            y(L) && x({ inst: L }),
            p(function () {
              y(L) && x({ inst: L });
            })
          );
        },
        [p],
      ),
      h(_),
      _
    );
  }
  function y(p) {
    var E = p.getSnapshot;
    p = p.value;
    try {
      var _ = E();
      return !u(p, _);
    } catch {
      return !0;
    }
  }
  function m(p, E) {
    return E();
  }
  var S =
    typeof window > "u" ||
    typeof window.document > "u" ||
    typeof window.document.createElement > "u"
      ? m
      : g;
  return (
    (rc.useSyncExternalStore =
      a.useSyncExternalStore !== void 0 ? a.useSyncExternalStore : S),
    rc
  );
}
var _y;
function J_() {
  return (_y || ((_y = 1), (sc.exports = K_())), sc.exports);
}
var Ey;
function k_() {
  if (Ey) return uc;
  Ey = 1;
  var a = Yi(),
    i = J_();
  function u(m, S) {
    return (m === S && (m !== 0 || 1 / m === 1 / S)) || (m !== m && S !== S);
  }
  var r = typeof Object.is == "function" ? Object.is : u,
    c = i.useSyncExternalStore,
    f = a.useRef,
    h = a.useEffect,
    g = a.useMemo,
    y = a.useDebugValue;
  return (
    (uc.useSyncExternalStoreWithSelector = function (m, S, p, E, _) {
      var z = f(null);
      if (z.current === null) {
        var L = { hasValue: !1, value: null };
        z.current = L;
      } else L = z.current;
      z = g(
        function () {
          function C(F) {
            if (!Y) {
              if (((Y = !0), (Q = F), (F = E(F)), _ !== void 0 && L.hasValue)) {
                var G = L.value;
                if (_(G, F)) return (H = G);
              }
              return (H = F);
            }
            if (((G = H), r(Q, F))) return G;
            var Z = E(F);
            return _ !== void 0 && _(G, Z) ? ((Q = F), G) : ((Q = F), (H = Z));
          }
          var Y = !1,
            Q,
            H,
            W = p === void 0 ? null : p;
          return [
            function () {
              return C(S());
            },
            W === null
              ? void 0
              : function () {
                  return C(W());
                },
          ];
        },
        [S, p, E, _],
      );
      var x = c(m, z[0], z[1]);
      return (
        h(
          function () {
            ((L.hasValue = !0), (L.value = x));
          },
          [x],
        ),
        y(x),
        x
      );
    }),
    uc
  );
}
var Ry;
function F_() {
  return (Ry || ((Ry = 1), (ic.exports = k_())), ic.exports);
}
var P_ = F_();
function I_(a, i) {
  return a === i;
}
function Ft(a, i, u = I_) {
  const r = nt.useCallback(
      (h) => {
        if (!a) return () => {};
        const { unsubscribe: g } = a.subscribe(h);
        return g;
      },
      [a],
    ),
    c = nt.useCallback(() => a?.get(), [a]);
  return P_.useSyncExternalStoreWithSelector(r, c, c, i, u);
}
var W_ = { get: () => {}, subscribe: () => ({ unsubscribe: () => {} }) };
function Ba(a) {
  const i = oe(),
    u = nt.useContext(a.from ? Y_ : Cs),
    r = a.from ?? u,
    c = r
      ? a.from
        ? i.stores.getRouteMatchStore(r)
        : i.stores.matchStores.get(r)
      : void 0,
    f = nt.useRef(void 0);
  return Ft(c ?? W_, (h) => {
    if (((a.shouldThrow ?? !0) && !h && Ee(), h === void 0)) return;
    const g = a.select ? a.select(h) : h;
    if (a.structuralSharing ?? i.options.defaultStructuralSharing) {
      const y = Oa(f.current, g);
      return ((f.current = y), y);
    }
    return g;
  });
}
function wp(a) {
  return Ba({
    from: a.from,
    strict: a.strict,
    structuralSharing: a.structuralSharing,
    select: (i) => (a.select ? a.select(i.loaderData) : i.loaderData),
  });
}
function Dp(a) {
  const { select: i, ...u } = a;
  return Ba({ ...u, select: (r) => (i ? i(r.loaderDeps) : r.loaderDeps) });
}
function Lp(a) {
  return Ba({
    from: a.from,
    shouldThrow: a.shouldThrow,
    structuralSharing: a.structuralSharing,
    strict: a.strict,
    select: (i) => {
      const u = a.strict === !1 ? i.params : i._strictParams;
      return a.select ? a.select(u) : u;
    },
  });
}
function Up(a) {
  return Ba({
    from: a.from,
    strict: a.strict,
    shouldThrow: a.shouldThrow,
    structuralSharing: a.structuralSharing,
    select: (i) => (a.select ? a.select(i.search) : i.search),
  });
}
function Np(a) {
  const i = oe();
  return nt.useCallback(
    (u) => i.navigate({ ...u, from: u.from ?? a?.from }),
    [a?.from, i],
  );
}
function Bp(a) {
  return Ba({
    ...a,
    select: (i) => (a.select ? a.select(i.context) : i.context),
  });
}
var jp = My();
const n2 = Ay(jp);
function $_(a, i) {
  const u = oe(),
    r = L_(i),
    {
      activeProps: c,
      inactiveProps: f,
      activeOptions: h,
      to: g,
      preload: y,
      preloadDelay: m,
      preloadIntentProximity: S,
      hashScrollIntoView: p,
      replace: E,
      startTransition: _,
      resetScroll: z,
      viewTransition: L,
      children: x,
      target: C,
      disabled: Y,
      style: Q,
      className: H,
      onClick: W,
      onBlur: F,
      onFocus: G,
      onMouseEnter: Z,
      onMouseLeave: P,
      onTouchStart: rt,
      ignoreBlocker: ut,
      params: it,
      search: $t,
      hash: Bt,
      state: Gt,
      mask: B,
      reloadDocument: J,
      unsafeRelative: lt,
      from: Et,
      _fromLocation: Tt,
      ...M
    } = a,
    X = Mc(),
    k = nt.useMemo(
      () => a,
      [
        u,
        a.from,
        a._fromLocation,
        a.hash,
        a.to,
        a.search,
        a.params,
        a.state,
        a.mask,
        a.unsafeRelative,
      ],
    ),
    I = Ft(
      u.stores.location,
      (Lt) => Lt,
      (Lt, ae) => Lt.href === ae.href,
    ),
    et = nt.useMemo(() => {
      const Lt = { _fromLocation: I, ...k };
      return u.buildLocation(Lt);
    }, [u, I, k]),
    dt = et.maskedLocation ? et.maskedLocation.publicHref : et.publicHref,
    vt = et.maskedLocation ? et.maskedLocation.external : et.external,
    Xt = nt.useMemo(() => iE(dt, vt, u.history, Y), [Y, vt, dt, u.history]),
    Ct = nt.useMemo(() => {
      if (Xt?.external)
        return ps(Xt.href, u.protocolAllowlist) ? void 0 : Xt.href;
      if (!uE(g) && !(typeof g != "string" || g.indexOf(":") === -1))
        try {
          return (new URL(g), ps(g, u.protocolAllowlist) ? void 0 : g);
        } catch {}
    }, [g, Xt, u.protocolAllowlist]),
    tn = nt.useMemo(() => {
      if (Ct) return !1;
      if (h?.exact) {
        if (!Z0(I.pathname, et.pathname, u.basepath)) return !1;
      } else {
        const Lt = gs(I.pathname, u.basepath),
          ae = gs(et.pathname, u.basepath);
        if (
          !(
            Lt.startsWith(ae) &&
            (Lt.length === ae.length || Lt[ae.length] === "/")
          )
        )
          return !1;
      }
      return (h?.includeSearch ?? !0) &&
        !be(I.search, et.search, {
          partial: !h?.exact,
          ignoreUndefined: !h?.explicitUndefined,
        })
        ? !1
        : h?.includeHash
          ? X && I.hash === et.hash
          : !0;
    }, [
      h?.exact,
      h?.explicitUndefined,
      h?.includeHash,
      h?.includeSearch,
      I,
      Ct,
      X,
      et.hash,
      et.pathname,
      et.search,
      u.basepath,
    ]),
    en = tn ? (ea(c, {}) ?? tE) : oc,
    Mn = tn ? oc : (ea(f, {}) ?? oc),
    wl = [H, en.className, Mn.className].filter(Boolean).join(" "),
    Ke = (Q || en.style || Mn.style) && { ...Q, ...en.style, ...Mn.style },
    [Dl, ja] = nt.useState(!1),
    Xi = nt.useRef(!1),
    nn = a.reloadDocument || Ct ? !1 : (y ?? u.options.defaultPreload),
    oa = m ?? u.options.defaultPreloadDelay ?? 0,
    Xe = nt.useCallback(() => {
      u.preloadRoute({ ...k, _builtLocation: et }).catch((Lt) => {
        (console.warn(Lt), console.warn(MS));
      });
    }, [u, k, et]);
  (D_(
    r,
    nt.useCallback(
      (Lt) => {
        Lt?.isIntersecting && Xe();
      },
      [Xe],
    ),
    lE,
    { disabled: !!Y || nn !== "viewport" },
  ),
    nt.useEffect(() => {
      Xi.current || (!Y && nn === "render" && (Xe(), (Xi.current = !0)));
    }, [Y, Xe, nn]));
  const Ll = (Lt) => {
    const ae = Lt.currentTarget.getAttribute("target"),
      Je = C !== void 0 ? C : ae;
    if (
      !Y &&
      !sE(Lt) &&
      !Lt.defaultPrevented &&
      (!Je || Je === "_self") &&
      Lt.button === 0
    ) {
      (Lt.preventDefault(),
        jp.flushSync(() => {
          ja(!0);
        }));
      const Ha = u.subscribe("onResolved", () => {
        (Ha(), ja(!1));
      });
      u.navigate({
        ...k,
        replace: E,
        resetScroll: z,
        hashScrollIntoView: p,
        startTransition: _,
        viewTransition: L,
        ignoreBlocker: ut,
      });
    }
  };
  if (Ct)
    return {
      ...M,
      ref: r,
      href: Ct,
      ...(x && { children: x }),
      ...(C && { target: C }),
      ...(Y && { disabled: Y }),
      ...(Q && { style: Q }),
      ...(H && { className: H }),
      ...(W && { onClick: W }),
      ...(F && { onBlur: F }),
      ...(G && { onFocus: G }),
      ...(Z && { onMouseEnter: Z }),
      ...(P && { onMouseLeave: P }),
      ...(rt && { onTouchStart: rt }),
    };
  const Vi = (Lt) => {
      if (Y || nn !== "intent") return;
      if (!oa) {
        Xe();
        return;
      }
      const ae = Lt.currentTarget;
      if (Li.has(ae)) return;
      const Je = setTimeout(() => {
        (Li.delete(ae), Xe());
      }, oa);
      Li.set(ae, Je);
    },
    ws = (Lt) => {
      Y || nn !== "intent" || Xe();
    },
    ce = (Lt) => {
      if (Y || !nn || !oa) return;
      const ae = Lt.currentTarget,
        Je = Li.get(ae);
      Je && (clearTimeout(Je), Li.delete(ae));
    };
  return {
    ...M,
    ...en,
    ...Mn,
    href: Xt?.href,
    ref: r,
    onClick: Rl([W, Ll]),
    onBlur: Rl([F, ce]),
    onFocus: Rl([G, Vi]),
    onMouseEnter: Rl([Z, Vi]),
    onMouseLeave: Rl([P, ce]),
    onTouchStart: Rl([rt, ws]),
    disabled: !!Y,
    target: C,
    ...(Ke && { style: Ke }),
    ...(wl && { className: wl }),
    ...(Y && eE),
    ...(tn && nE),
    ...(X && Dl && aE),
  };
}
var oc = {},
  tE = { className: "active" },
  eE = { role: "link", "aria-disabled": !0 },
  nE = { "data-status": "active", "aria-current": "page" },
  aE = { "data-transitioning": "transitioning" },
  Li = new WeakMap(),
  lE = { rootMargin: "100px" },
  Rl = (a) => (i) => {
    for (const u of a)
      if (u) {
        if (i.defaultPrevented) return;
        u(i);
      }
  };
function iE(a, i, u, r) {
  if (!r)
    return i
      ? { href: a, external: !0 }
      : { href: u.createHref(a) || "/", external: !1 };
}
function uE(a) {
  if (typeof a != "string") return !1;
  const i = a.charCodeAt(0);
  return i === 47 ? a.charCodeAt(1) !== 47 : i === 46;
}
var zc = nt.forwardRef((a, i) => {
  const { _asChild: u, ...r } = a,
    { type: c, ...f } = $_(r, i),
    h =
      typeof r.children == "function"
        ? r.children({ isActive: f["data-status"] === "active" })
        : r.children;
  if (!u) {
    const { disabled: g, ...y } = f;
    return nt.createElement("a", y, h);
  }
  return nt.createElement(u, f, h);
});
function sE(a) {
  return !!(a.metaKey || a.altKey || a.ctrlKey || a.shiftKey);
}
var rE = class extends Zy {
  constructor(i) {
    (super(i),
      (this.useMatch = (u) =>
        Ba({
          select: u?.select,
          from: this.id,
          structuralSharing: u?.structuralSharing,
        })),
      (this.useRouteContext = (u) => Bp({ ...u, from: this.id })),
      (this.useSearch = (u) =>
        Up({
          select: u?.select,
          structuralSharing: u?.structuralSharing,
          from: this.id,
        })),
      (this.useParams = (u) =>
        Lp({
          select: u?.select,
          structuralSharing: u?.structuralSharing,
          from: this.id,
        })),
      (this.useLoaderDeps = (u) => Dp({ ...u, from: this.id })),
      (this.useLoaderData = (u) => wp({ ...u, from: this.id })),
      (this.useNavigate = () => Np({ from: this.fullPath })),
      (this.Link = Ol.forwardRef((u, r) =>
        K.jsx(zc, { ref: r, from: this.fullPath, ...u }),
      )));
  }
};
function oE(a) {
  return new rE(a);
}
var cE = class extends wS {
  constructor(a) {
    (super(a),
      (this.useMatch = (i) =>
        Ba({
          select: i?.select,
          from: this.id,
          structuralSharing: i?.structuralSharing,
        })),
      (this.useRouteContext = (i) => Bp({ ...i, from: this.id })),
      (this.useSearch = (i) =>
        Up({
          select: i?.select,
          structuralSharing: i?.structuralSharing,
          from: this.id,
        })),
      (this.useParams = (i) =>
        Lp({
          select: i?.select,
          structuralSharing: i?.structuralSharing,
          from: this.id,
        })),
      (this.useLoaderDeps = (i) => Dp({ ...i, from: this.id })),
      (this.useLoaderData = (i) => wp({ ...i, from: this.id })),
      (this.useNavigate = () => Np({ from: this.fullPath })),
      (this.Link = Ol.forwardRef((i, u) =>
        K.jsx(zc, { ref: u, from: this.fullPath, ...i }),
      )));
  }
};
function fE(a) {
  return new cE(a);
}
function Hp(a) {
  return new dE(a, { silent: !0 }).createRoute;
}
var dE = class {
  constructor(a, i) {
    ((this.path = a),
      (this.createRoute = (u) => {
        const r = oE(u);
        return ((r.isRoot = !1), r);
      }),
      (this.silent = i?.silent));
  }
};
function qp(a, i) {
  let u, r, c, f;
  const h = () => (
      u ||
        (u = a()
          .then((y) => {
            ((u = void 0), (r = y[i]));
          })
          .catch((y) => {
            if (
              ((c = y),
              M0(c) &&
                c instanceof Error &&
                typeof window < "u" &&
                typeof sessionStorage < "u")
            ) {
              const m = `tanstack_router_reload:${c.message}`;
              sessionStorage.getItem(m) ||
                (sessionStorage.setItem(m, "1"), (f = !0));
            }
          })),
      u
    ),
    g = function (m) {
      if (f) throw (window.location.reload(), new Promise(() => {}));
      if (c) throw c;
      if (!r)
        if (_s) _s(h());
        else throw h();
      return nt.createElement(r, m);
    };
  return ((g.preload = h), g);
}
function hE(a) {
  const i = oe(),
    u = `not-found-${Ft(i.stores.location, (r) => r.pathname)}-${Ft(i.stores.status, (r) => r)}`;
  return K.jsx(xc, {
    getResetKey: () => u,
    onCatch: (r, c) => {
      if (ne(r)) a.onCatch?.(r, c);
      else throw r;
    },
    errorComponent: ({ error: r }) => {
      if (ne(r)) return a.fallback?.(r);
      throw r;
    },
    children: a.children,
  });
}
function mE() {
  return K.jsx("p", { children: "Not Found" });
}
function Tl(a) {
  return K.jsx(K.Fragment, { children: a.children });
}
function Yp(a, i, u) {
  return i.options.notFoundComponent
    ? K.jsx(i.options.notFoundComponent, { ...u })
    : a.options.defaultNotFoundComponent
      ? K.jsx(a.options.defaultNotFoundComponent, { ...u })
      : K.jsx(mE, {});
}
function yE(a) {
  return null;
}
function pE() {
  return (yE(oe()), null);
}
var Gp = nt.memo(function ({ matchId: i }) {
  const u = oe(),
    r = u.stores.matchStores.get(i);
  r || Ee();
  const c = Ft(u.stores.loadedAt, (h) => h),
    f = Ft(r, (h) => h);
  return K.jsx(gE, {
    router: u,
    matchId: i,
    resetKey: c,
    matchState: nt.useMemo(() => {
      const h = f.routeId,
        g = u.routesById[h].parentRoute?.id;
      return {
        routeId: h,
        ssr: f.ssr,
        _displayPending: f._displayPending,
        parentRouteId: g,
      };
    }, [f._displayPending, f.routeId, f.ssr, u.routesById]),
  });
});
function gE({ router: a, matchId: i, resetKey: u, matchState: r }) {
  const c = a.routesById[r.routeId],
    f = c.options.pendingComponent ?? a.options.defaultPendingComponent,
    h = f ? K.jsx(f, {}) : null,
    g = c.options.errorComponent ?? a.options.defaultErrorComponent,
    y = c.options.onCatch ?? a.options.defaultOnCatch,
    m = c.isRoot
      ? (c.options.notFoundComponent ??
        a.options.notFoundRoute?.options.component)
      : c.options.notFoundComponent,
    S = r.ssr === !1 || r.ssr === "data-only",
    p =
      (!c.isRoot || c.options.wrapInSuspense || S) &&
      (c.options.wrapInSuspense ??
        f ??
        (c.options.errorComponent?.preload || S))
        ? nt.Suspense
        : Tl,
    E = g ? xc : Tl,
    _ = m ? hE : Tl;
  return K.jsxs(c.isRoot ? (c.options.shellComponent ?? Tl) : Tl, {
    children: [
      K.jsx(Cs.Provider, {
        value: i,
        children: K.jsx(p, {
          fallback: h,
          children: K.jsx(E, {
            getResetKey: () => u,
            errorComponent: g || Ac,
            onCatch: (z, L) => {
              if (ne(z)) throw ((z.routeId ??= r.routeId), z);
              y?.(z, L);
            },
            children: K.jsx(_, {
              fallback: (z) => {
                if (
                  ((z.routeId ??= r.routeId),
                  !m ||
                    (z.routeId && z.routeId !== r.routeId) ||
                    (!z.routeId && !c.isRoot))
                )
                  throw z;
                return nt.createElement(m, z);
              },
              children:
                S || r._displayPending
                  ? K.jsx(H_, {
                      fallback: h,
                      children: K.jsx(Ty, { matchId: i }),
                    })
                  : K.jsx(Ty, { matchId: i }),
            }),
          }),
        }),
      }),
      r.parentRouteId === Da
        ? K.jsxs(K.Fragment, {
            children: [
              K.jsx(vE, { resetKey: u }),
              a.options.scrollRestoration && Oy ? K.jsx(pE, {}) : null,
            ],
          })
        : null,
    ],
  });
}
function vE({ resetKey: a }) {
  const i = oe(),
    u = nt.useRef(void 0);
  return (
    Ui(() => {
      const r = i.latestLocation.href;
      (u.current === void 0 || u.current !== r) &&
        (i.emit({
          type: "onRendered",
          ...Al(i.stores.location.get(), i.stores.resolvedLocation.get()),
        }),
        (u.current = r));
    }, [i.latestLocation.state.__TSR_key, a, i]),
    null
  );
}
var Ty = nt.memo(function ({ matchId: i }) {
    const u = oe(),
      r = (S, p) => u.getMatch(S.id)?._nonReactive[p] ?? S._nonReactive[p],
      c = u.stores.matchStores.get(i);
    c || Ee();
    const f = Ft(c, (S) => S),
      h = f.routeId,
      g = u.routesById[h],
      y = nt.useMemo(() => {
        const S = (
          u.routesById[h].options.remountDeps ?? u.options.defaultRemountDeps
        )?.({
          routeId: h,
          loaderDeps: f.loaderDeps,
          params: f._strictParams,
          search: f._strictSearch,
        });
        return S ? JSON.stringify(S) : void 0;
      }, [
        h,
        f.loaderDeps,
        f._strictParams,
        f._strictSearch,
        u.options.defaultRemountDeps,
        u.routesById,
      ]),
      m = nt.useMemo(() => {
        const S = g.options.component ?? u.options.defaultComponent;
        return S ? K.jsx(S, {}, y) : K.jsx(Xp, {});
      }, [y, g.options.component, u.options.defaultComponent]);
    if (f._displayPending) throw r(f, "displayPendingPromise");
    if (f._forcePending) throw r(f, "minPendingPromise");
    if (f.status === "pending") {
      const S = g.options.pendingMinMs ?? u.options.defaultPendingMinMs;
      if (S) {
        const p = u.getMatch(f.id);
        if (p && !p._nonReactive.minPendingPromise) {
          const E = Ua();
          ((p._nonReactive.minPendingPromise = E),
            setTimeout(() => {
              (E.resolve(), (p._nonReactive.minPendingPromise = void 0));
            }, S));
        }
      }
      throw r(f, "loadPromise");
    }
    if (f.status === "notFound")
      return (ne(f.error) || Ee(), Yp(u, g, f.error));
    if (f.status === "redirected")
      throw (_e(f.error) || Ee(), r(f, "loadPromise"));
    if (f.status === "error") throw f.error;
    return m;
  }),
  Xp = nt.memo(function () {
    const i = oe(),
      u = nt.useContext(Cs);
    let r,
      c = !1,
      f;
    {
      const m = u ? i.stores.matchStores.get(u) : void 0;
      (([r, c] = Ft(m, (S) => [S?.routeId, S?.globalNotFound ?? !1])),
        (f = Ft(
          i.stores.matchesId,
          (S) => S[S.findIndex((p) => p === u) + 1],
        )));
    }
    const h = r ? i.routesById[r] : void 0,
      g = i.options.defaultPendingComponent
        ? K.jsx(i.options.defaultPendingComponent, {})
        : null;
    if (c) return (h || Ee(), Yp(i, h, void 0));
    if (!f) return null;
    const y = K.jsx(Gp, { matchId: f });
    return r === Da ? K.jsx(nt.Suspense, { fallback: g, children: y }) : y;
  });
function SE() {
  const a = oe(),
    i = nt.useRef({ router: a, mounted: !1 }),
    [u, r] = nt.useState(!1),
    c = Ft(a.stores.isLoading, (p) => p),
    f = Ft(a.stores.hasPending, (p) => p),
    h = lc(c),
    g = c || u || f,
    y = lc(g),
    m = c || f,
    S = lc(m);
  return (
    (a.startTransition = (p) => {
      (r(!0),
        nt.startTransition(() => {
          (p(), r(!1));
        }));
    }),
    nt.useEffect(() => {
      const p = a.history.subscribe(a.load),
        E = a.buildLocation({
          to: a.latestLocation.pathname,
          search: !0,
          params: !0,
          hash: !0,
          state: !0,
          _includeValidateSearch: !0,
        });
      return (
        Rn(a.latestLocation.publicHref) !== Rn(E.publicHref) &&
          a.commitLocation({ ...E, replace: !0 }),
        () => {
          p();
        }
      );
    }, [a, a.history]),
    Ui(() => {
      if (
        (typeof window < "u" && a.ssr) ||
        (i.current.router === a && i.current.mounted)
      )
        return;
      ((i.current = { router: a, mounted: !0 }),
        (async () => {
          try {
            await a.load();
          } catch (E) {
            console.error(E);
          }
        })());
    }, [a]),
    Ui(() => {
      h &&
        !c &&
        a.emit({
          type: "onLoad",
          ...Al(a.stores.location.get(), a.stores.resolvedLocation.get()),
        });
    }, [h, a, c]),
    Ui(() => {
      S &&
        !m &&
        a.emit({
          type: "onBeforeRouteMount",
          ...Al(a.stores.location.get(), a.stores.resolvedLocation.get()),
        });
    }, [m, S, a]),
    Ui(() => {
      if (y && !g) {
        const p = Al(a.stores.location.get(), a.stores.resolvedLocation.get());
        (a.emit({ type: "onResolved", ...p }),
          Op(() => {
            (a.stores.status.set("idle"),
              a.stores.resolvedLocation.set(a.stores.location.get()));
          }),
          p.hrefChanged && DS(a));
      }
    }, [g, y, a]),
    null
  );
}
function bE() {
  const a = oe(),
    i =
      a.routesById[Da].options.pendingComponent ??
      a.options.defaultPendingComponent,
    u = i ? K.jsx(i, {}) : null,
    r = K.jsxs(typeof document < "u" && a.ssr ? Tl : nt.Suspense, {
      fallback: u,
      children: [K.jsx(SE, {}), K.jsx(_E, {})],
    });
  return a.options.InnerWrap ? K.jsx(a.options.InnerWrap, { children: r }) : r;
}
function _E() {
  const a = oe(),
    i = Ft(a.stores.firstId, (c) => c),
    u = Ft(a.stores.loadedAt, (c) => c),
    r = i ? K.jsx(Gp, { matchId: i }) : null;
  return K.jsx(Cs.Provider, {
    value: i,
    children: a.options.disableGlobalCatchBoundary
      ? r
      : K.jsx(xc, {
          getResetKey: () => u,
          errorComponent: Ac,
          onCatch: void 0,
          children: r,
        }),
  });
}
var EE = (a) => ({
    createMutableStore: Sy,
    createReadonlyStore: Sy,
    batch: Op,
  }),
  RE = (a) => new TE(a),
  TE = class extends SS {
    constructor(a) {
      super(a, EE);
    }
  };
function xE({ router: a, children: i, ...u }) {
  wy(u) &&
    a.update({
      ...a.options,
      ...u,
      context: { ...a.options.context, ...u.context },
    });
  const r = K.jsx(Mp.Provider, { value: a, children: i });
  return a.options.Wrap ? K.jsx(a.options.Wrap, { children: r }) : r;
}
function AE({ router: a, ...i }) {
  return K.jsx(xE, { router: a, ...i, children: K.jsx(bE, {}) });
}
function Vp(a) {
  const { attrs: i, children: u, nonce: r } = a;
  switch (a.tag) {
    case "title":
      return K.jsx("title", {
        ...i,
        suppressHydrationWarning: !0,
        children: u,
      });
    case "meta":
      return K.jsx("meta", { ...i, suppressHydrationWarning: !0 });
    case "link":
      return K.jsx("link", {
        ...i,
        precedence:
          i?.precedence ?? (i?.rel === "stylesheet" ? "default" : void 0),
        nonce: r,
        suppressHydrationWarning: !0,
      });
    case "style":
      return (
        a.inlineCss,
        K.jsx("style", {
          ...i,
          dangerouslySetInnerHTML: { __html: u },
          nonce: r,
        })
      );
    case "script":
      return K.jsx(ME, { attrs: i, children: u });
    default:
      return null;
  }
}
function ME({ attrs: a, children: i }) {
  oe();
  const u = Mc(),
    r =
      typeof a?.type == "string" &&
      a.type !== "" &&
      a.type !== "text/javascript" &&
      a.type !== "module";
  if (
    (nt.useEffect(() => {
      if (!r) {
        if (a?.src) {
          const c = (() => {
            try {
              const h = document.baseURI || window.location.href;
              return new URL(a.src, h).href;
            } catch {
              return a.src;
            }
          })();
          if (
            Array.from(document.querySelectorAll("script[src]")).find(
              (h) => h.src === c,
            )
          )
            return;
          const f = document.createElement("script");
          for (const [h, g] of Object.entries(a))
            h !== "suppressHydrationWarning" &&
              g !== void 0 &&
              g !== !1 &&
              f.setAttribute(h, typeof g == "boolean" ? "" : String(g));
          return (
            document.head.appendChild(f),
            () => {
              f.parentNode && f.parentNode.removeChild(f);
            }
          );
        }
        if (typeof i == "string") {
          const c = typeof a?.type == "string" ? a.type : "text/javascript",
            f = typeof a?.nonce == "string" ? a.nonce : void 0;
          if (
            Array.from(document.querySelectorAll("script:not([src])")).find(
              (g) => {
                if (!(g instanceof HTMLScriptElement)) return !1;
                const y = g.getAttribute("type") ?? "text/javascript",
                  m = g.getAttribute("nonce") ?? void 0;
                return g.textContent === i && y === c && m === f;
              },
            )
          )
            return;
          const h = document.createElement("script");
          if (((h.textContent = i), a))
            for (const [g, y] of Object.entries(a))
              g !== "suppressHydrationWarning" &&
                y !== void 0 &&
                y !== !1 &&
                h.setAttribute(g, typeof y == "boolean" ? "" : String(y));
          return (
            document.head.appendChild(h),
            () => {
              h.parentNode && h.parentNode.removeChild(h);
            }
          );
        }
      }
    }, [a, i, r]),
    r && typeof i == "string")
  )
    return K.jsx("script", {
      ...a,
      suppressHydrationWarning: !0,
      dangerouslySetInnerHTML: { __html: i },
    });
  if (!u) {
    if (a?.src) return K.jsx("script", { ...a, suppressHydrationWarning: !0 });
    if (typeof i == "string")
      return K.jsx("script", {
        ...a,
        dangerouslySetInnerHTML: { __html: i },
        suppressHydrationWarning: !0,
      });
  }
  return null;
}
var zE = (a) => {
  const i = oe(),
    u = i.options.ssr?.nonce,
    r = Ft(i.stores.matches, (m) => m.map((S) => S.meta).filter(Boolean), be),
    c = nt.useMemo(() => {
      const m = [],
        S = {};
      let p;
      for (let E = r.length - 1; E >= 0; E--) {
        const _ = r[E];
        for (let z = _.length - 1; z >= 0; z--) {
          const L = _[z];
          if (L)
            if (L.title) p || (p = { tag: "title", children: L.title });
            else if ("script:ld+json" in L)
              try {
                const x = JSON.stringify(L["script:ld+json"]);
                m.push({
                  tag: "script",
                  attrs: { type: "application/ld+json" },
                  children: D0(x),
                });
              } catch {}
            else {
              const x = L.name ?? L.property;
              if (x) {
                if (S[x]) continue;
                S[x] = !0;
              }
              m.push({ tag: "meta", attrs: { ...L, nonce: u } });
            }
        }
      }
      return (
        p && m.push(p),
        u &&
          m.push({ tag: "meta", attrs: { property: "csp-nonce", content: u } }),
        m.reverse(),
        m
      );
    }, [r, u]),
    f = Ft(
      i.stores.matches,
      (m) => {
        const S = m
            .map((_) => _.links)
            .filter(Boolean)
            .flat(1)
            .map((_) => ({ tag: "link", attrs: { ..._, nonce: u } })),
          p = i.ssr?.manifest,
          E = m
            .map((_) => p?.routes[_.routeId]?.assets ?? [])
            .filter(Boolean)
            .flat(1)
            .flatMap((_) =>
              _.tag === "link"
                ? CS(p, _)
                  ? []
                  : [
                      {
                        tag: "link",
                        attrs: {
                          ..._.attrs,
                          crossOrigin:
                            ay(a, "stylesheet") ?? _.attrs?.crossOrigin,
                          suppressHydrationWarning: !0,
                          nonce: u,
                        },
                      },
                    ]
                : _.tag === "style"
                  ? [
                      {
                        tag: "style",
                        attrs: { ..._.attrs, nonce: u },
                        children: _.children,
                        ...(_.inlineCss ? { inlineCss: !0 } : {}),
                      },
                    ]
                  : [],
            );
        return [...S, ...E];
      },
      be,
    ),
    h = Ft(
      i.stores.matches,
      (m) => {
        const S = [];
        return (
          m
            .map((p) => i.looseRoutesById[p.routeId])
            .forEach((p) =>
              i.ssr?.manifest?.routes[p.id]?.preloads
                ?.filter(Boolean)
                .forEach((E) => {
                  const _ = zS(E);
                  S.push({
                    tag: "link",
                    attrs: {
                      rel: "modulepreload",
                      href: _.href,
                      crossOrigin: ay(a, "modulepreload") ?? _.crossOrigin,
                      nonce: u,
                    },
                  });
                }),
            ),
          S
        );
      },
      be,
    ),
    g = Ft(
      i.stores.matches,
      (m) =>
        m
          .map((S) => S.styles)
          .flat(1)
          .filter(Boolean)
          .map(({ children: S, ...p }) => ({
            tag: "style",
            attrs: { ...p, nonce: u },
            children: S,
          })),
      be,
    ),
    y = Ft(
      i.stores.matches,
      (m) =>
        m
          .map((S) => S.headScripts)
          .flat(1)
          .filter(Boolean)
          .map(({ children: S, ...p }) => ({
            tag: "script",
            attrs: { ...p, nonce: u },
            children: S,
          })),
      be,
    );
  return OE([...c, ...h, ...f, ...g, ...y], (m) => JSON.stringify(m));
};
function OE(a, i) {
  const u = new Set();
  return a.filter((r) => {
    const c = i(r);
    return u.has(c) ? !1 : (u.add(c), !0);
  });
}
function CE(a) {
  const i = zE(a.assetCrossOrigin),
    u = oe().options.ssr?.nonce;
  return K.jsx(K.Fragment, {
    children: i.map((r) =>
      nt.createElement(Vp, {
        ...r,
        key: `tsr-meta-${JSON.stringify(r)}`,
        nonce: u,
      }),
    ),
  });
}
var wE = () => {
  const a = oe(),
    i = a.options.ssr?.nonce,
    u = (f) => {
      const h = [],
        g = a.ssr?.manifest;
      return g
        ? (f
            .map((y) => a.looseRoutesById[y.routeId])
            .forEach((y) =>
              g.routes[y.id]?.assets
                ?.filter((m) => m.tag === "script")
                .forEach((m) => {
                  h.push({
                    tag: "script",
                    attrs: { ...m.attrs, nonce: i },
                    children: m.children,
                  });
                }),
            ),
          h)
        : [];
    },
    r = (f) =>
      f
        .map((h) => h.scripts)
        .flat(1)
        .filter(Boolean)
        .map(({ children: h, ...g }) => ({
          tag: "script",
          attrs: { ...g, suppressHydrationWarning: !0, nonce: i },
          children: h,
        })),
    c = Ft(a.stores.matches, u, be);
  return DE(a, Ft(a.stores.matches, r, be), c);
};
function DE(a, i, u) {
  let r;
  a.serverSsr && (r = a.serverSsr.takeBufferedScripts());
  const c = [...i, ...u];
  return (
    r && c.unshift(r),
    K.jsx(K.Fragment, {
      children: c.map((f, h) =>
        nt.createElement(Vp, { ...f, key: `tsr-scripts-${f.tag}-${h}` }),
      ),
    })
  );
}
const LE = "/assets/styles-DR5uOlhJ.css";
function UE() {
  return K.jsx("div", {
    className:
      "flex min-h-screen items-center justify-center bg-background px-4",
    children: K.jsxs("div", {
      className: "max-w-md text-center",
      children: [
        K.jsx("h1", {
          className: "text-7xl font-bold text-foreground",
          children: "404",
        }),
        K.jsx("h2", {
          className: "mt-4 text-xl font-semibold text-foreground",
          children: "Page not found",
        }),
        K.jsx("p", {
          className: "mt-2 text-sm text-muted-foreground",
          children:
            "The page you're looking for doesn't exist or has been moved.",
        }),
        K.jsx("div", {
          className: "mt-6",
          children: K.jsx(zc, {
            to: "/",
            className:
              "inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90",
            children: "Go home",
          }),
        }),
      ],
    }),
  });
}
const Oc = fE({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1" },
      { title: "Lovable App" },
      { name: "description", content: "Lovable Generated Project" },
      { name: "author", content: "Lovable" },
      { property: "og:title", content: "Lovable App" },
      { property: "og:description", content: "Lovable Generated Project" },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary" },
      { name: "twitter:site", content: "@Lovable" },
    ],
    links: [{ rel: "stylesheet", href: LE }],
  }),
  shellComponent: NE,
  component: BE,
  notFoundComponent: UE,
});
function NE({ children: a }) {
  return K.jsxs("html", {
    lang: "en",
    children: [
      K.jsx("head", { children: K.jsx(CE, {}) }),
      K.jsxs("body", { children: [a, K.jsx(wE, {})] }),
    ],
  });
}
function BE() {
  return K.jsx(Xp, {});
}
const jE = "modulepreload",
  HE = function (a) {
    return "/" + a;
  },
  xy = {},
  Qp = function (i, u, r) {
    let c = Promise.resolve();
    if (u && u.length > 0) {
      let y = function (m) {
        return Promise.all(
          m.map((S) =>
            Promise.resolve(S).then(
              (p) => ({ status: "fulfilled", value: p }),
              (p) => ({ status: "rejected", reason: p }),
            ),
          ),
        );
      };
      document.getElementsByTagName("link");
      const h = document.querySelector("meta[property=csp-nonce]"),
        g = h?.nonce || h?.getAttribute("nonce");
      c = y(
        u.map((m) => {
          if (((m = HE(m)), m in xy)) return;
          xy[m] = !0;
          const S = m.endsWith(".css"),
            p = S ? '[rel="stylesheet"]' : "";
          if (document.querySelector(`link[href="${m}"]${p}`)) return;
          const E = document.createElement("link");
          if (
            ((E.rel = S ? "stylesheet" : jE),
            S || (E.as = "script"),
            (E.crossOrigin = ""),
            (E.href = m),
            g && E.setAttribute("nonce", g),
            document.head.appendChild(E),
            S)
          )
            return new Promise((_, z) => {
              (E.addEventListener("load", _),
                E.addEventListener("error", () =>
                  z(new Error(`Unable to preload CSS for ${m}`)),
                ));
            });
        }),
      );
    }
    function f(h) {
      const g = new Event("vite:preloadError", { cancelable: !0 });
      if (((g.payload = h), window.dispatchEvent(g), !g.defaultPrevented))
        throw h;
    }
    return c.then((h) => {
      for (const g of h || []) g.status === "rejected" && f(g.reason);
      return i().catch(f);
    });
  },
  qE = () => Qp(() => import("./index-CYQxVFKm.js"), __vite__mapDeps([0, 1])),
  YE = Hp("/")({ component: qp(qE, "component") }),
  GE = () =>
    Qp(() => import("./repo._repoId-HD62IApT.js"), __vite__mapDeps([2, 1])),
  XE = Hp("/repo/$repoId")({ component: qp(GE, "component") }),
  VE = YE.update({ id: "/", path: "/", getParentRoute: () => Oc }),
  QE = XE.update({
    id: "/repo/$repoId",
    path: "/repo/$repoId",
    getParentRoute: () => Oc,
  }),
  ZE = { IndexRoute: VE, RepoRepoIdRoute: QE },
  KE = Oc._addFileChildren(ZE);
function JE({ error: a, reset: i }) {
  const u = oe();
  return K.jsx("div", {
    className:
      "flex min-h-screen items-center justify-center bg-background px-4",
    children: K.jsxs("div", {
      className: "max-w-md text-center",
      children: [
        K.jsx("div", {
          className:
            "mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-destructive/10",
          children: K.jsx("svg", {
            xmlns: "http://www.w3.org/2000/svg",
            className: "h-8 w-8 text-destructive",
            fill: "none",
            viewBox: "0 0 24 24",
            stroke: "currentColor",
            strokeWidth: 2,
            children: K.jsx("path", {
              strokeLinecap: "round",
              strokeLinejoin: "round",
              d: "M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z",
            }),
          }),
        }),
        K.jsx("h1", {
          className: "text-2xl font-bold tracking-tight text-foreground",
          children: "Something went wrong",
        }),
        K.jsx("p", {
          className: "mt-2 text-sm text-muted-foreground",
          children: "An unexpected error occurred. Please try again.",
        }),
        !1,
        K.jsxs("div", {
          className: "mt-6 flex items-center justify-center gap-3",
          children: [
            K.jsx("button", {
              onClick: () => {
                (u.invalidate(), i());
              },
              className:
                "inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90",
              children: "Try again",
            }),
            K.jsx("a", {
              href: "/",
              className:
                "inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent",
              children: "Go home",
            }),
          ],
        }),
      ],
    }),
  });
}
const kE = () =>
  RE({
    routeTree: KE,
    context: {},
    scrollRestoration: !0,
    defaultPreloadStaleTime: 0,
    defaultErrorComponent: JE,
  });
async function FE() {
  const a = await kE();
  let i;
  return (
    (i = []),
    (window.__TSS_START_OPTIONS__ = { serializationAdapters: i }),
    i.push(O_),
    a.options.serializationAdapters &&
      i.push(...a.options.serializationAdapters),
    a.update({ basepath: "", serializationAdapters: i }),
    a.stores.matchesId.get().length || (await w_(a)),
    a
  );
}
async function PE() {
  const a = await FE();
  return (window.$_TSR?.h(), a);
}
var cc;
function IE() {
  return (
    cc || (cc = PE()),
    K.jsx(N_, { promise: cc, children: (a) => K.jsx(AE, { router: a }) })
  );
}
nt.startTransition(() => {
  S0.hydrateRoot(document, K.jsx(nt.StrictMode, { children: K.jsx(IE, {}) }));
});
export {
  zc as L,
  XE as R,
  Ft as a,
  Oa as b,
  WE as c,
  jp as d,
  n2 as e,
  Ol as f,
  K as j,
  nt as r,
  oe as u,
};
